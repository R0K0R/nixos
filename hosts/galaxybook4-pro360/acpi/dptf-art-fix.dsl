 ...
 */
// OEM Revision bumped from 0x00001000 -> 0x00001001 so the Linux ACPI
// initrd table-upgrade mechanism recognizes this as newer than the
// platform's own SSDT11 (DptfTb/DptfTabl) and actually replaces it --
// matching signature/OEMID/OEM-Table-ID with an equal-or-lower revision
// is ignored, not applied.
DefinitionBlock ("", "SSDT", 2, "DptfTb", "DptfTabl", 0x00001001)
{
    ...
    Scope (\_SB)
    {
        OperationRegion (DNVS, SystemMemory, 0x6CFB9000, 0x0021)
        ...
        Device (IETM)
        {
            ...
            {
                // Firmware bug: this platform's DPTF tables declare and
                // reference \_SB.IETM.SEN3 (a third thermal sensor) but no
                // loaded ACPI table ever actually defines that device -- the
                // stock check below dereferences SEN3.CTYP unconditionally,
                // unlike every other optional object in this method, which
                // aborts _ART with AE_NOT_FOUND. Guard it with CondRefOf
                // like ART1/ART0 already are (nested, since AML LAnd
                // evaluates both operands eagerly -- no && short-circuit),
                // so a missing SEN3 falls through to the ART0 path instead
                // of aborting.
                If (CondRefOf (\_SB.IETM.SEN3))
                {

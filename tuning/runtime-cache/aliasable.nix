{
  capturedAt = "2026-09-13T10:11:06Z";
  names = [
    "AMB-plugins"
    "ArchiSteamFarm"
    "DisnixWebService"
    "Fabric"
    "LAStools"
    "LycheeSlicer"
    "MMA"
    "OVMF"
    "OVMF-amdsev"
    "OVMF-cloud-hypervisor"
    "OVMF-inteltdx"
    "OVMF-xen"
    "OVMFFull"
    "QuadProgpp"
    "R"
    "SDL"
    "SDL2"
    "SDL2_Pango"
    "SDL2_gfx"
    "SDL2_image"
    "SDL2_mixer"
    "SDL2_net"
    "SDL2_sound"
    "SDL2_ttf"
    "SDL_audiolib"
    "SDL_compat"
    "SDL_gfx"
    "SDL_image"
    "SDL_mixer"
    "SDL_net"
    "SDL_sixel"
    "SDL_sound"
    "SDL_stretch"
    "SDL_ttf"
    "Xaw3d"
    "_010editor"
    "_0x"
    "_0xffff"
    "_0xpropo"
    "_0xproto"
    "_1fps"
    "_1oom"
    "_1password-cli"
    "_1password-gui"
    "_1password-gui-beta"
    "_2048-in-terminal"
    "_20kly"
    "_2bwm"
    "_2ship2harkinian"
    "_3270font"
    "_389-ds-base"
    "_3beans"
    "_3cpio"
    "_3llo"
    "_3mux"
    "_3proxy"
    "_4d-minesweeper"
    "_4th"
    "_4ti2"
    "_64gram"
    "_64tass"
    "_6tunnel"
    "_7-segment-font"
    "_7kaa"
    "_7z2hashcat"
    "_7zip-zstd"
    "_7zip-zstd-rar"
    "_7zz"
    "_7zz-rar"
    "_81voltd"
    "_86Box"
    "_86Box-with-roms"
    "_86box"
    "_86box-with-roms"
    "_90secondportraits"
    "_915resolution"
    "_9base"
    "_9menu"
    "_9ptls"
    "__flattenIncludeHackHook"
    "a-keys-path"
    "a2jmidid"
    "a2ps"
    "a4"
    "a52dec"
    "aaa"
    "aaaaxy"
    "aab"
    "aacgain"
    "aactivator"
    "aalib"
    "aaphoto"
    "aapt"
    "aarch64-esr-decoder"
    "aardvark-dns"
    "aasvg"
    "aasvg-rs"
    "aaxtomp3"
    "ab-av1"
    "aba"
    "abaddon"
    "abbaye-des-morts"
    "abc-verifier"
    "abcde"
    "abcl"
    "abcm2ps"
    "abcmidi"
    "abctl"
    "abduco"
    "abella"
    "abi-compliance-checker"
    "abi-dumper"
    "abiword"
    "ablog"
    "abook"
    "abootimg"
    "above"
    "abpoa"
    "abracadabra"
    "abseil-cpp"
    "abseil-cpp_202103"
    "abseil-cpp_202401"
    "abseil-cpp_202407"
    "abseil-cpp_202505"
    "abseil-cpp_202508"
    "abseil-cpp_202601"
    "abseil-cpp_202605"
    "abseil-cpp_202608"
    "absolute"
    "abtop"
    "abuild"
    "abuse"
    "ac-library"
    "acc"
    "accelergy"
    "accerciser"
    "account-utils"
    "accountsservice"
    "acct"
    "accuraterip-checksum"
    "acdi"
    "ace"
    "ace-of-penguins"
    "aces-container"
    "acgtk"
    "ack"
    "acl"
    "acl2"
    "acl2-minimal"
    "acli"
    "acltoolkit"
    "acme"
    "acme-client"
    "acme-dns"
    "acme-sh"
    "acorns"
    "acpi"
    "acpic"
    "acpica-tools"
    "acpid"
    "acpidump-all"
    "acpilight"
    "acpitool"
    "acquire"
    "acr"
    "acr-cli"
    "acsccid"
    "act"
    "act3"
    "actdiag"
    "action-validator"
    "actiona"
    "actionlint"
    "activate-linux"
    "activemq"
    "activitywatch"
    "actkbd"
    "actool"
    "actual-client"
    "actual-server"
    "ad"
    "ad-ldap-enum"
    "ad-miner"
    "ada"
    "adafruit-ampy"
    "adafruit-nrfutil"
    "adalanche"
    "adapta-backgrounds"
    "adapta-gtk-theme"
    "adapta-kde-theme"
    "adaptagrams"
    "adaptivecpp"
    "adaptivecppWithCuda"
    "adaptivecppWithRocm"
    "adbfs-rootless"
    "adbtuifm"
    "adcli"
    "adcskiller"
    "add-determinism"
    "addBinToPathHook"
    "addDriverRunpath"
    "addchain"
    "addic7ed-cli"
    "addlicense"
    "addr-book-combine"
    "addwater"
    "adementary-theme"
    "adenum"
    "adept"
    "adguardhome"
    "adguardian"
    "adhammer"
    "adi1090x-plymouth-themes"
    "adidnsdump"
    "adif-multitool"
    "adios2"
    "adlplug"
    "adminer"
    "adminerevo"
    "adminneo"
    "adms"
    "adns"
    "adolc"
    "adoptopenjdk-icedtea-web"
    "adpathfinder"
    "adr-tools"
    "adreaper"
    "adrgen"
    "adriconf"
    "adrs"
    "adscan"
    "adslib"
    "advancecomp"
    "advi"
    "adw-bluetooth"
    "adw-gtk3"
    "adwaita-fonts"
    "adwaita-icon-theme"
    "adwaita-icon-theme-legacy"
    "adwaita-qt"
    "adwaita-qt6"
    "adwsteamgtk"
    "aeacus"
    "aegan"
    "aegis-rs"
    "aegisub"
    "aegyptus"
    "aemu"
    "aeolus"
    "aeolus-stops"
    "aerc"
    "aerion"
    "aerion-creds"
    "aerogramme"
    "aeron"
    "aeron-cpp"
    "aerospace"
    "aerospike"
    "aescrypt"
    "aesfix"
    "aeskeyfind"
    "aespipe"
    "aether-lv2"
    "aewan"
    "afetch"
    "afew"
    "affine"
    "affine-bin"
    "afflib"
    "afio"
    "aflplusplus"
    "afsctool"
    "aften"
    "afterburn"
    "afterglow-cursors-recolored"
    "agari"
    "agate"
    "agave"
    "agda"
    "agdsn-zsh-config"
    "age"
    "age-plugin-1p"
    "age-plugin-fido2-hmac"
    "age-plugin-fido2prf"
    "age-plugin-ledger"
    "age-plugin-openpgp-card"
    "age-plugin-se"
    "age-plugin-sss"
    "age-plugin-tpm"
    "age-plugin-yubikey"
    "agebox"
    "agedu"
    "agenda"
    "agenix-cli"
    "agent-browser"
    "agent-safehouse"
    "agentsploit"
    "agevault"
    "agg"
    "aggregate6"
    "agi"
    "agkozak-zsh-prompt"
    "agneyastra"
    "agnos"
    "agola"
    "agorakit"
    "agru"
    "ags"
    "ags_1"
    "aha"
    "ahcpd"
    "ahk_x11"
    "ahoy"
    "aiac"
    "aichat"
    "aide"
    "aider-chat"
    "aider-chat-full"
    "aider-chat-with-bedrock"
    "aider-chat-with-browser"
    "aider-chat-with-help"
    "aider-chat-with-playwright"
    "aiger"
    "aiken"
    "aileron"
    "ain"
    "aioblescan"
    "aiodnsbrute"
    "aiovban-pyaudio"
    "air"
    "air-formatter"
    "airbuddy"
    "aircrack-ng"
    "airdrop-cli"
    "airgeddon"
    "airgorah"
    "airlift"
    "airscan"
    "airshipper"
    "airsonic"
    "airspy"
    "airspyhf"
    "airstrike"
    "airtame"
    "airwave"
    "airwin2rack"
    "airwindows"
    "airwindows-lv2"
    "aisleriot"
    "aitrack"
    "aixlog"
    "aj-snapshot"
    "aja-desktop-software"
    "akira-unstable"
    "akkadian"
    "akkoma"
    "akkoma-admin-fe"
    "akkoma-fe"
    "akku"
    "aks-mcp-server"
    "alac"
    "alacarte"
    "alacritty"
    "alacritty-graphics"
    "alacritty-theme"
    "alan"
    "alan_2"
    "alarm-clock-applet"
    "albatross"
    "albedo"
    "albert"
    "albyhub"
    "alcarin-tengwar"
    "alchemy"
    "alcom"
    "alcove"
    "alda"
    "aldente"
    "aldo"
    "alegreya"
    "alegreya-sans"
    "alejandra"
    "alembic"
    "aleo-fonts"
    "alephone"
    "alephone-apotheosis-x"
    "alephone-durandal"
    "alephone-eternal"
    "alephone-evil"
    "alephone-infinity"
    "alephone-marathon"
    "alephone-pathways-into-darkness"
    "alephone-pheonix"
    "alephone-red"
    "alephone-rubicon-x"
    "alephone-yuge"
    "alerta"
    "alerta-server"
    "alertmanager-gotify-bridge"
    "alertmanager-irc-relay"
    "alertmanager-ntfy"
    "alertmanager-webhook-logger"
    "alex"
    "alfaview"
    "alfis"
    "alfis-nogui"
    "alfred"
    "algia"
    "alglib"
    "algol68g"
    "algolia-cli"
    "ali"
    "aliae"
    "alibuild"
    "alice"
    "alice-lg"
    "alice-tools"
    "alice-tools-qt5"
    "alice-tools-qt6"
    "alienarena"
    "aligator"
    "align"
    "alioth"
    "alire"
    "alist"
    "alisthelper"
    "alistral"
    "alive2"
    "aliyun-cli"
    "aliyunpan"
    "alkalami"
    "alkimia"
    "all-cabal-hashes"
    "all-the-package-names"
    "allegro"
    "allegro4"
    "allegro5"
    "alliance"
    "allmark"
    "allmytoes"
    "alloy"
    "alloy5"
    "alloy6"
    "allure"
    "almanah"
    "almo"
    "almonds"
    "alock"
    "alot"
    "alp"
    "alpaca"
    "alpaca-proxy"
    "alpine"
    "alpine-make-rootfs"
    "alpine-make-vm-image"
    "alpnpass"
    "alps"
    "alsa-firmware"
    "alsa-lib"
    "alsa-lib-with-plugins"
    "alsa-oss"
    "alsa-plugins"
    "alsa-plugins-wrapper"
    "alsa-scarlett-gui"
    "alsa-tools"
    "alsa-topology-conf"
    "alsa-ucm-conf"
    "alsa-ucm-conf-asahi"
    "alsa-utils"
    "alsa-utils-nhlt"
    "alsaequal"
    "alt-ergo"
    "alt-ergo-free"
    "alt-tab-macos"
    "altair"
    "altdns"
    "altermime"
    "alterware-launcher"
    "alterx"
    "althttpd"
    "altserver-linux"
    "alttab"
    "alttpr-opentracker"
    "altus"
    "alure"
    "alure2"
    "alvr"
    "am2rlauncher"
    "amarena-theme"
    "amarok"
    "amass"
    "amazon-cloudwatch-agent"
    "amazon-ec2-net-utils"
    "amazon-ec2-utils"
    "amazon-ecr-credential-helper"
    "amazon-q-cli"
    "amazon-ssm-agent"
    "amb-plugins"
    "amber"
    "amber-lang"
    "amber-secret"
    "amberol"
    "amd-blis"
    "amd-debug-tools"
    "amd-libflame"
    "amd-ucodegen"
    "amdctl"
    "amdenc"
    "amdfwtool"
    "amdgpu_top"
    "amdtop"
    "ameba"
    "ameba-ls"
    "ament-cmake-core-jazzy"
    "amf"
    "amf-headers"
    "amfora"
    "amiberry"
    "amidst"
    "amiri"
    "aml"
    "ammonite"
    "ammonite_2_12"
    "ammonite_2_13"
    "ammonite_3_3"
    "amnezia-vpn"
    "amnezia-vpn-bin"
    "amneziawg-go"
    "amneziawg-tools"
    "amoco"
    "amoeba"
    "amoeba-data"
    "among-sus"
    "amp"
    "amp-cli"
    "amphetype"
    "amqpcat"
    "amrnb"
    "amrwb"
    "ams"
    "amtterm"
    "amule"
    "amule-cmd"
    "amule-daemon"
    "amule-gui"
    "amule-web"
    "anakron"
    "analog"
    "analyze-build"
    "ananicy-cpp"
    "ananicy-rules-cachyos"
    "anarch"
    "anarchism"
    "anchor"
    "anda"
    "andagii"
    "andcli"
    "anders"
    "andi"
    "andika"
    "androguard"
    "android-backup-extractor"
    "android-cli"
    "android-file-transfer"
    "android-image-kitchen"
    "android-mic"
    "android-ota-payload-extractor"
    "android-studio"
    "android-studio-for-platform"
    "android-studio-full"
    "android-studio-tools"
    "android-tools"
    "android-translation-layer"
    "androidsdk"
    "andyetitmoves"
    "anewer"
    "angband"
    "angelscript"
    "angie"
    "angie-console-light"
    "angle"
    "angle-grinder"
    "angr-management"
    "angrr"
    "angryipscanner"
    "angryoxide"
    "angsd"
    "angular-language-server"
    "ani-cli"
    "ani-skip"
    "anilibria-winmaclinux"
    "animatch"
    "anime4k"
    "animeko"
    "ankacoder"
    "ankacoder-condensed"
    "ankama-launcher"
    "anker-powerconf-c200-linux-tools"
    "anki"
    "anki-bin"
    "anki-sync-server"
    "anko"
    "annapurna-sil"
    "annextimelog"
    "annot"
    "annotation-mono"
    "annotator"
    "anonymous-pro-fonts"
    "anonymousPro"
    "ansel"
    "ansi"
    "ansi2html"
    "ansible"
    "ansible-builder"
    "ansible-cmdb"
    "ansible-doctor"
    "ansible-language-server"
    "ansible-lint"
    "ansible-navigator"
    "ansifilter"
    "ansilove"
    "ant"
    "antares"
    "ante"
    "antfs-cli"
    "anthy"
    "antidote"
    "antigen"
    "antigravity"
    "antigravity-acp"
    "antigravity-cli"
    "antigravity-fhs"
    "antigravity-ide"
    "antigravity-ide-fhs"
    "antimatter-dimensions"
    "antimicrox"
    "antimony"
    "antiprism"
    "antiword"
    "antlr"
    "antlr2"
    "antlr3"
    "antlr3_4"
    "antlr3_5"
    "antlr4"
    "antlr4_10"
    "antlr4_11"
    "antlr4_12"
    "antlr4_13"
    "antlr4_9"
    "antonio-font"
    "antora"
    "antora-lunr-extension"
    "antora-ui-default"
    "ants"
    "antsimulator"
    "anttweakbar"
    "antu-icons"
    "anubis"
    "anuko-time-tracker"
    "anup"
    "anvil-editor"
    "any-nix-shell"
    "anybadge"
    "anydesk"
    "anyk"
    "anyrun"
    "anyrun-provider"
    "anystyle-cli"
    "anytone-emu"
    "anytype"
    "anytype-cli"
    "anytype-heart"
    "ao3downloader"
    "aoc-cli"
    "aocd"
    "aocl-utils"
    "aonsoku"
    "apache-airflow"
    "apache-answer"
    "apache-directory-server"
    "apache-directory-studio"
    "apache-flex-sdk"
    "apache-jena"
    "apache-jena-fuseki"
    "apache-orc"
    "apache-users"
    "apacheHttpd"
    "apacheHttpd_2_4"
    "apacheKafka"
    "apacheKafka_4_1"
    "apacheKafka_4_2"
    "apacheKafka_4_3"
    "apachetomcatscanner"
    "apbs"
    "apc-temp-fetch"
    "apcupsd"
    "ape"
    "apeClex"
    "aperture"
    "apery"
    "apfel"
    "apfel-llm"
    "apfelgrid"
    "apfs-fuse"
    "apfsprogs"
    "apftool-rs"
    "apg"
    "apgdiff"
    "aphorme"
    "api-linter"
    "apidog"
    "apio"
    "apio-udev-rules"
    "apitrace"
    "apk-tools"
    "apkeditor"
    "apkeep"
    "apkg"
    "apkid"
    "apkleaks"
    "apko"
    "apksigcopier"
    "apksigner"
    "apktool"
    "apl2741-unicode"
    "apl386"
    "apl387"
    "apm-cli"
    "apng2gif"
    "apngasm"
    "apngasm_2"
    "apngopt"
    "aporetic"
    "aporetic-bin"
    "apostrophe"
    "apotris"
    "app-icon-preview"
    "app2unit"
    "apparency"
    "apparix"
    "apparmor-bin-utils"
    "apparmor-init"
    "apparmor-pam"
    "apparmor-parser"
    "apparmor-profiles"
    "apparmor-teardown"
    "apparmor-utils"
    "appcleaner"
    "appcsxcad"
    "appdaemon"
    "appeditor"
    "appfire-cli"
    "appflowy"
    "appgate-sdp"
    "appimage-run"
    "appimage-run-tests"
    "appimageupdate"
    "appimageupdate-qt"
    "appindicator-sharp"
    "appium-inspector"
    "apple-cursor"
    "apple-sdk"
    "apple-sdk_14"
    "apple-sdk_15"
    "apple-sdk_26"
    "applesauce"
    "applewin"
    "applgrid"
    "application-title-bar"
    "appmenu-glib-translator"
    "appres"
    "apprise"
    "approxmc"
    "apprun-cli"
    "appstream"
    "appstream-glib"
    "apptainer"
    "apptainer-overriden-nixos"
    "appvm"
    "apr"
    "apriltag"
    "aprutil"
    "aprx"
    "apt"
    "apt-cacher-ng"
    "apt-dater"
    "apt-mirror"
    "apt-offline"
    "apt-swarm"
    "aptakube"
    "aptdec"
    "aptly"
    "apulse"
    "apvlv"
    "apx"
    "apx-gui"
    "aqbanking"
    "aqtinstall"
    "aquamarine"
    "aquosctl"
    "arachne-pnr"
    "aragorn"
    "arandr"
    "aranym"
    "arashi"
    "aravis"
    "arc-icon-theme"
    "arc-kde-theme"
    "arc-theme"
    "arcan"
    "arcan-all-wrapped"
    "arcan-wrapped"
    "arcanechat-tui"
    "arcanist"
    "arcdps-log-manager"
    "arch-install-scripts"
    "archimede"
    "archimedes"
    "archipelago"
    "archisteamfarm"
    "archivemount"
    "archivy"
    "archon-lite"
    "arcticons-sans"
    "ardopc"
    "ardour"
    "ardour_8"
    "ardugotools"
    "arduino"
    "arduino-ci"
    "arduino-cli"
    "arduino-core"
    "arduino-core-unwrapped"
    "arduino-create-agent"
    "arduino-ide"
    "arduino-language-server"
    "arduino-mk"
    "arduino-ota"
    "arduinoOTA"
    "arelle"
    "ares"
    "ares-cli"
    "ares-rs"
    "arftracksat"
    "argagg"
    "argbash"
    "argc"
    "argo-expr"
    "argo-rollouts"
    "argo-workflows"
    "argocd"
    "argocd-autopilot"
    "argocd-vault-plugin"
    "argon"
    "argonaut"
    "argononed"
    "argp-standalone"
    "argparse"
    "argparse-manpage"
    "argpp"
    "argtable"
    "arguments"
    "argus"
    "argus-clients"
    "argyllcms"
    "aria2"
    "ariang"
    "aribb24"
    "aribb25"
    "ario"
    "arion"
    "aritim-dark"
    "arity"
    "arj"
    "arjun"
    "arjun-cnf"
    "ark"
    "ark-pixel-font"
    "arkade"
    "arkenfox-userjs"
    "arkpandora_ttf"
    "armTrustedFirmwareAllwinner"
    "armTrustedFirmwareAllwinnerH6"
    "armTrustedFirmwareAllwinnerH616"
    "armTrustedFirmwareQemu"
    "armTrustedFirmwareRK3328"
    "armTrustedFirmwareRK3399"
    "armTrustedFirmwareRK3568"
    "armTrustedFirmwareRK3588"
    "armTrustedFirmwareS905"
    "armTrustedFirmwareTools"
    "arma3-unix-launcher"
    "armadillo"
    "armagetronad"
    "armagetronad-dedicated"
    "armbian-firmware"
    "armips"
    "armitage"
    "arnis"
    "arouteserver"
    "arp-scan"
    "arp-scan-rs"
    "arpa2-leaf"
    "arpa2cm"
    "arpa2common"
    "arpack"
    "arpack-mpi"
    "arphic-ukai"
    "arphic-uming"
    "arping"
    "arpoison"
    "arq"
    "arrow"
    "arrow-adbc"
    "arrow-cpp"
    "arrow-glib"
    "arrpc"
    "arsenal"
    "arsenik"
    "art"
    "art-standalone"
    "artalk"
    "artem"
    "arti"
    "artim-dark"
    "artisan"
    "artix-games-launcher"
    "artyFX"
    "arubaotp-seed-extractor"
    "arwen"
    "arx-libertatis"
    "arxiv-latex-cleaner"
    "as-tree"
    "as31"
    "asahi-audio"
    "asahi-bless"
    "asahi-btsync"
    "asahi-fwextract"
    "asahi-nvram"
    "asahi-wifisync"
    "asak"
    "asap"
    "asar"
    "asc"
    "asc-key-to-qr-code-gif"
    "asccli"
    "ascii"
    "ascii-draw"
    "ascii-image-converter"
    "asciicam"
    "asciidoc"
    "asciidoc-full"
    "asciidoc-full-with-plugins"
    "asciidoctor"
    "asciidoctor-with-extensions"
    "asciidoctorj"
    "asciigraph"
    "asciijump"
    "asciimol"
    "asciinema"
    "asciinema-agg"
    "asciinema-scenario"
    "asciinema_3"
    "asciiquarium"
    "asciiquarium-transparent"
    "asdbctl"
    "asdf"
    "asdf-vm"
    "asdf_2_26"
    "asdf_3_1"
    "asdf_3_3"
    "aseprite"
    "aseq2json"
    "asfa"
    "asha-pipewire-sink"
    "ashell"
    "ashpd-demo"
    "ashuffle"
    "asio"
    "asio_1_32_0"
    "asio_1_36_0"
    "askalono"
    "asl"
    "asleap"
    "asm-lsp"
    "asmc-linux"
    "asmfmt"
    "asmjit"
    "asmrepl"
    "asn"
    "asn1c"
    "asn1editor"
    "asn2quickder"
    "asnmap"
    "asouldocs"
    "aspcud"
    "aspectj"
    "aspell"
    "aspino"
    "assaultcube"
    "assemblyscript"
    "assetfinder"
    "assetripper"
    "assh"
    "assign-lb-ip"
    "assimp"
    "assyrian"
    "ast-grep"
    "astartectl"
    "astc-encoder"
    "asterisk"
    "asterisk-ldap"
    "asterisk-lts"
    "asterisk-module-sccp"
    "asterisk-stable"
    "asterisk_20"
    "asterisk_22"
    "asterisk_23"
    "astral"
    "astro-language-server"
    "astroid"
    "astrolabe-generator"
    "astrolog"
    "astromenace"
    "astroterm"
    "astyle"
    "asuka"
    "asunder"
    "asus-wmi-screenpad-ctl"
    "asusctl"
    "asymptote"
    "async-profiler"
    "asyncapi"
    "at"
    "at-spi2-atk"
    "at-spi2-core"
    "atac"
    "atanks"
    "atari800"
    "ataripp"
    "atasm"
    "atf"
    "atftp"
    "ath9k-htc-blobless-firmware"
    "ath9k-htc-blobless-firmware-unstable"
    "atheme"
    "athens"
    "atinout"
    "atk"
    "atkinson-hyperlegible"
    "atkinson-hyperlegible-mono"
    "atkinson-hyperlegible-next"
    "atkinson-monolegible"
    "atkmm"
    "atkmm_2_36"
    "atlantis"
    "atlas"
    "atlassian-cli"
    "atlassian-plugin-sdk"
    "atlauncher"
    "atmos"
    "atomgit-cli"
    "atomic-operator"
    "atomic-swap"
    "atomicparsley"
    "atomix"
    "atool"
    "atop"
    "atopile"
    "atproto-goat"
    "atril"
    "ats-acc"
    "ats2"
    "attemptoClex"
    "attic-client"
    "attic-server"
    "attr"
    "attract-mode"
    "attyx"
    "atuin"
    "atuin-desktop"
    "aube"
    "aubio"
    "aucatctl"
    "auctex"
    "audacious"
    "audacious-bare"
    "audacious-plugins"
    "audacity"
    "audacity_3"
    "audacity_4"
    "audiality2"
    "audible-cli"
    "audiness"
    "audio-mirroring"
    "audio-sharing"
    "audiobookshelf"
    "audiofile"
    "audiomatch"
    "audion"
    "audiosource"
    "audiothekar"
    "audiowaveform"
    "audit"
    "auditbeat"
    "auditbeat7"
    "auditwheel"
    "auge"
    "augeas"
    "augustus"
    "augustus-go"
    "aurea"
    "aurulent-sans"
    "ausaxs"
    "ausweisapp"
    "ausweiskopie"
    "autenticacao-gov-pt-bin"
    "auth0-cli"
    "authelia"
    "authenticator"
    "authenticodeCheckHook"
    "authentik"
    "authoscope"
    "authz0"
    "auto-changelog"
    "auto-cpufreq"
    "auto-editor"
    "auto-epp"
    "auto-multiple-choice"
    "auto-patchelf"
    "autoAddDriverRunpath"
    "autoFixElfFiles"
    "autoPatchcilHook"
    "autoPatchelfHook"
    "autobloody"
    "autobrr"
    "autobuild"
    "autoconf"
    "autoconf-archive"
    "autoconf269"
    "autocorrect"
    "autocutsel"
    "autodiff"
    "autodock-vina"
    "autoenv"
    "autoflake"
    "autofs5"
    "autogen"
    "autojump"
    "autokbisw"
    "autokey"
    "autokuma"
    "autologin"
    "automake"
    "automake116x"
    "automake118x"
    "automatic-timezoned"
    "automaticcomponenttoolkit"
    "automirror"
    "automysqlbackup"
    "autopen"
    "autoprefixer"
    "autopsy"
    "autopush-rs"
    "autoraise"
    "autorandr"
    "autoreconfHook"
    "autoreconfHook269"
    "autorestic"
    "autorevision"
    "autospotting"
    "autossh"
    "autosubst2"
    "autosuspend"
    "autotalent"
    "autotier"
    "autotiling"
    "autotiling-rs"
    "autotools-language-server"
    "autotrace"
    "autotrash"
    "av1an"
    "av1an-unwrapped"
    "avahi"
    "avahi-compat"
    "avalanche-cli"
    "avalanchego"
    "avalonia"
    "avalonia-ilspy"
    "avbroot"
    "avd-fw"
    "avdl"
    "avdump3"
    "avell-unofficial-control-center"
    "aver"
    "avfs"
    "aviator"
    "avidemux"
    "avisynthplus"
    "avizo"
    "avml"
    "avogadro2"
    "avogadrolibs"
    "avr8burnomat"
    "avra"
    "avrdude"
    "avrdudess"
    "avrlibc"
    "avro-c"
    "avro-cpp"
    "avro-tools"
    "avy"
    "aw-notify"
    "aw-qt"
    "aw-server-rust"
    "aw-watcher-afk"
    "aw-watcher-window"
    "aw-watcher-window-wayland"
    "await"
    "awakened-poe-trade"
    "awatcher"
    "awesome"
    "awesomebump"
    "awk-language-server"
    "aws-adfs"
    "aws-assume-role"
    "aws-azure-login"
    "aws-c-auth"
    "aws-c-cal"
    "aws-c-common"
    "aws-c-compression"
    "aws-c-event-stream"
    "aws-c-http"
    "aws-c-io"
    "aws-c-mqtt"
    "aws-c-s3"
    "aws-c-sdkutils"
    "aws-cdk-cli"
    "aws-checksums"
    "aws-codeartifact-proxy"
    "aws-crt-cpp"
    "aws-encryption-sdk-cli"
    "aws-gate"
    "aws-iam-authenticator"
    "aws-lambda-rie"
    "aws-lc"
    "aws-mfa"
    "aws-nuke"
    "aws-ofi-nccl"
    "aws-rotate-key"
    "aws-sam-cli"
    "aws-sdk-cpp"
    "aws-signing-helper"
    "aws-spend-summary"
    "aws-sso-cli"
    "aws-sso-creds"
    "aws-sso-util"
    "aws-vault"
    "aws-workspaces"
    "awsbck"
    "awscli"
    "awscli2"
    "awsebcli"
    "awslimitchecker"
    "awslogs"
    "awsls"
    "awsrm"
    "awstats"
    "awsume"
    "awsweeper"
    "awww"
    "ax25-apps"
    "ax25-tools"
    "ax25ms"
    "axel"
    "axis2"
    "ayatana-ido"
    "ayatana-indicator-bluetooth"
    "ayatana-indicator-datetime"
    "ayatana-indicator-display"
    "ayatana-indicator-messages"
    "ayatana-indicator-power"
    "ayatana-indicator-session"
    "ayatana-indicator-sound"
    "ayatana-webmail"
    "ayugram-desktop"
    "az-pim-cli"
    "azahar"
    "azeret-mono"
    "azimuth"
    "azmq"
    "azpainter"
    "aztfexport"
    "azuki"
    "azure-artifacts-credprovider"
    "azure-cli"
    "azure-functions-core-tools"
    "azure-mcp"
    "azure-static-sites-client"
    "azure-storage-azcopy"
    "azuredatastudio"
    "azurefox"
    "azurehound"
    "azurite"
    "b2sum"
    "b3sum"
    "b4"
    "b43-fwcutter"
    "b43FirmwareCutter"
    "b43Firmware_5_1_138"
    "b43Firmware_6_30_163_46"
    "b612"
    "babashka"
    "babashka-unwrapped"
    "babeld"
    "babeldoc"
    "babelfish"
    "babelstone-han"
    "babeltrace"
    "babeltrace2"
    "babl"
    "baboossh"
    "backblaze-b2"
    "backdown"
    "backgroundremover"
    "backintime"
    "backintime-common"
    "backintime-qt"
    "backrest"
    "backward-cpp"
    "bacnet-stack"
    "bacon"
    "bacula"
    "bada-bib"
    "badchars"
    "baddns"
    "badgemagic-app"
    "badgemagic-rs"
    "badger"
    "badkeys"
    "badlion-client"
    "badness"
    "badrobot"
    "badsecrets"
    "badvpn"
    "baekmuk-ttf"
    "bagels"
    "bagr"
    "baidupcs-go"
    "baikal"
    "bakelite"
    "bakoma_ttf"
    "balanceofsatoshis"
    "balatro"
    "balatro-mod-manager"
    "balena-cli"
    "balena-compose-parser"
    "ballerburg"
    "ballerina"
    "balls"
    "balsa"
    "bam"
    "bambootracker"
    "bambootracker-qt6"
    "bambu-studio"
    "bambuddy"
    "bamf"
    "bamtools"
    "banana-accounting"
    "banana-cursor"
    "bandcamp-collection-downloader"
    "bandcampsync"
    "bandit"
    "bandwhich"
    "bangr"
    "banjorecomp"
    "banking"
    "bankstown-lv2"
    "banner"
    "bant"
    "bao"
    "baobab"
    "bar"
    "barbicanclient"
    "barcode"
    "bareboxTools"
    "baresip"
    "bark"
    "bark-server"
    "barlow"
    "barman"
    "barracuda"
    "barrage"
    "bartender"
    "bartib"
    "basalt"
    "basalt-monado"
    "base16-schemes"
    "base16-shell-preview"
    "base16-universal-manager"
    "base16384"
    "base24-schemes"
    "base45"
    "base91"
    "basedpyright"
    "basex"
    "basez"
    "bash-completion"
    "bash-env-json"
    "bash-git-prompt"
    "bash-language-server"
    "bash-my-aws"
    "bash-pinyin-completion-rs"
    "bash-preexec"
    "bash-snippets"
    "bash-supergenpass"
    "bashFHS"
    "bashInteractive"
    "bashInteractiveFHS"
    "bashNonInteractive"
    "bashSnippets"
    "bash_unit"
    "bashate"
    "bashblog"
    "bashcards"
    "bashd"
    "bashdb"
    "bashly"
    "bashmount"
    "bashplotlib"
    "bashrun2"
    "bashunit"
    "bashup-events32"
    "bashup-events44"
    "basicswap"
    "basiliskii"
    "basilk"
    "bastet"
    "basu"
    "bat"
    "batctl"
    "batik"
    "batmon"
    "bato"
    "bats"
    "batsignal"
    "battery-toolkit"
    "batticonplus"
    "baudline"
    "bayescan"
    "bazaar"
    "bazarr"
    "bazecor"
    "bazel"
    "bazel-buildtools"
    "bazel-gazelle"
    "bazel-kazel"
    "bazel-remote"
    "bazel-watcher"
    "bazel_7"
    "bazel_8"
    "bazel_9"
    "bazelisk"
    "bb"
    "bbe"
    "bbedit"
    "bbin"
    "bbot"
    "bbrew"
    "bc"
    "bc-ur"
    "bcache-tools"
    "bcachefs-tools"
    "bcal"
    "bcc"
    "bcftools"
    "bcg729"
    "bchoppr"
    "bchunk"
    "bcompare"
    "bcu"
    "bdep"
    "bdf2psf"
    "bdf2sfd"
    "bdf2ttf"
    "bdfresize"
    "bdftopcf"
    "bdsync"
    "bdt"
    "beads"
    "beadwork"
    "beamerpresenter"
    "beamerpresenter-mupdf"
    "beamerpresenter-poppler"
    "beammp-launcher"
    "bean-add"
    "beancount"
    "beancount-black"
    "beancount-ing-diba"
    "beancount-language-server"
    "beancount-share"
    "beancount_2"
    "beanhub-cli"
    "beanprice"
    "beanquery"
    "beans"
    "beanstalkd"
    "bear"
    "bearer"
    "bearssl"
    "beatprints"
    "beats"
    "beauty-line-icon-theme"
    "beautysh"
    "bed"
    "bedops"
    "bedtools"
    "bee"
    "beebeep"
    "beecrypt"
    "beedii"
    "beefi"
    "beekeeper-studio"
    "beep"
    "beeper"
    "beeper-bridge-manager"
    "beeref"
    "bees"
    "beets"
    "beets-minimal"
    "behave"
    "behdad-fonts"
    "beidconnect"
    "bella"
    "bellepoule"
    "below"
    "beluga"
    "bemenu"
    "bemoji"
    "bench"
    "bencher-cli"
    "benchexec"
    "bencodetools"
    "bend"
    "bender"
    "bendsql"
    "beneath-a-steel-sky"
    "benhsm-minesweeper"
    "benthos"
    "bento"
    "bento4"
    "bentopdf"
    "benzene"
    "beon"
    "bepass"
    "bepasty"
    "ber_metaocaml"
    "berglas"
    "berk76-tetris"
    "berry"
    "besley"
    "bespokesynth"
    "bespokesynth-with-vst2"
    "besu"
    "beszel"
    "betaflight-configurator"
    "better-adb-sync"
    "better-commits"
    "better-control"
    "betteralign"
    "bettercap"
    "betterdiscord-installer"
    "betterdiscordctl"
    "betterdisplay"
    "betterleaks"
    "betterlockscreen"
    "betula"
    "bevelbar"
    "beyond-all-reason"
    "beyond-identity"
    "bfcal"
    "bfetch"
    "bfg-repo-cleaner"
    "bfs"
    "bfscripts"
    "bftools"
    "bftpd"
    "bgnet"
    "bgpdump"
    "bgpq3"
    "bgpq4"
    "bgs"
    "bibata-cursors"
    "bibata-cursors-translucent"
    "bibclean"
    "biber"
    "biber-for-tectonic"
    "biber-ms"
    "bibiman"
    "biblatex-check"
    "biblesync"
    "bibletime"
    "biblioteca"
    "biboumi"
    "bibtex-tidy"
    "bibtex2html"
    "bibtool"
    "bibutils"
    "bic"
    "bicep"
    "bicep-lsp"
    "bicgl"
    "bichon"
    "bicon"
    "bicpl"
    "bigloo"
    "biglybt"
    "bign-handheld-thumbnailer"
    "bigpemu"
    "bigquery-emulator"
    "bikeshed"
    "bili-live-tool"
    "biliass"
    "bilibili"
    "bilibili-tui"
    "bililiverecorder"
    "biliup-rs"
    "bin2c"
    "binary"
    "binary-object-scanner"
    "binaryen"
    "binaryninja-free"
    "binbloom"
    "bind"
    "bindfs"
    "bingo"
    "bingrep"
    "bink-player"
    "bino3d"
    "binocle"
    "binsider"
    "binsort"
    "binwalk"
    "bio-gappa"
    "bioawk"
    "biodiff"
    "biome"
    "bionic"
    "bionic-translation"
    "biosdevname"
    "bip"
    "biplanes-revival"
    "bird-lg"
    "bird2"
    "bird3"
    "birdfont"
    "birdtray"
    "birdwatcher"
    "biscuit-cli"
    "bison"
    "bisoncpp"
    "bisq1"
    "bisq2"
    "bit"
    "bit-logo"
    "bitbake-language-server"
    "bitbox"
    "bitbox-bridge"
    "bitbucket-cli"
    "bitcoin"
    "bitcoin-knots"
    "bitcoind"
    "bitcoind-knots"
    "bitcomet"
    "bitcrook"
    "bite"
    "bitfocus-companion"
    "bitlbee"
    "bitlbee-discord"
    "bitlbee-facebook"
    "bitlbee-mastodon"
    "bitlbee-steam"
    "bitmagnet"
    "bitmap"
    "bitrise"
    "bitsnpicas"
    "bitwarden-cli"
    "bitwarden-desktop"
    "bitwarden-directory-connector"
    "bitwarden-directory-connector-cli"
    "bitwarden-menu"
    "bitwig-studio"
    "bitwig-studio3"
    "bitwig-studio4"
    "bitwig-studio5"
    "bitwig-studio5-unwrapped"
    "bitwig-studio6"
    "bitwise"
    "bitwuzla"
    "biwascheme"
    "biz-ud-gothic"
    "bjumblr"
    "bk"
    "bk_rom_compressor"
    "bkcrack"
    "bklk"
    "bkt"
    "black"
    "black-hole-solver"
    "black-macchiato"
    "blackbox-terminal"
    "blacken-docs"
    "blackfire"
    "blackhole"
    "blackmagic"
    "blackmagic-desktop-video"
    "blackout"
    "blackshades"
    "blackvoxel"
    "blade-formatter"
    "blahaj"
    "blahtexml"
    "blanket"
    "blas"
    "blas-ilp64"
    "blasfeo"
    "blast"
    "blast-bin"
    "blastem"
    "blaze"
    "blazesym-c"
    "blazingjj"
    "ble-serial"
    "bleachbit"
    "bleep"
    "blend2d"
    "blender"
    "blendfarm"
    "blesh"
    "bleur"
    "blflash"
    "blightmud"
    "blightmud-tts"
    "blink"
    "blink-qt"
    "blink1-tool"
    "blint"
    "blis"
    "blisp"
    "bliss"
    "blitz"
    "blivet-gui"
    "blktrace"
    "bloat"
    "bloaty"
    "blobby"
    "blobdrop"
    "blobfuse"
    "blobs_gg"
    "blobtools"
    "blobwars"
    "blockattack"
    "blockbench"
    "blockbook"
    "blockdiag"
    "blockhash"
    "blocksat-cli"
    "blockstream-electrs"
    "blocky"
    "blogc"
    "bloodhound"
    "bloodhound-ce"
    "bloodhound-py"
    "bloodspilot-client"
    "bloodspilot-server"
    "bloomrpc"
    "bloop"
    "blowfish-tools"
    "blst"
    "blucontrol"
    "bluebubbles"
    "bluefish"
    "bluej"
    "bluejay"
    "bluemail"
    "blueman"
    "bluemap"
    "bluemix-cli"
    "blueprint-compiler"
    "bluesky-pds"
    "bluesky-pdsadmin"
    "bluesnarfer"
    "bluesnooze"
    "bluespec"
    "bluetility"
    "bluetooth_battery"
    "bluetui"
    "bluetuith"
    "blueutil"
    "bluewalker"
    "bluez"
    "bluez-alsa"
    "bluez-experimental"
    "bluez-headers"
    "bluez-tools"
    "bluez5"
    "bluez5-experimental"
    "blugon"
    "blur-effect"
    "bmake"
    "bmaptool"
    "bmc-tools"
    "bmm"
    "bmon"
    "bmputil"
    "bmrsa"
    "bms-to-osu"
    "bngblaster"
    "boa"
    "boat-cli"
    "boatswain"
    "bob-nvim"
    "bobby"
    "bobcat"
    "bobgen"
    "boca"
    "bochs"
    "bodoni-moda"
    "boehmgc"
    "bogofilter"
    "bogofilter-db"
    "bogofilter-sqlite"
    "boilr"
    "boinc"
    "boinc-headless"
    "boinctui"
    "bold"
    "bolliedelayxt-lv2"
    "bolt"
    "bolt-launcher"
    "bolt_19"
    "bolt_20"
    "bolt_21"
    "bolt_22"
    "boltbrowser"
    "bom"
    "bombadillo"
    "bombardier"
    "bomber-go"
    "bombsquad"
    "bomly"
    "bomutils"
    "bonk"
    "bonmin"
    "bonnie"
    "bonnmotion"
    "bonsai"
    "bonzomatic"
    "boofuzz"
    "boogie"
    "boohu"
    "book-summary"
    "bookorbit"
    "bookstack"
    "bookup"
    "bookworm"
    "boolector"
    "boolstuff"
    "boomerang"
    "boops"
    "boost"
    "boost-build"
    "boost-sml"
    "boost178"
    "boost179"
    "boost180"
    "boost181"
    "boost182"
    "boost183"
    "boost186"
    "boost187"
    "boost188"
    "boost189"
    "boost190"
    "boost191"
    "booster"
    "boot"
    "bootc"
    "bootdev-cli"
    "bootil"
    "bootiso"
    "bootloadhid"
    "bootmac"
    "bootspec"
    "bootspec-lix"
    "bootstrap-studio"
    "bootterm"
    "bore"
    "bore-cli"
    "borealis-cursors"
    "borg-sans-mono"
    "borgbackup"
    "borgmatic"
    "boring"
    "boringssl"
    "boringtun"
    "bork"
    "boron"
    "bosh-cli"
    "bossa"
    "bossa-arduino"
    "botan3"
    "botanEsdm"
    "bottles"
    "bottles-unwrapped"
    "bottom"
    "bottom-rs"
    "boulder"
    "boundary"
    "bounty"
    "bowtie"
    "bowtie2"
    "box-cli"
    "box2d"
    "box2d_2"
    "box64"
    "box86"
    "boxbuddy"
    "boxed-cpp"
    "boxes"
    "boxflat"
    "boxfort"
    "boxxy"
    "bozohttpd"
    "bozohttpd-minimal"
    "bpf-linker"
    "bpfilter"
    "bpflinter"
    "bpfmon"
    "bpftools"
    "bpftop"
    "bpftrace"
    "bpftune"
    "bpfvet"
    "bpkg"
    "bpm-tools"
    "bpp-core"
    "bpp-phyl"
    "bpp-popgen"
    "bpp-seq"
    "bppsuite"
    "bqn"
    "bqn386"
    "braa"
    "braincurses"
    "brainflow"
    "brakeman"
    "brasero"
    "brasero-unwrapped"
    "brave"
    "brave-origin"
    "brave-search-cli"
    "breakpad"
    "breakpointHook"
    "breakpointHookCntr"
    "breaktimer"
    "breeze-hacked-cursor-theme"
    "breezy"
    "breitbandmessung"
    "breseq"
    "brev-cli"
    "brewtarget"
    "brgenml1cupswrapper"
    "brgenml1lpr"
    "brial"
    "briar-desktop"
    "brickd"
    "brickstore"
    "bridge-utils"
    "brig"
    "brightnessctl"
    "brill"
    "brillo"
    "brioche"
    "briss"
    "bristol"
    "brlaser"
    "brlcad"
    "brltty"
    "brmodelo"
    "broadcast-box"
    "broadcom-bt-firmware"
    "broadlink-cli"
    "brogue"
    "brogue-ce"
    "broken-sword-25"
    "bront_fonts"
    "brook"
    "broom"
    "broot"
    "brotab"
    "brotato"
    "brotli"
    "browser-sync"
    "browserpass"
    "browsers"
    "browsh"
    "browsr"
    "brscan4"
    "brscan5"
    "bruijn"
    "bruno"
    "bruno-cli"
    "brunsli"
    "brush"
    "brush-splat"
    "brutalmaze"
    "brutefir"
    "bruteforce-luks"
    "bruteforce-salted-openssl"
    "bruteforce-wallet"
    "brutespray"
    "brutus"
    "brygada-1918"
    "bs-manager"
    "bs2b-lv2"
    "bsc"
    "bschaffl"
    "bsd-finger"
    "bsd-fingerd"
    "bsdSetupHook"
    "bsdbuild"
    "bsdgames"
    "bsdiff"
    "bsequencer"
    "bsh"
    "bshapr"
    "bsky-cli"
    "bslizr"
    "bsnes-hd"
    "bsp-layout"
    "bspwm"
    "bstone"
    "bstring"
    "bsync"
    "bt-fw-converter"
    "bt-migrate"
    "btar"
    "btc-rpc-explorer"
    "btcd"
    "btcdeb"
    "btcli"
    "btcpayserver"
    "btcpayserver-altcoins"
    "btdu"
    "btest"
    "btfdump"
    "btfs"
    "btlejack"
    "btop"
    "btop-cuda"
    "btop-rocm"
    "btor2tools"
    "btrbk"
    "btrfs-assistant"
    "btrfs-auto-snapshot"
    "btrfs-heatmap"
    "btrfs-list"
    "btrfs-progs"
    "btrfs-snap"
    "btrpa-scan"
    "btrsync"
    "bttf"
    "bubblemail"
    "bubblewrap"
    "buck2"
    "buckets"
    "buckle"
    "bucklespring"
    "bucklespring-libinput"
    "bucklespring-x11"
    "buddy"
    "budget-tracker-tui"
    "budgie-analogue-clock-applet"
    "budgie-backgrounds"
    "budgie-control-center"
    "budgie-desktop"
    "budgie-desktop-services"
    "budgie-desktop-view"
    "budgie-desktop-with-plugins"
    "budgie-gsettings-overrides"
    "budgie-media-player-applet"
    "budgie-session"
    "budgie-systemmonitor-applet"
    "budgie-user-indicator-redux"
    "buf"
    "buffer"
    "buffrs"
    "buffybox"
    "bufisk"
    "bugdom"
    "bugstalker"
    "build2"
    "build2-bootstrap"
    "buildah"
    "buildah-unwrapped"
    "buildbot"
    "buildbot-full"
    "buildbot-ui"
    "buildbot-worker"
    "buildbox"
    "buildcatrust"
    "buildifier"
    "buildkit"
    "buildkit-nix"
    "buildkite-agent"
    "buildkite-agent-metrics"
    "buildkite-cli"
    "buildkite-test-collector-rust"
    "buildozer"
    "buildpack"
    "buildstream"
    "buildtorrent"
    "buku"
    "bukubrow"
    "bulk_extractor"
    "bulky"
    "bullet"
    "bulletml"
    "bulletty"
    "bulloak"
    "bullshit"
    "bully"
    "bulwark"
    "bumblebee"
    "bumblebee-status"
    "bump"
    "bump-my-version"
    "bump2version"
    "bumpp"
    "bumpver"
    "bun"
    "bunbun"
    "bundix"
    "bundler"
    "bundler-audit"
    "bundletool"
    "bundlewrap"
    "bunnyfetch"
    "bunster"
    "bunyan-rs"
    "bup"
    "bupc"
    "bupstash"
    "burn-central-cli"
    "burp"
    "burpsuite"
    "burstui"
    "bustle"
    "bustools"
    "busybox"
    "busybox-sandbox-shell"
    "butane"
    "buteo-syncfw"
    "butler"
    "butt"
    "buttercup-desktop"
    "butterfly"
    "buttermanager"
    "buttplug-lite"
    "buttplug-mcp"
    "buzz-cli"
    "buzztrax"
    "bvi"
    "bvibratr"
    "bviplus"
    "bwa"
    "bwa-mem2"
    "bwbasic"
    "bwm_ng"
    "bws"
    "byacc"
    "byedpi"
    "byobu"
    "bypass-url-parser"
    "bytecode-viewer"
    "byzanz"
    "bzflag"
    "bzip3"
    "bzmenu"
    "c-ares"
    "c-aresMinimal"
    "c-ascii-render"
    "c-blosc"
    "c-blosc2"
    "c-for-go"
    "c-graph"
    "c-intro-and-ref"
    "c-periphery"
    "c-siphash"
    "c-stdaux"
    "c2000-cgt"
    "c2ffi"
    "c2fmzq"
    "c2nim"
    "c2patool"
    "c3-lsp"
    "c3c"
    "c3d"
    "c64-debugger"
    "c99sh"
    "cabal-cli"
    "cabal-install"
    "cabal2nix"
    "cabal2nix-unwrapped"
    "cabextract"
    "cabin"
    "cabinpkg"
    "cables"
    "cacert"
    "cached-nix-shell"
    "cachefilesd"
    "cachix"
    "cadabra2"
    "cadaver"
    "caddy"
    "cadical"
    "cadical_2"
    "cadre"
    "cadvisor"
    "cadzinho"
    "caelestia-cli"
    "caelestia-shell"
    "caeml"
    "caerbannog"
    "caesura"
    "caf"
    "caffe"
    "caffeine"
    "caffeine-ng"
    "cage"
    "cagebreak"
    "cagent"
    "cai"
    "caido"
    "caido-cli"
    "caido-desktop"
    "cairo"
    "cairo-lang"
    "cairomm_1_0"
    "cairomm_1_16"
    "cairosvg"
    "caja"
    "caja-dropbox"
    "caja-extensions"
    "caja-with-extensions"
    "cakelisp"
    "caladea"
    "calamares"
    "calamares-nixos"
    "calamares-nixos-extensions"
    "calaos_installer"
    "calc"
    "calceph"
    "calcmysky"
    "calcoo"
    "calculix-ccx"
    "calcure"
    "calcurse"
    "calendar-cli"
    "calf"
    "calibre"
    "calibre-no-speech"
    "calibre-web"
    "calico-apiserver"
    "calico-app-policy"
    "calico-cni-plugin"
    "calico-kube-controllers"
    "calico-pod2daemon"
    "calico-typha"
    "calicoctl"
    "caligula"
    "callaudiod"
    "calligraphy"
    "calligraplan"
    "calls"
    "calyx-vpn"
    "cambalache"
    "cambia"
    "cambrinary"
    "camera-streamer"
    "cameractrls"
    "cameractrls-gtk3"
    "cameractrls-gtk4"
    "cameradar"
    "camilladsp"
    "camingo-code"
    "camset"
    "camunda-modeler"
    "can-utils"
    "canaille"
    "canard"
    "candle"
    "candy-icons"
    "caneda"
    "canfigger"
    "cannelloni"
    "cano"
    "canokey-qemu"
    "canokey-usbip"
    "canon-capt"
    "canon-cups-ufr2"
    "cansina"
    "cantarell-fonts"
    "cantata"
    "canto-curses"
    "canto-daemon"
    "cantoolz"
    "cantus"
    "capa"
    "capacities"
    "caper"
    "capitaine-cursors"
    "capitaine-cursors-themed"
    "capnproto"
    "capnproto-java"
    "capnproto-rust"
    "caprice32"
    "caprine"
    "caprine-bin"
    "caps"
    "capslock"
    "capstone"
    "capstone_4"
    "captive-browser"
    "capture"
    "capypdf"
    "carapace"
    "carapace-bridge"
    "carbon-now-cli"
    "carburetor"
    "carddav-util"
    "cardimpose"
    "cardinal"
    "cardo"
    "cardpeek"
    "cardwire"
    "cargo"
    "cargo-3ds"
    "cargo-about"
    "cargo-all-features"
    "cargo-ament-build"
    "cargo-aoc"
    "cargo-apk"
    "cargo-arc"
    "cargo-audit"
    "cargo-auditable"
    "cargo-auditable-cargo-wrapper"
    "cargo-autoinherit"
    "cargo-bazel"
    "cargo-benchcmp"
    "cargo-binstall"
    "cargo-binutils"
    "cargo-bisect-rustc"
    "cargo-bitbake"
    "cargo-bloat"
    "cargo-bolero"
    "cargo-bootimage"
    "cargo-bump"
    "cargo-bundle"
    "cargo-bundle-licenses"
    "cargo-c"
    "cargo-cache"
    "cargo-careful"
    "cargo-chef"
    "cargo-clean-recursive"
    "cargo-clone"
    "cargo-codspeed"
    "cargo-component"
    "cargo-cooldown"
    "cargo-cranky"
    "cargo-crev"
    "cargo-criterion"
    "cargo-cross"
    "cargo-cyclonedx"
    "cargo-deadlinks"
    "cargo-deb"
    "cargo-deny"
    "cargo-depgraph"
    "cargo-dephell"
    "cargo-diet"
    "cargo-dist"
    "cargo-docset"
    "cargo-duplicates"
    "cargo-edit"
    "cargo-embassy"
    "cargo-espmonitor"
    "cargo-expand"
    "cargo-feature"
    "cargo-feature-combinations"
    "cargo-features-manager"
    "cargo-flamegraph"
    "cargo-fund"
    "cargo-fuzz"
    "cargo-geiger"
    "cargo-generate"
    "cargo-gra"
    "cargo-group-imports"
    "cargo-guppy"
    "cargo-hack"
    "cargo-hakari"
    "cargo-hf2"
    "cargo-i18n"
    "cargo-info"
    "cargo-insta"
    "cargo-lambda"
    "cargo-leptos"
    "cargo-license"
    "cargo-limit"
    "cargo-llvm-cov"
    "cargo-llvm-lines"
    "cargo-local-registry"
    "cargo-lock"
    "cargo-machete"
    "cargo-make"
    "cargo-mobile2"
    "cargo-modules"
    "cargo-mommy"
    "cargo-msrv"
    "cargo-mutants"
    "cargo-ndk"
    "cargo-nextest"
    "cargo-outdated"
    "cargo-pgo"
    "cargo-pgrx"
    "cargo-pgrx_0_16_0"
    "cargo-pgrx_0_16_1"
    "cargo-pgrx_0_17_0"
    "cargo-pgrx_0_18_0"
    "cargo-pio"
    "cargo-play"
    "cargo-plumbing"
    "cargo-preflight"
    "cargo-profiler"
    "cargo-psp"
    "cargo-public-api"
    "cargo-rail"
    "cargo-rdme"
    "cargo-readme"
    "cargo-release"
    "cargo-risczero"
    "cargo-rr"
    "cargo-run-bin"
    "cargo-sbom"
    "cargo-seek"
    "cargo-semver-checks"
    "cargo-shear"
    "cargo-show-asm"
    "cargo-shuttle"
    "cargo-sonar"
    "cargo-sort"
    "cargo-sort-derives"
    "cargo-spellcheck"
    "cargo-supply-chain"
    "cargo-sweep"
    "cargo-swift"
    "cargo-tally"
    "cargo-tarpaulin"
    "cargo-tauri"
    "cargo-temp"
    "cargo-toml-lint"
    "cargo-typify"
    "cargo-udeps"
    "cargo-ui"
    "cargo-unfmt"
    "cargo-unused-features"
    "cargo-update"
    "cargo-v5"
    "cargo-valgrind"
    "cargo-vet"
    "cargo-vibe"
    "cargo-wasi"
    "cargo-watch"
    "cargo-whatfeatures"
    "cargo-wipe"
    "cargo-wizard"
    "cargo-workspaces"
    "cargo-xbuild"
    "cargo-xwin"
    "cargo-zigbuild"
    "cargo2junit"
    "caribou"
    "cariddi"
    "carl"
    "carla"
    "carlito"
    "caroline"
    "carps-cups"
    "cartero"
    "carto"
    "cartridges"
    "casacore"
    "casacpp"
    "casadi"
    "cascadia-code"
    "casilda"
    "cask-server"
    "cassandra"
    "cassandra-cpp-driver"
    "cassandra_4"
    "cassette"
    "cassowary"
    "castero"
    "castget"
    "castnow"
    "castopod"
    "castor"
    "castsponsorskip"
    "castty"
    "castxml"
    "casync"
    "cat9"
    "cat9-wrapped"
    "cataclysm-dda"
    "cataclysm-dda-git"
    "catalyst"
    "catamaran"
    "catatonit"
    "catboost"
    "catbox"
    "catch2"
    "catch2_3"
    "catclock"
    "catdoc"
    "catdocx"
    "catdvi"
    "catfish"
    "catgirl"
    "catgirldownloader"
    "catimg"
    "catnest"
    "catnip"
    "cato-client"
    "catppuccin"
    "catppuccin-catwalk"
    "catppuccin-cursors"
    "catppuccin-discord"
    "catppuccin-fcitx5"
    "catppuccin-grub"
    "catppuccin-gtk"
    "catppuccin-hyprland"
    "catppuccin-kde"
    "catppuccin-kvantum"
    "catppuccin-papirus-folders"
    "catppuccin-plymouth"
    "catppuccin-qt5ct"
    "catppuccin-sddm"
    "catppuccin-sddm-corners"
    "catppuccin-whiskers"
    "catppuccinifier-cli"
    "catppuccinifier-gui"
    "catt"
    "caudec"
    "cauwugo"
    "cava"
    "cavalcade"
    "cavalier"
    "cavasik"
    "cavif"
    "cayley"
    "cbatticon"
    "cbc"
    "cbconvert"
    "cbconvert-gui"
    "cbeams"
    "cbfmt"
    "cbfstool"
    "cbftp"
    "cbmbasic"
    "cbmc"
    "cbmc-viewer"
    "cbmem"
    "cbmp"
    "cbom"
    "cbonsai"
    "cbor-diag"
    "cbqn"
    "cbqn-replxx"
    "cbqn-standalone"
    "cbqn-standalone-replxx"
    "cbrowser"
    "cbtemulator"
    "cc-switch"
    "cc-switch-cli"
    "cc-token"
    "cc-tool"
    "cc1541"
    "cc2538-bsl"
    "cc65"
    "ccache"
    "ccal"
    "cccc"
    "ccd2iso"
    "ccemux"
    "ccextractor"
    "ccid"
    "ccl"
    "ccls"
    "ccmeter"
    "cconv"
    "ccrtp"
    "ccrypt"
    "ccsymbols"
    "cctag"
    "cctv-viewer"
    "cctz"
    "ccusage"
    "ccze"
    "cd-hit"
    "cdb"
    "cddiscid"
    "cddl"
    "cddlc"
    "cddlib"
    "cdecl"
    "cdecl-blocks"
    "cdecrypt"
    "cdemu-client"
    "cdemu-daemon"
    "cdesktopenv"
    "cdi2iso"
    "cdimgtools"
    "cdist"
    "cdk"
    "cdk-go"
    "cdk8s-cli"
    "cdktf-cli"
    "cdktn-cli"
    "cdncheck"
    "cdo"
    "cdogs-sdl"
    "cdparanoia"
    "cdparanoia-iii"
    "cdparanoiaIII"
    "cdpr"
    "cdrdao"
    "cdrkit"
    "cdrtools"
    "cdxgen"
    "cdxj-indexer"
    "cedar"
    "ceedling"
    "cef-binary"
    "cegui"
    "cel-go"
    "celeste"
    "celeste-classic"
    "celeste-classic-pm"
    "celeste64"
    "celestegame"
    "celestia"
    "celf"
    "cell"
    "celluloid"
    "celt"
    "celt_0_5_1"
    "celt_0_7"
    "cemu"
    "cemu-ti"
    "cen64"
    "censor"
    "cent"
    "centerpiece"
    "centrifugo"
    "ceph"
    "ceph-client"
    "ceph-csi"
    "ceph-dev"
    "ceptre"
    "cerbero"
    "cerberus"
    "cerca"
    "cereal"
    "ceres-solver"
    "cernlib"
    "cero"
    "cert-viewer"
    "cert-x-gen"
    "certbot"
    "certbot-full"
    "certdump"
    "certgraph"
    "certi"
    "certificate-ripper"
    "certigo"
    "certinfo"
    "certinfo-go"
    "certipy"
    "certmgr"
    "certmitm"
    "certspotter"
    "certstrap"
    "certstream-server-go"
    "certsync"
    "cewl"
    "cewler"
    "cf-hero"
    "cf-terraforming"
    "cf-tool"
    "cf-vault"
    "cfclient"
    "cfdg"
    "cfdyndns"
    "cffconvert"
    "cfitsio"
    "cflow"
    "cfm"
    "cfn-changeset-viewer"
    "cfn-nag"
    "cfonts"
    "cfr"
    "cfripper"
    "cfs-zen-tweaks"
    "cfspeedtest"
    "cfssl"
    "cfv"
    "cg3"
    "cgal"
    "cgal_5"
    "cgdb"
    "cgif"
    "cgiserver"
    "cgit"
    "cgl"
    "cglm"
    "cgminer"
    "cgns"
    "cgoban"
    "cgreen"
    "cgt-calc"
    "cgtcalc"
    "cgterm"
    "cgui"
    "ch341eeprom"
    "chafa"
    "chain-bench"
    "chainsaw"
    "chalk"
    "chamber"
    "chameleon-cli"
    "chameleonultragui"
    "chameleos"
    "chance"
    "changedetection-io"
    "changelog-d"
    "changelogen"
    "changelogger"
    "changelogging"
    "changetower"
    "changie"
    "chaos"
    "charasay"
    "charge-lnd"
    "charis"
    "charles"
    "charles3"
    "charles4"
    "charles5"
    "charliecloud"
    "charls"
    "charm"
    "charm-freeze"
    "chars"
    "chart-testing"
    "chase"
    "chatblade"
    "chatd"
    "chatgpt"
    "chatgpt-classic"
    "chatgpt-cli"
    "chatgpt-shell-cli"
    "chatterino2"
    "chatterino7"
    "chatty"
    "chatzone-desktop"
    "chawan"
    "chayang"
    "chcase"
    "cheat"
    "cheating-daddy"
    "check"
    "check-if-email-exists"
    "check-jsonschema"
    "check-sieve"
    "check50"
    "checkPhaseThreadLimitHook"
    "checkbashisms"
    "checkinstall"
    "checkip"
    "checkmake"
    "checkmate"
    "checkov"
    "checkpolicy"
    "checkpwn"
    "checkra1n"
    "checksec"
    "checkstyle"
    "cheese"
    "cheesecutter"
    "chef-cli"
    "chelf"
    "chemacs2"
    "cherry"
    "cherry-studio"
    "cherrybomb"
    "cherrytree"
    "chess-clock"
    "chess-tui"
    "chessdb"
    "chessx"
    "chewing-editor"
    "chez"
    "chez-matchable"
    "chez-mit"
    "chez-racket"
    "chez-scmutils"
    "chez-srfi"
    "chezmoi"
    "chglog"
    "chhoto-url"
    "chiaki"
    "chiaki-ng"
    "chibi"
    "chicago95"
    "chicken"
    "chipmunk"
    "chipsec"
    "chiptrack"
    "chiri"
    "chirp"
    "chirpstack-concentratord"
    "chirpstack-fuota-server"
    "chirpstack-gateway-bridge"
    "chirpstack-gateway-mesh"
    "chirpstack-mqtt-forwarder"
    "chirpstack-packet-multiplexer"
    "chirpstack-rest-api"
    "chirpstack-udp-forwarder"
    "chisel"
    "chkbit"
    "chkcrontab"
    "chmlib"
    "chntpw"
    "chocolate-doom"
    "cholmod-extra"
    "chonburi-font"
    "choose"
    "choose-gui"
    "choparp"
    "chopchop"
    "chow-centaur"
    "chow-kick"
    "chow-phaser"
    "chow-tape-model"
    "chroma"
    "chromaprint"
    "chrome-export"
    "chrome-pak-customizer"
    "chrome-token-signing"
    "chromedriver"
    "chromium"
    "chromium-bsu"
    "chromium-hsts-preload-list"
    "chroncal"
    "chrony"
    "chrpath"
    "chruby"
    "chruby-fish"
    "chrysalis"
    "chsrc"
    "cht-sh"
    "chuck"
    "chuffed"
    "chunk"
    "chunkfs"
    "chunksync"
    "ciano"
    "ciao"
    "cicero-tui"
    "cider-2"
    "ciderpress2"
    "cidr-merger"
    "cidrgrep"
    "cie-middleware-linux"
    "ciel"
    "cifs-utils"
    "ciftilib"
    "cilium-cli"
    "cimg"
    "cine"
    "cinelerra"
    "cinnamon"
    "cinnamon-common"
    "cinnamon-control-center"
    "cinnamon-desktop"
    "cinnamon-gsettings-overrides"
    "cinnamon-menus"
    "cinnamon-screensaver"
    "cinnamon-session"
    "cinnamon-settings-daemon"
    "cinnamon-translations"
    "cinny"
    "cinny-desktop"
    "cinny-unwrapped"
    "cinzel"
    "cipher"
    "circle-flags"
    "circleci-cli"
    "circom"
    "circt"
    "circumflex"
    "circup"
    "circus"
    "cirrus-cli"
    "cirrusgo"
    "cisco-packet-tracer_9"
    "ciscoPacketTracer9"
    "cista"
    "citations"
    "citrix-workspace"
    "citrix_workspace"
    "citrix_workspace_26_04_0"
    "citron"
    "civetweb"
    "civicrm"
    "civo"
    "cjdns"
    "cjdns-tools"
    "cjose"
    "cjs"
    "cjson"
    "ckan"
    "ckb-next"
    "ckbcomp"
    "ckdl"
    "cksfv"
    "cl"
    "cl-launch"
    "cl-wordle"
    "clac"
    "clair"
    "clairvoyance"
    "clairvoyant"
    "clamav"
    "clamtk"
    "clang-analyzer"
    "clang-manpages"
    "clang-tidy-sarif"
    "clang-tools"
    "clang-uml"
    "clangbuildanalyzer"
    "clanlib"
    "clap"
    "clapboard"
    "clapgrep"
    "clapper"
    "clapper-enhancers"
    "clapper-unwrapped"
    "clarity-city"
    "clash-meta"
    "clash-nyanpasu"
    "clash-rs"
    "clash-verge-rev"
    "clashtui"
    "clasp-common-lisp"
    "classicube"
    "clatd"
    "claude-agent-acp"
    "claude-code"
    "claude-code-acp"
    "claude-code-bin"
    "claude-code-router"
    "claude-mergetool"
    "claude-monitor"
    "claude-powerline"
    "claude-usage-tracker"
    "claws"
    "claws-mail"
    "clazy"
    "clblas"
    "clblast"
    "clboss"
    "cld2"
    "cldr-annotations"
    "clean"
    "clean-css-cli"
    "cleanit"
    "clearlooks-phenix"
    "clearly-u"
    "clearlyU"
    "clematis"
    "clementine"
    "clerk"
    "clever-tools"
    "clevercsv"
    "clevis"
    "clex"
    "clfft"
    "clhep"
    "cli-tips"
    "cli11"
    "cli50"
    "cli53"
    "cliairplay"
    "cliam"
    "cliamp"
    "clib"
    "click"
    "clickable"
    "clickclack"
    "clickgen"
    "clickhouse"
    "clickhouse-backup"
    "clickhouse-cli"
    "clickhouse-cpp"
    "clickhouse-lts"
    "clickup"
    "client-ip-echo"
    "cliflux"
    "clifm"
    "clight"
    "clight-gui"
    "clightd"
    "clightning"
    "clinfo"
    "cling"
    "clingcon"
    "clingo"
    "clini"
    "clipaste"
    "clipboard-jh"
    "clipcat"
    "cliphist"
    "clipman"
    "clipmenu"
    "clipnotify"
    "clipp"
    "clipper"
    "clipper2"
    "clippy"
    "clippy-copy"
    "clippy-sarif"
    "clipqr"
    "clips"
    "clipse"
    "clipse-x11"
    "clipster"
    "cliqr"
    "cliquer"
    "cliscord"
    "clisp"
    "clive"
    "clj-kondo"
    "cljfmt"
    "cljstyle"
    "clmagma"
    "cln"
    "cloak"
    "cloak-pt"
    "cloc"
    "clock-rs"
    "clockify"
    "clog-cli"
    "clojure"
    "clojure-lsp"
    "clojure-mcp-light"
    "clolcat"
    "clonehero"
    "cloneit"
    "clooj"
    "closurecompiler"
    "cloud-custodian"
    "cloud-hypervisor"
    "cloud-init"
    "cloud-nuke"
    "cloud-provider-kind"
    "cloud-utils"
    "cloudbrute"
    "cloudcheck"
    "cloudcompare"
    "clouddrive2"
    "cloudflare-cli"
    "cloudflare-ddns"
    "cloudflare-dynamic-dns"
    "cloudflare-dyndns"
    "cloudflare-speed-cli"
    "cloudflare-utils"
    "cloudflare-warp"
    "cloudflared"
    "cloudfoundry-cli"
    "cloudfox"
    "cloudgoat"
    "cloudhunter"
    "cloudlens"
    "cloudlist"
    "cloudlog"
    "cloudmonkey"
    "cloudprober"
    "cloudquery"
    "cloudrecon"
    "cloudsmith-cli"
    "clp"
    "clpeak"
    "clprover"
    "clps2c-compiler"
    "clucene-core"
    "clucene-core_2"
    "clucene_core"
    "clucene_core_2"
    "clustal-omega"
    "clusterctl"
    "clusterlint"
    "clusternet"
    "clutter"
    "clutter-gst"
    "clutter-gtk"
    "clyp"
    "clzip"
    "cm256cc"
    "cm_unicode"
    "cmake"
    "cmake-format"
    "cmake-language-server"
    "cmake-lint"
    "cmakeCurses"
    "cmakeMinimal"
    "cmakeWithGui"
    "cmakerc"
    "cmark"
    "cmark-gfm"
    "cmatrix"
    "cmc"
    "cmctl"
    "cmd-polkit"
    "cmd-wrapped"
    "cmdstan"
    "cmigemo"
    "cminpack"
    "cmocka"
    "cmospwd"
    "cmph"
    "cmrt"
    "cmst"
    "cmt"
    "cmucl_binary"
    "cmus"
    "cmusfm"
    "cmux"
    "cni"
    "cni-plugin-flannel"
    "cni-plugins"
    "cnijfilter2"
    "cnijfilter_2_80"
    "cnijfilter_4_00"
    "cnping"
    "cnpypp"
    "cnquery"
    "cns11643-kai"
    "cnspec"
    "cnsprcy"
    "cnstrokeorder"
    "cntb"
    "cntlm"
    "cntr"
    "coal"
    "coan"
    "cobalt"
    "cobang"
    "cobra-cli"
    "coc-basedpyright"
    "coc-clangd"
    "coc-css"
    "coc-diagnostic"
    "coc-docker"
    "coc-emmet"
    "coc-eslint"
    "coc-explorer"
    "coc-flutter"
    "coc-git"
    "coc-haxe"
    "coc-highlight"
    "coc-html"
    "coc-java"
    "coc-jest"
    "coc-json"
    "coc-lists"
    "coc-markdownlint"
    "coc-nginx"
    "coc-pairs"
    "coc-prettier"
    "coc-pyright"
    "coc-r-lsp"
    "coc-rust-analyzer"
    "coc-sh"
    "coc-smartf"
    "coc-snippets"
    "coc-solargraph"
    "coc-spell-checker"
    "coc-sqlfluff"
    "coc-stylelint"
    "coc-tabnine"
    "coc-texlab"
    "coc-toml"
    "coc-vimlsp"
    "coc-vimtex"
    "coc-wxml"
    "coc-yaml"
    "coc-yank"
    "coccinelle"
    "cockatrice"
    "cockpit"
    "cockpit-dockermanager"
    "cockpit-files"
    "cockpit-machines"
    "cockpit-podman"
    "cockpit-zfs"
    "cockroachdb"
    "cocoapods"
    "cocoapods-beta"
    "cocogitto"
    "cocom"
    "cocom-tool-set"
    "coconut"
    "coconutbattery"
    "cocoon"
    "cod"
    "codd"
    "code"
    "code-cursor"
    "code-cursor-fhs"
    "code-maat"
    "code-minimap"
    "code-nautilus"
    "code-server"
    "code-theme-converter"
    "code2prompt"
    "codebase-memory-mcp"
    "codeberg-cli"
    "codeberg-pages"
    "codeblocks"
    "codeblocksFull"
    "codebook"
    "codebraid"
    "codebuff"
    "codec2"
    "codechecker"
    "codecov-cli"
    "codecrafters-cli"
    "codecserver"
    "codefresh"
    "codegrab"
    "codegraph"
    "codeium"
    "codemov"
    "codeowners"
    "codeql"
    "coder"
    "codesearch"
    "codesnap"
    "codespell"
    "codespelunker"
    "codevis"
    "codex"
    "codex-acp"
    "codex-relay"
    "codexbar"
    "codipack"
    "codon"
    "codux"
    "coercer"
    "coeurl"
    "coffeegrindsize"
    "coffeescript"
    "cog"
    "cogl"
    "cohomcalg"
    "coin-utils"
    "coin3d"
    "coinlive"
    "coinmp"
    "cointop"
    "colcon"
    "coldsnap"
    "colemak-dh"
    "colibri2"
    "colima"
    "collabora-desktop"
    "collabora-online"
    "collada-dom"
    "collapseos-cvm"
    "collectd"
    "collectd-data"
    "collectl"
    "collector"
    "collision"
    "colloid-cursors"
    "colloid-gtk-theme"
    "colloid-icon-theme"
    "colm"
    "colmap"
    "colmapWithCuda"
    "colmena"
    "colo"
    "colobot"
    "coloquinte"
    "color-lsp"
    "color-transformation-language"
    "colord"
    "colord-gtk"
    "colord-gtk4"
    "colordiff"
    "coloredlogs"
    "colorfuldarkglobal6-kde"
    "colorgrind"
    "colorized-logs"
    "colorless"
    "colorls"
    "colormake"
    "colorpanes"
    "colort"
    "colorz"
    "coloursum"
    "colpack"
    "colstr"
    "coltrane"
    "comaps"
    "combinatorial_designs"
    "comby"
    "comedilib"
    "comet-gog"
    "comet-gog_heroic"
    "comfortaa"
    "comfyui"
    "comic-mandown"
    "comic-mono"
    "comic-neue"
    "comic-relief"
    "comical"
    "comigo"
    "comixcursors"
    "comma"
    "commafeed"
    "commandergenius"
    "commet-chat"
    "commit"
    "commit-formatter"
    "commit-mono"
    "commit-notifier"
    "commitizen"
    "commitizen-go"
    "commitlint"
    "commitlint-rs"
    "commitmsgfmt"
    "committed"
    "commix"
    "common-licenses"
    "common-updater-scripts"
    "commons-bcel"
    "commons-bsf"
    "commons-compress"
    "commons-daemon"
    "commons-fileupload"
    "commons-io"
    "commons-lang"
    "commons-logging"
    "commons-math"
    "commonsBcel"
    "commonsBsf"
    "commonsCompress"
    "commonsDaemon"
    "commonsFileUpload"
    "commonsIo"
    "commonsLang"
    "commonsLogging"
    "commonsMath"
    "communi"
    "communique"
    "comodoro"
    "compactor"
    "compare50"
    "compass"
    "compcert"
    "compdb"
    "compile-daemon"
    "compiledb"
    "complete-alias"
    "completely"
    "complexity"
    "complgen"
    "compose2nix"
    "composefs"
    "composer-require-checker"
    "compress-pptx"
    "compsize"
    "comrak"
    "conan"
    "conceal"
    "concessio"
    "concord"
    "concord-tui"
    "concurrently"
    "conda"
    "conduit"
    "conduktor-ctl"
    "confclerk"
    "confd"
    "confd-calico"
    "confetty"
    "config-store"
    "configurable-http-proxy"
    "confluencepot"
    "confluent-cli"
    "confluent-platform"
    "conform"
    "conftest"
    "confy"
    "confy-tui"
    "conglomerate"
    "conjure"
    "conjure-tor"
    "conkeyscan"
    "conky"
    "conman"
    "conmon"
    "conmon-rs"
    "connect"
    "connect-box"
    "connectome-workbench"
    "connman"
    "connman-notify"
    "connmanFull"
    "connmanMinimal"
    "connman_dmenu"
    "conntrack-tools"
    "conpass"
    "conserve"
    "conserver"
    "console-bridge"
    "console-setup"
    "conspy"
    "constrict"
    "construct"
    "constructium"
    "construo"
    "construoBase"
    "consul"
    "consul-alerts"
    "consul-template"
    "contact"
    "contacts"
    "container"
    "container-structure-test"
    "container2wasm"
    "containerd"
    "containerlab"
    "containeryard"
    "context-mode"
    "context7-mcp"
    "contour"
    "contrast"
    "controku"
    "convbin"
    "convchain"
    "convco"
    "conventional-changelog-cli"
    "converged-security-suite"
    "converseen"
    "convertall"
    "convertlit"
    "convertx"
    "convfont"
    "convimg"
    "convmv"
    "convos"
    "convoyeur"
    "cook-cli"
    "cook-framework"
    "cookiecutter"
    "cool-retro-term"
    "coolreader"
    "cooper"
    "cooper-hewitt"
    "coordgenlibs"
    "coost"
    "copacetic"
    "cope"
    "copier"
    "copilot-language-server"
    "copilot-language-server-fhs"
    "copilot-node-server"
    "coppwr"
    "copyDesktopItems"
    "copyPkgconfigItems"
    "copybara"
    "copycat"
    "copygen"
    "copyparty"
    "copyparty-full-buggy"
    "copyparty-min"
    "copyparty-most"
    "copyq"
    "copyright-update"
    "copywrite"
    "coq"
    "coq-kernel"
    "coq2html"
    "coq_8_10"
    "coq_8_11"
    "coq_8_12"
    "coq_8_13"
    "coq_8_14"
    "coq_8_15"
    "coq_8_16"
    "coq_8_17"
    "coq_8_18"
    "coq_8_19"
    "coq_8_20"
    "coq_8_7"
    "coq_8_8"
    "coq_8_9"
    "coq_9_0"
    "coq_9_1"
    "coq_9_2"
    "coq_9_3"
    "cordova"
    "coreboot-utils"
    "corectrl"
    "coredns"
    "corefonts"
    "corepack"
    "corepack_22"
    "corepack_24"
    "corerad"
    "coreutils-full"
    "coreutils-prefixed"
    "corkscrew"
    "cormorant"
    "corn-cli"
    "cornelis"
    "cornucopia"
    "coroot"
    "coroot-node-agent"
    "corosync"
    "corosync-qdevice"
    "corretto11"
    "corretto17"
    "corretto21"
    "corretto25"
    "corrode-scanner"
    "corrosion"
    "corrscope"
    "corrupter"
    "corsair"
    "corsix-th"
    "cortex-command-community-project"
    "cortex-tools"
    "corteza"
    "corto"
    "corundum"
    "cosign"
    "cosmic-app-library"
    "cosmic-applets"
    "cosmic-applibrary"
    "cosmic-bg"
    "cosmic-comp"
    "cosmic-design-demo"
    "cosmic-edit"
    "cosmic-ext-applet-caffeine"
    "cosmic-ext-applet-external-monitor-brightness"
    "cosmic-ext-applet-mare-player"
    "cosmic-ext-applet-minimon"
    "cosmic-ext-applet-privacy-indicator"
    "cosmic-ext-applet-sysinfo"
    "cosmic-ext-applet-weather"
    "cosmic-ext-calculator"
    "cosmic-ext-ctl"
    "cosmic-ext-tweaks"
    "cosmic-files"
    "cosmic-greeter"
    "cosmic-icons"
    "cosmic-idle"
    "cosmic-initial-setup"
    "cosmic-launcher"
    "cosmic-monitor"
    "cosmic-notifications"
    "cosmic-osd"
    "cosmic-panel"
    "cosmic-player"
    "cosmic-protocols"
    "cosmic-randr"
    "cosmic-reader"
    "cosmic-screenshot"
    "cosmic-session"
    "cosmic-settings"
    "cosmic-settings-daemon"
    "cosmic-sound-theme"
    "cosmic-store"
    "cosmic-term"
    "cosmic-viewer"
    "cosmic-wallpapers"
    "cosmic-workspaces-epoch"
    "cosmocc"
    "cosmopolitan"
    "costa"
    "cot"
    "cotp"
    "coturn"
    "couchbase-shell"
    "couchdb-dump"
    "couchdb3"
    "coulomb"
    "coulr"
    "countryfetch"
    "countryguess"
    "courier-prime"
    "courier-unicode"
    "coursera-dl"
    "coursier"
    "cov-build"
    "coverage-reporter"
    "cowpatty"
    "cowsay"
    "cowsql"
    "coyim"
    "coz"
    "coze"
    "cozette"
    "cozy"
    "cozy-drive"
    "cp210x-program"
    "cp2k"
    "cp437"
    "cpak"
    "cpat"
    "cpc"
    "cpcfs"
    "cpcm-x"
    "cpeditor"
    "cpio"
    "cplay-ng"
    "cplex"
    "cpm-cmake"
    "cpond"
    "cpp-hocon"
    "cpp-interop"
    "cpp-ipfs-http-client"
    "cpp-jwt"
    "cpp-redis"
    "cpp-utilities"
    "cppad"
    "cppcheck"
    "cppclean"
    "cppcms"
    "cppcodec"
    "cppdb"
    "cppe"
    "cppfront"
    "cppi"
    "cppitertools"
    "cpplint"
    "cppman"
    "cppreference-doc"
    "cpptest"
    "cpptoml"
    "cpptrace"
    "cppunit"
    "cpputest"
    "cppzmq"
    "cpr"
    "cproto"
    "cpu-energy-meter"
    "cpu-x"
    "cpu_features"
    "cpufetch"
    "cpufrequtils"
    "cpuid"
    "cpuinfo"
    "cpulimit"
    "cpuminer"
    "cpupower-gui"
    "cpuset"
    "cpustat"
    "cpx"
    "cq"
    "cqlsh-rs"
    "crab-hole"
    "crabz"
    "crackle"
    "cracklib"
    "crackql"
    "crackxls"
    "craftos-pc"
    "cramfsprogs"
    "cramfsswap"
    "crane"
    "crate2nix"
    "cratedb"
    "crates-lsp"
    "crates-tui"
    "crawl"
    "crawlTiles"
    "crawley"
    "crc"
    "crc32c"
    "crc64fast-nvme"
    "crcpp"
    "crd2pulumi"
    "cre2"
    "create-dmg"
    "createrepo_c"
    "credential-detector"
    "credhub-cli"
    "creds"
    "credspy"
    "credstash"
    "credsweeper"
    "creduce"
    "credwolf"
    "creek"
    "creep"
    "crengine-ng"
    "crewai"
    "crex"
    "cri-o"
    "cri-o-unwrapped"
    "cri-tools"
    "crictty"
    "crimson"
    "crimson-pro"
    "crip"
    "crispy-doom"
    "critcmp"
    "criterion"
    "criu"
    "crlfsuite"
    "crlfuzz"
    "cro-mag-rally"
    "croaring"
    "croc"
    "crocoddyl"
    "cron"
    "croncpp"
    "cronet-go"
    "cronie"
    "cronutils"
    "cropgui"
    "cross-seed"
    "crossfire-arch"
    "crossfire-gridarta"
    "crossfire-jxclient"
    "crossfire-maps"
    "crossfire-server"
    "crossguid"
    "crossmacro"
    "crossmacro-daemon"
    "crossover"
    "crosspatch"
    "crosspipe"
    "crossplane"
    "crossplane-cli"
    "crosswords"
    "crosvm"
    "crow"
    "crow-translate"
    "crowbar"
    "crowdin-cli"
    "crowdsec"
    "crowdsec-firewall-bouncer"
    "crrcsim"
    "crudini"
    "cruft"
    "cruise"
    "crumbs"
    "crun"
    "crunch"
    "crusader"
    "crush"
    "crypt4gh"
    "crypto-org-wallet"
    "crypto-tracker"
    "cryptodev"
    "cryptolyzer"
    "cryptomator"
    "cryptomator-cli"
    "cryptominisat"
    "cryptop"
    "cryptopp"
    "cryptor"
    "cryptoscan"
    "cryptoverif"
    "cryptpad"
    "cryptsetup"
    "crystal"
    "crystal-dock"
    "crystal2nix"
    "crystal_1_14"
    "crystal_1_15"
    "crystal_1_16"
    "crystal_1_17"
    "crystal_1_18"
    "crystal_1_19"
    "crystalline"
    "crystfel"
    "crystfel-headless"
    "csa"
    "csaf-tool"
    "cscope"
    "csdp"
    "csdr"
    "csfml"
    "csfml_2"
    "csharp-ls"
    "csharpier"
    "csharprepl"
    "cshatag"
    "csmith"
    "csound"
    "csound-manual"
    "csound-qt"
    "cspell"
    "css-checker"
    "css-html-js-minify"
    "css-variables-language-server"
    "csso-cli"
    "csv-tui"
    "csv2latex"
    "csv2md"
    "csv2odf"
    "csv2parquet"
    "csv2svg"
    "csvdiff"
    "csview"
    "csvkit"
    "csvlens"
    "csvq"
    "csvquote"
    "csvs-to-sqlite"
    "csvtk"
    "csvtool"
    "csxcad"
    "ctagdrc"
    "ctags"
    "ctags-lsp"
    "ctagsWrapped"
    "ctestCheckHook"
    "ctlptl"
    "ctmg"
    "ctodo"
    "ctop"
    "ctpl"
    "ctpv"
    "ctranslate2"
    "ctre"
    "ctrtool"
    "ctune"
    "ctx"
    "ctx7"
    "ctypes_sh"
    "cubeb"
    "cubelify"
    "cubicle"
    "cubicsdr"
    "cubiomes-viewer"
    "cucumber"
    "cudatext"
    "cudatext-gtk"
    "cudatext-qt"
    "cudatoolkit"
    "cudd"
    "cue"
    "cue2pops"
    "cuelsp"
    "cuetools"
    "cuetsy"
    "culmus"
    "culvert"
    "cum"
    "cuneiform"
    "cunicu"
    "cunit"
    "cups"
    "cups-bjnp"
    "cups-brother-dcp1610wlpr"
    "cups-brother-dcpl2550dw"
    "cups-brother-dcpl3550cdw"
    "cups-brother-dcpt310"
    "cups-brother-dcpt720dw"
    "cups-brother-dcpt725dw"
    "cups-brother-hl1110"
    "cups-brother-hl1210w"
    "cups-brother-hl2260d"
    "cups-brother-hl3140cw"
    "cups-brother-hl3150cdn"
    "cups-brother-hl3170cdw"
    "cups-brother-hll2340dw"
    "cups-brother-hll2350dw"
    "cups-brother-hll2375dw"
    "cups-brother-hll3230cdw"
    "cups-brother-mfcl2710dw"
    "cups-brother-mfcl2750dw"
    "cups-brother-mfcl2800dw"
    "cups-browsed"
    "cups-drv-rastertosag-gdi"
    "cups-dymo"
    "cups-filters"
    "cups-idprt-barcode"
    "cups-idprt-mt888"
    "cups-idprt-mt890"
    "cups-idprt-sp900"
    "cups-idprt-tspl"
    "cups-kyocera"
    "cups-kyocera-3500-4500"
    "cups-kyocera-ecosys-m552x-p502x"
    "cups-kyodialog"
    "cups-pdf-to-pdf"
    "cups-pk-helper"
    "cups-printers"
    "cups-toshiba-estudio"
    "cups-zj-58"
    "cura-appimage"
    "curaengine_stable"
    "curie"
    "curl"
    "curl-impersonate"
    "curl-impersonate-chrome"
    "curlFull"
    "curlHTTP3"
    "curlMinimal"
    "curlWithGnuTls"
    "curlconverter"
    "curlie"
    "curlpp"
    "curseofwar"
    "curseofwar-sdl"
    "curseradio"
    "cursewords"
    "cursor-cli"
    "cursor-clip"
    "curtail"
    "curv"
    "cutecapture"
    "cutechess"
    "cutecom"
    "cutecosmic"
    "cutee"
    "cutemaze"
    "cutentr"
    "cutter"
    "cuyo"
    "cv"
    "cvc3"
    "cvc4"
    "cvc5"
    "cve"
    "cve-bin-tool"
    "cve-prioritizer"
    "cvehound"
    "cvemap"
    "cvise"
    "cvs"
    "cvs-fast-export"
    "cvs2svn"
    "cvsps"
    "cvsq"
    "cw"
    "cwal"
    "cwebbin"
    "cwltool"
    "cwm"
    "cwtch"
    "cwtch-ui"
    "cxx-prettyprint"
    "cxx-rs"
    "cxxopts"
    "cxxtest"
    "cxxtools"
    "cyan"
    "cyan-skillfish-governor-smu"
    "cyanrip"
    "cyberchef"
    "cyberduck"
    "cyberia"
    "cyberpunk-neon"
    "cyberstrike"
    "cyclone"
    "cyclone-scheme"
    "cyclonedds"
    "cyclonedds-cxx"
    "cyclonedx-cli"
    "cyclonedx-gomod"
    "cyclonedx-python"
    "cyme"
    "cynthion"
    "cypress"
    "cyrus-imapd"
    "cyrus-sasl-xoauth2"
    "cyrus_sasl"
    "cytoscape"
    "cz-cli"
    "czkawka"
    "czkawka-full"
    "czmq"
    "d-seams"
    "d-spy"
    "d1x-rebirth-full"
    "d2"
    "d2coding"
    "d2hs"
    "d2x-rebirth-full"
    "dab_lib"
    "dabet"
    "dablin"
    "dabtools"
    "dadadodo"
    "dae"
    "daed"
    "daemon"
    "daemonize"
    "daemontools"
    "dafny"
    "daggerfall-unity"
    "daggerfall-unity-unfree"
    "daikhan"
    "dailies"
    "daisydisk"
    "daktari"
    "daktilo"
    "dalfox"
    "dalphaball"
    "damon"
    "dancing-script"
    "danger-gitlab"
    "dante"
    "dapl"
    "dapl-native"
    "dapper"
    "dapr-cli"
    "dar"
    "darcnes"
    "darcs"
    "darcs-to-git"
    "dark-mode-notify"
    "darkhttpd"
    "darkice"
    "darklua"
    "darkly"
    "darkman"
    "darkplaces"
    "darkradiant"
    "darkstat"
    "darktable"
    "darktile"
    "darling-dmg"
    "dart"
    "dart-bin"
    "dart-sass"
    "dart-source"
    "dart_frog_cli"
    "dartsim"
    "das"
    "das_watchdog"
    "dasel"
    "dash"
    "dash-mpd-cli"
    "dasher"
    "dashing"
    "dasht"
    "dashy-ui"
    "dasm"
    "databricks-cli"
    "databricks-sql-cli"
    "dataclass-wizard"
    "datadog-agent"
    "datadog-process-agent"
    "datadog-pup"
    "dataexplorer"
    "dataform"
    "datafusion-cli"
    "datalab"
    "datalad"
    "datalad-gooey"
    "datamash"
    "datamodel-code-generator"
    "datasette"
    "datatrove"
    "datefmt"
    "datefudge"
    "dateutils"
    "datovka"
    "datree"
    "dav1d"
    "davfs2"
    "davinci-resolve"
    "davinci-resolve-studio"
    "davis"
    "davix"
    "davix-copy"
    "davmail"
    "davs2"
    "davtest"
    "dawarich"
    "dawn"
    "dawncut"
    "dayon"
    "dazel"
    "db"
    "db-rest"
    "db4"
    "db48"
    "db5"
    "db53"
    "db6"
    "db60"
    "db62"
    "dbacl"
    "dbcsr"
    "dbd"
    "dbeaver-bin"
    "dbgate"
    "dbip-asn-lite"
    "dbip-city-lite"
    "dbip-country-lite"
    "dblab"
    "dblatex"
    "dblatexFull"
    "dbmail"
    "dbmate"
    "dbmonster"
    "dbqn"
    "dbqn-native"
    "dbt"
    "dbtpl"
    "dbus"
    "dbus-broker"
    "dbus-cpp"
    "dbus-glib"
    "dbus-test-runner"
    "dbus_cplusplus"
    "dbus_java"
    "dbvisualizer"
    "dbx"
    "dbxml"
    "dc3dd"
    "dcadec"
    "dcap"
    "dcfldd"
    "dcgm"
    "dclock"
    "dclxvi"
    "dcm2niix"
    "dcmtk"
    "dcnnt"
    "dconf"
    "dconf-editor"
    "dconf2nix"
    "dcp"
    "dcp375cw-cupswrapper"
    "dcp375cwlpr"
    "dcp9020cdw-cupswrapper"
    "dcp9020cdwlpr"
    "dcpj785dw"
    "dcpj785dw-cupswrapper"
    "dcpomatic"
    "dcraw"
    "dcrctl"
    "dcrd"
    "dcrwallet"
    "dcs"
    "dcv"
    "dcw-gmt"
    "dd_rescue"
    "ddate"
    "ddccontrol"
    "ddccontrol-db"
    "ddclient"
    "ddcui"
    "ddcutil"
    "ddcutil-service"
    "ddd"
    "ddev"
    "ddgr"
    "ddh"
    "ddhx"
    "ddm"
    "ddnet"
    "ddnet-server"
    "ddns-go"
    "ddns-updater"
    "ddosify"
    "ddrescue"
    "ddrescueview"
    "ddrutility"
    "ddsmt"
    "dduper"
    "deadbeef"
    "deadbeef-with-plugins"
    "deadbranch"
    "deadd-notification-center"
    "deadlock-mod-manager"
    "deadnix"
    "deadpixi-sam-unstable"
    "dealii"
    "deark"
    "debase"
    "debcraft"
    "debian-archive-keyring"
    "debian-devscripts"
    "debian-goodies"
    "debianutils"
    "debootstrap"
    "debos"
    "debsigs"
    "debugedit"
    "dec-decode"
    "decasify"
    "decent-sampler"
    "decibels"
    "deck"
    "deck-app"
    "decker"
    "deckmaster"
    "decktape"
    "decode-spam-headers"
    "decoder"
    "deconz"
    "dediprog-sf100"
    "dedup"
    "dedup-darwin"
    "dee"
    "deej"
    "deep-translator"
    "deepcool-digital-linux"
    "deepdiff"
    "deepfilternet"
    "deepgit"
    "deepsecrets"
    "deepsource"
    "deeptools"
    "deer"
    "deezer-desktop"
    "deezer-enhanced"
    "deface"
    "defaultbrowser"
    "deff"
    "defuddle"
    "defuddle-cli"
    "degate"
    "degit"
    "deheader"
    "dehinter"
    "dehydrated"
    "deja"
    "deja-dup"
    "dejagnu"
    "dejavu_fonts"
    "dejavu_fontsEnv"
    "dejsonlz4"
    "delayarchitect"
    "delfin"
    "delineate"
    "dell-530cdn"
    "dell-bios-fan-control"
    "dell-command-configure"
    "delly"
    "delsum"
    "delta"
    "deltachat-desktop"
    "deltachat-repl"
    "deltachat-rpc-server"
    "deltachat-tauri"
    "deluge"
    "deluge-2_x"
    "deluge-gtk"
    "deluged"
    "delve"
    "demoit"
    "demucs-rs"
    "denaro"
    "dendrite"
    "denemo"
    "deniseemu"
    "deno"
    "densify"
    "dep-scan"
    "dep-tree"
    "departure-mono"
    "dependabot-cli"
    "dependency-track"
    "dependency-track-exporter"
    "deploy-rs"
    "deployer"
    "depotdownloader"
    "deps-fnl"
    "depthcharge-tools"
    "deptry"
    "deputy"
    "deqp-runner"
    "der-ascii"
    "descent1-assets"
    "descent2-assets"
    "descent3"
    "descent3-unwrapped"
    "desed"
    "design"
    "desk-exec"
    "deskew"
    "deskflow"
    "deskreen"
    "desktop-file-utils"
    "desktopToDarwinBundle"
    "desktoppr"
    "desmume"
    "desync"
    "detach"
    "detect"
    "detect-it-easy"
    "detect-secrets"
    "detekt"
    "deterministic-host-uname"
    "deterministic-uname"
    "deterministic-zip"
    "dethrace"
    "detox"
    "deutex"
    "dev86"
    "devbox"
    "devcontainer"
    "devd"
    "devede"
    "devenv"
    "devhelp"
    "device-tree_rpi"
    "devilspie2"
    "devilutionx"
    "devin-cli"
    "devin-desktop"
    "devmode"
    "devour"
    "devpi-client"
    "devpi-server"
    "devpod"
    "devpod-desktop"
    "devspace"
    "devtodo"
    "devtoolbox"
    "dex"
    "dex-oidc"
    "dex2jar"
    "dexed"
    "dexter"
    "deye-dummycloud"
    "dezoomify-rs"
    "dfasma"
    "dfc"
    "dfhack"
    "dfl-applications"
    "dfl-ipc"
    "dfl-login1"
    "dfl-utils"
    "dfmt"
    "dformat"
    "dfrs"
    "dftd4"
    "dfu-programmer"
    "dfu-util"
    "dgen-sdl"
    "dgop"
    "dgoss"
    "dgraph"
    "dhall"
    "dhall-bash"
    "dhall-docs"
    "dhall-json"
    "dhall-lsp-server"
    "dhall-nix"
    "dhall-nixpkgs"
    "dhall-yaml"
    "dhcpcd"
    "dhcpdump"
    "dhcpig"
    "dhcping"
    "dhcpm"
    "dhewm3"
    "dhex"
    "dht"
    "dhtbsign"
    "di"
    "di-tui"
    "dia"
    "diagnostic-languageserver"
    "diagrams-as-code"
    "diagrams-builder"
    "dialect"
    "dialog"
    "dialogbox"
    "diamond"
    "diceware"
    "dico"
    "dict"
    "dict-cc-py"
    "diction"
    "dictu"
    "didder"
    "didu"
    "didyoumean"
    "dieHook"
    "diebahn"
    "dieharder"
    "diesel-cli"
    "diesel-cli-ext"
    "diff-pdf"
    "diff-so-fancy"
    "diff2html-cli"
    "diffedit3"
    "diffnav"
    "diffoci"
    "diffoscope"
    "diffoscopeMinimal"
    "diffpdf"
    "diffr"
    "diffsitter"
    "diffstat"
    "difftastic"
    "diffuse"
    "diffyml"
    "dig"
    "digestpp"
    "digibyte"
    "digibyted"
    "digidrie"
    "digiham"
    "digikam"
    "digital"
    "digitalbitbox"
    "digitemp"
    "dijo"
    "dillo"
    "dillo-plus"
    "dim"
    "din"
    "dina-font"
    "ding"
    "ding-libs"
    "dinghy"
    "dinish"
    "dinit"
    "dino"
    "diod"
    "diodon"
    "dioxionary"
    "dioxus-cli"
    "dipc"
    "dippi"
    "dirb"
    "dirbuster"
    "dircolors-solarized"
    "dirdiff"
    "directfb"
    "directx-headers"
    "directx-shader-compiler"
    "direnv"
    "direvent"
    "direwolf"
    "direwolf-unstable"
    "dirstalk"
    "dirt"
    "dirvish"
    "disarchive"
    "discocss"
    "discord"
    "discord-canary"
    "discord-development"
    "discord-gamesdk"
    "discord-ptb"
    "discord-rich-presence-plex"
    "discord-rpc"
    "discord-sh"
    "discordchatexporter-cli"
    "discordchatexporter-desktop"
    "discordo"
    "discount"
    "discourse"
    "discourse-mail-receiver"
    "discourseAllPlugins"
    "discover-overlay"
    "discrete-scroll"
    "disfetch"
    "disk-filltest"
    "disk_indicator"
    "diskbloom"
    "diskgraph"
    "disko"
    "diskonaut-ng"
    "diskrsync"
    "diskscan"
    "disktui"
    "diskus"
    "diskwatch"
    "dislocker"
    "dismap"
    "dismember"
    "disnix"
    "disnix-web-service"
    "disnixos"
    "disorderfs"
    "display3d"
    "displaycal"
    "displaylink"
    "dissent"
    "distant"
    "distcc"
    "distccMasquerade"
    "distgen"
    "distrho-ports"
    "distribution"
    "distro-info-data"
    "distrobox"
    "distrobox-tui"
    "distrobuilder"
    "distroshelf"
    "diswall"
    "dit"
    "dita-ot"
    "ditaa"
    "diun"
    "dive"
    "dix"
    "diylc"
    "django-upgrade"
    "djbdns"
    "djenrandom"
    "djent"
    "djgpp"
    "djgpp_i586"
    "djgpp_i686"
    "djhtml"
    "djlint"
    "djot-js"
    "djv"
    "djview"
    "djview4"
    "djvu2pdf"
    "djvulibre"
    "dk"
    "dkh"
    "dkimproxy"
    "dkimpy"
    "dl-librescore"
    "dl-poly-classic-mpi"
    "dleyna"
    "dli"
    "dlib"
    "dlpack"
    "dlx"
    "dm-mono"
    "dm-sans"
    "dma"
    "dmalloc"
    "dmarc-metrics-exporter"
    "dmarc-report-converter"
    "dmd"
    "dmensamenu"
    "dmenu"
    "dmenu-bluetooth"
    "dmenu-rs"
    "dmenu-rs-enable-plugins"
    "dmenu-wayland"
    "dmg2img"
    "dmidecode"
    "dmitry"
    "dmlive"
    "dmraid"
    "dmrconfig"
    "dms"
    "dms-shell"
    "dmsdos"
    "dmtcp"
    "dmtx-utils"
    "dn42-cacert"
    "dn42-registry-wizard"
    "dnadd"
    "dnd-tools"
    "dness"
    "dnf-plugins-core"
    "dnf4"
    "dnf5"
    "dnglab"
    "dnote"
    "dns-collector"
    "dns-over-https"
    "dns-root-data"
    "dns2tcp"
    "dnschef"
    "dnscontrol"
    "dnscrypt-proxy"
    "dnsdbq"
    "dnsdiag"
    "dnsdist"
    "dnsenum"
    "dnsi"
    "dnsight"
    "dnslink-std-go"
    "dnslookup"
    "dnsmap"
    "dnsmasq"
    "dnsmon-go"
    "dnsmonster"
    "dnsname-cni"
    "dnspeep"
    "dnsperf"
    "dnsproxy"
    "dnsrecon"
    "dnss"
    "dnst"
    "dnstake"
    "dnstap"
    "dnstop"
    "dnstracer"
    "dnstwist"
    "dnsutils"
    "dnsvalidator"
    "dnsviz"
    "dnsx"
    "do-agent"
    "doas"
    "doas-sudo-shim"
    "doc2dash"
    "doc2go"
    "docbook-xsl-nons"
    "docbook-xsl-ns"
    "docbook2mdoc"
    "docbook2odf"
    "docbook2x"
    "docbook5"
    "docbook_sgml_dtd_31"
    "docbook_sgml_dtd_41"
    "docbook_sgml_dtd_45"
    "docbook_xml_dtd_412"
    "docbook_xml_dtd_42"
    "docbook_xml_dtd_43"
    "docbook_xml_dtd_44"
    "docbook_xml_dtd_45"
    "docbook_xml_ebnf_dtd"
    "docbook_xsl"
    "docbook_xsl_ns"
    "docfd"
    "docfx"
    "docify"
    "dockbarx"
    "docker"
    "docker-buildx"
    "docker-client"
    "docker-color-output"
    "docker-compose"
    "docker-compose-language-service"
    "docker-credential-env"
    "docker-credential-gcr"
    "docker-credential-helpers"
    "docker-gc"
    "docker-init"
    "docker-language-server"
    "docker-ls"
    "docker-sbom"
    "docker-sbx"
    "docker-slim"
    "docker-vackup"
    "docker_25"
    "docker_29"
    "dockerfile-language-server"
    "dockerfile-language-server-nodejs"
    "dockerfile-pin"
    "dockerfmt"
    "dockfmt"
    "dockle"
    "dockmate"
    "dockrtui"
    "dockstarter"
    "dockutil"
    "doclifter"
    "docling"
    "docling-serve"
    "docopt_cpp"
    "docopts"
    "docstrfmt"
    "doctest"
    "doctl"
    "doctoc"
    "documenso"
    "documentation-highlighter"
    "documize-community"
    "docuseal"
    "docutils"
    "docuum"
    "dodgy"
    "dog"
    "doge"
    "dogedns"
    "doggo"
    "doh-proxy-rust"
    "doing"
    "doit"
    "doitlive"
    "dokieli"
    "dokuwiki"
    "dolbybcsoftwaredecode"
    "dolfinx"
    "dolibarr"
    "dolphin-emu"
    "dolphin-emu-primehack"
    "dolt"
    "domain-check"
    "domination"
    "domine"
    "domino-chain"
    "domoticz"
    "donkey"
    "donpapi"
    "doodle"
    "dooit"
    "dooit-extras"
    "dool"
    "doom-bcc"
    "doomretro"
    "doomrunner"
    "doomseeker"
    "doona"
    "door-knocker"
    "dopamine"
    "dopewars"
    "doppler"
    "doq"
    "dorion"
    "dorkscout"
    "dos2unix"
    "dosage"
    "dosage-tracker"
    "dosbox"
    "dosbox-staging"
    "dosbox-x"
    "dosemu_fonts"
    "dosfstools"
    "dosis"
    "dot-language-server"
    "dot-merlin-reader"
    "dot2tex"
    "dotacat"
    "dotbot"
    "dotcolon-fonts"
    "dotconf"
    "dotenv-cli"
    "dotenv-linter"
    "dotenvx"
    "dotenvy"
    "dotfiles"
    "dotherside"
    "dotnet-aspnetcore"
    "dotnet-aspnetcore_10"
    "dotnet-aspnetcore_11"
    "dotnet-aspnetcore_6"
    "dotnet-aspnetcore_7"
    "dotnet-aspnetcore_8"
    "dotnet-aspnetcore_9"
    "dotnet-ef"
    "dotnet-outdated"
    "dotnet-repl"
    "dotnet-runtime"
    "dotnet-runtime_10"
    "dotnet-runtime_11"
    "dotnet-runtime_6"
    "dotnet-runtime_7"
    "dotnet-runtime_8"
    "dotnet-runtime_9"
    "dotnet-sdk"
    "dotnet-sdk_10"
    "dotnet-sdk_11"
    "dotnet-sdk_6"
    "dotnet-sdk_7"
    "dotnet-sdk_8"
    "dotnet-sdk_9"
    "dotnetbuildhelpers"
    "dotool"
    "dotslash"
    "dotter"
    "double-conversion"
    "double-entry-generator"
    "doublecmd"
    "doulos-sil"
    "dovecot"
    "dovecot-fts-flatcurve"
    "dovecot_2_3"
    "dovecot_2_4"
    "dovecot_exporter"
    "dovecot_pigeonhole"
    "dovecot_pigeonhole_0_5"
    "dovecot_pigeonhole_2_4"
    "dovi-tool"
    "downkyicore"
    "doxx"
    "doxygen"
    "doxygen-awesome-css"
    "doxygen_gui"
    "dpdk"
    "dpic"
    "dpkg"
    "dplusplus"
    "dpms-off"
    "dpp"
    "dprint"
    "dps8m"
    "dpt-rp1-py"
    "dput-ng"
    "dq"
    "dr14_tmeter"
    "dra-cla"
    "draco"
    "dracula-icon-theme"
    "dracula-qt5-theme"
    "dracula-theme"
    "dracut"
    "drafting-mono"
    "dragmap"
    "dragon-drop"
    "dragonfly-reverb"
    "dragonflydb"
    "drake"
    "drascula-the-vampire-strikes-back"
    "drasl"
    "drat"
    "drat-trim"
    "draupnir"
    "drawing"
    "drawio"
    "drawio-headless"
    "drawpile"
    "drawpile-server-headless"
    "drawterm"
    "drawterm-wayland"
    "drawy"
    "drbd"
    "dreamchess"
    "dreamweb"
    "dri-pkgconfig-stub"
    "drift"
    "driftctl"
    "driftnet"
    "drill"
    "drip"
    "drive"
    "drm_info"
    "drogon"
    "droidcam"
    "droidmote"
    "drone"
    "drone-cli"
    "drone-oss"
    "drone-runner-docker"
    "drone-runner-ssh"
    "drone-scp"
    "droopy"
    "dropbear"
    "dropbox"
    "dropbox-cli"
    "dropwatch"
    "druid"
    "drum-machine"
    "drumgizmo"
    "drumkv1"
    "drupal"
    "dry"
    "ds4u"
    "dscanner"
    "dsda-doom"
    "dsda-launcher"
    "dsdcc"
    "dsearch"
    "dsf2flac"
    "dsnet"
    "dsniff"
    "dsp"
    "dspam"
    "dsq"
    "dsremote"
    "dssat"
    "dssd"
    "dsseries"
    "dssi"
    "dssp"
    "dstask"
    "dstep"
    "dstp"
    "dsview"
    "dsvpn"
    "dt"
    "dt-schema"
    "dtach"
    "dtbloader"
    "dtc"
    "dtcmp"
    "dte"
    "dtee"
    "dterm"
    "dtool"
    "dtools"
    "dtop"
    "dtrx"
    "dts-lsp"
    "dtsfmt"
    "dtui"
    "dtv-scan-tables"
    "dua"
    "dualsensectl"
    "duat"
    "dub"
    "dub-to-nix"
    "dubBuildHook"
    "dubCheckHook"
    "dubSetupHook"
    "dublin-traceroute"
    "duc"
    "duckdb"
    "duckdb-odbc"
    "ducker"
    "duckling-proxy"
    "duckmarines"
    "duckscript"
    "duckx"
    "duden"
    "duf"
    "duff"
    "dufs"
    "duktape"
    "dum"
    "dumb-init"
    "dumbpipe"
    "dummyhttp"
    "dump"
    "dump1090-fa"
    "dump_syms"
    "dumpasn1"
    "dumphfdl"
    "dumpifs"
    "dumpnar"
    "dumptorrent"
    "dumpvdl2"
    "dumpyara"
    "dune"
    "dune-release"
    "dune3d"
    "dune_2"
    "dune_3"
    "dunst"
    "duo-unix"
    "dupd"
    "dupe-krill"
    "dupeguru"
    "duperemove"
    "duplicacy"
    "duplicati"
    "duplicity"
    "duply"
    "dura"
    "durden"
    "durden-wrapped"
    "dusklight"
    "dust"
    "dustracing2d"
    "dut"
    "dutctl"
    "duti"
    "dutree"
    "dvb-apps"
    "dvc"
    "dvc-with-remotes"
    "dvd-slideshow"
    "dvd-vr"
    "dvdauthor"
    "dvdbackup"
    "dvdisaster"
    "dvdplusrwtools"
    "dvdstyler"
    "dvgrab"
    "dvr-scan"
    "dvtm"
    "dvtm-unstable"
    "dwarf-fortress"
    "dwarf-fortress-full"
    "dwarf-therapist"
    "dwarf2json"
    "dwarfdump"
    "dwarfs"
    "dwdiff"
    "dwl"
    "dwlb"
    "dwm"
    "dwm-status"
    "dwmbar"
    "dwmblocks"
    "dwproton-bin"
    "dwt1-shell-color-scripts"
    "dwz"
    "dxa"
    "dxvk"
    "dxvk_1"
    "dxvk_2"
    "dxx-rebirth"
    "dyalog"
    "dydisnix"
    "dyff"
    "dylib"
    "dynamic-colors"
    "dynamic-wallpaper"
    "dynamips"
    "dynamodb-local"
    "dynarmic"
    "dyncall"
    "dyndnsc"
    "dynein"
    "dynisland"
    "dysk"
    "dysnomia"
    "dzen2"
    "e-imzo"
    "e-imzo-manager"
    "e16"
    "e1s"
    "e2fsprogs"
    "e2tools"
    "e57inspector"
    "ea"
    "each"
    "eaglemode"
    "ear2ctl"
    "earbuds"
    "earlybird"
    "earlyoom"
    "eartag"
    "earthbuild"
    "earthly"
    "earthlyls"
    "eas-cli"
    "easel"
    "easeprobe"
    "eask-cli"
    "easyabc"
    "easyaudiosync"
    "easycrypt"
    "easycrypt-runtest"
    "easyeasm"
    "easyeda2kicad"
    "easyeffects"
    "easyjson"
    "easylpac"
    "easyocr"
    "easypdkprog"
    "easyroam-connect-desktop"
    "easyrpg-player"
    "easyrsa"
    "easysnap"
    "easytag"
    "easytier"
    "eb-garamond"
    "ebook2cw"
    "ebook_tools"
    "ebpf-usb"
    "ebpf-verifier"
    "ebtables"
    "ebtks"
    "ebumeter"
    "ebusd"
    "ec"
    "ec2-ami-tools"
    "ec2-api-tools"
    "ec2-instance-selector"
    "ec2-metadata-mock"
    "ec2stepshell"
    "ecapture"
    "ecasound"
    "ecc"
    "eccodes"
    "ecdsatool"
    "ecdsautils"
    "echidna"
    "echoip"
    "ecl"
    "eclair"
    "eclib"
    "eclint"
    "eclipse-mat"
    "ecm"
    "ecmtools"
    "ecopcr"
    "ecos"
    "ecpdap"
    "ecs-agent"
    "ecsk"
    "ecspresso"
    "ectool"
    "ecwolf"
    "ed"
    "ed-odyssey-materials-helper"
    "edac-utils"
    "edb"
    "eddie"
    "eddy"
    "eden"
    "edencommon"
    "edgetpu-compiler"
    "edgetx"
    "edhm-ui"
    "edid-decode"
    "edid-generator"
    "edido"
    "edir"
    "edit"
    "editline"
    "editorconfig-checker"
    "editorconfig-core-c"
    "editres"
    "edk2"
    "edk2-uefi-shell"
    "edl"
    "edlib"
    "edlin"
    "edmarketconnector"
    "edname"
    "edopro"
    "edukai"
    "eduke32"
    "eduli"
    "edusong"
    "eduvpn-client"
    "edwin"
    "edwood"
    "ee"
    "eemusic"
    "eepers"
    "eff"
    "efficient-compression-tool"
    "effitask"
    "efibooteditor"
    "efibootmgr"
    "efitools"
    "efivar"
    "eflite"
    "efm-langserver"
    "efmt"
    "efont-unicode"
    "eg25-manager"
    "egctl"
    "eget"
    "egg2nix"
    "eggdbus"
    "eggdrop"
    "egglog"
    "eggnog-mapper"
    "egl-gbm"
    "egl-wayland"
    "egl-wayland2"
    "egl-x11"
    "eglexternalplatform"
    "ego"
    "egoboo"
    "egypt"
    "eiciel"
    "eid-mw"
    "eigen"
    "eigen2"
    "eigen_3_4_0"
    "eigen_5"
    "eigenlayer"
    "eigenmath"
    "eigenrand"
    "eigenwallet"
    "eilmeldung"
    "eiquadprog"
    "eiskaltdcpp"
    "eiwd"
    "ejabberd"
    "eject"
    "ejs"
    "ejson"
    "ejson2env"
    "ejsonkms"
    "ekam"
    "ekho"
    "ekrhyper"
    "eks-node-viewer"
    "eksctl"
    "ela-widget-tools"
    "elan"
    "elastic"
    "elasticmq-server-bin"
    "elasticsearch"
    "elasticsearch-curator"
    "elasticsearch7"
    "elastix"
    "electricsheep"
    "electron"
    "electron-bin"
    "electron-cash"
    "electron-chromedriver"
    "electron-chromedriver_39"
    "electron-chromedriver_40"
    "electron-chromedriver_41"
    "electron-chromedriver_42"
    "electron-chromedriver_43"
    "electron-fiddle"
    "electron-mail"
    "electron_39"
    "electron_39-bin"
    "electron_40"
    "electron_40-bin"
    "electron_41"
    "electron_41-bin"
    "electron_42"
    "electron_42-bin"
    "electron_43"
    "electron_43-bin"
    "electrs"
    "electrum"
    "electrum-grs"
    "electrum-ltc"
    "elegant-sddm"
    "elektroid"
    "element"
    "element-call"
    "element-desktop"
    "element-web"
    "element-web-unwrapped"
    "elementary-xfce-icon-theme"
    "elements"
    "elementsd"
    "elephant"
    "elf-dissector"
    "elf-header-real"
    "elf-info"
    "elf2nucleus"
    "elf2uf2-rs"
    "elfcat"
    "elfdeps"
    "elfinfo"
    "elfio"
    "elfkickers"
    "elftool"
    "elfutils"
    "elfx86exts"
    "eli"
    "elinks"
    "eliot-tree"
    "elixir"
    "elixir-ls"
    "elixir_1_18"
    "elixir_1_19"
    "eliza"
    "elkhound"
    "ell"
    "elliptic_curves"
    "elm-land"
    "elm2nix"
    "elmerfem"
    "elogind"
    "eloquent"
    "elpa"
    "eltclsh"
    "eludris"
    "elvis"
    "elvis-erlang"
    "elvish"
    "em"
    "em100"
    "emacs"
    "emacs-all-the-icons-fonts"
    "emacs-anywhere"
    "emacs-gtk"
    "emacs-lsp-booster"
    "emacs-macport"
    "emacs-nox"
    "emacs-pgtk"
    "emacs30-macport"
    "emacs31"
    "emacs31-gtk3"
    "emacs31-nox"
    "emacs31-pgtk"
    "emacsclient-commands"
    "emacspeak"
    "email"
    "embedxpl"
    "embellish"
    "ember-cli"
    "ember-language-server"
    "emblem"
    "emborg"
    "emboss"
    "embree"
    "emcee"
    "emerald-legacy-launcher"
    "emerald-legacy-launcher-unwrapped"
    "emhash"
    "emilua"
    "emissary"
    "emitter"
    "emma"
    "emmet-language-server"
    "emmet-ls"
    "emmy-lua-code-style"
    "emmylua-check"
    "emmylua-doc-cli"
    "emmylua-formatter"
    "emmylua-ls"
    "emocli"
    "emoji-picker"
    "emoji-runner"
    "emojify"
    "emojipick"
    "emote"
    "empire-compiler"
    "emplace"
    "empserver"
    "emptty"
    "empty"
    "empty-epsilon"
    "empty-pdf"
    "emptyDirectory"
    "emptyFile"
    "emu2"
    "emuflight-configurator"
    "emulsion"
    "emulsion-palette"
    "emv"
    "en-croissant"
    "enableGCOVInstrumentation"
    "enblend-enfuse"
    "enc"
    "enca"
    "enchant"
    "enchant2"
    "enchant_2"
    "enchive"
    "encled"
    "encode-sans"
    "encodings"
    "encpipe"
    "encrypted-dns-server"
    "endeavour"
    "endgame-singularity"
    "endless-sky"
    "endlessh"
    "endlessh-go"
    "endlines"
    "enet"
    "engage"
    "engauge-digitizer"
    "engelsystem"
    "engrampa"
    "enhanced-ctorrent"
    "enigma"
    "enjarify"
    "enkei"
    "enochecker-test"
    "enpass"
    "enpass-cli"
    "enpass-mac"
    "enroot"
    "enry"
    "enscript"
    "ensemble-chorus"
    "ensureNewerSourcesForZipFilesHook"
    "ent"
    "ent-go"
    "entangle"
    "ente-auth"
    "ente-cli"
    "ente-desktop"
    "ente-web"
    "enter-tex"
    "entire"
    "entr"
    "entropy"
    "entt"
    "entwine"
    "enum4linux"
    "enum4linux-ng"
    "enumer"
    "enumerepo"
    "envchain"
    "envconsul"
    "envfs"
    "envio"
    "envision"
    "envision-unwrapped"
    "envoluntary"
    "envoy"
    "envoy-bin"
    "envsubst"
    "envypn-font"
    "enyo-launcher"
    "enzyme"
    "eog"
    "eom"
    "eos-installer"
    "eot_utilities"
    "epeg"
    "epgstation"
    "ephemeralpg"
    "epic5"
    "epilys-bb"
    "epiphany"
    "epkowa"
    "eplot"
    "epm"
    "epoll-shim"
    "epr"
    "eprover"
    "eprover-ho"
    "epson-201106w"
    "epson-201401w"
    "epson-202101w"
    "epson-alc1100"
    "epson-escpr"
    "epson-escpr2"
    "epson-inkjet-printer-workforce-840-series"
    "epson-tm-t88vi"
    "epson-workforce-635-nx625-series"
    "epson_201207w"
    "epson_201310w"
    "epsonscan2"
    "epstool"
    "epub-thumbnailer"
    "epub2txt2"
    "epubcheck"
    "epy"
    "equalx"
    "equibop"
    "equicord"
    "equihash"
    "er-patcher"
    "era"
    "erdtree"
    "eresi"
    "erg"
    "ergo"
    "ergochat"
    "ergogen"
    "ergohaven-entropy"
    "ergoscf"
    "ericw-tools"
    "erigon"
    "erlang"
    "erlang-language-platform"
    "erlang_27"
    "erlang_28"
    "erlfmt"
    "erofs-utils"
    "erosmb"
    "errands"
    "errbot"
    "errcheck"
    "ersatztv"
    "es"
    "esbuild"
    "escambo"
    "eschalot"
    "escrotum"
    "esdm"
    "esh"
    "eslint"
    "eslint_d"
    "esp-generate"
    "espanso"
    "espanso-wayland"
    "espeak"
    "espeak-classic"
    "espeak-ng"
    "espeakedit"
    "espeakup"
    "espflash"
    "esphome"
    "esphome-device-builder"
    "espresso"
    "esptool"
    "esptool-ck"
    "espup"
    "essentia-extractor"
    "essh"
    "esshader"
    "et"
    "et-book"
    "etBook"
    "eta"
    "etcd"
    "etcd_3_5"
    "etcd_3_6"
    "ete"
    "ete-unwrapped"
    "etebase-server"
    "eternal-terminal"
    "eternity"
    "etesync-dav"
    "ethash"
    "etherape"
    "ethercat"
    "etherguard"
    "ethernet-connection-status"
    "etherpad-lite"
    "ethersync"
    "ethq"
    "ethr"
    "ethtool"
    "etlegacy"
    "etlegacy-assets"
    "etlegacy-unwrapped"
    "ets"
    "ettercap"
    "etterna"
    "eudev"
    "eudic"
    "eukleides"
    "eunomia"
    "euphonica"
    "eureka-editor"
    "eurofurence"
    "eva"
    "evans"
    "evcc"
    "evcxr"
    "evdev-proto"
    "evdevhook2"
    "evdevremapkeys"
    "evebox"
    "evemu"
    "eventlog"
    "eventstat"
    "eventstore"
    "everdo"
    "everest"
    "everest-bin"
    "everest-mons"
    "everforest-cursors"
    "everforest-gtk-theme"
    "evhz"
    "evil-helix"
    "evil-winrm"
    "evil-winrm-py"
    "evillimiter"
    "evilpixie"
    "evilwm"
    "evince"
    "evmdis"
    "evolution"
    "evolution-data-server"
    "evolution-data-server-gtk4"
    "evolution-ews"
    "evolutionWithPlugins"
    "evremap"
    "evscript"
    "evsieve"
    "evtest"
    "evtest-qt"
    "evtx"
    "eweb"
    "eww"
    "ex_doc"
    "exabgp"
    "exactaudiocopy"
    "exaile"
    "examine"
    "example-robot-data"
    "exatorrent"
    "excalidraw_export"
    "excalifont"
    "exe2hex"
    "execline"
    "execline-man-pages"
    "executor"
    "exegol"
    "exegol4"
    "exempi"
    "exercise-timer"
    "exercism"
    "exfat"
    "exfatprogs"
    "exhaustive"
    "exhibit"
    "exif"
    "exificient"
    "exiflooter"
    "exifprobe"
    "exiftags"
    "exiftool"
    "exim"
    "exiv2"
    "exo"
    "exodus"
    "exomizer"
    "exonerate"
    "exoscale-cli"
    "expand-response-params"
    "expat"
    "expect"
    "expected-lite"
    "expenses"
    "experienced-pixel-dungeon"
    "explain"
    "explex"
    "explex-nf"
    "expliot"
    "explo"
    "exploitdb"
    "exportarr"
    "exposor"
    "expr"
    "expresslrs-configurator"
    "expressvpn"
    "exprtk"
    "exrtools"
    "ext3grep"
    "ext4magic"
    "external-editor-revived"
    "extest"
    "extism-cli"
    "extism-js"
    "extism-js-core"
    "extra-container"
    "extrace"
    "extract-dtb"
    "extract-xiso"
    "extract_url"
    "extractpdfmark"
    "extremetuxracer"
    "extrude"
    "extundelete"
    "exult"
    "eyed3"
    "eyedropper"
    "eyewitness"
    "eza"
    "ezminc"
    "ezno"
    "ezquake"
    "ezra-sil"
    "ezstream"
    "eztrace"
    "f1_8"
    "f1viewer"
    "f2"
    "f2c"
    "f2fs-tools"
    "f3"
    "f3d"
    "f3d_egl"
    "f5_6"
    "fa_1"
    "faac"
    "faad2"
    "faas-cli"
    "faba-icon-theme"
    "faba-mono-icons"
    "fable"
    "fabric"
    "fabric-ai"
    "fabric-installer"
    "fac"
    "fac-build"
    "facedetect"
    "facetimehd-calibration"
    "facetimehd-firmware"
    "facter"
    "factor-lang"
    "factor-lang-0_100"
    "factor-lang-0_101"
    "factor-lang-0_99"
    "factorio"
    "factorio-demo"
    "factorio-demo-experimental"
    "factorio-experimental"
    "factorio-headless"
    "factorio-headless-experimental"
    "factorio-space-age"
    "factorio-space-age-experimental"
    "factoriolab"
    "factplusplus"
    "fadein"
    "fahclient"
    "fahrplan"
    "fail2ban"
    "faircamp"
    "fairfax"
    "fairfax-hd"
    "fairymax"
    "fairywren"
    "faiss"
    "faissWithCuda"
    "fake-gcs-server"
    "fakeNss"
    "fakechroot"
    "fakedir"
    "fakehttp"
    "fakeroot"
    "fakeroute"
    "fakesip"
    "faketty"
    "falcoctl"
    "falkor"
    "fallout-ce"
    "fallout2-ce"
    "famistudio"
    "famly-fetch"
    "fan2go"
    "fanbox-dl"
    "fanctl"
    "fancy-cat"
    "fanficfare"
    "fangfrisch"
    "fantasque-sans-mono"
    "fantomas"
    "fanwood"
    "faq"
    "far2l"
    "faraday"
    "faraday-agent-dispatcher"
    "faraday-cli"
    "farbfeld"
    "farge"
    "farstream"
    "fasm"
    "fasm-bin"
    "fasmarm"
    "fasmg"
    "fasole"
    "fast"
    "fast-cli-zig"
    "fast-cpp-csv-parser"
    "fast-downward"
    "fast-float"
    "fast-ssh"
    "fastJson"
    "fastahack"
    "fastapi-cli"
    "fastcap"
    "fastcdr"
    "fastchess"
    "fastcompmgr"
    "fastcov"
    "fastcrw"
    "fastd"
    "fastd-exporter"
    "fastdds"
    "fastddsgen"
    "fastfetch"
    "fastfetch-unwrapped"
    "fastfetchMinimal"
    "fastflix"
    "fastgron"
    "fasthenry"
    "fastjar"
    "fastjet"
    "fastjet-contrib"
    "fastlane"
    "fastly"
    "fastmail-desktop"
    "fastmod"
    "fastnetmon-advanced"
    "fastnlo-toolkit"
    "fastp"
    "fastpbkdf2"
    "fastqc"
    "fasttext"
    "fasttrackml"
    "fatcat"
    "fatou"
    "fatrace"
    "fatresize"
    "fatrop"
    "fatsort"
    "faudio"
    "faugus-launcher"
    "fauna-shell"
    "faust"
    "faust-benchmark-tools"
    "faust-physicalmodeling"
    "faust2"
    "faust2alqt"
    "faust2csound"
    "faust2firefox"
    "faust2jack"
    "faust2jackrust"
    "faust2jaqt"
    "faust2ladspa"
    "faust2lv2"
    "faust2sc"
    "faust2sndfile"
    "faustPhysicalModeling"
    "faustfmt"
    "faustlive"
    "faustlsp"
    "fava"
    "fava-investor"
    "favirecon"
    "fawltydeps"
    "fb303"
    "fbc"
    "fbcat"
    "fbida"
    "fblog"
    "fbmark"
    "fbmenugen"
    "fbset"
    "fbterm"
    "fbthrift"
    "fbv"
    "fbvnc"
    "fcast-client"
    "fcast-receiver"
    "fceux"
    "fceux-qt5"
    "fceux-qt6"
    "fcft"
    "fcgi"
    "fcgiwrap"
    "fcitx5"
    "fcitx5-anthy"
    "fcitx5-areca"
    "fcitx5-array"
    "fcitx5-bamboo"
    "fcitx5-black-simplicity"
    "fcitx5-chewing"
    "fcitx5-fluent"
    "fcitx5-gtk"
    "fcitx5-hangul"
    "fcitx5-inflex-themes"
    "fcitx5-lotus"
    "fcitx5-lua"
    "fcitx5-m17n"
    "fcitx5-material-color"
    "fcitx5-mcbopomofo"
    "fcitx5-mellow-themes"
    "fcitx5-mozc"
    "fcitx5-mozc-ut"
    "fcitx5-nord"
    "fcitx5-openbangla-keyboard"
    "fcitx5-pinyin-minecraft"
    "fcitx5-pinyin-moegirl"
    "fcitx5-pinyin-zhwiki"
    "fcitx5-rime"
    "fcitx5-rose-pine"
    "fcitx5-sitelen-pona"
    "fcitx5-skk"
    "fcitx5-table-extra"
    "fcitx5-table-other"
    "fcitx5-tokyonight"
    "fcl"
    "fclones"
    "fclones-gui"
    "fcp"
    "fcppt"
    "fcrackzip"
    "fcron"
    "fd"
    "fdk-aac-encoder"
    "fdk_aac"
    "fdm"
    "fdroidcl"
    "fdroidserver"
    "fdt-viewer"
    "fdtools"
    "fdupes"
    "fead"
    "feather"
    "feather-tk"
    "featherpad"
    "febio"
    "febio-studio"
    "fedigroups"
    "fedimint"
    "fedistar"
    "fediwall"
    "fediwall-unwrapped"
    "feed2imap-go"
    "feedbackd"
    "feedbackd-device-themes"
    "feedgnuplot"
    "feedr"
    "feh"
    "fehlstart"
    "feishin"
    "feishin-web"
    "feishu"
    "feishu-cli"
    "felix"
    "felix-fm"
    "felix86"
    "felix_remoteshell"
    "femtolisp"
    "fence"
    "fend"
    "fennel-ls"
    "ferdium"
    "ferium"
    "ferm"
    "fermi2"
    "fermyon-spin"
    "fernglas"
    "feroxbuster"
    "ferretdb"
    "ferrishot"
    "ferron"
    "ferrum"
    "fertilizer"
    "fet-sh"
    "fetch"
    "fetch-scm"
    "fetchit"
    "fetchmail"
    "fetchmail_7"
    "fetchtastic"
    "fetchutils"
    "fex"
    "fex-headless"
    "feyngame"
    "ff2mpv"
    "ff2mpv-go"
    "ff2mpv-rust"
    "ffado"
    "ffado-mixer"
    "ffcast"
    "fff"
    "fff-mcp"
    "fffuu"
    "ffizer"
    "fflas-ffpack"
    "fflinuxprint"
    "ffmpeg"
    "ffmpeg-full"
    "ffmpeg-headless"
    "ffmpeg-livepeer"
    "ffmpeg-normalize"
    "ffmpeg_4"
    "ffmpeg_4-full"
    "ffmpeg_4-headless"
    "ffmpeg_6"
    "ffmpeg_6-full"
    "ffmpeg_6-headless"
    "ffmpeg_7"
    "ffmpeg_7-full"
    "ffmpeg_7-headless"
    "ffmpeg_8"
    "ffmpeg_8-full"
    "ffmpeg_8-headless"
    "ffmpeg_9"
    "ffmpeg_9-full"
    "ffmpeg_9-headless"
    "ffmpegthumbnailer"
    "ffmpreg"
    "ffms"
    "ffpb"
    "ffsend"
    "ffsubsync"
    "ffts"
    "fftw"
    "fftwFloat"
    "fftwLongDouble"
    "fftwMpi"
    "fftwQuad"
    "fftwSinglePrec"
    "ffuf"
    "fg-virgil"
    "fgallery"
    "fgqcanvas"
    "fh"
    "fheroes2"
    "fiano"
    "fiche"
    "fichub-cli"
    "ficsit-cli"
    "fiddler-everywhere"
    "fider"
    "fido2-manage"
    "fido2luks"
    "field-monitor"
    "fierce"
    "fig2dev"
    "figlet"
    "figma-agent"
    "figma-linux"
    "figtree"
    "figurine"
    "fiji"
    "fil-plugins"
    "file-rename"
    "file-roller"
    "filebeat"
    "filebeat7"
    "filebeat8"
    "filebench"
    "filebot"
    "filebrowser"
    "filebrowser-quantum"
    "filecheck"
    "filegive"
    "fileinfo"
    "filen-cli"
    "filen-desktop"
    "files-cli"
    "files-to-prompt"
    "fileshare"
    "fileshelter"
    "filezilla"
    "filius"
    "filter-audio"
    "filterpath"
    "filtr"
    "filtron"
    "fim"
    "fim-rs"
    "finalcut"
    "finalfrontier"
    "finalfusion-utils"
    "finalmouse-udev-rules"
    "finamp"
    "find-billy"
    "find-cursor"
    "findXMLCatalogs"
    "findbugs"
    "findent"
    "findex"
    "findimagedupes"
    "findnewest"
    "findomain"
    "findup"
    "finetune"
    "fingerprintx"
    "finit"
    "finufft"
    "fio"
    "fioctl"
    "fira"
    "fira-code"
    "fira-code-symbols"
    "fira-go"
    "fira-math"
    "fira-mono"
    "fira-sans"
    "fire"
    "firebase-tools"
    "firebird"
    "firebird-emu"
    "firebird_3"
    "firebird_4"
    "firebird_5"
    "firecracker"
    "firecrawl-cli"
    "firecrawl-mcp"
    "firectl"
    "firefly"
    "firefly-desktop"
    "firefly-iii"
    "firefly-iii-data-importer"
    "firefox"
    "firefox-beta"
    "firefox-beta-bin"
    "firefox-beta-unwrapped"
    "firefox-bin"
    "firefox-bin-unwrapped"
    "firefox-devedition"
    "firefox-devedition-bin"
    "firefox-devedition-unwrapped"
    "firefox-devtools-mcp"
    "firefox-esr"
    "firefox-esr-140"
    "firefox-esr-140-unwrapped"
    "firefox-esr-153"
    "firefox-esr-153-unwrapped"
    "firefox-esr-unwrapped"
    "firefox-gnome-theme"
    "firefox-mobile"
    "firefox-sync-client"
    "firefox-unwrapped"
    "firefox_decrypt"
    "firefoxpwa"
    "firefoxpwa-unwrapped"
    "firehol"
    "firejail"
    "fireplace"
    "firestarter"
    "firewalk"
    "firewalld"
    "firewalld-gui"
    "firezone-gateway"
    "firezone-gui-client"
    "firezone-headless-client"
    "firezone-relay"
    "firezone-server"
    "firezone-server-api"
    "firezone-server-domain"
    "firezone-server-web"
    "firmware-manager"
    "firmware-updater"
    "firrtl"
    "fish"
    "fish-fillets-ng"
    "fish-irssi"
    "fish-lsp"
    "fishMinimal"
    "fishnet"
    "fission"
    "fits-cloudctl"
    "fitsverify"
    "fittrackee"
    "fityk"
    "five-or-more"
    "fixedsys-excelsior"
    "fixjson"
    "fixup-yarn-lock"
    "fizz"
    "fjo"
    "flac"
    "flac123"
    "flac2all"
    "flac2mp3"
    "flaca"
    "flaci"
    "flacon"
    "fladder"
    "flake-checker"
    "flake-du"
    "flake-edit"
    "flam3"
    "flamegraph"
    "flamelens"
    "flamerobin"
    "flameshot"
    "flamp"
    "flann"
    "flannel"
    "flap-alerted"
    "flare"
    "flare-floss"
    "flare-signal"
    "flarectl"
    "flaresolverr"
    "flarum"
    "flashbench"
    "flashfocus"
    "flashgbx"
    "flashlabel-yxwl"
    "flashmq"
    "flashprint"
    "flashprog"
    "flashrom"
    "flashspace"
    "flask-unsign"
    "flasm"
    "flat-remix-gnome"
    "flat-remix-icon-theme"
    "flatbuffers"
    "flatbuffers_23"
    "flatcc"
    "flatito"
    "flatpak"
    "flatpak-builder"
    "flatpak-builder-tools"
    "flatpak-xdg-utils"
    "flatten-references-graph"
    "flattenReferencesGraph"
    "flatter"
    "flavours"
    "flawfinder"
    "flawz"
    "fldigi"
    "fleet"
    "fleet-desktop"
    "fleet-orbit"
    "fleetctl"
    "fleeting-plugin-aws"
    "fleng"
    "flent"
    "flet-client-flutter"
    "flex"
    "flex-launcher"
    "flex-ncat"
    "flex-ndax"
    "flex_2_5_35"
    "flexcpp"
    "flexget"
    "flexibee"
    "flexoptix-app"
    "flight-of-the-amazon-queen"
    "flightcore"
    "flightgear"
    "flink"
    "flint"
    "flint3"
    "flintlock"
    "flip"
    "flip-link"
    "flips"
    "flirc"
    "flirt"
    "flite"
    "flitter"
    "flix"
    "fllog"
    "flmsg"
    "flock"
    "flockit"
    "flood"
    "flood-for-transmission"
    "floorp-bin"
    "floorp-bin-unwrapped"
    "flootty"
    "flopoco"
    "florist"
    "flottbot"
    "flow"
    "flow-control"
    "flow-state"
    "flowblade"
    "flowgger"
    "flowtime"
    "flpsed"
    "flrig"
    "fltk"
    "fltk-minimal"
    "fltk13"
    "fltk13-minimal"
    "fltk14"
    "fltk14-minimal"
    "fltk_1_3"
    "fltk_1_3-minimal"
    "fltk_1_4"
    "fltk_1_4-minimal"
    "fltrator"
    "fltrdr"
    "fluent-bit"
    "fluent-gtk-theme"
    "fluent-icon-theme"
    "fluent-reader"
    "fluentd"
    "fluffychat"
    "fluffychat-web"
    "fluida-lv2"
    "fluidd"
    "fluidsynth"
    "flutter"
    "flutter329"
    "flutter332"
    "flutter335"
    "flutter338"
    "flutter341"
    "flutter344"
    "flutter347"
    "flutter_rust_bridge_codegen"
    "fluux-messenger"
    "fluvio"
    "flux"
    "flux-build"
    "flux9s"
    "fluxbox"
    "fluxboxlauncher"
    "fluxcd"
    "fluxcd-operator"
    "fluxcd-operator-mcp"
    "flvstreamer"
    "flwrap"
    "fly"
    "flycast"
    "flyctl"
    "flye"
    "flying-carpet"
    "flyspray"
    "flytectl"
    "flyway"
    "flywheel-cli"
    "fm"
    "fm-go"
    "fm-tune"
    "fmd-server"
    "fmi-reference-fmus"
    "fmit"
    "fmodex"
    "fmt"
    "fmt_10"
    "fmt_11"
    "fmt_12"
    "fmt_9"
    "fmtoy"
    "fn-cli"
    "fna3d"
    "fnc"
    "fnlfmt"
    "fnm"
    "fnotifystat"
    "fnott"
    "fnox"
    "focus"
    "focus-stack"
    "focuswriter"
    "foks"
    "foks-server"
    "fokus"
    "folder-color-switcher"
    "folia-major"
    "foliate"
    "folio"
    "folks"
    "follow"
    "folly"
    "folo"
    "foma"
    "fomp"
    "fondu"
    "font-adobe-100dpi"
    "font-adobe-75dpi"
    "font-adobe-utopia-100dpi"
    "font-adobe-utopia-75dpi"
    "font-adobe-utopia-type1"
    "font-alias"
    "font-arabic-misc"
    "font-awesome"
    "font-awesome_4"
    "font-awesome_5"
    "font-awesome_6"
    "font-awesome_7"
    "font-bh-100dpi"
    "font-bh-75dpi"
    "font-bh-lucidatypewriter-100dpi"
    "font-bh-lucidatypewriter-75dpi"
    "font-bh-ttf"
    "font-bh-type1"
    "font-bitstream-100dpi"
    "font-bitstream-75dpi"
    "font-bitstream-type1"
    "font-config-info"
    "font-cronyx-cyrillic"
    "font-cursor-misc"
    "font-daewoo-misc"
    "font-dec-misc"
    "font-encodings"
    "font-ibm-type1"
    "font-isas-misc"
    "font-jis-misc"
    "font-manager"
    "font-micro-misc"
    "font-misc-cyrillic"
    "font-misc-ethiopic"
    "font-misc-meltho"
    "font-misc-misc"
    "font-mutt-misc"
    "font-schumacher-misc"
    "font-screen-cyrillic"
    "font-sony-misc"
    "font-sun-misc"
    "font-util"
    "font-v"
    "font-winitzki-cyrillic"
    "font-xfree86-type1"
    "fontadobe100dpi"
    "fontadobe75dpi"
    "fontadobeutopia100dpi"
    "fontadobeutopia75dpi"
    "fontadobeutopiatype1"
    "fontalias"
    "fontarabicmisc"
    "fontbakery"
    "fontbh100dpi"
    "fontbh75dpi"
    "fontbhlucidatypewriter100dpi"
    "fontbhlucidatypewriter75dpi"
    "fontbhttf"
    "fontbhtype1"
    "fontbitstream100dpi"
    "fontbitstream75dpi"
    "fontbitstreamtype1"
    "fontc"
    "fontconfig"
    "fontcronyxcyrillic"
    "fontcursormisc"
    "fontdaewoomisc"
    "fontdecmisc"
    "fontfor"
    "fontforge"
    "fontforge-fonttools"
    "fontforge-gtk"
    "fontibmtype1"
    "fontisasmisc"
    "fontjismisc"
    "fontmicromisc"
    "fontmisccyrillic"
    "fontmiscethiopic"
    "fontmiscmeltho"
    "fontmiscmisc"
    "fontmuttmisc"
    "fontpreview"
    "fontschumachermisc"
    "fontscreencyrillic"
    "fontsonymisc"
    "fontsunmisc"
    "fonttosfnt"
    "fontutil"
    "fontwinitzkicyrillic"
    "fontxfree86type1"
    "foo2zjs"
    "foodfetch"
    "foomatic-db"
    "foomatic-db-engine"
    "foomatic-db-nonfree"
    "foomatic-db-ppds"
    "foomatic-db-ppds-withNonfreeDb"
    "foomatic-filters"
    "foonathan-memory"
    "foot"
    "footage"
    "footswitch"
    "fooyin"
    "fop"
    "fopnu"
    "forbidden"
    "ford"
    "forecast"
    "foreman"
    "foremost"
    "forge-mtg"
    "forge-sparks"
    "forgejo"
    "forgejo-cli"
    "forgejo-lts"
    "forgejo-mcp"
    "forgejo-runner"
    "fork-cleaner"
    "forkstat"
    "forktty"
    "form"
    "formatjson5"
    "formatter"
    "formiko"
    "formula"
    "forscape"
    "fortanix-sgx-tools"
    "fortify-headers"
    "fortitude"
    "fortls"
    "fortran-fpm"
    "fortran-language-server"
    "fortune"
    "fortune-kind"
    "fosrl-gerbil"
    "fosrl-newt"
    "fosrl-olm"
    "fosrl-pangolin"
    "foss-flame"
    "fossil"
    "fotema"
    "fotowall"
    "foundationdb"
    "foundry"
    "four-in-a-row"
    "fourmolu"
    "fox"
    "fox_1_6"
    "foxdot"
    "foxglove-cli"
    "foxglove-studio"
    "foxmarks"
    "foxotron"
    "fp16"
    "fparser"
    "fpart"
    "fpattern"
    "fpc"
    "fping"
    "fplll"
    "fplll_20160331"
    "fpm"
    "fpm2"
    "fpp"
    "fprettify"
    "fprintd"
    "fprintd-tod"
    "fq"
    "fracjson"
    "fractal"
    "fragment-mono"
    "fragments"
    "frama-c"
    "frama-c-gui"
    "framac"
    "frame"
    "frame-media-converter"
    "framesh"
    "framework-control"
    "framework-tool"
    "framework-tool-tui"
    "frangipanni"
    "frankenphp"
    "franz"
    "fraunces"
    "frawk"
    "fre"
    "freac"
    "frece"
    "free42"
    "free5gc-amf"
    "free5gc-ausf"
    "free5gc-bsf"
    "free5gc-chf"
    "free5gc-n3iwf"
    "free5gc-nef"
    "free5gc-nrf"
    "free5gc-nssf"
    "free5gc-pcf"
    "free5gc-smf"
    "free5gc-tngf"
    "free5gc-udm"
    "free5gc-udr"
    "free5gc-upf"
    "freealut"
    "freebayes"
    "freebind"
    "freecad"
    "freecad-qt6"
    "freecad-wayland"
    "freecell-solver"
    "freeciv"
    "freeciv_gtk"
    "freeciv_qt"
    "freeciv_sdl2"
    "freediameter"
    "freedink"
    "freedoom"
    "freedroid"
    "freedroidrpg"
    "freedv"
    "freefall"
    "freefilesync"
    "freefont_ttf"
    "freeglut"
    "freeipa"
    "freeipmi"
    "freelens-bin"
    "freemind"
    "freenect"
    "freenet"
    "freenukum"
    "freeoffice"
    "freeorion"
    "freepats"
    "freeplane"
    "freepv"
    "freeradius"
    "freerdp"
    "freerouting"
    "freesasa"
    "freescout"
    "freesweep"
    "freeswitch"
    "freetalk"
    "freetds"
    "freetts"
    "freetube"
    "freetype"
    "freetype-demos"
    "freewheeling"
    "freexl"
    "freeze"
    "frei"
    "frei0r"
    "freifunk-meshviewer"
    "french-numbers"
    "freqle"
    "freqtweak"
    "frequest"
    "frescobaldi"
    "fresh-editor"
    "freshfetch"
    "freshrss"
    "fretboard"
    "fribidi"
    "fricas"
    "friction-graphics"
    "frida-tools"
    "frigate"
    "frink"
    "friture"
    "fritz-exporter"
    "fritzing"
    "frobby"
    "frodo4"
    "frog"
    "frog-protocols"
    "frogatto"
    "frogdata"
    "frogmouth"
    "froide"
    "froide-govplan"
    "fromager"
    "frostwire-bin"
    "frotz"
    "frozen"
    "frozen-containers"
    "frp"
    "frr"
    "frugally-deep"
    "fs-watcher-lsp"
    "fsarchiver"
    "fsatrace"
    "fsautocomplete"
    "fscan"
    "fscrypt"
    "fscrypt-experimental"
    "fscryptctl"
    "fsearch"
    "fsel"
    "fselect"
    "fsg"
    "fsharp"
    "fsmark"
    "fsmon"
    "fsnav"
    "fsnotifier"
    "fspy"
    "fsql"
    "fsrx"
    "fst"
    "fstar"
    "fstl"
    "fstrcmp"
    "fstrm"
    "fsuae"
    "fsuae-launcher"
    "fsv"
    "fsverity-utils"
    "fswatch"
    "fswebcam"
    "ft2-clone"
    "ftb-app"
    "fte"
    "fteqcc"
    "fteqw"
    "fteqw-dedicated"
    "ftgl"
    "ftop"
    "ftx-prog"
    "ftxui"
    "fuc"
    "fuchsia-cursor"
    "fujprog"
    "fulcio"
    "fulcrum"
    "fum"
    "funambol"
    "func"
    "function-runner"
    "functionalplus"
    "functiontrace-server"
    "fundoc"
    "funnelweb"
    "funzzy"
    "furmark"
    "furnace"
    "furtherance"
    "fuse"
    "fuse-archive"
    "fuse-emulator"
    "fuse-overlayfs"
    "fuse2"
    "fuse2fs"
    "fuse3"
    "fusee-interfacee-tk"
    "fusee-nano"
    "fusesoc"
    "fusionInventory"
    "fusioninventory-agent"
    "fusuma"
    "fut"
    "futhark"
    "futility"
    "fuzzdb"
    "fuzzel"
    "fuzzylite"
    "fverb"
    "fvm"
    "fvs2"
    "fvwm"
    "fvwm2"
    "fvwm3"
    "fw"
    "fw-ectool"
    "fw-fanctrl"
    "fwanalyzer"
    "fwbuilder"
    "fwknop"
    "fwts"
    "fwup"
    "fwupd"
    "fwupd-efi"
    "fx"
    "fx-cast-bridge"
    "fxlinuxprint"
    "fxlinuxprintutil"
    "fxload"
    "fyi"
    "fyne"
    "fypp"
    "fzf"
    "fzf-git-sh"
    "fzf-make"
    "fzf-obc"
    "fzf-preview"
    "fzf-zsh-plugin"
    "fzssh"
    "fzy"
    "g-ls"
    "g-wrap"
    "g15daemon"
    "g203-led"
    "g2o"
    "g3kb-switch"
    "g3proxy"
    "g810-led"
    "g933-utils"
    "gImageReader"
    "gImageReader-qt"
    "gabutdm"
    "gaiasky"
    "gajim"
    "galah"
    "galario"
    "galatia-sil"
    "galaxis"
    "galaxy-buds-client"
    "galculator"
    "gale"
    "galen"
    "galene"
    "galene-file-transfer"
    "galene-ldap"
    "galene-stream"
    "galer"
    "galerio"
    "gallery-dl"
    "galleta"
    "gallia"
    "gam"
    "gama"
    "gama-tui"
    "gambit"
    "gambit-chess"
    "gambit-project"
    "gambit-unstable"
    "game-devices-udev-rules"
    "game-music-emu"
    "game-rs"
    "gamemac"
    "gamemode"
    "gamenetworkingsockets"
    "gamepad-mirror"
    "gamepad-tool"
    "gamescope"
    "gamescope-wsi"
    "gamja"
    "gamma-launcher"
    "gammaray"
    "gammastep"
    "gammu"
    "gams"
    "gamt"
    "gancio"
    "gandom-fonts"
    "ganttproject-bin"
    "gap"
    "gap-full"
    "gap-minimal"
    "gapcast"
    "gaphor"
    "gapless"
    "gappa"
    "garage"
    "garage_1"
    "garage_1_x"
    "garage_2"
    "garamond-libre"
    "garble"
    "garcon"
    "garden-of-coloured-lights"
    "gardendevd"
    "gargoyle"
    "garmin-grafana"
    "garmindb"
    "garnet"
    "gash"
    "gash-utils"
    "gat"
    "gate"
    "gate12"
    "gatekeeper"
    "gatk"
    "gatling"
    "gato"
    "gatsby-cli"
    "gatus"
    "gatus-cli"
    "gau"
    "gauche"
    "gaucheBootstrap"
    "gauge"
    "gauge-unwrapped"
    "gav"
    "gavin-bc"
    "gavl"
    "gaw"
    "gawd"
    "gawk-with-extensions"
    "gawkInteractive"
    "gay"
    "gazelle-origin"
    "gb-backup"
    "gbar"
    "gbenchmark"
    "gbforth"
    "gbsplay"
    "gcab"
    "gcal"
    "gcalcli"
    "gcan"
    "gcc-arm-embedded"
    "gcc-arm-embedded-13"
    "gcc-arm-embedded-14"
    "gcc-arm-embedded-15"
    "gcc-unwrapped"
    "gccForLibs"
    "gccmakedep"
    "gcdemu"
    "gcfflasher"
    "gci"
    "gcl"
    "gcli"
    "gclient2nix"
    "gcolor3"
    "gcompris"
    "gcov2lcov"
    "gcovr"
    "gcp-scanner"
    "gcr_3"
    "gcr_4"
    "gcrane"
    "gcs"
    "gcsfuse"
    "gcstar"
    "gcx"
    "gd"
    "gdal"
    "gdalMinimal"
    "gdash"
    "gdb"
    "gdb-dashboard"
    "gdbHostCpuOnly"
    "gdbgui"
    "gdbm"
    "gdbuspp"
    "gdcm"
    "gdevelop"
    "gdk-pixbuf"
    "gdk-pixbuf-xlib"
    "gdl"
    "gdlauncher-carbon"
    "gdlv"
    "gdm"
    "gdm-settings"
    "gdmap"
    "gdown"
    "gdrive"
    "gdrive3"
    "gdscript-formatter"
    "gdtoolkit_3"
    "gdtoolkit_4"
    "gdu"
    "geant4"
    "geany"
    "geany-with-vte"
    "gearboy"
    "gearlever"
    "geary"
    "gebaar-libinput"
    "geckodriver"
    "gecode"
    "gecode_3"
    "gede"
    "gedit"
    "geek-life"
    "geekbench"
    "geekbench_4"
    "geekbench_5"
    "geekbench_6"
    "geekbench_7"
    "geeqie"
    "geesefs"
    "gef"
    "gefyra"
    "gegl"
    "geis"
    "geist-font"
    "gel"
    "gelasio"
    "gelly"
    "gem"
    "gema"
    "gemget"
    "gemini-cli"
    "gemini-cli-bin"
    "geminicommit"
    "gemmi"
    "gemrb"
    "gemstash"
    "gen-license"
    "gen-oath-safe"
    "gen6dns"
    "genact"
    "genann"
    "gendef"
    "genealogos-api"
    "genealogos-cli"
    "genefer"
    "genemichaels"
    "generic-sycl-components"
    "genesys"
    "genext2fs"
    "gengetopt"
    "genimage"
    "genmap"
    "genpass"
    "genromfs"
    "gensio"
    "gensort"
    "gentium"
    "gentium-book"
    "gentium-book-basic"
    "gentium-plus"
    "genxword"
    "genymotion"
    "genzai"
    "geo"
    "geobug"
    "geoclock"
    "geoclue2"
    "geoclue2-with-demo-agent"
    "geocode-glib_2"
    "geogebra"
    "geogebra6"
    "geogram"
    "geographiclib"
    "geoip"
    "geoipWithDatabase"
    "geoipjava"
    "geoipupdate"
    "geolite-legacy"
    "geomcpp"
    "geomyidae"
    "geonkick"
    "geopard"
    "geos"
    "geoserver"
    "geph"
    "gephgui-wry"
    "gephi"
    "gerbera"
    "gerbil"
    "gerbil-unstable"
    "gerbolyze"
    "gerbv"
    "germinal"
    "gerrit"
    "gerrit-queue"
    "gerrit_3_12"
    "gerrit_3_13"
    "gerrit_3_14"
    "gersemi"
    "gershwin-system"
    "get-google-fonts"
    "get-graphql-schema"
    "get_iplayer"
    "getconf"
    "getdata"
    "getdns"
    "getdp"
    "geteduroam"
    "geteduroam-cli"
    "geteltorito"
    "getent"
    "geticons"
    "getmail6"
    "getopt"
    "getoptions"
    "getsploit"
    "gettext"
    "getxbook"
    "gex"
    "gexiv2"
    "gexiv2_0_16"
    "gf"
    "gf2x"
    "gfal2"
    "gfal2-util"
    "gfan"
    "gflags"
    "gfold"
    "gforth"
    "gfs2-utils"
    "gfshare"
    "gftp"
    "gfxreconstruct"
    "gfxstream"
    "gfxtablet"
    "gg-jj"
    "gg-scm"
    "ggh"
    "ggml"
    "ggmorse"
    "ggshield"
    "ggz_base_libs"
    "gh"
    "gh-aw"
    "gh-cal"
    "gh-cherry-pick"
    "gh-classroom"
    "gh-contribs"
    "gh-dash"
    "gh-do"
    "gh-eco"
    "gh-enhance"
    "gh-f"
    "gh-gei"
    "gh-gonest"
    "gh-i"
    "gh-markdown-preview"
    "gh-notify"
    "gh-ost"
    "gh-poi"
    "gh-pr-review"
    "gh-review-conductor"
    "gh-s"
    "gh-screensaver"
    "gh-signoff"
    "gh-skyline"
    "gh-stack"
    "gh-token"
    "gh-webhook"
    "gh2md"
    "ghalint"
    "ghatm"
    "ghauri"
    "ghbackup"
    "ghc_filesystem"
    "ghcherry"
    "ghcid"
    "ghciwatch"
    "ghdl"
    "ghdl-gcc"
    "ghdl-llvm"
    "ghdl-llvm-jit"
    "ghdl-mcode"
    "ghdorker"
    "ghdump"
    "ghex"
    "ghfetch"
    "ghgrab"
    "ghi"
    "ghidra"
    "ghidra-bin"
    "ghmap"
    "ghorg"
    "ghost"
    "ghost-cli"
    "ghost-complete"
    "ghostfolio"
    "ghostie"
    "ghostscript"
    "ghostscriptX"
    "ghostscript_headless"
    "ghosttohugo"
    "ghostty"
    "ghostty-bin"
    "ghostunnel"
    "ghp-import"
    "ghq"
    "ghqr"
    "ghr"
    "ghr-cli"
    "ghrepo-stats"
    "ghstack"
    "ghui"
    "ghunt"
    "ghw"
    "ghz"
    "gi-crystal"
    "gi-docgen"
    "giac"
    "giac-with-xcas"
    "giada"
    "giara"
    "gibberish-detector"
    "gibo"
    "gickup"
    "gif-for-cli"
    "gif2apng"
    "giff"
    "gifgen"
    "giflib"
    "gifsicle"
    "gifski"
    "gifticlib"
    "gigalixir"
    "gigawork"
    "gigedit"
    "gigolo"
    "gildas"
    "gilt-rs"
    "gimagereader"
    "gimagereader-qt"
    "gimme-aws-creds"
    "gimoji"
    "gimp"
    "gimp-with-plugins"
    "gimp2"
    "gimp2-with-plugins"
    "gimp3"
    "gimp3-with-plugins"
    "gimx"
    "ginac"
    "ginkgo"
    "ginko"
    "gip"
    "giph"
    "gir-rs"
    "girara"
    "girouette"
    "girsh"
    "gist"
    "git"
    "git-absorb"
    "git-agecrypt"
    "git-aggregator"
    "git-annex"
    "git-annex-metadata-gui"
    "git-annex-remote-dbx"
    "git-annex-remote-googledrive"
    "git-annex-remote-rclone"
    "git-appraise"
    "git-archive-all"
    "git-autofixup"
    "git-autoshare"
    "git-backdate"
    "git-backup-go"
    "git-bars"
    "git-big-picture"
    "git-branchless"
    "git-branchstack"
    "git-brunch"
    "git-bug"
    "git-bug-migration"
    "git-buildpackage"
    "git-cache"
    "git-chain"
    "git-chglog"
    "git-cinnabar"
    "git-cliff"
    "git-codereview"
    "git-cola"
    "git-conventional-commits"
    "git-crecord"
    "git-credential-aol"
    "git-credential-email"
    "git-credential-gmail"
    "git-credential-gopass"
    "git-credential-keepassxc"
    "git-credential-manager"
    "git-credential-oauth"
    "git-credential-outlook"
    "git-credential-yahoo"
    "git-crypt"
    "git-delete-merged-branches"
    "git-dive"
    "git-doc"
    "git-dumper"
    "git-extras"
    "git-fame"
    "git-fast-export"
    "git-filter-repo"
    "git-fire"
    "git-fixup"
    "git-fork"
    "git-ftp"
    "git-gamble"
    "git-get"
    "git-gone"
    "git-gr"
    "git-graph"
    "git-hound"
    "git-hub"
    "git-identity"
    "git-igitt"
    "git-ignore"
    "git-imerge"
    "git-instafix"
    "git-interactive-rebase-tool"
    "git-jump"
    "git-latexdiff"
    "git-lfs"
    "git-lfs-transfer"
    "git-ls"
    "git-machete"
    "git-metrics"
    "git-mit"
    "git-msgraph"
    "git-my"
    "git-nomad"
    "git-octopus"
    "git-open"
    "git-pages"
    "git-pages-cli"
    "git-pkgs"
    "git-point"
    "git-pr"
    "git-privacy"
    "git-prole"
    "git-protonmail"
    "git-ps-rs"
    "git-publish"
    "git-pw"
    "git-quick-stats"
    "git-quickfix"
    "git-radar"
    "git-recent"
    "git-releaser"
    "git-relevant-history"
    "git-remote-codecommit"
    "git-remote-gcrypt"
    "git-remote-hg"
    "git-reparent"
    "git-repo"
    "git-repo-updater"
    "git-review"
    "git-revise"
    "git-run"
    "git-secret"
    "git-secrets"
    "git-series"
    "git-sim"
    "git-sizer"
    "git-smash"
    "git-spice"
    "git-stack"
    "git-standup"
    "git-statuses"
    "git-subrepo"
    "git-subtrac"
    "git-sync"
    "git-team"
    "git-test"
    "git-together"
    "git-toolbelt"
    "git-tools"
    "git-town"
    "git-trim"
    "git-unroll"
    "git-up"
    "git-upstream"
    "git-vanity-hash"
    "git-vendor"
    "git-wait"
    "git-warp-time"
    "git-when-merged"
    "git-who"
    "git-workspace"
    "git-worktree-switcher"
    "git-wt"
    "git-xet"
    "git2-cpp"
    "git2cl"
    "gitFull"
    "gitMinimal"
    "gitRepo"
    "gitSVN"
    "gitSetupHook"
    "gita"
    "gitaly"
    "gitbatch"
    "gitbeaker-cli"
    "gitbutler"
    "gitcomet"
    "gitcs"
    "gitea"
    "gitea-actions-runner"
    "gitea-mcp-server"
    "giter8"
    "gitfetch"
    "gitflow"
    "gitg"
    "githooks"
    "github-act-runner"
    "github-backup"
    "github-changelog-generator"
    "github-cli"
    "github-commenter"
    "github-copilot-app"
    "github-copilot-cli"
    "github-copilot-intellij-agent"
    "github-desktop"
    "github-distributed-owners"
    "github-linguist"
    "github-markdown-toc-go"
    "github-mcp-server"
    "github-release"
    "github-runner"
    "github-to-sops"
    "github-to-sqlite"
    "gitify"
    "gitingest"
    "gitit"
    "gitjacker"
    "gitkraken"
    "gitlab"
    "gitlab-art"
    "gitlab-ci-linter"
    "gitlab-ci-local"
    "gitlab-ci-ls"
    "gitlab-ci-validate"
    "gitlab-clippy"
    "gitlab-container-registry"
    "gitlab-duo"
    "gitlab-ee"
    "gitlab-elasticsearch-indexer"
    "gitlab-kas"
    "gitlab-pages"
    "gitlab-release-cli"
    "gitlab-runner"
    "gitlab-shell"
    "gitlab-timelogs"
    "gitlab-triage"
    "gitlab-workhorse"
    "gitleaks"
    "gitless"
    "gitlint"
    "gitlogue"
    "gitls"
    "gitmoji-cli"
    "gitmux"
    "gitnapped"
    "gitnr"
    "gitnuro"
    "gitolite"
    "gitopper"
    "gitoxide"
    "gitprompt-rs"
    "gitpulsar"
    "gitqlient"
    "gitrs"
    "gitsign"
    "gitsnip"
    "gitstats"
    "gitstatus"
    "gitte"
    "gittuf"
    "gitty"
    "gittype"
    "gittyup"
    "gitu"
    "gitui"
    "gitwatch"
    "gitweb"
    "gitxray"
    "givaro"
    "gixy"
    "giza"
    "gjo"
    "gjs"
    "gk-cli"
    "gklib"
    "gl-gsync-demo"
    "gl117"
    "gl2ps"
    "gl3w"
    "glab"
    "glab-tui"
    "glabels-qt"
    "glade"
    "gladtex"
    "glamoroustoolkit"
    "glance"
    "glanceclient"
    "glances"
    "glas"
    "glasgow"
    "glasskube"
    "glasstty-ttf"
    "glauth"
    "glava"
    "glaxnimate"
    "glaze"
    "glbinding"
    "gle"
    "gleam"
    "glee"
    "glew"
    "glew110"
    "glew_1_10"
    "glfw"
    "glfw2"
    "glfw3"
    "glfw3-minecraft"
    "glib"
    "glib-networking"
    "glib-testing"
    "glibc"
    "glibcInfo"
    "glibcLocales"
    "glibcLocalesUtf8"
    "glibc_memusage"
    "glibc_multi"
    "glibmm"
    "glibmm_2_68"
    "glibtool"
    "glicol-cli"
    "glide-media-player"
    "glider"
    "glirc"
    "glitchtip"
    "glitter"
    "gllvm"
    "glm"
    "glm_1_0_1"
    "glmark2"
    "global"
    "global-platform-pro"
    "globalarrays"
    "globalping-cli"
    "globalplatform"
    "globe-cli"
    "globulation2"
    "globus-cli"
    "glock"
    "glog"
    "glogg"
    "glooctl"
    "gloox"
    "glow"
    "glow-lang"
    "glowing-bear"
    "glpaper"
    "glpi-agent"
    "glpk"
    "glpng"
    "glrnvim"
    "glsl_analyzer"
    "glslang"
    "glslls"
    "glslviewer"
    "glsurf"
    "gltron"
    "glucose"
    "glucose-syrup"
    "gluesql"
    "glui"
    "glusterfs"
    "glycin-loaders"
    "glycin-thumbnailer"
    "glyr"
    "gmad"
    "gmailctl"
    "gmap"
    "gmetronome"
    "gmic"
    "gmic-qt"
    "gmid"
    "gmime"
    "gmime2"
    "gmime3"
    "gmm"
    "gmnitohtml"
    "gmobile"
    "gmp"
    "gmp6"
    "gmpxx"
    "gmqcc"
    "gmrender-resurrect"
    "gmrun"
    "gmsh"
    "gmt"
    "gn"
    "gnatprove"
    "gnet"
    "gnirehtet"
    "gnmic"
    "gnome-2048"
    "gnome-applets"
    "gnome-autoar"
    "gnome-backgrounds"
    "gnome-bluetooth"
    "gnome-boxes"
    "gnome-browser-connector"
    "gnome-builder"
    "gnome-calculator"
    "gnome-calendar"
    "gnome-characters"
    "gnome-chess"
    "gnome-clocks"
    "gnome-color-manager"
    "gnome-commander"
    "gnome-common"
    "gnome-connections"
    "gnome-console"
    "gnome-contacts"
    "gnome-control-center"
    "gnome-decoder"
    "gnome-desktop"
    "gnome-desktop-testing"
    "gnome-disk-utility"
    "gnome-doc-utils"
    "gnome-epub-thumbnailer"
    "gnome-extension-manager"
    "gnome-extensions-cli"
    "gnome-feeds"
    "gnome-firmware"
    "gnome-flashback"
    "gnome-font-viewer"
    "gnome-frog"
    "gnome-graphs"
    "gnome-inform7"
    "gnome-initial-setup"
    "gnome-keyring"
    "gnome-keysign"
    "gnome-klotski"
    "gnome-kra-ora-thumbnailer"
    "gnome-logs"
    "gnome-mahjongg"
    "gnome-manuals"
    "gnome-maps"
    "gnome-menus"
    "gnome-mines"
    "gnome-monitor-config"
    "gnome-multi-writer"
    "gnome-music"
    "gnome-network-displays"
    "gnome-nibbles"
    "gnome-notes"
    "gnome-obfuscate"
    "gnome-online-accounts"
    "gnome-online-accounts-gtk"
    "gnome-packagekit"
    "gnome-panel"
    "gnome-panel-with-modules"
    "gnome-pass-search-provider"
    "gnome-podcasts"
    "gnome-pomodoro"
    "gnome-pomodoro-watcher"
    "gnome-power-manager"
    "gnome-randr"
    "gnome-remote-desktop"
    "gnome-robots"
    "gnome-screenshot"
    "gnome-secrets"
    "gnome-session"
    "gnome-session-ctl"
    "gnome-settings-daemon"
    "gnome-settings-daemon48"
    "gnome-shell"
    "gnome-shell-extensions"
    "gnome-software"
    "gnome-solanum"
    "gnome-sound-recorder"
    "gnome-sudoku"
    "gnome-system-monitor"
    "gnome-taquin"
    "gnome-tecla"
    "gnome-terminal"
    "gnome-tetravex"
    "gnome-text-editor"
    "gnome-themes-extra"
    "gnome-tour"
    "gnome-tweaks"
    "gnome-usage"
    "gnome-user-docs"
    "gnome-user-share"
    "gnome-video-effects"
    "gnome-weather"
    "gnomecast"
    "gnonograms"
    "gnostic"
    "gnote"
    "gns3-gui"
    "gns3-server"
    "gnss-sdr"
    "gnss-share"
    "gnu-cim"
    "gnu-config"
    "gnu-efi"
    "gnu-pw-mgr"
    "gnu-shepherd"
    "gnu-smalltalk"
    "gnuapl"
    "gnuastro"
    "gnubg"
    "gnucap"
    "gnucap-full"
    "gnucash"
    "gnuchess"
    "gnuclad"
    "gnucobol"
    "gnudatalanguage"
    "gnugo"
    "gnujump"
    "gnuk"
    "gnulib"
    "gnum4"
    "gnumake42"
    "gnumeric"
    "gnunet"
    "gnunet-gtk"
    "gnunet-messenger-cli"
    "gnupatch"
    "gnupg"
    "gnupg-pkcs11-scd"
    "gnupg1"
    "gnupg1compat"
    "gnupg24"
    "gnupgMinimal"
    "gnuplot"
    "gnuplot_aquaterm"
    "gnuplot_qt"
    "gnupth"
    "gnuradio"
    "gnuradioMinimal"
    "gnushogi"
    "gnustep-back"
    "gnustep-base"
    "gnustep-gui"
    "gnustep-libobjc"
    "gnustep-make"
    "gnustep-systempreferences"
    "gnutls"
    "gnvim"
    "gnvim-unwrapped"
    "go"
    "go-2fa"
    "go-arch-lint"
    "go-audit"
    "go-autoconfig"
    "go-avahi-cname"
    "go-away"
    "go-bare"
    "go-bindata"
    "go-bindata-assetfs"
    "go-blueprint"
    "go-callvis"
    "go-camo"
    "go-car"
    "go-check"
    "go-chromecast"
    "go-containerregistry"
    "go-cover-treemap"
    "go-cqhttp"
    "go-critic"
    "go-crx3"
    "go-csp-collector"
    "go-cve-search"
    "go-dnscollector"
    "go-dork"
    "go-ecoflow-exporter"
    "go-enum"
    "go-errorlint"
    "go-ethereum"
    "go-exploitdb"
    "go-font"
    "go-graft"
    "go-grip"
    "go-hass-agent"
    "go-httpbin"
    "go-i18n"
    "go-import-lint"
    "go-ios"
    "go-jet"
    "go-jira"
    "go-jsonnet"
    "go-jsonschema"
    "go-judge"
    "go-junit-report"
    "go-landlock"
    "go-libp2p-daemon"
    "go-librespot"
    "go-licence-detector"
    "go-license-detector"
    "go-licenses"
    "go-md2man"
    "go-migrate"
    "go-minimock"
    "go-mockery"
    "go-mockery_2"
    "go-mod-graph-chart"
    "go-mod-upgrade"
    "go-motion"
    "go-mtpfs"
    "go-musicfox"
    "go-org"
    "go-outline"
    "go-passbolt-cli"
    "go-protobuf"
    "go-rice"
    "go-sct"
    "go-secdump"
    "go-sendxmpp"
    "go-shadowsocks2"
    "go-swag"
    "go-swagger"
    "go-symbols"
    "go-task"
    "go-toml"
    "go-tools"
    "go-tpc"
    "go-xmlstruct"
    "go2rtc"
    "go2tv"
    "go2tv-lite"
    "go365"
    "go9p"
    "go_1_25"
    "go_1_26"
    "go_1_27"
    "go_latest"
    "goa"
    "goaccess"
    "goarista"
    "goat"
    "goat-cli"
    "goatcounter"
    "goattracker"
    "goattracker-stereo"
    "goaway"
    "goawk"
    "gob2"
    "gobatmon"
    "gobble"
    "gobby"
    "gobetween"
    "gobgp"
    "gobgpd"
    "gobi_loader"
    "gobject-introspection"
    "gobject-introspection-unwrapped"
    "goblob"
    "gobuster"
    "gobusybox"
    "gocatcli"
    "gocd-agent"
    "gocd-server"
    "gocheat"
    "gocode-gomod"
    "goconst"
    "goconvey"
    "gocover-cobertura"
    "gocovsh"
    "gocr"
    "gocron"
    "gocryptfs"
    "goctl"
    "gocyclo"
    "goda"
    "godef"
    "godini"
    "godns"
    "godot"
    "godot-export-templates-bin"
    "godot-mcp"
    "godot-mono"
    "godot3"
    "godot3-debug-server"
    "godot3-export-templates"
    "godot3-headless"
    "godot3-mono"
    "godot3-mono-debug-server"
    "godot3-mono-export-templates"
    "godot3-mono-headless"
    "godot3-mono-server"
    "godot3-server"
    "godot_4"
    "godot_4-export-templates-bin"
    "godot_4-mono"
    "godot_4_3"
    "godot_4_3-export-templates-bin"
    "godot_4_3-mono"
    "godot_4_4"
    "godot_4_4-export-templates-bin"
    "godot_4_4-mono"
    "godot_4_5"
    "godot_4_5-export-templates-bin"
    "godot_4_5-mono"
    "godot_4_6"
    "godot_4_6-export-templates-bin"
    "godot_4_6-mono"
    "godot_4_7"
    "godot_4_7-export-templates-bin"
    "godot_4_7-mono"
    "godotpcktool"
    "godspeed"
    "godsvg"
    "godu"
    "goeland"
    "gof5"
    "gofaxip"
    "goffice"
    "gofish"
    "goflow"
    "goflow2"
    "gofu"
    "gofumpt"
    "gogUnpackHook"
    "gogcli"
    "gogdl"
    "gogetdoc"
    "gogh"
    "gogogadget"
    "gogui"
    "gogup"
    "gohai"
    "gohufont"
    "goimapnotify"
    "goimports-reviser"
    "gojo"
    "gojq"
    "gojsontoyaml"
    "gokapi"
    "gokart"
    "gokey"
    "gokrazy"
    "goku"
    "gol"
    "golangci-lint"
    "golangci-lint-langserver"
    "golazo"
    "goldberg-emu"
    "goldboot"
    "golden-cheetah"
    "golden-cheetah-bin"
    "goldendict-ng"
    "golds"
    "golem"
    "golex"
    "golines"
    "golink"
    "golint"
    "gollama"
    "gollum"
    "golly"
    "gom"
    "gomacro"
    "gomanagedocker"
    "gomapenum"
    "gomarkdoc"
    "gomatrix"
    "gomi"
    "gomobile"
    "gomodifytags"
    "gomp"
    "gomplate"
    "gomtree"
    "gomuks"
    "gomuks-desktop"
    "gomuks-web"
    "gonic"
    "gonzo"
    "goobook"
    "goocanvas2"
    "goocanvas3"
    "goocanvas_2"
    "goocanvas_3"
    "goocanvasmm2"
    "goodhosts"
    "goodvibes"
    "goofcord"
    "goofys"
    "google-alloydb-auth-proxy"
    "google-amber"
    "google-app-engine-go-sdk"
    "google-authenticator"
    "google-chat-linux"
    "google-chrome"
    "google-clasp"
    "google-cloud-bigtable-tool"
    "google-cloud-cpp"
    "google-cloud-sdk"
    "google-cloud-sdk-gce"
    "google-cloud-sql-proxy"
    "google-compute-engine"
    "google-cursor"
    "google-drive-ocamlfuse"
    "google-fonts"
    "google-guest-agent"
    "google-guest-configs"
    "google-guest-oslogin"
    "google-java-format"
    "google-lighthouse"
    "google-play"
    "googleearth-pro"
    "googlesans-code"
    "goose"
    "goose-cli"
    "goose-desktop"
    "goose-lang"
    "gopacked"
    "gopass"
    "gopass-hibp"
    "gopass-jsonapi"
    "gopass-summon-provider"
    "gopatch"
    "gopeed"
    "goperf"
    "gopher"
    "gopher64"
    "gopher64-netplay-server"
    "gophernotes"
    "gophertube"
    "gophish"
    "gopkgs"
    "goplantuml"
    "gopls"
    "gopodder"
    "gopro"
    "gopro-tool"
    "gops"
    "gopsuinfo"
    "gorched"
    "gordonflashtool"
    "gore"
    "goread"
    "goredo"
    "goreleaser"
    "goreman"
    "goreplay"
    "goresym"
    "gorilla-bin"
    "gorilla-cli"
    "gorm"
    "gorm-gentool"
    "gortr"
    "gose"
    "gosec"
    "gosh"
    "goshs"
    "gosimports"
    "gosmee"
    "gospider"
    "goss"
    "gossa"
    "gossip"
    "gost"
    "gostatic"
    "gosu"
    "got"
    "gotags"
    "gotemplate"
    "gotenberg"
    "gotest"
    "gotestdox"
    "gotestfmt"
    "gotests"
    "gotestsum"
    "gotestwaf"
    "gotify-cli"
    "gotify-desktop"
    "gotify-server"
    "gotip"
    "gotlsaflare"
    "gotmplfmt"
    "gotmplfmt-hugo"
    "goto"
    "gotools"
    "gotop"
    "gotosocial"
    "gotraceui"
    "gotree"
    "gotrue"
    "gotrue-supabase"
    "gotty"
    "gotypist"
    "goudy-bookletter-1911"
    "goupile"
    "gource"
    "govc"
    "govee2mqtt"
    "goverlay"
    "govers"
    "goverter"
    "goverview"
    "govulncheck"
    "gow"
    "gowall"
    "gowebly"
    "gowitness"
    "gox"
    "goxel"
    "goxlr-utility"
    "gp-saml-gui"
    "gp2c"
    "gpa"
    "gpac"
    "gpac-unstable"
    "gpart"
    "gparted"
    "gparted-full"
    "gpaste"
    "gpauth"
    "gpclient"
    "gpd-pocket-4-pipewire"
    "gperf"
    "gperftools"
    "gpg-mdp"
    "gpg-tui"
    "gpgme"
    "gpgmepp"
    "gphoto2"
    "gphoto2fs"
    "gpick"
    "gping"
    "gpio-utils"
    "gplates"
    "gpm"
    "gpm-ncurses"
    "gpodder"
    "gpp"
    "gprbuild"
    "gpredict"
    "gprename"
    "gprof2dot"
    "gprojector"
    "gprolog"
    "gpsbabel"
    "gpsbabel-gui"
    "gpscorrelate"
    "gpsd"
    "gpsprune"
    "gpt-cli"
    "gpt2tc"
    "gpt4all"
    "gpt4all-cuda"
    "gptcommit"
    "gptfdisk"
    "gptman"
    "gptscript"
    "gpu-burn"
    "gpu-screen-recorder"
    "gpu-screen-recorder-gtk"
    "gpu-screen-recorder-notification"
    "gpu-screen-recorder-ui"
    "gpu-switch"
    "gpu-usage-waybar"
    "gpu-viewer"
    "gpufetch"
    "gpupad"
    "gpustat"
    "gputils"
    "gpuvis"
    "gpx"
    "gpx-animator"
    "gpx-viewer"
    "gpxlab"
    "gpxsee"
    "gpxsee-qt6"
    "gpython"
    "gql"
    "gqlgen"
    "gqlgenc"
    "gqlint"
    "gqmqtt"
    "gqrx"
    "gqrx-gr-audio"
    "gqrx-portaudio"
    "gr-dect2"
    "grabserial"
    "gradescope-submit"
    "gradia"
    "gradle"
    "gradle-completion"
    "gradle-dependency-tree-diff"
    "gradle-native-platform"
    "gradle-unwrapped"
    "gradle_8"
    "gradle_8-unwrapped"
    "gradle_9"
    "gradle_9-unwrapped"
    "gradlew-fdroid"
    "gradm"
    "grafana"
    "grafana-alloy"
    "grafana-dash-n-grab"
    "grafana-image-renderer"
    "grafana-kiosk"
    "grafana-loki"
    "grafana-reporter"
    "grafana-to-ntfy"
    "grafanactl"
    "grafterm"
    "grafx2"
    "grail"
    "grails"
    "gram"
    "gramma"
    "gramps"
    "grandorgue"
    "grandperspective"
    "granian"
    "granted"
    "grantlee"
    "grap"
    "graph-cli"
    "graph-easy"
    "graphene"
    "graphene-hardened-malloc"
    "graphest"
    "graphhopper"
    "graphicsmagick"
    "graphicsmagick-imagemagick-compat"
    "graphicsmagick_q16"
    "graphify"
    "graphinder"
    "graphite"
    "graphite-cli"
    "graphite-cursors"
    "graphite2"
    "graphlan"
    "graphpython"
    "graphql-client"
    "graphql-language-service-cli"
    "graphqlmaker"
    "graphqlmap"
    "graphqurl"
    "graphs"
    "graphviz"
    "graphviz-nox"
    "graphw00f"
    "graphwar"
    "graplang"
    "grasp-backend"
    "grass"
    "grass-sass"
    "grav"
    "gravit"
    "grayjay"
    "grayjay-frontend"
    "grayjay-libcurlshim"
    "graylog-6_0"
    "graylog-6_1"
    "grb"
    "grc"
    "grcov"
    "greaseweazle"
    "greed"
    "greenbone-feed-sync"
    "greenfoot"
    "greenmask"
    "greenx"
    "greetd"
    "greetd-mini-wl-greeter"
    "greg"
    "gremlin-console"
    "gremlin-server"
    "gren"
    "grenedalf"
    "grepcidr"
    "grepm"
    "greptimedb"
    "gretl"
    "grex"
    "greybird"
    "gridcoin-research"
    "gridcoin-researchd"
    "gridlock"
    "gridtracker"
    "gridtracker2"
    "griffe"
    "grilo"
    "grilo-plugins"
    "grim"
    "grimblast"
    "grimoire"
    "grin"
    "grip-grab"
    "grip-search"
    "grisbi"
    "grit"
    "grive2"
    "grizzly"
    "grml-zsh-config"
    "grmon"
    "grobi"
    "grocy"
    "groestlcoin"
    "groestlcoind"
    "groff"
    "grok-build"
    "grok-cli"
    "gromacs"
    "gromacsCudaMpi"
    "gromacsDouble"
    "gromacsDoubleMpi"
    "gromacsMpi"
    "gromacsPlumed"
    "gromit-mpx"
    "gron"
    "groonga"
    "groove"
    "grooveauthor"
    "groovy"
    "groovy-language-server"
    "grpc"
    "grpc-client-cli"
    "grpc-gateway"
    "grpc-health-probe"
    "grpc-tools"
    "grpc_cli"
    "grpcui"
    "grpcurl"
    "grsync"
    "grub2"
    "grub2_efi"
    "grub2_ieee1275"
    "grub2_light"
    "grub2_pvgrub_image"
    "grub2_pvhgrub_image"
    "grub2_xen"
    "grub2_xen_pvh"
    "grummage"
    "grunt-cli"
    "gruppled-black-cursors"
    "gruppled-black-lite-cursors"
    "gruppled-white-cursors"
    "gruppled-white-lite-cursors"
    "gruut"
    "gruut-ipa"
    "gruvbox-dark-gtk"
    "gruvbox-dark-icons-gtk"
    "gruvbox-gtk-theme"
    "gruvbox-kvantum"
    "gruvbox-plus-icons"
    "gruvterial-theme"
    "grype"
    "gs1200-exporter"
    "gsan"
    "gsasl"
    "gscan2pdf"
    "gscreenshot"
    "gsctl"
    "gsd"
    "gsender"
    "gsettings-desktop-schemas"
    "gsettings-qt"
    "gshhg-gmt"
    "gsimplecal"
    "gsl"
    "gsl-lite"
    "gsm"
    "gsmartcontrol"
    "gsmlib"
    "gsoap"
    "gsocket"
    "gsound"
    "gspeech"
    "gspell"
    "gss"
    "gssdp-tools"
    "gssdp_1_6"
    "gssg"
    "gssproxy"
    "gst"
    "gst-thumbnailers"
    "gst123"
    "gswatcher"
    "gt"
    "gt5"
    "gtest"
    "gtfocli"
    "gtg"
    "gthree"
    "gthumb"
    "gti"
    "gtimelog"
    "gtk-doc"
    "gtk-frdp"
    "gtk-gnutella"
    "gtk-layer-shell"
    "gtk-mac-bundler"
    "gtk-mac-integration"
    "gtk-mac-integration-gtk2"
    "gtk-mac-integration-gtk3"
    "gtk-pipe-viewer"
    "gtk-server"
    "gtk-session-lock"
    "gtk-sharp-3_0"
    "gtk-vnc"
    "gtk2"
    "gtk2-x11"
    "gtk3"
    "gtk3-x11"
    "gtk4"
    "gtk4-layer-shell"
    "gtkclipblock"
    "gtkd"
    "gtkdatabox"
    "gtkdialog"
    "gtkglext"
    "gtkgnutella"
    "gtkgreet"
    "gtkhash"
    "gtkhtml"
    "gtklock"
    "gtklock-playerctl-module"
    "gtklock-powerbar-module"
    "gtklock-userinfo-module"
    "gtklock-virtkb-module"
    "gtkmm3"
    "gtkmm4"
    "gtksheet"
    "gtksourceview"
    "gtksourceview3"
    "gtksourceview4"
    "gtksourceview5"
    "gtksourceviewmm"
    "gtksourceviewmm4"
    "gtkspell3"
    "gtkspellmm"
    "gtkterm"
    "gtkwave"
    "gtmathematics"
    "gtop"
    "gtranslator"
    "gtrash"
    "gtree"
    "gts"
    "gtt"
    "gtypist"
    "guacamole-client"
    "guacamole-server"
    "guake"
    "gubbi-font"
    "gucci"
    "gucharmap"
    "guesswidth"
    "guestfs-tools"
    "guetzli"
    "guff"
    "guglielmo"
    "gui-for-singbox"
    "guile"
    "guile-aspell"
    "guile-avahi"
    "guile-cairo"
    "guile-chickadee"
    "guile-colorized"
    "guile-commonmark"
    "guile-config"
    "guile-curl"
    "guile-fibers"
    "guile-gcrypt"
    "guile-git"
    "guile-gnutls"
    "guile-goblins"
    "guile-hall"
    "guile-hoot"
    "guile-irregex"
    "guile-json"
    "guile-json-rpc"
    "guile-lib"
    "guile-lsp-server"
    "guile-lzlib"
    "guile-lzma"
    "guile-mqtt"
    "guile-ncurses"
    "guile-opengl"
    "guile-quickcheck"
    "guile-reader"
    "guile-redis"
    "guile-sdl2"
    "guile-semver"
    "guile-sjson"
    "guile-sqlite3"
    "guile-srfi-145"
    "guile-srfi-180"
    "guile-ssh"
    "guile-websocket"
    "guile-xcb"
    "guile-zlib"
    "guile-zstd"
    "guile_1_8"
    "guile_2_0"
    "guile_2_2"
    "guile_3_0"
    "guilt"
    "guitarix"
    "guitarix-vst"
    "guix"
    "gum"
    "gumbo"
    "gummi"
    "gummy"
    "gungnir"
    "gup"
    "gupnp-av"
    "gupnp-dlna"
    "gupnp-igd"
    "gupnp-tools"
    "gupnp_1_6"
    "gurk-rs"
    "gurobi"
    "gusb"
    "gut"
    "gutenprint"
    "gutenprint-bin"
    "gutenprintBin"
    "guvcview"
    "guymager"
    "gv"
    "gvfs"
    "gvisor"
    "gvm-libs"
    "gvm-tools"
    "gvpe"
    "gvproxy"
    "gwe"
    "gweather-locations"
    "gweled"
    "gwenhywfar"
    "gworkspace"
    "gwq"
    "gwrap"
    "gws"
    "gwt240"
    "gwyddion"
    "gx"
    "gx-go"
    "gxemul"
    "gxkb"
    "gxmatcheq-lv2"
    "gxmessage"
    "gxml"
    "gxplugins-lv2"
    "gyb"
    "gyre-fonts"
    "gyroflow"
    "gyroflow-bin"
    "gz-cmake"
    "gz-math"
    "gz-utils"
    "gzdoom"
    "gzrt"
    "h"
    "h2"
    "h2o"
    "h2spec"
    "h3"
    "h3_3"
    "h3_4"
    "h5glance"
    "h5utils"
    "h5v"
    "h8mail"
    "ha-mcp"
    "ha-mqtt-discoverable-cli"
    "habitat"
    "hachimarupop"
    "hachoir"
    "hack-font"
    "hackedbox"
    "hackernews-tui"
    "hackertyper"
    "hackgen-font"
    "hackgen-nf-font"
    "hackgregator"
    "hackneyed"
    "hackrf"
    "hacksguard"
    "hacompanion"
    "hactool"
    "hadolint"
    "hadolint-sarif"
    "hadoop"
    "hadoop2"
    "hadoop3"
    "hadoop_3_3"
    "hadoop_3_4"
    "haguichi"
    "hail"
    "haiti"
    "hakrawler"
    "hakuneko"
    "hal-hardware-analyzer"
    "half"
    "halfempty"
    "halftone"
    "halibut"
    "halide"
    "halloy"
    "halo"
    "halp"
    "ham"
    "hamlib"
    "hamlib_3"
    "hamlib_4"
    "hammer"
    "hamr"
    "hamrs"
    "hamrs-pro"
    "hamsket"
    "hamster"
    "hamtransfer"
    "hanazono"
    "handbrake"
    "handheld-daemon"
    "handheld-daemon-ui"
    "handlr"
    "handlr-regex"
    "handy"
    "hanken-grotesk"
    "hanko"
    "hannom"
    "hans"
    "hap-py"
    "happy"
    "happy-coder"
    "haproxy"
    "har-to-k6"
    "harbor-cli"
    "hardinfo2"
    "hare"
    "hareHook"
    "harec"
    "haredo"
    "haredoc"
    "harfbuzz"
    "harfbuzzFull"
    "harlequin"
    "harminv"
    "harmonia"
    "harmonist"
    "harmonoid"
    "harper"
    "harper-desktop"
    "harsh"
    "haruna"
    "harvid"
    "has"
    "hasciicam"
    "hase"
    "hash-identifier"
    "hash-slinger"
    "hash_extender"
    "hashcash"
    "hashcat"
    "hashcat-utils"
    "hashdeep"
    "hashes"
    "hashi-ui"
    "hashi-up"
    "hashid"
    "hashit"
    "hashlink"
    "hashpump"
    "hashrat"
    "haskell-ci"
    "haskell-language-server"
    "hasklig"
    "hassil"
    "haste-client"
    "haste-server"
    "hasura-cli"
    "hat-trie"
    "hatari"
    "hatch"
    "hatchet"
    "hath-rust"
    "hatsu"
    "haunt"
    "haveged"
    "haven"
    "havn"
    "havoc"
    "hawkeye"
    "hawkthorne-journey"
    "hax11"
    "haxe"
    "haxe_4_3"
    "hayabusa"
    "hayabusa-sec"
    "hayagriva"
    "haylxon"
    "hb-honeypot"
    "hbase"
    "hbase2"
    "hbase3"
    "hbase_2_4"
    "hbase_2_5"
    "hbase_2_6"
    "hbase_3_0"
    "hblock"
    "hcdiag"
    "hci"
    "hck"
    "hcl2json"
    "hcledit"
    "hclfmt"
    "hcli"
    "hcloud"
    "hcloud-upload-image"
    "hcom"
    "hcp"
    "hcxdumptool"
    "hcxtools"
    "hd-idle"
    "hdaps-gl"
    "hdapsd"
    "hdate"
    "hddfancontrol"
    "hddtemp"
    "hdf4"
    "hdf5"
    "hdf5-blosc"
    "hdf5-cpp"
    "hdf5-fortran"
    "hdf5-fortran-mpi"
    "hdf5-mpi"
    "hdf5-threadsafe"
    "hdf5_1_10"
    "hdfview"
    "hdhomerun-config-gui"
    "hdl-dump"
    "hdos"
    "hdparm"
    "hdr10plus"
    "hdr10plus_tool"
    "hdrhistogram_c"
    "hdrmerge"
    "hdrop"
    "hdt"
    "headache"
    "headlamp"
    "headlamp-frontend"
    "headlamp-server"
    "headlines"
    "headphones"
    "headphones-toolbox"
    "headplane"
    "headplane-agent"
    "headscale"
    "headset"
    "headset-charge-indicator"
    "headsetcontrol"
    "health"
    "health-check"
    "healthchecks"
    "heaptrack"
    "heartbeat"
    "heartbeat7"
    "heatclient"
    "heatseeker"
    "heatshrink"
    "hebbot"
    "hebcal"
    "hecate"
    "hedgedoc"
    "hedgedoc-cli"
    "hedgemodmanager"
    "hedgewars"
    "hednsextractor"
    "heh"
    "heidisql"
    "heimdal"
    "heimdall"
    "heimdall-gui"
    "heimdall-proxy"
    "heimer"
    "heisenbridge"
    "hekatomb"
    "helio-workstation"
    "heliocron"
    "helix"
    "helix-db"
    "helix-unwrapped"
    "hellmaker"
    "hello"
    "hello-cpp"
    "hello-go"
    "hello-unfree"
    "hello-wayland"
    "hellwal"
    "helm"
    "helm-dashboard"
    "helm-docs"
    "helm-ls"
    "helmfile"
    "helmfile-wrapped"
    "helmholtz"
    "helmify"
    "helmsman"
    "help2man"
    "helvetica-neue-lt-std"
    "helvum"
    "hentai-at-home"
    "hepmc2"
    "hepmc3"
    "heptagon"
    "herbe"
    "herbstluftwm"
    "hercules"
    "hercules-ci-agent"
    "herdr"
    "heretic"
    "hermit"
    "hermitcli"
    "heroic"
    "heroic-unwrapped"
    "heroku"
    "herqq"
    "herwig"
    "hetzner-kube"
    "hevea"
    "hevi"
    "hex"
    "hex-a-hop"
    "hexagonrpc"
    "hexapdf"
    "hexbinhex"
    "hexcurse"
    "hexd"
    "hexdiff"
    "hexdino"
    "hexdump"
    "hexedit"
    "hexfiend"
    "hexgui"
    "hexio"
    "hexo-cli"
    "hexpatch"
    "hextazy"
    "hexxy"
    "hexyl"
    "hey"
    "hey-mail"
    "heygpt"
    "heynote"
    "hfinger"
    "hfsprogs"
    "hfst"
    "hfst-ospell"
    "hfsutils"
    "hgrep"
    "hh-suite"
    "hheretic"
    "hhexen"
    "hhpc"
    "hickory-dns"
    "hicolor-icon-theme"
    "hid-listen"
    "hid-tools"
    "hidapi"
    "hidapitester"
    "hidden-bar"
    "hidrd"
    "hidviz"
    "hiera-eyaml"
    "hieroglyphic"
    "hifile"
    "higan"
    "high-tide"
    "highfive"
    "highfive-mpi"
    "highlight"
    "highlight-assertions"
    "highlight-pointer"
    "highs"
    "highscore"
    "highscore-blastem"
    "highscore-bsnes"
    "highscore-desmume"
    "highscore-gearsystem"
    "highscore-mednafen"
    "highscore-mgba"
    "highscore-mupen64plus"
    "highscore-nestopia"
    "highscore-prosystem"
    "highscore-sameboy"
    "highscore-stella"
    "highscore-unwrapped"
    "hii"
    "hikounomizu"
    "hiksink"
    "hilbish"
    "himalaya"
    "hime"
    "himitsu"
    "hinit"
    "hinoirisetr"
    "hired"
    "hiredis"
    "hiredis-vip"
    "hisat2"
    "hishtory"
    "hister"
    "hitch"
    "hitori"
    "hittekaart"
    "hivelytracker"
    "hivemind"
    "hivex"
    "hjson"
    "hjson-go"
    "hk"
    "hl-log-viewer"
    "hledger"
    "hledger-check-fancyassertions"
    "hledger-fmt"
    "hledger-iadd"
    "hledger-interest"
    "hledger-ui"
    "hledger-utils"
    "hledger-web"
    "hlint"
    "hlsdk-portable"
    "hm"
    "hmat-oss"
    "hmcl"
    "hmm"
    "hmmer"
    "hn-text"
    "hns"
    "hnswlib"
    "hoard"
    "hobbits"
    "hockeypuck"
    "hockeypuck-web"
    "hocr-tools"
    "hof"
    "hol"
    "hol_light"
    "holehe"
    "hollywood"
    "holo-build"
    "holo-cli"
    "holo-daemon"
    "holochain-launcher"
    "hologram"
    "holos"
    "home-assistant"
    "home-assistant-cli"
    "home-assistant-matter-hub"
    "home-manager"
    "homebank"
    "homebox"
    "homebridge"
    "homebridge-config-ui-x"
    "homepage-dashboard"
    "homer"
    "homerow"
    "homesick"
    "honcho"
    "honey-home"
    "honeycomb-refinery"
    "honeyfetch"
    "honeymarker"
    "honeytail"
    "honeytrap"
    "honeyvent"
    "hongdown"
    "honggfuzz"
    "honk"
    "hooky"
    "hopper"
    "hoppet"
    "hoppscotch"
    "horcrux"
    "horizon-eda"
    "horst"
    "host"
    "host-spawn"
    "hostapd"
    "hostapd-mana"
    "hostctl"
    "hostess"
    "hostmux"
    "hostname"
    "hostname-debian"
    "hosts-bl"
    "hostsblock"
    "hot-resize"
    "hotdoc"
    "hotpatch"
    "hotspot"
    "hottext"
    "houdini"
    "hound"
    "hours"
    "hover"
    "hovercraft"
    "howard-hinnant-date"
    "howdy"
    "howl"
    "hp-unified-linux-driver"
    "hp2p"
    "hpack"
    "hpccm"
    "hpcg"
    "hpe-ltfs"
    "hping"
    "hpipm"
    "hpl"
    "hplip"
    "hplipWithPlugin"
    "hpp2plantuml"
    "hpx"
    "hqplayer-desktop"
    "hqplayerd"
    "hr"
    "hred"
    "hscolour"
    "hsd"
    "hsetroot"
    "hspell"
    "hsphfpd"
    "hsqldb"
    "hss"
    "hstr"
    "hstsparser"
    "ht"
    "htcondor"
    "htgettoken"
    "html-minifier"
    "html-proofer"
    "html-tidy"
    "html-xml-utils"
    "html2pdf"
    "html2text"
    "html5validator"
    "htmlcxx"
    "htmldoc"
    "htmlhint"
    "htmlq"
    "htmltest"
    "htmlunit-driver"
    "htmx-lsp"
    "htmx-lsp2"
    "htop"
    "htop-vim"
    "htpdate"
    "htslib"
    "httm"
    "http-getter"
    "http-nu"
    "http-parser"
    "http-scanner"
    "http-server"
    "http2tcp"
    "http3-ytproxy"
    "httpdirfs"
    "httpdump"
    "httpie"
    "httpie-desktop"
    "httping"
    "httplab"
    "httplib"
    "httpref"
    "httprobe"
    "https-dns-proxy"
    "httpstat"
    "httptap"
    "httptoolkit"
    "httptoolkit-server"
    "httptunnel"
    "httpunit"
    "httpx"
    "httpy-cli"
    "httpyac"
    "httrack"
    "httraqt"
    "hub"
    "hubble"
    "hubot-sans"
    "hubstaff"
    "hue-cli"
    "hue-plus"
    "hueadm"
    "huggle"
    "hugin"
    "hugo"
    "hugs"
    "huion-switcher"
    "hujsonfmt"
    "hullcaster"
    "humanfriendly"
    "humanity-icon-theme"
    "humblebundle-downloader"
    "humility"
    "humioctl"
    "huniq"
    "hunk"
    "hunspell"
    "hunt"
    "hurl"
    "hushboard"
    "husky"
    "hut"
    "hvm"
    "hw-probe"
    "hwatch"
    "hwdata"
    "hwi"
    "hwinfo"
    "hwloc"
    "hx-lsp"
    "hxcpp"
    "hxtools"
    "hy"
    "hybridreverb2"
    "hydra"
    "hydra-ant-logger"
    "hydra-check"
    "hydra-cli"
    "hydraAntLogger"
    "hydralauncher"
    "hydrapaper"
    "hydrasect"
    "hydrogen"
    "hydrogen-web"
    "hydrogen-web-unwrapped"
    "hydroxide"
    "hydrus"
    "hyfetch"
    "hygg"
    "hylafaxplus"
    "hyp"
    "hyp2mat"
    "hyper"
    "hyper-cmd-utils"
    "hyper8"
    "hyperbeam"
    "hyperblobs"
    "hyperfine"
    "hyperglot"
    "hyperhdr"
    "hyperion-ng"
    "hyperledger-fabric"
    "hyperlink"
    "hyperpotamus"
    "hyperrogue"
    "hyperscan"
    "hyperscrypt-font"
    "hypershell"
    "hyperspeedcube"
    "hyperssh"
    "hyphen"
    "hypnotix"
    "hypr"
    "hyprcursor"
    "hyprdim"
    "hyprdynamicmonitors"
    "hypre"
    "hyprfreeze"
    "hyprgraphics"
    "hypridle"
    "hyprkeys"
    "hyprland"
    "hyprland-activewindow"
    "hyprland-autoname-workspaces"
    "hyprland-monitor-attached"
    "hyprland-per-window-layout"
    "hyprland-preview-share-picker"
    "hyprland-protocols"
    "hyprland-qt-support"
    "hyprland-qtutils"
    "hyprland-workspaces"
    "hyprland-workspaces-tui"
    "hyprlang"
    "hyprlauncher"
    "hyprlax"
    "hyprlock"
    "hyprls"
    "hyprmag"
    "hyprmod"
    "hyprmon"
    "hyprmoncfg"
    "hyprnome"
    "hyprnotify"
    "hyprpaper"
    "hyprpicker"
    "hyprpolkitagent"
    "hyprprop"
    "hyprproxlock"
    "hyprpwcenter"
    "hyprquickframe"
    "hyprscratch"
    "hyprshade"
    "hyprshell"
    "hyprshot"
    "hyprshutdown"
    "hyprsome"
    "hyprspace"
    "hyprsunset"
    "hyprsysteminfo"
    "hyprtoolkit"
    "hyprutils"
    "hyprviz"
    "hyprwayland-scanner"
    "hyprwhspr-rs"
    "hyprwire"
    "hypseus-singe"
    "hysteria"
    "hyx"
    "i-dot-ming"
    "i-pi"
    "i18next-cli"
    "i2c-tools"
    "i2p"
    "i2pd"
    "i2pd-tools"
    "i3"
    "i3-auto-layout"
    "i3-back"
    "i3-balance-workspace"
    "i3-cycle-focus"
    "i3-easyfocus"
    "i3-layout-manager"
    "i3-open-next-ws"
    "i3-ratiosplit"
    "i3-resurrect"
    "i3-rounded"
    "i3-swallow"
    "i3-volume"
    "i3-wk-switch"
    "i3a"
    "i3altlayout"
    "i3bar-river"
    "i3blocks"
    "i3blocks-gaps"
    "i3ipc-glib"
    "i3lock"
    "i3lock-blur"
    "i3lock-color"
    "i3lock-fancy"
    "i3lock-fancy-rapid"
    "i3minator"
    "i3pystatus"
    "i3status"
    "i3status-rust"
    "i3wsr"
    "i7z"
    "i810switch"
    "ia-writer-duospace"
    "ia-writer-mono"
    "ia-writer-quattro"
    "iaca"
    "iaca_2_1"
    "iaca_3_0"
    "iagno"
    "iaito"
    "iam-policy-json-to-terraform"
    "iamb"
    "iamy"
    "iana-etc"
    "iannix"
    "ianny"
    "iat"
    "iay"
    "ibm-plex"
    "ibmcloud-cli"
    "ibtool"
    "ibus"
    "ibus-theme-tools"
    "ibus-with-plugins"
    "ibusMinimal"
    "ic-keysmith"
    "ical2orgpy"
    "icbm3d"
    "icdiff"
    "ice-bar"
    "iceauth"
    "icebreaker"
    "icecast"
    "icecream"
    "icemon"
    "icepeak"
    "iceshelf"
    "icesl"
    "icesprog"
    "icestorm"
    "icestudio"
    "icewm"
    "icinga2"
    "icinga2-agent"
    "icingaweb2"
    "icingaweb2-ipl"
    "icingaweb2-thirdparty"
    "icloudpd"
    "icm"
    "icmake"
    "icnsify"
    "ico"
    "icoextract"
    "icomoon-feather"
    "icon-lang"
    "icon-library"
    "icon-slicer"
    "iconConvTools"
    "iconic"
    "iconnamingutils"
    "iconpack-jade"
    "iconpack-obsidian"
    "iconv"
    "icoutils"
    "icr"
    "ictree"
    "icu"
    "icu60"
    "icu63"
    "icu64"
    "icu66"
    "icu67"
    "icu70"
    "icu71"
    "icu72"
    "icu73"
    "icu74"
    "icu75"
    "icu76"
    "icu77"
    "icu78"
    "id3"
    "id3lib"
    "id3v2"
    "ida-free"
    "idasen"
    "idb-companion"
    "ideamaker"
    "identity"
    "ideogram"
    "idescriptor"
    "ideviceinstaller"
    "idevicerestore"
    "idle3tools"
    "idmail"
    "idnkit"
    "idris"
    "idris2"
    "idsk"
    "idutils"
    "ieda"
    "ietf-cli"
    "ifdnfc"
    "ifdnfc-nci"
    "ifdtool"
    "ifenslave"
    "iferr"
    "ifm"
    "ifm-web"
    "ifmetric"
    "ifrextractor-rs"
    "ifstat-legacy"
    "ifstate"
    "iftop"
    "ifupdown-ng"
    "ifuse"
    "ifwifi"
    "igir"
    "igmpproxy"
    "ignite-cli"
    "ignition"
    "igprof"
    "igraph"
    "igraph_0"
    "igrep"
    "igsc"
    "igv"
    "ihaskell"
    "ihp-new"
    "ii"
    "iina"
    "iio-hyprland"
    "iio-niri"
    "iio-oscilloscope"
    "iio-sensor-proxy"
    "iir1"
    "ijhttp"
    "ijq"
    "ijs"
    "ike-scan"
    "ikill"
    "ikiwiki"
    "ikiwiki-full"
    "iksemel"
    "ilbc"
    "ili2c"
    "illum"
    "illuminanced"
    "iloader"
    "ilspycmd"
    "ima-evm-utils"
    "image-analyzer"
    "image-roll"
    "image_optim"
    "imagej"
    "imagelol"
    "imagemagick"
    "imagemagickBig"
    "imagemagick_light"
    "imager"
    "imageworsener"
    "imaginary"
    "imagination"
    "imagineer"
    "imake"
    "imapdedup"
    "imapfilter"
    "imapgoose"
    "imapsync"
    "imath"
    "imdshift"
    "imewlconverter"
    "img"
    "img-cat"
    "img2pdf"
    "img4lib"
    "img4tool"
    "imgbrd-grabber"
    "imgcat"
    "imgcrypt"
    "imgp"
    "imgpatchtools"
    "imgpkg"
    "imgproxy"
    "imgui"
    "imgui-file-dialog"
    "imgur-screenshot"
    "imgurbash2"
    "imhex"
    "iml"
    "imlib2"
    "imlib2-nox"
    "imlib2Full"
    "imlibsetroot"
    "immer"
    "immersed"
    "immich"
    "immich-cli"
    "immich-go"
    "immich-kiosk"
    "immich-machine-learning"
    "immich-public-proxy"
    "immichframe"
    "immudb"
    "imnodes"
    "impala"
    "impl"
    "implot"
    "imposm"
    "impression"
    "improv-setup"
    "impy"
    "imsprog"
    "imtui"
    "imv"
    "imwheel"
    "in-formant"
    "inadyn"
    "inav-blackbox-tools"
    "inav-configurator"
    "inchi"
    "include-what-you-use"
    "inconsolata"
    "inconsolata-lgc"
    "incron"
    "incus"
    "incus-compose"
    "incus-lts"
    "incus-spawn"
    "incus-ui-canonical"
    "indent"
    "indexed-bzip2"
    "indi-full"
    "indi-full-nonfree"
    "indi-with-drivers"
    "indicator-application-gtk3"
    "indicator-sound-switcher"
    "indilib"
    "indradb-client"
    "indradb-server"
    "industrializer"
    "ineffassign"
    "inetutils"
    "infamousPlugins"
    "infamousplugins"
    "infat"
    "infer"
    "inferno"
    "infisical"
    "infisicalsdk"
    "inflow"
    "influxdb"
    "influxdb-cxx"
    "influxdb-mcp-server"
    "influxdb2"
    "influxdb2-cli"
    "influxdb2-provision"
    "influxdb2-server"
    "influxdb2-token-manipulator"
    "influxdb3"
    "infnoise"
    "inform6"
    "inform7"
    "infra"
    "infra-arcana"
    "infracost"
    "inframap"
    "infrastructure-agent"
    "ingen"
    "ingress2gateway"
    "inherd-quake"
    "inhibridge"
    "inih"
    "iniparser"
    "initool"
    "inja"
    "ink"
    "inkcut"
    "inklecate"
    "inklingreader"
    "inko"
    "inkscape"
    "inkscape-with-extensions"
    "inlyne"
    "inmarsatc"
    "innernet"
    "inngest"
    "innoextract"
    "inochi-creator"
    "inochi-session"
    "inori"
    "inormalize"
    "inotify-info"
    "inotify-tools"
    "input-fonts"
    "input-leap"
    "input-remapper"
    "inputmodule-control"
    "inputplug"
    "inputplumber"
    "inql"
    "inriafonts"
    "insert-dylib"
    "inshellisense"
    "insomnia"
    "inspec"
    "inspector"
    "inspectrum"
    "inspircd"
    "inspircdMinimal"
    "install-nothing"
    "installFonts"
    "installShellFiles"
    "instaloader"
    "instant-space-switcher"
    "instawow"
    "instead"
    "insulator2"
    "insync"
    "insync-nautilus"
    "integratorxx"
    "integresql"
    "integrity-scrub"
    "intel-cmt-cat"
    "intel-compute-runtime"
    "intel-compute-runtime-legacy1"
    "intel-gmmlib"
    "intel-gpu-tools"
    "intel-graphics-compiler"
    "intel-ipsec-mb"
    "intel-llvm"
    "intel-media-driver"
    "intel-media-sdk"
    "intel-npu-driver"
    "intel-ocl"
    "intel-one-mono"
    "intel-oneapi-toolkit"
    "intel-undervolt"
    "intel-vaapi-driver"
    "intel2200BGFirmware"
    "intelephense"
    "intelli-shell"
    "intelmetool"
    "intelp2m"
    "inteltool"
    "intensity-normalization"
    "intentrace"
    "inter"
    "inter-alia"
    "inter-nerdfont"
    "interactive-html-bom"
    "interactsh"
    "intercal"
    "intercept-build"
    "interception-tools"
    "interlock"
    "interlude"
    "intermodal"
    "internalallthethings"
    "internetarchive"
    "intersphinx-registry"
    "interstellar"
    "interval-tree"
    "intiface-central"
    "intltool"
    "introspectme"
    "introspectre"
    "intune-portal"
    "inv-sig-helper"
    "inventree"
    "inventree-part-import"
    "invidious"
    "invidious-router"
    "invidtui"
    "invoice"
    "invoice2data"
    "invoiceplane"
    "inxi"
    "io"
    "ioc-scan"
    "iocaine"
    "iocextract"
    "iocsearcher"
    "iodash"
    "iodine"
    "iomelt"
    "ion"
    "ionic-cli"
    "ioping"
    "ioport"
    "ioq3-scion"
    "ioquake3"
    "ior"
    "iortcw"
    "iortcw_sp"
    "ios-cross-compile"
    "ios-deploy"
    "ios-safari-remote-debug"
    "ios-webkit-debug-proxy"
    "iosevka"
    "iosevka-bin"
    "iotas"
    "iotools"
    "iotop"
    "iotop-c"
    "iozone"
    "ioztat"
    "ip2asn"
    "ip2location"
    "ip2location-c"
    "ip2unix"
    "ipad_charge"
    "ipaexfont"
    "ipafont"
    "ipam"
    "ipamjfont"
    "ipatool"
    "ipbt"
    "ipbus-uhal"
    "ipcalc"
    "ipe"
    "iperf"
    "iperf2"
    "iperf3"
    "iperf3d"
    "ipfetch"
    "ipfs-cluster"
    "ipfs-upload-client"
    "ipget"
    "ipgrep"
    "ipinfo"
    "iplan"
    "iplookup-gtk"
    "ipmicfg"
    "ipmitool"
    "ipmiutil"
    "ipmiview"
    "ipopt"
    "ipp-usb"
    "iprange"
    "iproute2"
    "iproute2mac"
    "iprover"
    "ipscan"
    "ipset"
    "ipsw"
    "iptables"
    "iptables-legacy"
    "iptables-nftables-compat"
    "iptraf-ng"
    "iptsd"
    "iptstate"
    "iptvnator"
    "ipu6-camera-bins"
    "ipu6-camera-hal"
    "ipu6ep-camera-hal"
    "ipu6epmtl-camera-hal"
    "iputils"
    "ipv6calc"
    "ipvsadm"
    "ipw2200-firmware"
    "ipxe"
    "iqtree"
    "ir-lv2"
    "ir-standard-fonts"
    "irccat"
    "irccloud"
    "ircd-hybrid"
    "ircdHybrid"
    "ircdog"
    "iredis"
    "irker"
    "iro"
    "irods"
    "irods-icommands"
    "iroh-dns-server"
    "iroh-relay"
    "iroh-ssh"
    "ironbar"
    "ironcalc"
    "ironicclient"
    "ironwail"
    "irony-server"
    "irpf"
    "irqbalance"
    "irrd"
    "irrlicht"
    "irssi"
    "irtt"
    "iruby"
    "irust"
    "is-fast"
    "isa-l"
    "isabelle"
    "isc-cron"
    "iscc"
    "isd"
    "ised"
    "isisdl"
    "isl"
    "isl_0_20"
    "isl_0_23"
    "isl_0_27"
    "isle-portable"
    "iso-flags"
    "iso-flags-png-320x240"
    "isocodes"
    "isolate"
    "isolyzer"
    "isomd5sum"
    "isort"
    "ispc"
    "ispell"
    "ispike"
    "isponsorblocktv"
    "isrcsubmit"
    "isso"
    "issue2md"
    "istat-menus"
    "istioctl"
    "isync"
    "it-tools"
    "itamae"
    "itch"
    "itch-dl"
    "itchiodl"
    "itd"
    "ite-backlight"
    "iterm2"
    "itgmania"
    "itk"
    "itk_5"
    "itk_5_2"
    "itools"
    "itstool"
    "itsx"
    "itsycal"
    "iucode-tool"
    "ivan"
    "iverilog"
    "ivette"
    "ivm"
    "ivpn"
    "ivpn-service"
    "ivpn-ui"
    "ivsc-firmware"
    "ivy"
    "ivyterm"
    "iw"
    "iw4x-launcher"
    "iwd"
    "iwe"
    "iwgtk"
    "iwmenu"
    "iwona"
    "iwqr"
    "izrss"
    "j"
    "j2cli"
    "j2lint"
    "j4-dmenu-desktop"
    "ja2-stracciatella"
    "jaaa"
    "jabcode"
    "jabcode-reader"
    "jabcode-writer"
    "jabref"
    "jacinda"
    "jack-autoconnect"
    "jack-example-tools"
    "jack-link"
    "jack-passthrough"
    "jack1"
    "jack2"
    "jack_autoconnect"
    "jack_capture"
    "jack_oscrolloscope"
    "jackass"
    "jackett"
    "jackline"
    "jackmeter"
    "jackmix"
    "jackmix_jack1"
    "jacktrip"
    "jacoco"
    "jade"
    "jadx"
    "jaeles"
    "jaero"
    "jags"
    "jai"
    "jai-jail"
    "jailer"
    "jake"
    "jalv"
    "jalv-qt"
    "jam"
    "jama"
    "jameica"
    "jamesdsp"
    "jamesdsp-pulse"
    "jami"
    "jamin"
    "jamulus"
    "jan"
    "janet"
    "jankyborders"
    "jansson"
    "janus-gateway"
    "janusgraph"
    "japa"
    "jaq"
    "jarl"
    "jart-jsoncpp"
    "jasmin"
    "jasmin-compiler"
    "jasp-desktop"
    "jasper"
    "jaspr_cli"
    "jasterix"
    "java-cup"
    "java-hamcrest"
    "java-language-server"
    "java-service-wrapper"
    "javaCup"
    "javacc"
    "javascript-typescript-langserver"
    "jawiki-all-titles-in-ns0"
    "jaxodraw"
    "jay"
    "jazz2"
    "jazz2-content"
    "jazzy"
    "jbake"
    "jbang"
    "jbig2dec"
    "jbig2enc"
    "jbigkit"
    "jbofihe"
    "jboss"
    "jboss_mysql_jdbc"
    "jbrowse"
    "jc"
    "jc303"
    "jcal"
    "jchempaint"
    "jcli"
    "jconvolver"
    "jd-diff-patch"
    "jdd"
    "jdenticon-cli"
    "jdepend"
    "jdiskreport"
    "jdk"
    "jdk11"
    "jdk11_headless"
    "jdk17"
    "jdk17_headless"
    "jdk21"
    "jdk21_headless"
    "jdk25"
    "jdk25_headless"
    "jdk8"
    "jdk8_headless"
    "jdk_headless"
    "jdom"
    "jdt-language-server"
    "jdupes"
    "jed"
    "jedit"
    "jefferson"
    "jekyll"
    "jello"
    "jello-lang"
    "jelly"
    "jellycli"
    "jellyfin"
    "jellyfin-desktop"
    "jellyfin-ffmpeg"
    "jellyfin-media-player"
    "jellyfin-mpv-shim"
    "jellyfin-rpc"
    "jellyfin-tui"
    "jellyfin-web"
    "jellyfish"
    "jellyflix"
    "jellyseerr"
    "jellystat"
    "jellytui"
    "jemalloc"
    "jen"
    "jenkins"
    "jenkins-job-builder"
    "jerryscript"
    "jet"
    "jet-pilot"
    "jetbrains-libdbm"
    "jetbrains-mono"
    "jetbrains-runner"
    "jetbrains-toolbox"
    "jetring"
    "jetty"
    "jetty_12"
    "jetuml"
    "jextract"
    "jf"
    "jfbview"
    "jffi"
    "jflap"
    "jflex"
    "jfmt"
    "jfrog-cli"
    "jfsutils"
    "jfsw"
    "jftui"
    "jgmenu"
    "jhead"
    "jhiccup"
    "jib-cli"
    "jibri"
    "jicofo"
    "jid"
    "jigasi"
    "jigdo"
    "jigmo"
    "jikken"
    "jimtcl"
    "jing"
    "jing-trang"
    "jinja-lsp"
    "jinja2-cli"
    "jiq"
    "jira-cli-go"
    "jirafeau"
    "jiratui"
    "jiten"
    "jitsi-excalidraw"
    "jitsi-meet"
    "jitsi-meet-electron"
    "jitsi-meet-prosody"
    "jitsi-videobridge"
    "jitterentropy"
    "jitterentropy-rngd"
    "jive"
    "jj"
    "jj-fzf"
    "jj-pre-push"
    "jj-starship"
    "jj-vine"
    "jjui"
    "jkqtplotter"
    "jl"
    "jless"
    "jmc2obj"
    "jmespath"
    "jmeter"
    "jmol"
    "jmusicbot"
    "jna"
    "jnetmap"
    "jnettop"
    "jnitrace"
    "jnr-posix"
    "jnv"
    "jo"
    "job-security"
    "jobber"
    "jocalsend"
    "joe"
    "jogger"
    "jogl"
    "john"
    "johnny"
    "johnny-reborn"
    "johnny-reborn-engine"
    "joincap"
    "joker"
    "jol"
    "jolt-tui"
    "jonquil"
    "jool-cli"
    "joomscan"
    "joplin"
    "joplin-cli"
    "joplin-desktop"
    "jose"
    "josh"
    "joshuto"
    "josm"
    "jost"
    "jot"
    "jotdown"
    "jotta-cli"
    "joularjx"
    "journaldriver"
    "journalist"
    "journalwatch"
    "jove"
    "joycond"
    "joycond-cemuhook"
    "joypixels"
    "joystickwake"
    "jp"
    "jp-zip-codes"
    "jp2a"
    "jpcre2"
    "jpeg-archive"
    "jpegexiforient"
    "jpeginfo"
    "jpegli"
    "jpegoptim"
    "jpegrescan"
    "jpexs"
    "jpilot"
    "jpm"
    "jpmml-evaluator"
    "jprofiler"
    "jprq"
    "jpsxdec"
    "jpylyzer"
    "jq"
    "jq-lsp"
    "jq-zsh-plugin"
    "jqfmt"
    "jql"
    "jqp"
    "jquake"
    "jre"
    "jre-generate-cacerts"
    "jre11_minimal"
    "jre17_minimal"
    "jre21_minimal"
    "jre25_minimal"
    "jre8"
    "jre8_headless"
    "jre_headless"
    "jre_minimal"
    "jreleaser-cli"
    "jrl-cmakemodules"
    "jrl-cmakemodules-scripts"
    "jrnl"
    "jrsonnet"
    "jruby"
    "js-beautify"
    "js8call"
    "jsbeautifier"
    "jsdoc"
    "jshint"
    "jshon"
    "jsign"
    "jsluice"
    "jsmn"
    "json-diff"
    "json-fortran"
    "json-glib"
    "json-patch"
    "json-plot"
    "json-repair"
    "json-schema-catalog-rs"
    "json-schema-for-humans"
    "json-server"
    "json-sort"
    "json-sort-cli"
    "json-tui"
    "json2cdn"
    "json2hcl"
    "json2ts"
    "json2tsv"
    "json2yaml"
    "json_c"
    "jsoncons"
    "jsoncpp"
    "jsoncppSecureMemory"
    "jsonfmt"
    "jsongrep"
    "jsonkdl"
    "jsonld-cli"
    "jsonnet"
    "jsonnet-bundler"
    "jsonnet-language-server"
    "jsonplaceholder"
    "jsonrpc-glib"
    "jsonschema"
    "jsonschema-cli"
    "jsonwatch"
    "jspecify"
    "jstest-gtk"
    "jsubfinder"
    "jsvc"
    "jtag-remote-server"
    "jtbl"
    "jtc"
    "jtds_jdbc"
    "jtdx"
    "juce"
    "judy"
    "jugglinglab"
    "juicefs"
    "juicity"
    "juju"
    "jujutsu"
    "jujuutils"
    "julec"
    "juledoc"
    "julefmt"
    "julia"
    "julia-bin"
    "julia-lts"
    "julia-lts-bin"
    "julia-mono"
    "julia-stable"
    "julia-stable-bin"
    "julia_110"
    "julia_110-bin"
    "julia_111"
    "julia_111-bin"
    "julia_112"
    "julia_112-bin"
    "julius"
    "jumanpp"
    "jump"
    "jumpapp"
    "jumpnbump"
    "jumppad"
    "jumpy"
    "junction"
    "junction-font"
    "junest"
    "junicode"
    "juniper"
    "junixsocket-common"
    "junixsocket-native-common"
    "junkie"
    "juno-theme"
    "jupp"
    "jupyter"
    "jupyter-all"
    "jupyterlite"
    "just"
    "just-formatter"
    "just-lsp"
    "justbuild"
    "justify"
    "jwasm"
    "jwhois"
    "jwm"
    "jwt-cli"
    "jwt-hack"
    "jwtcat"
    "jwtinfo"
    "jwx"
    "jx"
    "jxplorer"
    "jxrlib"
    "jxscout"
    "jython"
    "k0sctl"
    "k2pdfopt"
    "k2tf"
    "k3d"
    "k3s"
    "k3s_1_34"
    "k3s_1_35"
    "k3s_1_36"
    "k3sup"
    "k40-whisperer"
    "k6"
    "k8s-manifest-sigstore"
    "k8sgpt"
    "k9s"
    "kabeljau"
    "kacst"
    "kaf"
    "kafka-cmak"
    "kafka-delta-ingest"
    "kafkactl"
    "kagen"
    "kaggle"
    "kahip"
    "kaidan"
    "kail"
    "kaitai-struct-compiler"
    "kaitai-struct-cpp-stl-runtime"
    "kak-tree-sitter"
    "kak-tree-sitter-unwrapped"
    "kakasi"
    "kakoune"
    "kakoune-cr"
    "kakoune-lsp"
    "kakoune-unwrapped"
    "kalamine"
    "kaldi"
    "kalibrate-hackrf"
    "kalibrate-rtl"
    "kalign"
    "kalker"
    "kallisto"
    "kamailio"
    "kamal"
    "kamal-proxy"
    "kamid"
    "kamilalisp"
    "kaminpar"
    "kamp"
    "kana"
    "kanagawa-icon-theme"
    "kanarenshu"
    "kanata"
    "kanata-with-cmd"
    "kanban"
    "kanboard"
    "kando"
    "kangaru"
    "kanha"
    "kanidm-provision"
    "kanidmWithSecretProvisioning_1_10"
    "kanidmWithSecretProvisioning_1_11"
    "kanidmWithSecretProvisioning_1_8"
    "kanidmWithSecretProvisioning_1_9"
    "kanidm_1_10"
    "kanidm_1_11"
    "kanidm_1_8"
    "kanidm_1_9"
    "kanif"
    "kaniko"
    "kanit-font"
    "kanji-stroke-order-font"
    "kanjidraw"
    "kanri"
    "kanshi"
    "kapacitor"
    "kapitonov-plugins-pack"
    "kapow"
    "kapowbang"
    "kapp"
    "kappanhang"
    "kaput-cli"
    "kara"
    "karabiner-dk"
    "karabiner-elements"
    "karakeep"
    "karate"
    "kardolus-chatgpt-cli"
    "karere"
    "kargo"
    "karla"
    "karlender"
    "karlyriceditor"
    "karma"
    "karma-runner"
    "karmor"
    "karp"
    "kas"
    "kasasa"
    "kasmweb"
    "kassert"
    "kata-runtime"
    "katago"
    "katagoCPU"
    "katagoTensorRT"
    "katagoWithCuda"
    "katana"
    "katawa-shoujo-re-engineered"
    "katex"
    "kati"
    "katriawm"
    "katvan"
    "kaufkauflist"
    "kavita"
    "kawa"
    "kawkab-mono-font"
    "kazam"
    "kazumi"
    "kazv"
    "kb"
    "kbd"
    "kbd-ergol"
    "kbdVlock"
    "kbdd"
    "kbdlight"
    "kbfs"
    "kbld"
    "kbs2"
    "kbst"
    "kbt"
    "kcat"
    "kcc"
    "kcgi"
    "kchat"
    "kcl"
    "kcl-kafka"
    "kcl-language-server"
    "kclvm"
    "kclvm_cli"
    "kcollectd"
    "kconf"
    "kconfig-frontends"
    "kcov"
    "kcp"
    "kdbg"
    "kddockwidgets"
    "kde-gruvbox"
    "kde-rounded-corners"
    "kdiff3"
    "kdigger"
    "kdiskmark"
    "kdlfmt"
    "kdocker"
    "kdoctor"
    "kdotool"
    "kdsingleapplication"
    "kea"
    "keama"
    "keedump"
    "keen4"
    "keep-sorted"
    "keepBuildTree"
    "keepalived"
    "keepass"
    "keepass-charactercopy"
    "keepass-diff"
    "keepass-keeagent"
    "keepass-keepasshttp"
    "keepass-keepassrpc"
    "keepass-keetraytotp"
    "keepass-otpkeyprov"
    "keepass-qrcodeview"
    "keepassxc"
    "keepassxc-go"
    "keeperrl"
    "keepingyouawake"
    "keepmenu"
    "keepwn"
    "keet"
    "keeweb"
    "kega-fusion"
    "keifu"
    "keka"
    "kemai"
    "kener"
    "kent"
    "kepler"
    "kepubify"
    "kerbrute"
    "kerbwolf"
    "kerf"
    "kerf_1"
    "kermit"
    "kermit-terminal"
    "kernel-hardening-checker"
    "kernelshark"
    "kestrel"
    "ketch"
    "ketesa"
    "keto"
    "kew"
    "kexec-tools"
    "key"
    "key-rack"
    "keybase"
    "keybase-gui"
    "keybinder"
    "keybinder3"
    "keyboard-layout-editor"
    "keycard-cli"
    "keycastr"
    "keychain"
    "keychron-udev-rules"
    "keycloak"
    "keycloak-config-cli"
    "keyd"
    "keyfinder-cli"
    "keyfuzz"
    "keyguard"
    "keyleds"
    "keylight-cli"
    "keylight-controller-mschneider82"
    "keylights"
    "keym"
    "keymap-drawer"
    "keymapp"
    "keymapper"
    "keymapviz"
    "keynav"
    "keyoxide-cli"
    "keypunch"
    "keyscope"
    "keyspersecond"
    "keystone"
    "keystore-explorer"
    "keyutils"
    "kfilt"
    "kftray"
    "kgeotag"
    "kgt"
    "khal"
    "khard"
    "khmeros"
    "khronos"
    "khronos-ocl-icd-loader"
    "ki"
    "kibi"
    "kicad"
    "kicad-small"
    "kicad-testing"
    "kicad-testing-small"
    "kicad-unstable"
    "kicad-unstable-small"
    "kickoff"
    "kickstart"
    "kicli"
    "kics"
    "kid3"
    "kid3-cli"
    "kid3-kde"
    "kid3-qt"
    "kide"
    "kiesel"
    "kikit"
    "kikoplay"
    "kile"
    "kile-wl"
    "killall"
    "killerbee"
    "killport"
    "kiln"
    "kilo"
    "kilocode-cli"
    "kimai"
    "kime"
    "kin-openapi"
    "kind"
    "kind-lang"
    "kind2"
    "kine"
    "kinect-audio-setup"
    "kingfisher"
    "kingstvis"
    "kip"
    "kirc"
    "kirimoto"
    "kiro"
    "kiro-cli"
    "kiro-cli-unwrapped"
    "kiro-fhs"
    "kirsch"
    "kismet"
    "kissat"
    "kissfft"
    "kissfftFloat"
    "kisslicer"
    "kiterunner"
    "kitex"
    "kitget"
    "kitops"
    "kitsas"
    "kitti3"
    "kitty"
    "kitty-bin"
    "kitty-img"
    "kitty-themes"
    "kittycad-kcl-lsp"
    "kittysay"
    "kiwix"
    "kiwix-apple"
    "kiwix-tools"
    "kjv"
    "klassy"
    "klaus"
    "klavaro"
    "klayout"
    "klee"
    "klibc"
    "klibcShrunk"
    "klick"
    "klipper"
    "klipper-estimator"
    "klipper-firmware"
    "klipper-flash"
    "klipper-genconf"
    "klipperscreen"
    "kloak"
    "klog"
    "klog-rs"
    "klog-time-tracker"
    "klong"
    "kluctl"
    "klystrack"
    "kmeet"
    "kmetronome"
    "kmidimon"
    "kminion"
    "kmod"
    "kmod-blacklist-ubuntu"
    "kmod-debian-aliases"
    "kmon"
    "kmonad"
    "kmscon"
    "kmscube"
    "kmsvnc"
    "kmsxx"
    "kmymoney"
    "kn"
    "knewave"
    "knightos-genkfs"
    "knightos-kcc"
    "knightos-kimg"
    "knightos-kpack"
    "knightos-mkrom"
    "knightos-mktiupgrade"
    "knightos-patchrom"
    "knightos-scas"
    "knightos-z80e"
    "knit"
    "knobkraft-orm"
    "knock"
    "knockoutcitylauncher"
    "knockpy"
    "knope"
    "knossosnet"
    "knot-dns"
    "knot-resolver"
    "knot-resolver-manager_6"
    "knot-resolver_5"
    "knot-resolver_6"
    "knowsmore"
    "kns"
    "knxd"
    "ko"
    "kobodeluxe"
    "koboldcpp"
    "koboredux"
    "koboredux-free"
    "kochi-substitute"
    "kochi-substitute-naga10"
    "kode-mono"
    "kodelife"
    "kodi"
    "kodi-cli"
    "kodi-gbm"
    "kodi-retroarch-advanced-launchers"
    "kodi-wayland"
    "koffan"
    "kohighlights"
    "koito"
    "koji"
    "koka"
    "kokkos"
    "kolla"
    "komac"
    "komari-agent"
    "komga"
    "komika-fonts"
    "komikahuna"
    "komikandy"
    "komikazba"
    "komikaze"
    "komikazoom"
    "komikku"
    "komodo"
    "komorebi"
    "kompose"
    "kompute"
    "kona"
    "konbucase"
    "kondo"
    "konf"
    "konsave"
    "konstraint"
    "kontainer"
    "kontemplate"
    "kontroll"
    "koodo-reader"
    "koodousfinder"
    "kooha"
    "kool"
    "kopia"
    "kopia-ui"
    "kops"
    "kops_1_31"
    "kops_1_32"
    "kops_1_33"
    "kopuz"
    "kor"
    "kora-icon-theme"
    "kord"
    "kore"
    "koreader"
    "korrect"
    "koruri"
    "kotatogram-desktop"
    "kotlin"
    "kotlin-cli"
    "kotlin-interactive-shell"
    "kotlin-language-server"
    "kotlin-native"
    "koto"
    "koto-ls"
    "kotofetch"
    "kotonoha"
    "koules"
    "kpcli"
    "kphotoalbum"
    "kplex"
    "kpt"
    "kpx"
    "kr106"
    "krabby"
    "kraft"
    "krakatau2"
    "krakenx"
    "kramdown-asciidoc"
    "krane"
    "krank"
    "krankerl"
    "krapslog"
    "kratos"
    "krb5"
    "krbjack"
    "kreative-square"
    "krelay"
    "krename"
    "krep"
    "krew"
    "kreya"
    "krill"
    "krillinai"
    "kristall"
    "krita"
    "krita-plugin-gmic"
    "krita-unwrapped"
    "kronometer"
    "kronosnet"
    "krop"
    "krr"
    "krunker"
    "krunkit"
    "krunner-vscodeprojects"
    "krunvm"
    "krusader"
    "kryoflux"
    "kryoptic"
    "kryptor"
    "ks"
    "kseexpr"
    "ksh"
    "kshutdown"
    "ksmbd-tools"
    "ksnip"
    "kssd"
    "kstars"
    "kstart"
    "ksuperkey"
    "kt"
    "ktailctl"
    "ktfmt"
    "kthxbye"
    "ktimetracker"
    "ktlint"
    "ktls-utils"
    "ktop"
    "ktor-cli"
    "ktunnel"
    "ktx-tools"
    "kty"
    "ku"
    "kubazip"
    "kube-bench"
    "kube-capacity"
    "kube-hunter"
    "kube-linter"
    "kube-prompt"
    "kube-review"
    "kube-router"
    "kube-score"
    "kube-state-metrics"
    "kubeaudit"
    "kubebuilder"
    "kubecfg"
    "kubeclarity"
    "kubecm"
    "kubecolor"
    "kubeconform"
    "kubectl"
    "kubectl-ai"
    "kubectl-cnpg"
    "kubectl-convert"
    "kubectl-df-pv"
    "kubectl-doctor"
    "kubectl-evict-pod"
    "kubectl-example"
    "kubectl-explore"
    "kubectl-gadget"
    "kubectl-graph"
    "kubectl-images"
    "kubectl-kcl"
    "kubectl-klock"
    "kubectl-ktop"
    "kubectl-linstor"
    "kubectl-neat"
    "kubectl-node-shell"
    "kubectl-rabbitmq"
    "kubectl-rook-ceph"
    "kubectl-tree"
    "kubectl-validate"
    "kubectl-view-allocations"
    "kubectl-view-secret"
    "kubectx"
    "kubedb-cli"
    "kubedock"
    "kubedog"
    "kubefetch"
    "kubefirst"
    "kubefwd"
    "kubelogin"
    "kubelogin-oidc"
    "kubemq-community"
    "kubemqctl"
    "kubent"
    "kubeone"
    "kubeprompt"
    "kubepug"
    "kubergrunt"
    "kubernetes"
    "kubernetes-code-generator"
    "kubernetes-controller-tools"
    "kubernetes-helm"
    "kubernetes-helm-wrapped"
    "kubernetes-kcp"
    "kubernetes-metrics-server"
    "kubernetes-polaris"
    "kubernetes-validate"
    "kubernix"
    "kubescape"
    "kubeseal"
    "kubesec"
    "kubeshark"
    "kubespy"
    "kubestroyer"
    "kubeswitch"
    "kubetail"
    "kubetrim"
    "kubetui"
    "kubeval"
    "kubeval-schema"
    "kubevela"
    "kubevirt"
    "kubevpn"
    "kubexhunt"
    "kubexit"
    "kubexporter"
    "kubie"
    "kubo"
    "kubo-fs-repo-migrations"
    "kubo-migrator"
    "kubo-migrator-unwrapped"
    "kulala-core"
    "kulala-fmt"
    "kuma"
    "kuma-cp"
    "kuma-dp"
    "kuma-experimental"
    "kumactl"
    "kupfer"
    "kuro"
    "kurve"
    "kustomize"
    "kustomize-lint"
    "kustomize-sops"
    "kustomize_3"
    "kustomize_4"
    "kuttl"
    "kuyen-icons"
    "kuzu"
    "kvazaar"
    "kvirc"
    "kvmarwaita"
    "kvmtool"
    "kvrocks"
    "kwakd"
    "kwin-script-geometry-change"
    "kwok"
    "kwybars"
    "kx-aspe-cli"
    "kxstitch"
    "kyotocabinet"
    "kytea"
    "kythe"
    "kyua"
    "kyverno"
    "kyverno-chainsaw"
    "kzipmix"
    "l-smash"
    "l2md"
    "l8w8jwt"
    "la-capitaine-icon-theme"
    "lab"
    "labctl"
    "labelImg"
    "labelife-label-printer"
    "labelle"
    "labplot"
    "labwc"
    "labwc-gtktheme"
    "labwc-menu-generator"
    "labwc-tweaks"
    "labwc-tweaks-gtk"
    "labymod-launcher"
    "lact"
    "lacus"
    "lacy"
    "ladspa-header"
    "ladspa-sdk"
    "ladspaH"
    "ladspaPlugins"
    "ladybird"
    "ladybugdb"
    "lager"
    "lagrange"
    "lagrange-tui"
    "lakectl"
    "lakefs"
    "lalezar-fonts"
    "lalrpop"
    "lamb"
    "lambda-delta"
    "lambda-lisp"
    "lambda-lisp-blc"
    "lambda-mod-zsh-theme"
    "lambdabot"
    "lambdock"
    "lame"
    "laminar"
    "lammps"
    "lammps-mpi"
    "lan-mouse"
    "landrun"
    "langgraph-cli"
    "languagetool"
    "languagetool-rust"
    "lanraragi"
    "lao"
    "lapack"
    "lapack-ilp64"
    "lapack-reference"
    "lapce"
    "laravel"
    "lark-cli"
    "lasem"
    "lasso"
    "last"
    "last-resort"
    "lastfm"
    "lastools"
    "lastpass-cli"
    "lastversion"
    "lasuite-docs"
    "lasuite-docs-collaboration-server"
    "lasuite-docs-frontend"
    "lasuite-drive"
    "lasuite-meet"
    "laszip"
    "laszip_2"
    "latex2html"
    "latex2mathml"
    "latexminted"
    "latexml-oxide"
    "latexrun"
    "lato"
    "latte-integrale"
    "lattice-diamond"
    "laudanum"
    "launchnext"
    "laurel"
    "lauti"
    "lavacli"
    "lavalauncher"
    "lavalink"
    "lavat"
    "layan-cursors"
    "layerx"
    "lazarus"
    "lazarus-qt5"
    "lazarus-qt6"
    "laze"
    "lazpaint"
    "lazycli"
    "lazycommit"
    "lazydocker"
    "lazygit"
    "lazyhetzner"
    "lazyjj"
    "lazyjournal"
    "lazymc"
    "lazynpm"
    "lazyrsync"
    "lazyspotify"
    "lazysql"
    "lazyssh"
    "lazytrivy"
    "lazyworktree"
    "lbd"
    "lbdb"
    "lbreakout2"
    "lbreakouthd"
    "lbry"
    "lbzip2"
    "lc0"
    "lc3tools"
    "lcab"
    "lcagent"
    "lcalc"
    "lcdf-typetools"
    "lcdproc"
    "lcevcdec"
    "lci"
    "lcm"
    "lcms"
    "lcms1"
    "lcms2"
    "lcov"
    "lcrq"
    "lcsync"
    "lctime"
    "ld-audit-search-mod"
    "ld-is-cc-hook"
    "ld64"
    "ldacbt"
    "ldap-manager"
    "ldapdomaindump"
    "ldapmonitor"
    "ldapnomnom"
    "ldapvi"
    "ldash"
    "ldb"
    "ldc"
    "ldeep"
    "ldid"
    "ldid-procursus"
    "ldm"
    "ldmtool"
    "ldmud"
    "ldmud-full"
    "ldns"
    "ldproxy"
    "ldtk"
    "leafnode1"
    "league-gothic"
    "league-mono"
    "league-of-moveable-type"
    "league-script-number-one"
    "league-spartan"
    "lean"
    "lean-lsp-mcp"
    "lean2"
    "lean3"
    "lean4"
    "leanblueprint"
    "leangz"
    "leanify"
    "leaps"
    "learn6502"
    "leatherman"
    "leaves"
    "leddy"
    "ledfx"
    "ledger"
    "ledger-autosync"
    "ledger-live-desktop"
    "ledger-udev-rules"
    "ledger-web"
    "ledger2beancount"
    "ledit"
    "ledmon"
    "leela"
    "leela-zero"
    "leet-helix"
    "leetcode-cli"
    "leetgo"
    "leetgpu"
    "leetsolv"
    "left4gore-bin"
    "lefthook"
    "leftwm"
    "leftwm-config"
    "leftwm-theme"
    "legba"
    "legcord"
    "legendary-gl"
    "legends-of-equestria"
    "legendsbrowser2"
    "legendsviewer-next"
    "legit"
    "legit-web"
    "legitify"
    "lego"
    "leiningen"
    "lektor"
    "lely-core"
    "lemmeknow"
    "lemminx"
    "lemmy-help"
    "lemmy-server"
    "lemmy-ui"
    "lemon"
    "lemon-graph"
    "lemonade"
    "lemonbar"
    "lemonbar-xft"
    "lemurs"
    "lenmus"
    "lenovo-legion"
    "lenpaste"
    "lens"
    "lensfun"
    "lenspect"
    "leo-editor"
    "leo-lang"
    "leo2"
    "leo3-bin"
    "leocad"
    "lepton-eda"
    "lepton-jpeg-util"
    "leptonica"
    "leptosfmt"
    "lerc"
    "lerna"
    "lerna_6"
    "lerna_8"
    "lerpn"
    "lesbar"
    "less"
    "lessc"
    "lesscpy"
    "lesspass-cli"
    "lesspipe"
    "lethe"
    "letos"
    "letos-plugins"
    "letsdns"
    "letterpress"
    "levant"
    "level-zero"
    "leveldb"
    "leviculum"
    "levmar"
    "lexbor"
    "lexend"
    "lexicon"
    "lexido"
    "lexmark-aex"
    "lexy"
    "lf"
    "lfe"
    "lfk"
    "lftp"
    "lgogdownloader"
    "lgogdownloader-gui"
    "lgpio"
    "lguf-brightness"
    "lha"
    "lhapdf"
    "lhasa"
    "lhs2tex"
    "li-ri"
    "liana"
    "lianad"
    "lib25519"
    "lib2geom"
    "lib3270"
    "lib3ds"
    "lib3mf"
    "lib45d"
    "lib60870"
    "libAppleWM"
    "libFS"
    "libGDSII"
    "libGL"
    "libGLU"
    "libGLX"
    "libHX"
    "libICE"
    "libLAS"
    "libSM"
    "libWindowsWM"
    "libX11"
    "libXScrnSaver"
    "libXau"
    "libXaw"
    "libXcomposite"
    "libXcursor"
    "libXdamage"
    "libXdmcp"
    "libXext"
    "libXfixes"
    "libXfont"
    "libXfont2"
    "libXft"
    "libXi"
    "libXinerama"
    "libXmu"
    "libXp"
    "libXpm"
    "libXpresent"
    "libXrandr"
    "libXrender"
    "libXres"
    "libXt"
    "libXtst"
    "libXv"
    "libXvMC"
    "libXxf86dga"
    "libXxf86misc"
    "libXxf86vm"
    "libaacs"
    "libaal"
    "libabigail"
    "libabw"
    "libacars"
    "libaccounts-glib"
    "libacr38u"
    "libad9361"
    "libadwaita"
    "libaec"
    "libagar"
    "libagar_test"
    "libaio"
    "libajantv2"
    "libamplsolver"
    "libamqpcpp"
    "libansilove"
    "libantlr3c"
    "libao"
    "libaom"
    "libaosd"
    "libapparmor"
    "libappimage"
    "libappindicator"
    "libappindicator-gtk3"
    "libapplewm"
    "libarchive"
    "libarchive-qt"
    "libargon2"
    "libargs"
    "libaribcaption"
    "libasn1c"
    "libasr"
    "libass"
    "libassuan"
    "libastyle"
    "libasyncns"
    "libatasmart"
    "libation"
    "libatomic_ops"
    "libaudclient"
    "libaudec"
    "libavc1394"
    "libavif"
    "libax25"
    "libayatana-appindicator"
    "libayatana-common"
    "libayatana-indicator"
    "libb2"
    "libb64"
    "libbacktrace"
    "libbap"
    "libbase58"
    "libbass"
    "libbass_fx"
    "libbassmidi"
    "libbassmix"
    "libbde"
    "libbdplus"
    "libbfd"
    "libbfd_2_38"
    "libbfio"
    "libbgcode"
    "libbladeRF"
    "libblake3"
    "libblockdev"
    "libblocksruntime"
    "libbluray"
    "libbluray-full"
    "libbpf"
    "libbpf_0"
    "libbpkg"
    "libbraiding"
    "libbs2b"
    "libbsd"
    "libbtbb"
    "libburn"
    "libbutl"
    "libbytesize"
    "libc"
    "libcaca"
    "libcacard"
    "libcamera"
    "libcamera-qcam"
    "libcanberra"
    "libcanberra-gtk3"
    "libcanberra_kde"
    "libcangjie"
    "libcap"
    "libcap-text-verifier"
    "libcap_ng"
    "libcaption"
    "libcardiacarrest"
    "libcava"
    "libcbor"
    "libccd"
    "libcdada"
    "libcdaudio"
    "libcddb"
    "libcdio"
    "libcdio-paranoia"
    "libcdr"
    "libcec"
    "libcec_platform"
    "libcello"
    "libceph"
    "libcerf"
    "libcgroup"
    "libchamplain_libsoup3"
    "libchardet"
    "libchdr"
    "libchewing"
    "libchipcard"
    "libcifpp"
    "libcint"
    "libcircle"
    "libck"
    "libclang"
    "libcli"
    "libclipboard"
    "libcloudproviders"
    "libclthreads"
    "libclxclient"
    "libcmatrix"
    "libcmis"
    "libcoap"
    "libcollectdclient"
    "libcomps"
    "libconfig"
    "libconfuse"
    "libcork"
    "libcorrect"
    "libcosmicAppHook"
    "libcotp"
    "libcouchbase"
    "libcpr"
    "libcpr_1_10_5"
    "libcpucycles"
    "libcpuid"
    "libcrafter"
    "libcredis"
    "libcrossguid"
    "libcryptui"
    "libcs50"
    "libcsa"
    "libcsptr"
    "libcss"
    "libctb"
    "libctemplate"
    "libctl"
    "libcue"
    "libcue-lang"
    "libcupsfilters"
    "libcutl"
    "libcxx"
    "libcxxrt"
    "libcyaml"
    "libdaemon"
    "libdaq"
    "libdatachannel"
    "libdatovka"
    "libdatrie"
    "libdazzle"
    "libdbi"
    "libdbi-drivers"
    "libdbi-drivers-base"
    "libdbiDrivers"
    "libdbiDriversBase"
    "libdbusmenu"
    "libdbusmenu-gtk3"
    "libdbusmenu-qt5"
    "libdc1394"
    "libde265"
    "libdecor"
    "libdeflate"
    "libdeltachat"
    "libdex"
    "libdicom"
    "libdict"
    "libdigidocpp"
    "libdiscid"
    "libdislocator"
    "libdisplay-info"
    "libdisplay-info_0_3"
    "libdivecomputer"
    "libdivsufsort"
    "libdjinterop"
    "libdlcr"
    "libdmapsharing"
    "libdmtx"
    "libdmx"
    "libdnet"
    "libdnf"
    "libdng"
    "libdom"
    "libdovi"
    "libdrm"
    "libdsk"
    "libdv"
    "libdvbcsa"
    "libdvbpsi"
    "libdvdcss"
    "libdvdnav"
    "libdvdnav_4_2_1"
    "libdvdread"
    "libdvdread_4_9_9"
    "libdwarf"
    "libe-book"
    "libe57format"
    "libeatmydata"
    "libeb"
    "libebml"
    "libebur128"
    "libeconf"
    "libedgetpu"
    "libedit"
    "libeduvpn-common"
    "libee"
    "libei"
    "libelf"
    "libelfin"
    "libemf2svg"
    "libepoxy"
    "libepubgen"
    "liberal-crime-squad"
    "liberastika"
    "liberasurecode"
    "liberation-circuit"
    "liberation-sans-narrow"
    "liberation_ttf"
    "liberation_ttf_v1"
    "liberation_ttf_v2"
    "liberfa"
    "liberio"
    "libertine"
    "libertine-g"
    "libertinus"
    "libesmtp"
    "libestr"
    "libetebase"
    "libetonyek"
    "libetpan"
    "libeufin"
    "libev"
    "libevdev"
    "libevdevc"
    "libevent"
    "libewf"
    "libewf-legacy"
    "libexecinfo"
    "libexif"
    "libexosip"
    "libexsid"
    "libextractor"
    "libexttextcat"
    "libf2c"
    "libfabric"
    "libfakeXinerama"
    "libfakekey"
    "libfaketime"
    "libfakexinerama"
    "libfann"
    "libfastjson"
    "libff"
    "libffado"
    "libffcall"
    "libffi"
    "libffiReal"
    "libffi_3_3"
    "libfido2"
    "libfilezilla"
    "libfishsound"
    "libfive"
    "libfixposix"
    "libflowmanager"
    "libfm"
    "libfm-extra"
    "libfmvoice"
    "libfolia"
    "libfontenc"
    "libfoundry"
    "libfprint"
    "libfprint-2-tod1-broadcom"
    "libfprint-2-tod1-broadcom-cv3plus"
    "libfprint-2-tod1-elan"
    "libfprint-2-tod1-goodix"
    "libfprint-2-tod1-goodix-550a"
    "libfprint-2-tod1-vfs0090"
    "libfprint-tod"
    "libfreeaptx"
    "libfreefare"
    "libfreehand"
    "libfrida-core"
    "libfs"
    "libfsm"
    "libftdi"
    "libftdi1"
    "libfx2"
    "libfyaml"
    "libgaminggear"
    "libgbinder"
    "libgbm"
    "libgcc"
    "libgccjit"
    "libgcrypt"
    "libgda5"
    "libgda6"
    "libgdamm"
    "libgdiplus"
    "libgdsii"
    "libgdstk"
    "libgedit-amtk"
    "libgedit-gfls"
    "libgedit-gtksourceview"
    "libgedit-tepl"
    "libgee"
    "libgen-cli"
    "libgeneral"
    "libgeotiff"
    "libgepub"
    "libgff"
    "libgflow"
    "libghostty-vt"
    "libgig"
    "libgit2"
    "libgit2-glib"
    "libglibutil"
    "libglut"
    "libglvnd"
    "libglycin"
    "libglycin-gtk4"
    "libgmpris"
    "libgnome-games-support"
    "libgnome-keyring"
    "libgnomekbd"
    "libgnt"
    "libgnunetchat"
    "libgnurl"
    "libgourou"
    "libgovirt"
    "libgpg-error"
    "libgpg-error-gen-posix-lock-obj"
    "libgphoto2"
    "libgpiod"
    "libgpiod_1"
    "libgpod"
    "libgrapheme"
    "libgringotts"
    "libgsf"
    "libgssglue"
    "libgtkflow3"
    "libgtkflow4"
    "libgtop"
    "libgudev"
    "libguestfs"
    "libguestfs-appliance"
    "libguestfs-with-appliance"
    "libgumath"
    "libguytools"
    "libgweather"
    "libgxps"
    "libhandy"
    "libhandy_0"
    "libhangul"
    "libharu"
    "libhdhomerun"
    "libheif"
    "libhighscore"
    "libhomfly"
    "libhsts"
    "libhttpserver"
    "libhubbub"
    "libhugetlbfs"
    "libhv"
    "libhwy"
    "libhx"
    "libhydrogen"
    "libiberty"
    "libibmad"
    "libibumad"
    "libical"
    "libice"
    "libicns"
    "libiconv"
    "libiconvReal"
    "libid3tag"
    "libideviceactivation"
    "libidn"
    "libidn2"
    "libiec61850"
    "libiec61883"
    "libieee1284"
    "libiff"
    "libigl"
    "libigloo"
    "libiio"
    "libilbc"
    "libilbm"
    "libimagequant"
    "libime"
    "libimobiledevice"
    "libimobiledevice-glue"
    "libindicator"
    "libindicator-gtk3"
    "libinfinity"
    "libinjection"
    "libinklevel"
    "libinotify-kqueue"
    "libinput"
    "libinput-gestures"
    "libinsane"
    "libinstpatch"
    "libint"
    "libintPsi4"
    "libiodbc"
    "libipfix"
    "libipt"
    "libiptcdata"
    "libipuz"
    "libirc"
    "libircclient"
    "libirecovery"
    "libiscsi"
    "libisds"
    "libisoburn"
    "libisocodes"
    "libisofs"
    "libite"
    "libitl"
    "libivykis"
    "libixion"
    "libixp"
    "libjack2"
    "libjaylink"
    "libjcat"
    "libjodycode"
    "libjpeg"
    "libjpeg-tools"
    "libjpeg8"
    "libjpeg_original"
    "libjpeg_turbo"
    "libjson"
    "libjson-rpc-cpp"
    "libjwt"
    "libjxl"
    "libkate"
    "libkazv"
    "libkcapi"
    "libkeyfinder"
    "libkiwix"
    "libkqueue"
    "libkrb5"
    "libkrun"
    "libkrun-efi"
    "libkrun-sev"
    "libkrun-tdx"
    "libkrunfw"
    "libks"
    "libksba"
    "libksi"
    "libkvmi"
    "liblangtag"
    "liblapack"
    "liblapin"
    "liblas"
    "liblastfm-vambrose"
    "liblastfmSF"
    "liblaxjson"
    "liblbfgs"
    "liblc3"
    "liblcf"
    "libleidenalg"
    "libliftoff"
    "libliftoff_0_4"
    "libliftoff_0_5"
    "liblinear"
    "libllvm"
    "liblo"
    "liblockfile"
    "liblogging"
    "liblognorm"
    "libloot"
    "libloragw-2g4"
    "libloragw-sx1301"
    "libloragw-sx1302"
    "liblouis"
    "liblqr1"
    "liblscp"
    "liblsl"
    "libltc"
    "liblxi"
    "liblzf"
    "libmad"
    "libmaddy-markdown"
    "libmamba"
    "libmanette"
    "libmatchbox"
    "libmatekbd"
    "libmatemixer"
    "libmateweather"
    "libmatheval"
    "libmatroska"
    "libmatthew_java"
    "libmaxminddb"
    "libmbd"
    "libmbim"
    "libmcfp"
    "libmcrypt"
    "libmd"
    "libmediaart"
    "libmediainfo"
    "libmegapixels"
    "libmemcached"
    "libmeshb"
    "libmhash"
    "libmicrodns"
    "libmicrohttpd"
    "libmicrohttpd_1_0"
    "libmikmod"
    "libmilter"
    "libminc"
    "libmirage"
    "libmms"
    "libmng"
    "libmnl"
    "libmodbus"
    "libmodplug"
    "libmodsecurity"
    "libmodule"
    "libmodulemd"
    "libmongocrypt"
    "libmowgli"
    "libmpack"
    "libmpc"
    "libmpcdec"
    "libmpd"
    "libmpdclient"
    "libmpeg2"
    "libmpg123"
    "libmrss"
    "libmsgraph"
    "libmspack"
    "libmspub"
    "libmsquic"
    "libmsym"
    "libmt32emu"
    "libmtp"
    "libmusicbrainz"
    "libmusicbrainz5"
    "libmwaw"
    "libmypaint"
    "libmysofa"
    "libmysqlclient"
    "libmysqlclient_3_1"
    "libmysqlclient_3_2"
    "libmysqlclient_3_3"
    "libmysqlconnectorcpp"
    "libnabo"
    "libnatpmp"
    "libnats-c"
    "libnatspec"
    "libnbd"
    "libndctl"
    "libndp"
    "libndtypes"
    "libnest2d"
    "libnet"
    "libnetfilter_acct"
    "libnetfilter_conntrack"
    "libnetfilter_cthelper"
    "libnetfilter_cttimeout"
    "libnetfilter_log"
    "libnetfilter_queue"
    "libnfc"
    "libnfc-nci"
    "libnfnetlink"
    "libnfs"
    "libnftnl"
    "libnghttp2"
    "libnghttp2_asio"
    "libngspice"
    "libnice"
    "libnick"
    "libnids"
    "libnitrokey"
    "libnixxml"
    "libnl"
    "libnl-tiny"
    "libnma"
    "libnma-gtk4"
    "libnoise"
    "libnop"
    "libnotify"
    "libnova"
    "libnsbmp"
    "libnsfb"
    "libnsgif"
    "libnsl"
    "libnslog"
    "libnspsl"
    "libnss-mysql"
    "libnss_nis"
    "libnsutils"
    "libnut"
    "libnvidia-container"
    "libnvme"
    "libnxml"
    "liboauth"
    "libodb"
    "libodb-sqlite"
    "libodfgen"
    "libofa"
    "libofx"
    "libogg"
    "liboggz"
    "liboil"
    "libomemo-c"
    "libomxil-bellagio"
    "libopaque"
    "libopcodes"
    "libopcodes_2_38"
    "libopenaptx"
    "libopenglrecorder"
    "libopenmpt"
    "libopenmpt-modplug"
    "libopenraw"
    "libopenshot"
    "libopenshot-audio"
    "libopensmtpd"
    "liboping"
    "liboprf"
    "libopus"
    "libopusenc"
    "liboqs"
    "liborbispkg-pkgtool"
    "liborcus"
    "liborigin"
    "libosinfo"
    "libosip"
    "libosmium"
    "libosmo-netif"
    "libosmo-sigtran"
    "libosmoabis"
    "libosmocore"
    "libosmscout"
    "libossp_uuid"
    "libotf"
    "libotr"
    "libow"
    "libowfat"
    "libowlevelzs"
    "libp11"
    "libpagemaker"
    "libpam-wrapper"
    "libpanel"
    "libpaper"
    "libpar2"
    "libparserutils"
    "libpcap"
    "libpciaccess"
    "libpeas"
    "libpeas2"
    "libpff"
    "libpfm"
    "libpg_query"
    "libpgf"
    "libphidget22"
    "libphidget22extra"
    "libphonenumber"
    "libpinyin"
    "libpipeline"
    "libpisp"
    "libpkgconf"
    "libpkgmanifest"
    "libplacebo"
    "libplacebo_5"
    "libplctag"
    "libplist"
    "libpng"
    "libpng12"
    "libpointmatcher"
    "libpoly"
    "libportal"
    "libportal-gtk3"
    "libportal-gtk4"
    "libportal-qt5"
    "libportal-qt6"
    "libpostal"
    "libpostalWithData"
    "libppd"
    "libpq"
    "libpqxx"
    "libpressureaudio"
    "libprojectm"
    "libprom"
    "libprotoident"
    "libproxy"
    "libpsl"
    "libpsl-with-scripts"
    "libpsm2"
    "libpst"
    "libpthread-stubs"
    "libpthreadstubs"
    "libptytty"
    "libpulsar"
    "libpulseaudio"
    "libpwquality"
    "libqalculate"
    "libqb"
    "libqmi"
    "libqrtr-glib"
    "libqtdbusmock"
    "libqtdbustest"
    "libqxp"
    "libr3"
    "librandombytes"
    "libraop"
    "libraqm"
    "librashader"
    "libraspberrypi"
    "librasterlite2"
    "libratbag"
    "libraw"
    "libraw1394"
    "librclone"
    "librda"
    "librdata"
    "librdf_raptor2"
    "librdf_rasqal"
    "librdf_redland"
    "libre"
    "libre-baskerville"
    "libre-bodoni"
    "libre-caslon"
    "libre-franklin"
    "libre-graph-api-cpp-qt-client"
    "librealsense"
    "librealsense-gui"
    "librealsenseWithCuda"
    "librealsenseWithoutCuda"
    "librearp"
    "librearp-lv2"
    "librecad"
    "librecast"
    "librechat"
    "libredirect"
    "libredwg"
    "libreelec-dvb-firmware"
    "librelane"
    "librelp"
    "librem"
    "libremidi"
    "libremines"
    "librenms"
    "libreoffice"
    "libreoffice-bin"
    "libreoffice-collabora"
    "libreoffice-fresh"
    "libreoffice-qt"
    "libreoffice-qt-fresh"
    "libreoffice-qt-fresh-unwrapped"
    "libreoffice-qt-stable"
    "libreoffice-qt-stable-unwrapped"
    "libreoffice-qt-still"
    "libreoffice-qt-still-unwrapped"
    "libreoffice-qt-unwrapped"
    "libreoffice-qt6"
    "libreoffice-qt6-fresh"
    "libreoffice-qt6-fresh-unwrapped"
    "libreoffice-qt6-still"
    "libreoffice-qt6-still-unwrapped"
    "libreoffice-qt6-unwrapped"
    "libreoffice-stable"
    "libreoffice-stable-unwrapped"
    "libreoffice-still"
    "libreoffice-unwrapped"
    "librep"
    "librepcb"
    "librepo"
    "librepods"
    "libresample"
    "libresidfp"
    "librespeed-cli"
    "librespeed-rust"
    "libresplit"
    "librespot"
    "librespot-ma"
    "libresprite"
    "libressl"
    "libressl_4_2"
    "libressl_4_3"
    "librest"
    "librest_1_0"
    "libreswan"
    "libretls"
    "libretrack"
    "libretranslate"
    "libretro-core-info"
    "libretro-shaders-slang"
    "libretroPackages"
    "librevenge"
    "librevna"
    "librewolf"
    "librewolf-bin"
    "librewolf-bin-unwrapped"
    "librewolf-unwrapped"
    "librice"
    "librime"
    "librime-lua"
    "librime-octagram"
    "librist"
    "libroxml"
    "librsb"
    "librseq"
    "librsvg"
    "librsync"
    "librtprocess"
    "librttopo"
    "librum"
    "libs3"
    "libsakura"
    "libsamplerate"
    "libsass"
    "libsbml"
    "libsbsms"
    "libsbsms_2_0_2"
    "libsbsms_2_3_0"
    "libscfg"
    "libschrift"
    "libsciter"
    "libscrypt"
    "libsearpc"
    "libseccomp"
    "libsecret"
    "libsegfault"
    "libselinux"
    "libsemanage"
    "libsepol"
    "libserdes"
    "libserialport"
    "libsfdo"
    "libshout"
    "libshumate"
    "libsidplayfp"
    "libsieve"
    "libsigcxx"
    "libsigcxx30"
    "libsignal-ffi"
    "libsignal-protocol-c"
    "libsignon-glib"
    "libsigrok"
    "libsigrok-sipeed"
    "libsigrokdecode"
    "libsigsegv"
    "libsixel"
    "libskk"
    "libslirp"
    "libsm"
    "libsmartcols"
    "libsmbios"
    "libsmf"
    "libsmi"
    "libsnark"
    "libsndfile"
    "libsodium"
    "libsolv"
    "libsoundio"
    "libsoup_3"
    "libspatialaudio"
    "libspatialindex"
    "libspatialite"
    "libspecbleach"
    "libspectre"
    "libspectrum"
    "libspelling"
    "libspf2"
    "libspiro"
    "libspnav"
    "libspng"
    "libsquish"
    "libsrs2"
    "libssc"
    "libssh"
    "libssh2"
    "libsshWithGssapi"
    "libstaden-read"
    "libstartup_notification"
    "libstatgrab"
    "libstemmer"
    "libstrangle"
    "libstroke"
    "libstrophe"
    "libstudxml"
    "libsupermesh"
    "libsurvive"
    "libsv"
    "libsvgtiny"
    "libsvm"
    "libsysprof-capture"
    "libsystemtap"
    "libtapi"
    "libtar"
    "libtas"
    "libtasn1"
    "libtatsu"
    "libtelnet"
    "libtensorflow"
    "libtermkey"
    "libthai"
    "libtheora"
    "libthreadar"
    "libticables2"
    "libticalcs2"
    "libtickit"
    "libticonv"
    "libtiff"
    "libtifiles2"
    "libtiger"
    "libtinfo"
    "libtins"
    "libtirpc"
    "libtlsrpt"
    "libtomcrypt"
    "libtommath"
    "libtool"
    "libtool_1_5"
    "libtool_2"
    "libtorch-bin"
    "libtorrent-rakshasa"
    "libtorrent-rasterbar"
    "libtorrent-rasterbar-1_2_x"
    "libtorrent-rasterbar-2_0_x"
    "libtoxcore"
    "libtpms"
    "libtrace"
    "libtraceevent"
    "libtracefs"
    "libtransmission_4"
    "libtree"
    "libtsm"
    "libtypec"
    "libu2f-emu"
    "libu2f-host"
    "libu2f-server"
    "libubox"
    "libubox-mbedtls"
    "libubox-nossl"
    "libuchardet"
    "libucl"
    "libucontext"
    "libudev-devd"
    "libudev-garden"
    "libudev-zero"
    "libudev0-shim"
    "libudfread"
    "libuecc"
    "libuev"
    "libui"
    "libui-ng"
    "libuiohook"
    "libuldaq"
    "libultrahdr"
    "libunarr"
    "libunibreak"
    "libunicode"
    "libuninameslist"
    "libunique3"
    "libunistring"
    "libunity"
    "libunwind"
    "libupnp"
    "liburcu"
    "liburing"
    "libusb-compat-0_1"
    "libusb1"
    "libusbgx"
    "libusbmuxd"
    "libusbp"
    "libusbsio"
    "libutempter"
    "libutf8proc"
    "libutp"
    "libutp_3_4"
    "libuuid"
    "libuv"
    "libuvc"
    "libv3270"
    "libv4l"
    "libva"
    "libva-minimal"
    "libva-utils"
    "libva-vdpau-driver"
    "libvarlink"
    "libvdpau"
    "libvdpau-va-gl"
    "libvdwxc"
    "libversion"
    "libverto"
    "libvfn"
    "libvgm"
    "libvibrant"
    "libvirt"
    "libvirt-dbus"
    "libvirt-glib"
    "libvisio"
    "libvisio2svg"
    "libvlc"
    "libvlcpp"
    "libvmaf"
    "libvmi"
    "libvncserver"
    "libvoikko"
    "libvolume_id"
    "libvorbis"
    "libvori"
    "libvpl"
    "libvpx"
    "libvterm"
    "libvterm-neovim"
    "libwacom"
    "libwacom-surface"
    "libwapcaplet"
    "libwbxml"
    "libweaver"
    "libwebcam"
    "libwebm"
    "libwebp"
    "libwebsockets"
    "libwhereami"
    "libwhich"
    "libwignernj"
    "libwindowswm"
    "libwmf"
    "libwnck"
    "libwpd"
    "libwpd_08"
    "libwpe"
    "libwpe-fdo"
    "libwpg"
    "libwps"
    "libwtk-sdl2"
    "libx11"
    "libx86"
    "libx86emu"
    "libxau"
    "libxaw"
    "libxaw3d"
    "libxc"
    "libxc_7"
    "libxcb"
    "libxcb-cursor"
    "libxcb-errors"
    "libxcb-image"
    "libxcb-keysyms"
    "libxcb-render-util"
    "libxcb-util"
    "libxcb-wm"
    "libxcomp"
    "libxcomposite"
    "libxcrypt"
    "libxcrypt-legacy"
    "libxcursor"
    "libxcvt"
    "libxdamage"
    "libxdg_basedir"
    "libxdmcp"
    "libxeddsa"
    "libxext"
    "libxfce4ui"
    "libxfce4util"
    "libxfce4windowing"
    "libxfixes"
    "libxfont_1"
    "libxfont_2"
    "libxfs"
    "libxft"
    "libxi"
    "libxinerama"
    "libxisf"
    "libxkbcommon"
    "libxkbcommon_8"
    "libxkbfile"
    "libxklavier"
    "libxl"
    "libxls"
    "libxlsxwriter"
    "libxmi"
    "libxml2"
    "libxml2Python"
    "libxml2_13"
    "libxmlb"
    "libxmlxx"
    "libxmlxx3"
    "libxmlxx5"
    "libxmp"
    "libxmu"
    "libxnd"
    "libxo"
    "libxp"
    "libxpdf"
    "libxpm"
    "libxpresent"
    "libxrandr"
    "libxrender"
    "libxres"
    "libxscrnsaver"
    "libxshmfence"
    "libxslt"
    "libxsmm"
    "libxt"
    "libxtst"
    "libxv"
    "libxvmc"
    "libxxf86dga"
    "libxxf86misc"
    "libxxf86vm"
    "libyafaray"
    "libyaml"
    "libyang"
    "libykclient"
    "libykneomgr"
    "libytnef"
    "libyubikey"
    "libyuv"
    "libz"
    "libzakalwe"
    "libzbc"
    "libzbd"
    "libzdb"
    "libzen"
    "libzim"
    "libzim-glib"
    "libzint"
    "libzip"
    "libzmf"
    "libzra"
    "lice"
    "license-cli"
    "license-generator"
    "license-go"
    "license-plist"
    "license-scanner"
    "license_finder"
    "licenseclassifier"
    "licensed"
    "licensedigger"
    "licensee"
    "licensure"
    "lichess-bot"
    "lichess-external-engine"
    "licomp-toolkit"
    "lidarr"
    "lie"
    "lieer"
    "lief"
    "lifecycled"
    "lifelines"
    "lifeograph"
    "liferea"
    "lightburn"
    "lightdm"
    "lightdm-gtk-greeter"
    "lightdm-mini-greeter"
    "lightdm-mobile-greeter"
    "lightdm-slick-greeter"
    "lightdm-tiny-greeter"
    "lightdm_qt"
    "lightgbm"
    "lighthouse"
    "lighthouse-steamvr"
    "lightlocker"
    "lightning"
    "lightning-loop"
    "lightning-pool"
    "lightning-terminal"
    "lightningcss"
    "lightningstream"
    "lightsoff"
    "lightspark"
    "lighttpd"
    "lightum"
    "lightwalletd"
    "lightway"
    "lightworks"
    "ligolo-ng"
    "likwid"
    "lil-pwny"
    "lilex"
    "lilipod"
    "lillydap"
    "lilo"
    "lilv"
    "lilypond"
    "lilypond-unstable"
    "lilypond-unstable-with-fonts"
    "lilypond-with-fonts"
    "lima"
    "lima-additional-guestagents"
    "lima-bin"
    "lima-full"
    "limbo"
    "limesuite"
    "limesuiteWithGui"
    "limesurvey"
    "limine"
    "limine-full"
    "limitcpu"
    "limo"
    "linalg"
    "linbox"
    "lincity"
    "lincity-ng"
    "lincity_ng"
    "linden-hill"
    "line-awesome"
    "linear"
    "linearicons-free"
    "linecast"
    "linenoise"
    "lineselect"
    "lingeling"
    "lingot"
    "lingua-franca"
    "linien-gui"
    "linja-pi-pu-lukin"
    "linja-sike"
    "link-grammar"
    "linkchecker"
    "linkding"
    "linkerd"
    "linkerd_edge"
    "linkerd_stable"
    "links2"
    "linkwarden"
    "linode-cli"
    "linphone"
    "linssid"
    "lint-staged"
    "linthesia"
    "lintspec"
    "linux"
    "linux-doc"
    "linux-enable-ir-emitter"
    "linux-exploit-suggester"
    "linux-firmware"
    "linux-gpib"
    "linux-libertine-g"
    "linux-manual"
    "linux-pam"
    "linux-router"
    "linux-router-without-wifi"
    "linux-scripts"
    "linux-wallpaperengine"
    "linux-wifi-hotspot"
    "linuxConsoleTools"
    "linuxHeaders"
    "linuxPackages_custom_tinyconfig_kernel"
    "linux_5_10"
    "linux_5_15"
    "linux_6_1"
    "linux_6_12"
    "linux_6_18"
    "linux_6_6"
    "linux_7_2"
    "linux_latest"
    "linux_logo"
    "linux_rpi0"
    "linux_rpi02w"
    "linux_rpi1"
    "linux_rpi2"
    "linux_rpi3"
    "linux_rpi4"
    "linux_testing"
    "linux_xanmod"
    "linux_xanmod_latest"
    "linux_xanmod_stable"
    "linux_zen"
    "linuxdeploy"
    "linuxdeploy-plugin-qt"
    "linuxdoc-tools"
    "linuxhw-edid-fetcher"
    "linuxkit"
    "linuxptp"
    "linuxquota"
    "linuxsampler"
    "linuxwave"
    "linvstmanager"
    "linx-server"
    "linyaps"
    "linyaps-box"
    "linyaps-web-store-installer"
    "lipl"
    "lipo-go"
    "lips"
    "liquibase"
    "liquibase_redshift_extension"
    "liquid-dsp"
    "liquidctl"
    "liquidprompt"
    "liquidsoap"
    "liquidwar"
    "liquidwar5"
    "lirc"
    "lis"
    "lisette"
    "lisgd"
    "listadmin"
    "listen1"
    "listenbrainz-mpd"
    "listmonk"
    "listres"
    "lit"
    "lite"
    "lite-xl"
    "litebrowser"
    "litecli"
    "litefs"
    "litehtml"
    "litellm"
    "litemdview"
    "liteparse"
    "literata"
    "literate"
    "litestar"
    "litestream"
    "litmus"
    "litmusctl"
    "litterbox"
    "little_boxes"
    "littlegptracker"
    "live-server"
    "live555"
    "livebook"
    "livecaptions"
    "livedl"
    "livegrep"
    "livekit"
    "livekit-cli"
    "livekit-ingress"
    "livekit-libwebrtc"
    "livepeer"
    "livi"
    "lix"
    "lix-diff"
    "lixStatic"
    "lk-jwt-service"
    "lkl"
    "lklWithFirewall"
    "lklug-sinhala"
    "lkproof"
    "lksctp-tools"
    "lla"
    "llama-cpp"
    "llama-cpp-cuda"
    "llama-cpp-rocm"
    "llama-cpp-vulkan"
    "llama-swap"
    "llama-swap-minimal"
    "lld"
    "lld_18"
    "lld_19"
    "lld_20"
    "lld_21"
    "lld_22"
    "lldap"
    "lldap-cli"
    "lldb"
    "lldb_18"
    "lldb_19"
    "lldb_20"
    "lldb_21"
    "lldb_22"
    "lldpd"
    "llhttp"
    "llm"
    "llm-ls"
    "llmfit"
    "llmserve"
    "llpp"
    "llr"
    "llrCUDA"
    "lls"
    "llvm"
    "llvm-manpages"
    "llvm_18"
    "llvm_19"
    "llvm_20"
    "llvm_21"
    "llvm_22"
    "lm4flash"
    "lm_sensors"
    "lmath"
    "lmdb"
    "lmdbxx"
    "lmmath"
    "lmms"
    "lmms-full"
    "lmodern"
    "lmp"
    "lms"
    "lmstudio"
    "lnav"
    "lnch"
    "lnd"
    "lndconnect"
    "lndhub-go"
    "lndinit"
    "lndir"
    "lndmanage"
    "lngen"
    "lnreader"
    "lnx"
    "loadbox"
    "loadlibrary"
    "loadwatch"
    "lobster"
    "lobtui"
    "local-ai"
    "local-content-share"
    "locale"
    "localproxy"
    "localsearch"
    "localsend"
    "localstack"
    "localtime"
    "localtunnel"
    "locate-dominating-file"
    "loccount"
    "lock"
    "lockbook"
    "lockbook-desktop"
    "lockdep"
    "locker"
    "lockfile-progs"
    "lockfileProgs"
    "loco"
    "log4brains"
    "log4cplus"
    "log4cpp"
    "log4cxx"
    "log4shib"
    "loganalyzer"
    "logcheck"
    "logdy"
    "logger"
    "logiops"
    "logiops_0_2_3"
    "logisim"
    "logisim-evolution"
    "logitech-udev-rules"
    "logkeys"
    "logmap"
    "logmein-hamachi"
    "logmich"
    "logrotate"
    "logseq"
    "logstalgia"
    "logstash"
    "logstash-contrib"
    "logstash7"
    "logstash7-oss"
    "logtop"
    "loguru"
    "lokalise2-cli"
    "loki"
    "loki-tool"
    "lokinet"
    "loksh"
    "lolcat"
    "lolcode"
    "lollypop"
    "lombok"
    "lon"
    "long-shebang"
    "longcat"
    "longview"
    "looking-glass-client"
    "lookout"
    "lookup-license"
    "loopwm"
    "lora"
    "loramon"
    "lorem"
    "lorien"
    "lorri"
    "losange"
    "losslessaudiochecker"
    "losslesscut"
    "losslesscut-bin"
    "lottieconverter"
    "loudgain"
    "loudmouth"
    "loupe"
    "lout"
    "louvain-community"
    "louvre"
    "love"
    "love_0_10"
    "love_11"
    "lovely-injector"
    "lovr"
    "lowdown"
    "lowdown-unsandboxed"
    "lowfi"
    "loxodo"
    "lp_solve"
    "lpac"
    "lpairs2"
    "lparchive2epub"
    "lpcnet"
    "lpd8editor"
    "lprint"
    "lprobe"
    "lr"
    "lrcalc"
    "lrcget"
    "lrcsnc"
    "lrdf"
    "lrs"
    "lrzip"
    "lrzsz"
    "ls-lint"
    "lsb-release"
    "lscolors"
    "lsd"
    "lsd2dsl"
    "lsdvd"
    "lse"
    "lsfg-vk"
    "lsfg-vk-ui"
    "lsh"
    "lshw"
    "lshw-gui"
    "lsirec"
    "lsiutil"
    "lsix"
    "lsof"
    "lsp-ai"
    "lsp-plugins"
    "lsplug"
    "lspmux"
    "lsr"
    "lsscsi"
    "lssecret"
    "lstk"
    "lstr"
    "lsv"
    "lswt"
    "lsyncd"
    "ltc-tools"
    "ltex-ls"
    "ltex-ls-plus"
    "ltl2ba"
    "ltrace"
    "ltris"
    "ltspice"
    "lttng-tools"
    "lttng-ust"
    "lttng-ust_2_12"
    "lttoolbox"
    "ltunify"
    "lua"
    "lua-language-server"
    "lua5"
    "lua5_1"
    "lua5_2"
    "lua5_2_compat"
    "lua5_3"
    "lua5_3_compat"
    "lua5_4"
    "lua5_4_compat"
    "lua5_5"
    "lua5_5_compat"
    "luabind"
    "luabind_luajit"
    "luabridge"
    "luaformatter"
    "luajit"
    "luajit_2_0"
    "luajit_2_1"
    "luajit_openresty"
    "luakit"
    "luanti"
    "luanti-client"
    "luanti-server"
    "luarenamer"
    "luarocks"
    "luarocks-nix"
    "luarocks-packages-updater"
    "luastatus"
    "luau"
    "luau-lsp"
    "lubelogger"
    "lucenepp"
    "lucida-downloader"
    "lucide"
    "lucky-cli"
    "lucky-commit"
    "luckybackup"
    "luculent"
    "luddite"
    "ludtwig"
    "ludusavi"
    "lue"
    "lufus"
    "lug-helper"
    "lugaru"
    "luigi"
    "luit"
    "lukesmithxyz-bible-kjv"
    "luksmeta"
    "lulu"
    "lumafly"
    "lumen"
    "luminance"
    "luminaut"
    "luminous-ttv"
    "lunacy"
    "lunar"
    "lunar-client"
    "lunarml"
    "lunasvg"
    "lunatask"
    "lune"
    "luneta"
    "luppp"
    "lure-of-the-temptress"
    "lurk"
    "lutgen"
    "lutgen-studio"
    "lutok"
    "lutris"
    "lutris-free"
    "lutris-unwrapped"
    "luwen"
    "lux"
    "lux-cli"
    "lv"
    "lv2"
    "lv2bm"
    "lv2lint"
    "lv_font_conv"
    "lvm2"
    "lvm2_dmeventd"
    "lvm2_vdo"
    "lvmsync"
    "lvtk"
    "lwan"
    "lwc"
    "lwgrp"
    "lwm"
    "lx-music-desktop"
    "lxappearance"
    "lxc"
    "lxcfs"
    "lxd-image-server"
    "lxgw-fusionkai"
    "lxgw-neoxihei"
    "lxgw-wenkai"
    "lxgw-wenkai-screen"
    "lxgw-wenkai-tc"
    "lxi-tools"
    "lxi-tools-gui"
    "lxmenu-data"
    "lxmf"
    "lxmf-rs"
    "lxpanel"
    "lxqt-panel-profiles"
    "lxrandr"
    "lxsession"
    "lxtask"
    "lxterminal"
    "ly"
    "lychee"
    "lycheeslicer"
    "lynis"
    "lynx"
    "lyra"
    "lyra-cursors"
    "lyrebird"
    "lyricspot"
    "lyto"
    "lyx"
    "lz4"
    "lzbench"
    "lzfse"
    "lzham"
    "lzip"
    "lziprecover"
    "lzlib"
    "lzo"
    "lzop"
    "lzsa"
    "m-cli"
    "m17-cxx-demod"
    "m17n_db"
    "m17n_lib"
    "m1ddc"
    "m1n1"
    "m2-mesoplanet"
    "m2-planet"
    "m2libc"
    "m32edit"
    "m33-linux"
    "m4"
    "m4acut"
    "m4ri"
    "m4rie"
    "m8c"
    "ma"
    "maa-assistant-arknights"
    "maa-cli"
    "mac-fdisk"
    "mac-robber"
    "macaulay2"
    "macchanger"
    "macchina"
    "maccy"
    "macdylibbundler"
    "macfuse-stubs"
    "macism"
    "mackerel-agent"
    "mackup"
    "macmon"
    "macopix"
    "macos-defaults"
    "macos-instantview"
    "macpm"
    "macs2"
    "macse"
    "macshot"
    "macskk"
    "mactop"
    "mactracker"
    "mactrix"
    "macvim"
    "maddy"
    "madlang"
    "mado"
    "madonctl"
    "maelstrom"
    "maelstrom-clj"
    "maeparser"
    "maestral"
    "maestral-gui"
    "maestro"
    "mafft"
    "mage"
    "magento-cloud"
    "magic-enum"
    "magic-vlsi"
    "magic-wormhole"
    "magic-wormhole-rs"
    "magicard-cups-driver"
    "magicq"
    "magicrescue"
    "magika"
    "magika-cli"
    "magma"
    "magma-cuda"
    "magma-cuda-static"
    "magma-hip"
    "magnetic-catppuccin-gtk"
    "magnetico"
    "magnumclient"
    "mago"
    "maia3"
    "maid"
    "maigret"
    "maiko"
    "mail-tlsa-check-exporter"
    "mailcap"
    "mailcatcher"
    "mailcheck"
    "maildir-rank-addr"
    "maildrop"
    "mailhog"
    "mailman"
    "mailman-hyperkitty"
    "mailman-web"
    "mailpit"
    "mailsend"
    "mailspring"
    "mailsy"
    "mailutils"
    "maim"
    "mainsail"
    "mairix"
    "maizzle"
    "majima"
    "major-mono-display"
    "make-minimal-bootstrap-sources"
    "makeBinaryWrapper"
    "makeDBusConf"
    "makeGCOVReport"
    "makeInitrdNGTool"
    "makeShellWrapper"
    "makeWrapper"
    "makebootfat"
    "makedepend"
    "makefile2graph"
    "makehuman"
    "makejinja"
    "makemkv"
    "maker-panel"
    "makerpm"
    "makeself"
    "makima"
    "mako"
    "malcontent"
    "malcontent-ui"
    "maliit-framework"
    "maliit-keyboard"
    "mallard-ducktype"
    "maloader"
    "malt"
    "maltego"
    "malwoverview"
    "mamba"
    "mamba-cpp"
    "mame"
    "mame-tools"
    "man"
    "man-db"
    "man-pages"
    "man-pages-posix"
    "managarr"
    "mandelbrot-cli"
    "mandelbulber"
    "mandoc"
    "mandown"
    "manga-cli"
    "manga-tui"
    "mangal"
    "mangareader"
    "mangayomi"
    "mangl"
    "mango"
    "mangohud"
    "mangojuice"
    "mangowc"
    "mani"
    "manifest-tool"
    "manifold"
    "manilaclient"
    "manim"
    "manim-slides"
    "manix"
    "manticoresearch"
    "mantra"
    "manuskript"
    "map-cmd"
    "mapcache"
    "mapcidr"
    "maphosts"
    "maplestory-cursor"
    "mapnik"
    "mapproxy"
    "mapscii"
    "mapserver"
    "maptool"
    "mar1d"
    "marathi-cursive"
    "marathonctl"
    "marble-marcher-ce"
    "marble-shell-theme"
    "marcel"
    "marco"
    "marelle"
    "marge-bot"
    "mari0"
    "mariadb"
    "mariadb-connector-c"
    "mariadb-connector-c_3_1"
    "mariadb-connector-c_3_2"
    "mariadb-connector-c_3_3"
    "mariadb-connector-java"
    "mariadb-embedded"
    "mariadb-galera"
    "mariadb_1011"
    "mariadb_106"
    "mariadb_114"
    "mariadb_118"
    "marimo"
    "mariokart64recomp"
    "marisa"
    "mark"
    "markdown-anki-decks"
    "markdown-code-runner"
    "markdown-link-check"
    "markdown-oxide"
    "markdown-toc"
    "markdown2html-converter"
    "markdownlint-cli"
    "markdownlint-cli2"
    "marked-man"
    "marker"
    "markitdown"
    "markitdown-mcp"
    "markless"
    "markscribe"
    "marksman"
    "marktext"
    "markuplinkchecker"
    "marl"
    "marlin-calc"
    "marmite"
    "marp-cli"
    "mars"
    "mars-mips"
    "marst"
    "martian-mono"
    "martin"
    "marvin"
    "marwaita-icons"
    "marytts"
    "mas"
    "mask"
    "masklint"
    "maskprocessor"
    "maskromtool"
    "masscan"
    "massdns"
    "massren"
    "master_me"
    "mastermind"
    "masterpdfeditor"
    "masterpdfeditor4"
    "mastodon"
    "mastodon-archive"
    "mastotool"
    "mat2"
    "matcha"
    "matcha-gtk-theme"
    "matcha-rss-digest"
    "matchbox"
    "matchbox-server"
    "mate-applets"
    "mate-backgrounds"
    "mate-calc"
    "mate-common"
    "mate-control-center"
    "mate-desktop"
    "mate-gsettings-overrides"
    "mate-icon-theme"
    "mate-icon-theme-faenza"
    "mate-indicator-applet"
    "mate-media"
    "mate-menus"
    "mate-netbook"
    "mate-notification-daemon"
    "mate-panel"
    "mate-panel-with-applets"
    "mate-polkit"
    "mate-power-manager"
    "mate-screensaver"
    "mate-sensors-applet"
    "mate-session-manager"
    "mate-settings-daemon"
    "mate-settings-daemon-wrapped"
    "mate-system-monitor"
    "mate-terminal"
    "mate-themes"
    "mate-tweak"
    "mate-user-guide"
    "mate-user-share"
    "mate-utils"
    "mate-wayland-session"
    "materia-everforest-kvantum"
    "materia-kde-theme"
    "material-black-colors"
    "material-cursors"
    "material-design-icons"
    "material-icons"
    "material-maker"
    "material-symbols"
    "materialgram"
    "materialize"
    "materialx"
    "math-preview"
    "mathemagix"
    "mathematica"
    "mathematica-cuda"
    "mathematica-webdoc"
    "mathematica-webdoc-cuda"
    "mathgl"
    "mathic"
    "mathicgb"
    "mathjax"
    "mathjax-node-cli"
    "mathmod"
    "mathpix-snipping-tool"
    "matio"
    "matlab-language-server"
    "matomo"
    "matrix-alertmanager"
    "matrix-alertmanager-receiver"
    "matrix-appservice-irc"
    "matrix-authentication-service"
    "matrix-brandy"
    "matrix-commander"
    "matrix-commander-rs"
    "matrix-conduit"
    "matrix-continuwuity"
    "matrix-corporal"
    "matrix-dl"
    "matrix-hook"
    "matrix-hookshot"
    "matrix-media-repo"
    "matrix-sdk-crypto-nodejs"
    "matrix-synapse"
    "matrix-synapse-unwrapped"
    "matrix-tuwunel"
    "matrix-zulip-bridge"
    "matter-compiler"
    "matterbridge"
    "matterhorn"
    "matterircd"
    "matterjs-server"
    "mattermost"
    "mattermost-desktop"
    "mattermostLatest"
    "matugen"
    "maturin"
    "maubot"
    "maude"
    "mautrix-discord"
    "mautrix-gmessages"
    "mautrix-googlechat"
    "mautrix-meta"
    "mautrix-signal"
    "mautrix-slack"
    "mautrix-telegram"
    "mautrix-whatsapp"
    "maven"
    "maven3"
    "maven_4"
    "mavproxy"
    "mawk"
    "maxcso"
    "maxfetch"
    "maxflow"
    "maxima"
    "maxima-ecl"
    "maxlib"
    "maya"
    "mayo"
    "mb2md"
    "mbake"
    "mbed-cli"
    "mbedtls"
    "mbedtls_4"
    "mbelib"
    "mbidled"
    "mblaze"
    "mblock-mlink"
    "mbmd"
    "mboot"
    "mbpfan"
    "mbpoll"
    "mbqn"
    "mbrola"
    "mbrola-voices"
    "mbt"
    "mbtileserver"
    "mbuffer"
    "mbusd"
    "mbutil"
    "mc"
    "mcabber"
    "mcaimi-st"
    "mcap-cli"
    "mcaselector"
    "mcat"
    "mcat-unwrapped"
    "mcdreforged"
    "mceinject"
    "mcelog"
    "mcfly"
    "mcfly-fzf"
    "mcfm"
    "mchprs"
    "mcl-cpp-utility-lib"
    "mcomix"
    "mcontrolcenter"
    "mcp-gateway"
    "mcp-grafana"
    "mcp-k8s-go"
    "mcp-language-server"
    "mcp-nixos"
    "mcp-proxy"
    "mcp-searxng"
    "mcp-server-fetch"
    "mcp-server-filesystem"
    "mcp-server-git"
    "mcp-server-memory"
    "mcp-server-sequential-thinking"
    "mcp-server-time"
    "mcpelauncher-client"
    "mcpelauncher-ui-qt"
    "mcporter"
    "mcpp"
    "mcrcon"
    "mcrl2"
    "mcron"
    "mcrypt"
    "mcstatus"
    "mctc-lib"
    "mcuboot-imgtool"
    "mcumgr-client"
    "mcy"
    "md-lsp"
    "md-tangle"
    "md-tui"
    "md2gemini"
    "md2pdf"
    "md4c"
    "mda_lv2"
    "mdadm"
    "mdadm4"
    "mdbook"
    "mdbook-admonish"
    "mdbook-bib"
    "mdbook-cmdrun"
    "mdbook-d2"
    "mdbook-emojicodes"
    "mdbook-epub"
    "mdbook-footnote"
    "mdbook-graphviz"
    "mdbook-i18n-helpers"
    "mdbook-katex"
    "mdbook-kroki-preprocessor"
    "mdbook-linkcheck2"
    "mdbook-man"
    "mdbook-mermaid"
    "mdbook-open-on-gh"
    "mdbook-pagecrypt"
    "mdbook-pagetoc"
    "mdbook-pandoc"
    "mdbook-pdf"
    "mdbook-pdf-outline"
    "mdbook-plantuml"
    "mdbook-plugins"
    "mdbook-rss-feed"
    "mdbook-rss-feed-full"
    "mdbook-toc"
    "mdbook-variables"
    "mdbook-yml-header"
    "mdbtools"
    "mdcat"
    "mdctags"
    "mdds"
    "mdevctl"
    "mdevd"
    "mdf2iso"
    "mdformat"
    "mdfried"
    "mdhtml"
    "mdio-tools"
    "mdk"
    "mdk-sdk"
    "mdk3-master"
    "mdk4"
    "mdl"
    "mdns"
    "mdns-scanner"
    "mdp"
    "mdq"
    "mdr"
    "mdschema"
    "mdserve"
    "mdsf"
    "mdsh"
    "mdslides"
    "mdterm"
    "mdtsql"
    "mdwatch"
    "mdx-language-server"
    "mdzk"
    "me3"
    "me_cleaner"
    "mealie"
    "measureme"
    "mecab"
    "med"
    "medfile"
    "media-downloader"
    "media-player-info"
    "mediaelch"
    "mediaelch-qt5"
    "mediaelch-qt6"
    "mediagoblin"
    "mediaharbor"
    "mediainfo"
    "mediainfo-gui"
    "mediamate"
    "mediamtx"
    "mediasynclite"
    "mediathekview"
    "mediawiki"
    "mediawriter"
    "medio"
    "mednafen"
    "mednafen-server"
    "mednaffe"
    "medusa"
    "meek"
    "meerk40t"
    "meerk40t-camera"
    "meetingbar"
    "meg"
    "megabasterd"
    "megacli"
    "megacmd"
    "megaglest"
    "megahit"
    "megam"
    "megapixels"
    "megasync"
    "megatools"
    "meh"
    "meilisearch"
    "mekuteriya"
    "mela"
    "melange"
    "meld"
    "melete"
    "meletrix-udev-rules"
    "meli"
    "melmatcheq-lv2"
    "melodeon"
    "melody"
    "melonDS"
    "melonds"
    "melonloader-installer"
    "melos"
    "melt"
    "memcached"
    "memcached-exporter"
    "memcachedTestHook"
    "meme-bingo-web"
    "meme-image-generator"
    "meme-suite"
    "memento"
    "memogram"
    "memorado"
    "memos"
    "memray"
    "memtailor"
    "memtest86-efi"
    "memtest86plus"
    "memtest_vulkan"
    "memtester"
    "memtier-benchmark"
    "memtree"
    "mencal"
    "mendeley"
    "mender-artifact"
    "mendingwall"
    "menu-cache"
    "menulibre"
    "menumaker"
    "menyoki"
    "meow"
    "meowfetch"
    "meowlnir"
    "meowpdf"
    "mepo"
    "meraki-cli"
    "mercure"
    "mercurial"
    "mercurialFull"
    "mercury"
    "merecat"
    "merge-fmt"
    "merge-ut-dictionaries"
    "mergerfs"
    "mergerfs-tools"
    "mergiraf"
    "meritous"
    "merkaartor"
    "mermaid-cli"
    "mermaid-filter"
    "mermerd"
    "merriweather"
    "merriweather-sans"
    "merve"
    "mesa"
    "mesa-demos"
    "mesa-gl-headers"
    "mesa-libclc"
    "mesa_glu"
    "mesa_i686"
    "mescc-tools"
    "mescc-tools-extra"
    "mesen"
    "meshcentral"
    "meshcore-cli"
    "meshlab"
    "meshoptimizer"
    "meshtastic"
    "meshtastic-web"
    "meshtasticd"
    "meslo-lg"
    "meslo-lgs-nf"
    "meson"
    "meson-tools"
    "mesonlsp"
    "mesos-dns"
    "messer-slim"
    "metabase"
    "metabigor"
    "metacity"
    "metacubexd"
    "metadata"
    "metadata-cleaner"
    "metal-cli"
    "metals"
    "metamath"
    "metamorphose2"
    "metapac"
    "metapixel"
    "metar"
    "metasploit"
    "metastore"
    "metee"
    "meteo-qt"
    "meteor"
    "meteor-git"
    "meterbridge"
    "meters-lv2"
    "metersLv2"
    "methane"
    "metis"
    "metis-prover"
    "metricbeat"
    "metricbeat7"
    "metronome"
    "mev-boost"
    "mew"
    "mfaktc"
    "mfakto"
    "mfaomp"
    "mfc465cncupswrapper"
    "mfc465cnlpr"
    "mfc5890cncupswrapper"
    "mfc5890cnlpr"
    "mfc9140cdncupswrapper"
    "mfc9140cdnlpr"
    "mfcj470dw-cupswrapper"
    "mfcj470dwlpr"
    "mfcj6510dw-cupswrapper"
    "mfcj6510dwlpr"
    "mfcj880dwcupswrapper"
    "mfcj880dwlpr"
    "mfcl2700dncupswrapper"
    "mfcl2700dnlpr"
    "mfcl2720dwcupswrapper"
    "mfcl2720dwlpr"
    "mfcl2740dwcupswrapper"
    "mfcl2740dwlpr"
    "mfcl3770cdwcupswrapper"
    "mfcl3770cdwlpr"
    "mfcl5750dw"
    "mfcl8690cdwcupswrapper"
    "mfcl8690cdwlpr"
    "mfcuk"
    "mfoc"
    "mfoc-hardnested"
    "mftrace"
    "mg"
    "mgba"
    "mgitstatus"
    "mgmt"
    "mgrep"
    "mhabit"
    "mhonarc"
    "mhost"
    "mi2ly"
    "michabo"
    "micro"
    "micro-full"
    "micro-httpd"
    "micro-with-wl-clipboard"
    "micro-with-xclip"
    "microbin"
    "microcad"
    "microcode-amd"
    "microcode-intel"
    "microcom"
    "microdnf"
    "microfetch"
    "micromamba"
    "micromdm"
    "micronaut"
    "micronucleus"
    "microplane"
    "micropython"
    "microscheme"
    "microserver"
    "microsocks"
    "microsoft-edge"
    "microsoft-gsl"
    "microsoft-identity-broker"
    "microsoft-rush"
    "mictray"
    "mid2key"
    "middleclick"
    "midi-trigger"
    "midicsv"
    "midimonster"
    "midivisualizer"
    "mieru"
    "mighty-mike"
    "migmix"
    "migrate"
    "migrate-to-uv"
    "migu"
    "mihomo"
    "mike"
    "mikmod"
    "miktex"
    "milkytracker"
    "mill"
    "miller"
    "millet"
    "millipixels"
    "millisecond"
    "milu"
    "mim-solvers"
    "mimalloc"
    "mimeo"
    "mimetic"
    "mimic"
    "mimikatz"
    "mimir"
    "min"
    "min-ed-launcher"
    "minari"
    "minc_tools"
    "minc_widgets"
    "minder"
    "mindustry"
    "mindustry-server"
    "mindustry-wayland"
    "minecraft-server"
    "minecraft-server-hibernation"
    "minecraftia"
    "minefair"
    "minesector"
    "minesweep-rs"
    "minetest-mapserver"
    "mingetty"
    "mingtest"
    "minhtml"
    "mini-calc"
    "mini-httpd"
    "mini-pqc"
    "mini-redis"
    "miniHttpd"
    "minia"
    "miniaudicle"
    "miniaudio"
    "miniball"
    "minica"
    "minicom"
    "minidjvu"
    "minidlna"
    "minidsp"
    "miniflux"
    "minify"
    "minigalaxy"
    "minijail"
    "minijail-tools"
    "minijinja"
    "minikube"
    "minilibx"
    "minimacy"
    "minimal-grub-theme"
    "minimap2"
    "minimax-coding-plan-mcp"
    "minimax-mcp"
    "minimodem"
    "minimp3"
    "mininet"
    "minio"
    "minio-certgen"
    "minio-client"
    "minio-cpp"
    "minio-warp"
    "minion"
    "miniplayer"
    "minipro"
    "minisat"
    "miniscript"
    "miniserve"
    "minisign"
    "minissdpd"
    "ministat"
    "minitube"
    "miniupnpc"
    "miniupnpd"
    "miniupnpd-nftables"
    "minivmac"
    "minixml"
    "miniz"
    "minizign"
    "minizinc"
    "minizinc-ide"
    "minizincide"
    "minizip"
    "minizip-ng"
    "minizip-ng-compat"
    "mint"
    "mint-artwork"
    "mint-cursor-themes"
    "mint-l-icons"
    "mint-l-theme"
    "mint-themes"
    "mint-x-icons"
    "mint-y-icons"
    "mintotp"
    "minutor"
    "mio"
    "mir"
    "mir-qualia"
    "mir_2_15"
    "miracle-wm"
    "miraclecast"
    "miracode"
    "mirakurun"
    "miranda"
    "miredo"
    "miriway"
    "miro"
    "mirrorbits"
    "mirrord"
    "mirth"
    "mirtk"
    "miru"
    "misc-gitology"
    "miscfiles"
    "misconfig-mapper"
    "mise"
    "missidentify"
    "mission-center"
    "mission-planner"
    "misskey"
    "mist"
    "mist-cli"
    "mistral-inference"
    "mistral-rs"
    "mistral-vibe"
    "mistralclient"
    "mitama-cpp-result"
    "mitex"
    "mitimasu"
    "mitm-cache"
    "mitm6"
    "mitmproxy"
    "mitmproxy2swagger"
    "mitra"
    "mitscheme"
    "mitschemeX11"
    "mix2nix"
    "mixing-station"
    "mixtool"
    "mixxc"
    "mixxx"
    "mjmap"
    "mjolnir"
    "mjpegtools"
    "mjpegtoolsFull"
    "mjpg-streamer"
    "mkalias"
    "mkbootimage"
    "mkbootimg-osm0sis"
    "mkbrr"
    "mkcal"
    "mkcert"
    "mkchromecast"
    "mkcl"
    "mkclean"
    "mkcue"
    "mkdocs"
    "mkfontdir"
    "mkfontscale"
    "mkgmap"
    "mkgmap-splitter"
    "mkinitcpio-nfs-utils"
    "mkjson"
    "mkl"
    "mklittlefs"
    "mkmtkhdr"
    "mkosi"
    "mkosi-full"
    "mkp224o"
    "mkpasswd"
    "mkrand"
    "mkroot-utils"
    "mksh"
    "mkspiffs"
    "mktemp"
    "mktoc"
    "mktorrent"
    "mktxp"
    "mkuimage"
    "mkvtoolnix"
    "mkvtoolnix-cli"
    "mlarchive2maildir"
    "mlc"
    "mldonkey"
    "mle"
    "mlflow-server"
    "mlib"
    "mlkit"
    "mlmmj"
    "mloader"
    "mlocate"
    "mlt"
    "mlterm"
    "mlterm-wayland"
    "mlton"
    "mlton20130715"
    "mlton20180207"
    "mlton20180207Binary"
    "mlton20210117"
    "mlton20210117Binary"
    "mlton20241230"
    "mlton20241230Binary"
    "mltonHEAD"
    "mlucas"
    "mlv-app"
    "mlvwm"
    "mlx42"
    "mlxbf-bootctl"
    "mlxbf-bootimages"
    "mm"
    "mm-common"
    "mma"
    "mmake"
    "mmark"
    "mmc-utils"
    "mmctl"
    "mmdbctl"
    "mmdbinspect"
    "mmdoc"
    "mmex"
    "mmg"
    "mmh"
    "mmixware"
    "mmj2"
    "mmlgui"
    "mmsd-tng"
    "mmseqs2"
    "mmtc"
    "mmtf-cpp"
    "mmtui"
    "mmv"
    "mmv-go"
    "mnamer"
    "mnc"
    "mnemonicode"
    "mnemosyne"
    "mni_autoreg"
    "mnist"
    "mno16"
    "mnxdom"
    "mo"
    "mo-viewer"
    "moar"
    "moarvm"
    "mob"
    "mobile-broadband-provider-info"
    "mobilecoin-wallet"
    "mobilizon"
    "mobroute"
    "mobsql"
    "moc"
    "mocha"
    "mochi"
    "mockgen"
    "mockobjects"
    "mockoon"
    "mod"
    "mod-arpeggiator-lv2"
    "mod-distortion"
    "modal"
    "modctl"
    "modd"
    "modelio"
    "models-dev"
    "modelscan"
    "modem-manager-gui"
    "modemmanager"
    "modern-cpp-kafka"
    "modest"
    "modprobed-db"
    "modrinth-app"
    "modrinth-app-unwrapped"
    "mods"
    "modsecurity-crs"
    "modsecurity_standalone"
    "moe"
    "moeli"
    "moji"
    "mojoshader"
    "moka-icon-theme"
    "mokuro"
    "mokutil"
    "molbar"
    "mold-unwrapped"
    "molden"
    "mole"
    "mole-cleaner"
    "molecule"
    "molequeue"
    "molly-brown"
    "molly-guard"
    "mollysocket"
    "molot-lite"
    "molotov"
    "molsketch"
    "moltengamepad"
    "moltenvk"
    "mommy"
    "mona-sans"
    "monado"
    "monado-vulkan-layers"
    "monaspace"
    "monero-cli"
    "monero-gui"
    "monetdb"
    "moneydance"
    "mongoaudit"
    "mongoc"
    "mongocxx"
    "mongodb"
    "mongodb-7_0"
    "mongodb-atlas-cli"
    "mongodb-ce"
    "mongodb-cli"
    "mongodb-compass"
    "mongodb-tools"
    "mongoose"
    "mongosh"
    "monica"
    "monit"
    "monitor"
    "monitorcontrol"
    "monitorets"
    "monitoring-plugins"
    "monkeys-audio"
    "monkeysAudio"
    "monkeysphere"
    "mono"
    "mono4"
    "mono5"
    "mono6"
    "monoDLLFixer"
    "mono_repo"
    "monocle"
    "monocraft"
    "monocypher"
    "monodraw"
    "monoid"
    "monolith"
    "mononoki"
    "monophony"
    "monosat"
    "monotone"
    "mons"
    "monsoon"
    "montserrat"
    "mooSpace"
    "moodle"
    "moodle-dl"
    "moolticute"
    "moon"
    "moon-buggy"
    "moon-child-fe"
    "moon-phases"
    "moonfire-nvr"
    "moonlight"
    "moonlight-embedded"
    "moonlight-qt"
    "moonpalace"
    "moonraker"
    "moonshine"
    "moor"
    "moosefs"
    "moospace"
    "mop"
    "mopac"
    "mopidy"
    "mopidy-argos"
    "mopidy-bandcamp"
    "mopidy-iris"
    "mopidy-jellyfin"
    "mopidy-listenbrainz"
    "mopidy-local"
    "mopidy-moped"
    "mopidy-mopify"
    "mopidy-mpd"
    "mopidy-mpris"
    "mopidy-muse"
    "mopidy-musicbox-webclient"
    "mopidy-notify"
    "mopidy-podcast"
    "mopidy-scrobbler"
    "mopidy-somafm"
    "mopidy-soundcloud"
    "mopidy-spotify"
    "mopidy-subidy"
    "mopidy-tidal"
    "mopidy-tunein"
    "mopidy-youtube"
    "mopidy-ytmusic"
    "moproxy"
    "mopsa"
    "moq"
    "moralerspace"
    "moralerspace-hw"
    "moralerspace-hwjpdoc"
    "moralerspace-jpdoc"
    "more"
    "moreutils"
    "morewaita-icon-theme"
    "morf"
    "morgen"
    "moribito"
    "morph"
    "morphosis"
    "morse-linux"
    "morse_cli"
    "morsel"
    "mos"
    "mosdepth"
    "mosdns"
    "moserial"
    "mosh"
    "moshi"
    "mosml"
    "mosquitto"
    "most"
    "motif"
    "motion"
    "motioneye"
    "motoc"
    "motrix"
    "motrix-next"
    "motus"
    "mount"
    "mount-zip"
    "mountain-duck"
    "mountpoint-s3"
    "mousai"
    "mousam"
    "mouse-actions"
    "mouse_m908"
    "mousecape"
    "mouseless"
    "mousepad"
    "mouser"
    "mousetweaks"
    "mov-cli"
    "moveBuildTree"
    "movim"
    "movine"
    "movit"
    "mox"
    "moxide"
    "moxnotify"
    "mozart2"
    "mozc"
    "mozc-ut"
    "mozcdic-ut-alt-cannadic"
    "mozcdic-ut-edict2"
    "mozcdic-ut-jawiki"
    "mozcdic-ut-neologd"
    "mozcdic-ut-personal-names"
    "mozcdic-ut-place-names"
    "mozcdic-ut-skk-jisyo"
    "mozcdic-ut-sudachidict"
    "mozhi"
    "mozillavpn"
    "mozjpeg"
    "mozlz4a"
    "mozo"
    "mozphab"
    "mozwire"
    "mp3blaster"
    "mp3cat"
    "mp3fs"
    "mp3gain"
    "mp3val"
    "mp4fpsmod"
    "mp4v2"
    "mpack"
    "mpb"
    "mpc"
    "mpc-qt"
    "mpc123"
    "mpd"
    "mpd-discord-rpc"
    "mpd-mpris"
    "mpd-notification"
    "mpd-sima"
    "mpd-small"
    "mpd-touch-screen-gui"
    "mpdas"
    "mpdcron"
    "mpdecimal"
    "mpdris2"
    "mpdris2-rs"
    "mpdscribble"
    "mpeg2dec"
    "mpfi"
    "mpfr"
    "mpfshell"
    "mpg123"
    "mph_2b_damase"
    "mpi"
    "mpiCheckPhaseHook"
    "mpich"
    "mpich-pmix"
    "mpifileutils"
    "mpir"
    "mplayer"
    "mpls"
    "mpop"
    "mpremote"
    "mprime"
    "mpris-notifier"
    "mpris-scrobbler"
    "mpris-timer"
    "mprisence"
    "mprocs"
    "mps"
    "mpsolve"
    "mptcpd"
    "mpv"
    "mpv-handler"
    "mpv-shim-default-shaders"
    "mpv-subs-popout"
    "mpv-unwrapped"
    "mpvc"
    "mpvpaper"
    "mpy-utils"
    "mqattack"
    "mqtt-benchmark"
    "mqtt-explorer"
    "mqtt-exporter"
    "mqtt-randompub"
    "mqtt_cpp"
    "mqttmultimeter"
    "mqttui"
    "mqttx"
    "mqttx-cli"
    "mr"
    "mrbayes"
    "mro-unicode"
    "mrpack-install"
    "mrrescue"
    "mrsh"
    "mrtg"
    "mrtrix"
    "mruby"
    "mrustc"
    "mrustc-bootstrap"
    "mrustc-minicargo"
    "ms-sys"
    "msat"
    "msbuild"
    "msbuild-structured-log-viewer"
    "mscgen"
    "mscompress"
    "mscp"
    "msecli"
    "msedgedriver"
    "msedit"
    "msfpc"
    "msgpack-c"
    "msgpack-cxx"
    "msgpack-tools"
    "msgpuck"
    "msgraph-cli"
    "msgviewer"
    "msieve"
    "msilbc"
    "msitools"
    "msldapdump"
    "mslicer"
    "mslink"
    "msm-modem"
    "msmtp"
    "msnake"
    "msoffcrypto-tool"
    "msolve"
    "msp430GccSupport"
    "msp430Newlib"
    "mspdebug"
    "mspds"
    "mspds-bin"
    "msr"
    "msr-tools"
    "msrtool"
    "mssql_jdbc"
    "mssqlhound"
    "mstflint"
    "mstore"
    "mstpd"
    "mswatch"
    "mt-st"
    "mt32emu-qt"
    "mt32emu-smf2wav"
    "mtail"
    "mtdev"
    "mtdutils"
    "mtk-uartboot"
    "mtkclient"
    "mtm"
    "mtools"
    "mtpaint"
    "mtprotoproxy"
    "mtproxy"
    "mtr"
    "mtr-exporter"
    "mtr-gui"
    "mtrace"
    "mtx"
    "mtxclient"
    "mu"
    "mu-repo"
    "mubeng"
    "muchsync"
    "mucommander"
    "mud"
    "mudlet"
    "mueval"
    "mufetch"
    "muffet"
    "muffin"
    "muffon"
    "mujmap"
    "mujoco"
    "mujs"
    "mullvad"
    "mullvad-browser"
    "mullvad-compass"
    "mullvad-vpn"
    "multi-scrobbler"
    "multica-cli"
    "multicharge"
    "multilockscreen"
    "multimarkdown"
    "multimon-ng"
    "multipart-parser-c"
    "multipath-tools"
    "multiplex"
    "multiqc"
    "multirun"
    "multitail"
    "multitime"
    "multiviewer-for-f1"
    "multivnc"
    "multus-cni"
    "mum"
    "mumble"
    "mumble_overlay"
    "mumps"
    "mumps-mpi"
    "munge"
    "munin"
    "muon"
    "muonStandalone"
    "muonlang"
    "mup"
    "muparser"
    "muparserx"
    "mupdf"
    "mupdf-headless"
    "mupen64plus"
    "murex"
    "murmur"
    "murmure"
    "mus"
    "muscle"
    "muse"
    "muse-sounds-manager"
    "museeks"
    "musescore"
    "musescore-evolution"
    "museum"
    "music-assistant"
    "music-assistant-desktop"
    "music-discord-rpc"
    "musicfree-desktop"
    "musicpod"
    "musicpresence"
    "musicus"
    "musikcube"
    "musl"
    "musl-fts"
    "musl-obstack"
    "musly"
    "muso"
    "mustache-go"
    "mustache-hpp"
    "mustache-spec"
    "mustang-cli"
    "mutagen"
    "mutagen-compose"
    "mutest"
    "mutmut"
    "mutt"
    "mutt-ics"
    "mutt-wizard"
    "mutter"
    "mutter48"
    "muvm"
    "mvapich"
    "mvebu64boot"
    "mvfst"
    "mvnd"
    "mvoice"
    "mvt"
    "mw"
    "mwic"
    "mx-takeover"
    "mxnet"
    "mxt-app"
    "mya"
    "myanon"
    "mybbscan"
    "mycelium"
    "mycli"
    "mycorrhiza"
    "mycrypto"
    "mydumper"
    "myfitnesspal"
    "mygpoclient"
    "mygui"
    "myjwt"
    "myks"
    "mylvmbackup"
    "mymake"
    "myman"
    "mymcplus"
    "mympd"
    "mynewt-newt"
    "mynewt-newtmgr"
    "mypaint"
    "mypaint-brushes"
    "mypaint-brushes1"
    "mypy"
    "mypy-protobuf"
    "myrddin"
    "myrescue"
    "myrica"
    "mysides"
    "mysql-shell"
    "mysql-shell-innovation"
    "mysql-shell_8"
    "mysql-shell_9"
    "mysql-workbench"
    "mysql2pgsql"
    "mysql84"
    "mysql_jdbc"
    "mysqltuner"
    "mystem"
    "mystmd"
    "mytetra"
    "mythes"
    "mythtv"
    "myx"
    "myxer"
    "n-m3u8dl-re"
    "n2"
    "n2048"
    "n2n"
    "n3"
    "n64recomp"
    "n8n"
    "n8n-nodes-carbonejs"
    "n8n-nodes-evolution-api"
    "n8n-task-runner-launcher"
    "n98-magerun2"
    "naabu"
    "nabi"
    "nacelle"
    "naev"
    "nafees"
    "nagelfar"
    "nagios"
    "nagstamon"
    "nahid-fonts"
    "nail-parquet"
    "naja"
    "nak"
    "nakama"
    "nali"
    "namaka"
    "namecoind"
    "namespace-cli"
    "namespaced-openvpn"
    "nano"
    "nano-syntax-highlighting"
    "nano-wallet"
    "nanoarrow"
    "nanobench"
    "nanoboyadvance"
    "nanodbc"
    "nanoemoji"
    "nanoflann"
    "nanomq"
    "nanomsg"
    "nanopb"
    "nanopbMalloc"
    "nanorc"
    "nanosaur"
    "nanosaur2"
    "nanosvg"
    "nanotts"
    "nanovna-qt"
    "nanovna-saver"
    "nanum"
    "nanum-gothic-coding"
    "nap"
    "napalm"
    "napari"
    "napi-rs-cli"
    "napi-rs-cli_3"
    "naproche"
    "naps2"
    "nar-serve"
    "narrowlink"
    "narsil"
    "nas"
    "nasctui"
    "nasin-nanpa"
    "nasin-nanpa-helvetica"
    "nasin-nanpa-ucsur"
    "nasm"
    "nasmfmt"
    "nasty"
    "nat-traverse"
    "national-park-typeface"
    "nativefiledialog-extended"
    "natls"
    "natron"
    "nats-kafka"
    "nats-server"
    "nats-top"
    "natscli"
    "natural-docs"
    "nautilus"
    "nautilus-open-any-terminal"
    "nautilus-open-in-blackbox"
    "nautilus-python"
    "nauty"
    "nav"
    "navi"
    "navicat-premium"
    "navidrome"
    "navilu-font"
    "nawk"
    "nb"
    "nb-cli"
    "nbd"
    "nbdkit"
    "nbench"
    "nbfc-linux"
    "nbmerge"
    "nbping"
    "nbqa"
    "nbsdgames"
    "nbstripout"
    "nbted"
    "nbtscan"
    "nbtscanner"
    "nbutools"
    "nbxplorer"
    "nbytes"
    "nc4nix"
    "ncbi-vdb"
    "ncdc"
    "ncdns"
    "ncdu"
    "ncdu_1"
    "ncftp"
    "ncgopher"
    "nchat"
    "ncmpc"
    "ncmpcpp"
    "ncnn"
    "nco"
    "ncompress"
    "ncpamixer"
    "ncps"
    "ncrack"
    "ncspot"
    "ncurses"
    "ncurses5"
    "ncurses6"
    "ncview"
    "ndcurves"
    "ndg"
    "ndi"
    "ndi-6"
    "ndisc6"
    "ndn-cxx"
    "ndn-tools"
    "ndpi"
    "ndppd"
    "ndstool"
    "ndstrim"
    "ndstrim-rs"
    "ne"
    "near-facsimile"
    "nearcore"
    "neard"
    "neargye-semver"
    "neatvnc"
    "nebula"
    "nebula-de-esser"
    "nebula-lighthouse-service"
    "nebula-sans"
    "necesse-server"
    "nedit"
    "needle"
    "neil"
    "neko"
    "nekoray"
    "nelm"
    "nelua"
    "nemo"
    "nemo-emblems"
    "nemo-fileroller"
    "nemo-preview"
    "nemo-python"
    "nemo-qml-plugin-dbus"
    "nemo-seahorse"
    "nemo-with-extensions"
    "nemorosa"
    "nemu"
    "nengo-gui"
    "neo"
    "neo-cowsay"
    "neo4j"
    "neocities"
    "neocmakelsp"
    "neohtop"
    "neomutt"
    "neon"
    "neonmodem"
    "neosay"
    "neothesia"
    "neovide"
    "neovim"
    "neovim-gtk"
    "neovim-node-client"
    "neovim-qt"
    "neovim-qt-unwrapped"
    "neovim-remote"
    "neovim-unwrapped"
    "neowall"
    "nerd-font-patcher"
    "nerdctl"
    "nerdfetch"
    "nerdfix"
    "nerdlog"
    "nero-umu"
    "nerva"
    "nest"
    "nest-cli"
    "nest-mpi"
    "nesting"
    "nestopia-ue"
    "net-cpp"
    "net-news-wire"
    "net-snmp"
    "net-tools"
    "netassert"
    "netatalk"
    "netavark"
    "netbeans"
    "netbird"
    "netbird-combined"
    "netbird-dashboard"
    "netbird-management"
    "netbird-proxy"
    "netbird-relay"
    "netbird-signal"
    "netbird-ui"
    "netbird-upload"
    "netboot"
    "netbootxyz-efi"
    "netbox"
    "netbox2netshot"
    "netbox_4_6"
    "netcap"
    "netcap-nodpi"
    "netcat"
    "netcat-gnu"
    "netcat-openbsd"
    "netcdf"
    "netcdf-mpi"
    "netcdfcxx4"
    "netcdffortran"
    "netclient"
    "netcoredbg"
    "netdata"
    "netdataCloud"
    "netdiscover"
    "netease-cloud-music-gtk"
    "netevent"
    "netexec"
    "netfetch"
    "netflix"
    "netflow2ng"
    "netfoil"
    "netgen"
    "netgen-vlsi"
    "nethack"
    "nethack-qt"
    "nethack-x11"
    "nethogs"
    "nethoscope"
    "netifd"
    "netkittftp"
    "netlify-cli"
    "netlistsvg"
    "netlogo"
    "netmaker"
    "netmaker-full"
    "netmask"
    "netns-proxy"
    "netop"
    "netpbm"
    "netpeek"
    "netperf"
    "netplan"
    "netproc"
    "netproxrc"
    "netris"
    "netron"
    "netrw"
    "netscan"
    "netscanner"
    "netselect"
    "netsniff-ng"
    "netsurf-browser"
    "netsurf-buildsystem"
    "nettee"
    "nettle"
    "nettle_4"
    "nettools"
    "netwatch"
    "networkaudiod"
    "networkd-dispatcher"
    "networkd-notify"
    "networking-ts-cxx"
    "networkmanager"
    "networkmanager-fortisslvpn"
    "networkmanager-iodine"
    "networkmanager-l2tp"
    "networkmanager-openconnect"
    "networkmanager-openvpn"
    "networkmanager-ssh"
    "networkmanager-sstp"
    "networkmanager-strongswan"
    "networkmanager-vpnc"
    "networkmanager_dmenu"
    "networkmanager_strongswan"
    "networkmanagerapplet"
    "networkminer"
    "neural-amp-modeler-lv2"
    "neuralrack"
    "neuron"
    "neuron-full"
    "neuron-mpi"
    "neutronclient"
    "neverball"
    "neverest"
    "new-session-manager"
    "newcomputermodern"
    "newdoc"
    "newelle"
    "newflasher"
    "newlib"
    "newlib-nano"
    "newlisp"
    "newman"
    "newrelic-sysmond"
    "newsboat"
    "newsflash"
    "newsraft"
    "newt"
    "newt-go"
    "newtonwars"
    "nex"
    "nexa"
    "nextcloud-client"
    "nextcloud-notify_push"
    "nextcloud-spreed-signaling"
    "nextcloud-talk-desktop"
    "nextcloud-whiteboard-server"
    "nextcloud32"
    "nextcloud33"
    "nextcloud34"
    "nextdns"
    "nextflow"
    "nextinspace"
    "nextjs-ollama-llm-ui"
    "nextpnr"
    "nextpnr-xilinx"
    "nextpnrWithGui"
    "nexttrace"
    "nextvi"
    "nexuiz"
    "nexus"
    "nexusd"
    "nexusmods-app"
    "nexusmods-app-unfree"
    "nezha"
    "nezha-agent"
    "nezha-theme-admin"
    "nezha-theme-nazhua"
    "nezha-theme-user"
    "nf-test"
    "nfd"
    "nfdump"
    "nfft"
    "nflz"
    "nfpm"
    "nfs-ganesha"
    "nfs-utils"
    "nfswolf"
    "nftables"
    "nftrace"
    "ngadmin"
    "nghttp2"
    "nghttp3"
    "nginx"
    "nginx-config-formatter"
    "nginx-doc"
    "nginx-language-server"
    "nginx-sso"
    "nginxMainline"
    "nginxShibboleth"
    "nginxStable"
    "ngircd"
    "ngn-k"
    "ngrep"
    "ngrok"
    "ngrrram"
    "ngspice"
    "ngt"
    "ngtcp2"
    "ngtcp2-gnutls"
    "nh"
    "nh-unwrapped"
    "nheko"
    "nhentai"
    "nhost-cli"
    "ni"
    "nibtools"
    "nice-dcv-client"
    "nickel"
    "nicotine-plus"
    "nicstat"
    "niff"
    "nifi"
    "nifskope"
    "nifticlib"
    "niftyreg"
    "niftyseg"
    "nightdiamond-cursors"
    "nightlight"
    "nih-plug"
    "nihstro"
    "niimath"
    "niimblue"
    "nika-fonts"
    "niks3"
    "nikto"
    "nil"
    "nilaway"
    "nile"
    "nilfs-utils"
    "nim"
    "nim-atlas"
    "nim-unwrapped"
    "nim_builder"
    "nim_lk"
    "nimble"
    "nimdow"
    "nimlangserver"
    "nimlsp"
    "nimmm"
    "ninja"
    "ninja_1_11"
    "ninjabrain-bot"
    "ninjas2"
    "ninjavis"
    "ninka"
    "ninvaders"
    "nipap"
    "nipap-cli"
    "nipap-www"
    "niri"
    "niri-sidebar"
    "nirimod"
    "nirimon"
    "niriswitcher"
    "nirius"
    "nitch"
    "nitrocli"
    "nitrokey-app"
    "nitrokey-app2"
    "nitrokey-pro-firmware"
    "nitrokey-start-firmware"
    "nitrokey-storage-firmware"
    "nitrokey-trng-rs232-firmware"
    "nitrokey-udev-rules"
    "nitrotpm-tools"
    "nitter"
    "niv"
    "nix"
    "nix-auth"
    "nix-bash-completions"
    "nix-binary-cache"
    "nix-bisect"
    "nix-btm"
    "nix-build-uncached"
    "nix-bundle"
    "nix-check-deps"
    "nix-converter"
    "nix-delegate"
    "nix-deploy"
    "nix-derivation"
    "nix-diff"
    "nix-direnv"
    "nix-doc"
    "nix-du"
    "nix-eval-jobs"
    "nix-fast-build"
    "nix-forecast"
    "nix-generate-from-cpan"
    "nix-health"
    "nix-heuristic-gc"
    "nix-index"
    "nix-index-unwrapped"
    "nix-info"
    "nix-info-tested"
    "nix-init"
    "nix-inspect"
    "nix-janitor"
    "nix-ld"
    "nix-lib-nmd"
    "nix-lib-nmt"
    "nix-melt"
    "nix-olde"
    "nix-output-monitor"
    "nix-pin"
    "nix-playground"
    "nix-plugins"
    "nix-prefetch"
    "nix-prefetch-bzr"
    "nix-prefetch-cvs"
    "nix-prefetch-darcs"
    "nix-prefetch-docker"
    "nix-prefetch-fossil"
    "nix-prefetch-git"
    "nix-prefetch-github"
    "nix-prefetch-hg"
    "nix-prefetch-pijul"
    "nix-prefetch-scripts"
    "nix-prefetch-svn"
    "nix-query-tree-viewer"
    "nix-required-mounts"
    "nix-run"
    "nix-scheduler-hook"
    "nix-script"
    "nix-search"
    "nix-search-cli"
    "nix-search-tv"
    "nix-serve"
    "nix-serve-ng"
    "nix-snapshotter"
    "nix-store-gcs-proxy"
    "nix-store-veritysetup-generator"
    "nix-sweep"
    "nix-template"
    "nix-top"
    "nix-tour"
    "nix-tree"
    "nix-unit"
    "nix-update"
    "nix-update-source"
    "nix-user-chroot"
    "nix-visualize"
    "nix-weather"
    "nix-web"
    "nix-your-shell"
    "nix-zsh-completions"
    "nixStatic"
    "nixbit"
    "nixci"
    "nixd"
    "nixdoc"
    "nixel"
    "nixf"
    "nixf-diagnose"
    "nixfmt"
    "nixfmt-rfc-style"
    "nixfmt-rs"
    "nixfmt-tree"
    "nixl"
    "nixmate"
    "nixops-dns"
    "nixops_unstable_full"
    "nixops_unstable_minimal"
    "nixos-anywhere"
    "nixos-bgrt-plymouth"
    "nixos-build-vms"
    "nixos-container"
    "nixos-enter"
    "nixos-facter"
    "nixos-firewall-tool"
    "nixos-generators"
    "nixos-grub2-theme"
    "nixos-icons"
    "nixos-init"
    "nixos-install"
    "nixos-install-tools"
    "nixos-option"
    "nixos-rebuild"
    "nixos-rebuild-ng"
    "nixos-render-docs"
    "nixos-render-docs-redirects"
    "nixos-shell"
    "nixos-test-driver"
    "nixoscope"
    "nixpacks"
    "nixpkgs-fmt"
    "nixpkgs-hammering"
    "nixpkgs-lint"
    "nixpkgs-lint-community"
    "nixpkgs-manual"
    "nixpkgs-openjdk-updater"
    "nixpkgs-pytools"
    "nixpkgs-review"
    "nixpkgs-reviewFull"
    "nixpkgs-track"
    "nixpkgs-vet"
    "nixseparatedebuginfod2"
    "nixt"
    "nixtamal"
    "nixtract"
    "njam"
    "njq"
    "nkeys"
    "nkf"
    "nload"
    "nlohmann_json"
    "nlohmann_json_schema_validator"
    "nlojet"
    "nlopt"
    "nls"
    "nm-file-secret-agent"
    "nmap"
    "nmap-formatter"
    "nmap-parse"
    "nmgui"
    "nmh"
    "nmon"
    "nmrpflash"
    "nms"
    "nmstate"
    "nmtrust"
    "nn"
    "nncp"
    "nnd"
    "nng"
    "nnn"
    "nnpdf"
    "nntp-proxy"
    "noaa-apt"
    "noctalia"
    "noctalia-greeter"
    "noctalia-qs"
    "noctalia-shell"
    "nocturne"
    "node-core-utils"
    "node-glob"
    "node-gyp"
    "node-gyp-build"
    "node-hp-scan-to"
    "node-manta"
    "node-pre-gyp"
    "node-problem-detector"
    "node-red"
    "nodeenv"
    "nodehun"
    "nodeinfo"
    "nodejs"
    "nodejs-slim"
    "nodejs-slim_22"
    "nodejs-slim_24"
    "nodejs-slim_26"
    "nodejs-slim_latest"
    "nodejsInstallExecutables"
    "nodejsInstallManuals"
    "nodejs_22"
    "nodejs_24"
    "nodejs_26"
    "nodejs_latest"
    "nodemon"
    "nodenv"
    "nodepy-runtime"
    "nodezator"
    "nodtool"
    "nohang"
    "noice"
    "noip"
    "noise-repellent"
    "noisetorch"
    "nom"
    "nomachine-client"
    "nomacs"
    "nomacs-qt6"
    "nomad"
    "nomad-autoscaler"
    "nomad-driver-containerd"
    "nomad-driver-podman"
    "nomad-pack"
    "nomad_1_10"
    "nomad_1_11"
    "nomad_1_9"
    "nomad_2_0"
    "nomadnet"
    "nominatim"
    "nominatim-ui"
    "nomino"
    "nomnatong"
    "nomore403"
    "non"
    "nono"
    "noogle-search"
    "nootka"
    "nordic"
    "nordpass"
    "nordvpn"
    "nordzy-cursor-theme"
    "nordzy-icon-theme"
    "norgolith"
    "norgolith-mcp"
    "norgolith-plugin-sdk"
    "noriskclient-launcher"
    "noriskclient-launcher-unwrapped"
    "normaliz"
    "normalize"
    "normcap"
    "norminette"
    "norwester-font"
    "nose2pytest"
    "noseyparker"
    "noson"
    "nosql-booster"
    "nosql-workbench"
    "nosqli"
    "nostr-rs-relay"
    "nostui"
    "nota"
    "notable"
    "notation"
    "notcurses"
    "notejot"
    "notemap"
    "notepad-next"
    "notes"
    "noteshrink"
    "notesnook"
    "noti"
    "notify"
    "notify-client"
    "notify-desktop"
    "notify-osd"
    "notifymuch"
    "notion"
    "notion-app"
    "notion-app-enhanced"
    "notion-electron"
    "notmuch"
    "notmuch-addrlookup"
    "notmuch-bower"
    "notmuch-mailmover"
    "notmuch-mutt"
    "noto-fonts"
    "noto-fonts-cjk-sans"
    "noto-fonts-cjk-sans-static"
    "noto-fonts-cjk-serif"
    "noto-fonts-cjk-serif-static"
    "noto-fonts-color-emoji"
    "noto-fonts-emoji-blob-bin"
    "noto-fonts-lgc-plus"
    "noto-fonts-monochrome-emoji"
    "notonoto"
    "notonoto-35"
    "notonoto-console"
    "notonoto-hs"
    "notonoto-hs-35"
    "notonoto-hs-console"
    "nototools"
    "notus-scanner"
    "nova"
    "nova-filters"
    "nova-password"
    "novelwriter"
    "novnc"
    "novops"
    "noweb"
    "nowplaying-cli"
    "nox"
    "noxdir"
    "np"
    "np2kai"
    "npapi_sdk"
    "npb"
    "npc"
    "nph"
    "npiet"
    "npingler"
    "npins"
    "npkill"
    "npm-check"
    "npm-check-updates"
    "npm-groovy-lint"
    "npm-lockfile-fix"
    "nps"
    "npth"
    "npush"
    "nq"
    "nqc"
    "nqp"
    "nqptp"
    "nray"
    "nrf-command-line-tools"
    "nrf-udev"
    "nrf5-sdk"
    "nrfconnect"
    "nrfconnect-bluetooth-low-energy"
    "nrfutil"
    "nrg2iso"
    "nrm"
    "nrr"
    "nrsc5"
    "ns-3"
    "ns-usbloader"
    "nsc"
    "nsd"
    "nsdiff"
    "nsf-ordlista"
    "nsgenbind"
    "nsh"
    "nsis"
    "nsjail"
    "nsnake"
    "nsncd"
    "nsplist"
    "nspr"
    "nsq"
    "nss"
    "nssTools"
    "nss_esr"
    "nss_latest"
    "nss_ldap"
    "nss_pam_ldapd"
    "nss_wrapper"
    "nssmdns"
    "nsss"
    "nstool"
    "nsxiv"
    "nsync"
    "nsz"
    "ntbtls"
    "ntdswolf"
    "ntfs2btrfs"
    "ntfs3g"
    "ntfsprogs"
    "ntfsprogs-plus"
    "ntfy"
    "ntfy-alertmanager"
    "ntfy-sh"
    "nth"
    "ntirpc"
    "ntk"
    "ntl"
    "ntlm-challenger"
    "ntlmrecon"
    "ntopng"
    "ntp"
    "ntpd-rs"
    "ntpstat"
    "nttcp"
    "ntttcp"
    "nu-lint"
    "nuXmv"
    "nu_scripts"
    "nuclear"
    "nuclei"
    "nuclei-templates"
    "nucleiparser"
    "nucleus"
    "nudoku"
    "nufmt"
    "nuget"
    "nuget-to-json"
    "nugget-doom"
    "nukeReferences"
    "nuked-md"
    "nullidentdmod"
    "nullmailer"
    "nulloy"
    "num-utils"
    "numactl"
    "numad"
    "numatop"
    "numbat"
    "numberstation"
    "numcpp"
    "numdiff"
    "numen"
    "numi"
    "numix-cursor-theme"
    "numix-icon-theme"
    "numix-icon-theme-circle"
    "numix-icon-theme-square"
    "numkong"
    "numlockx"
    "numr"
    "numsa"
    "numworks-epsilon"
    "numworks-udev-rules"
    "nunicode"
    "nunito"
    "nunito-sans"
    "nuraft"
    "nurl"
    "nushell"
    "nusmv"
    "nuspell"
    "nut"
    "nuttcp"
    "nuv"
    "nuweb"
    "nuxmv"
    "nv-codec-headers"
    "nv-codec-headers-10"
    "nv-codec-headers-11"
    "nv-codec-headers-12"
    "nv-codec-headers-9"
    "nvc"
    "nvcat"
    "nvchecker"
    "nvd"
    "nvdtools"
    "nvfancontrol"
    "nvfetcher"
    "nvi"
    "nvibrant"
    "nvidia-container-toolkit"
    "nvidia-docker"
    "nvidia-mig-parted"
    "nvidia-modprobe"
    "nvidia-optical-flow-sdk"
    "nvidia-system-monitor-qt"
    "nvidia-texture-tools"
    "nvidia-vaapi-driver"
    "nvidia_cg_toolkit"
    "nvidia_oc"
    "nvimpager"
    "nvitop"
    "nvme-cli"
    "nvme-rs"
    "nvmet-cli"
    "nvmetcfg"
    "nvpy"
    "nvramtool"
    "nvrh"
    "nvrs"
    "nvs"
    "nwchem"
    "nwdiag"
    "nwg-bar"
    "nwg-clipman"
    "nwg-displays"
    "nwg-dock"
    "nwg-dock-hyprland"
    "nwg-drawer"
    "nwg-hello"
    "nwg-icon-picker"
    "nwg-launchers"
    "nwg-look"
    "nwg-menu"
    "nwg-panel"
    "nwg-wrapper"
    "nwipe"
    "nwjs"
    "nwjs-ffmpeg-prebuilt"
    "nwjs-sdk"
    "nx-libs"
    "nx2elf"
    "nxdomain"
    "nxdumpclient"
    "nxengine-evo"
    "nxpmicro-mfgtools"
    "nxv"
    "nxwitness-client"
    "nyaa"
    "nyan"
    "nyancat"
    "nyarr"
    "nylon"
    "nym"
    "nyx"
    "nyxt"
    "nzbget"
    "nzbhydra2"
    "nzportable"
    "oak"
    "oakctl"
    "oaknut"
    "oama"
    "oapi-codegen"
    "oath-toolkit"
    "oathkeeper"
    "oatpp"
    "oauth2-proxy"
    "oauth2c"
    "oauth2l"
    "oauth2ms"
    "ob-xf"
    "obah"
    "obconf"
    "obdautodoctor"
    "obelisk"
    "oberon-risc-emu"
    "obex_data_server"
    "obexd"
    "obexftp"
    "obfs4"
    "obitools3"
    "obj-magic"
    "obj2tiles"
    "objconv"
    "objdiff"
    "objection"
    "objfw"
    "obliteratus"
    "oboete"
    "obs-cli"
    "obs-cmd"
    "obs-do"
    "obs-studio"
    "obscura"
    "obsidian"
    "obsidian-export"
    "obsidian-headless"
    "obuild"
    "ocaml"
    "ocaml-crunch"
    "ocaml-pds"
    "ocaml-top"
    "ocaml_make"
    "ocamlformat"
    "ocamlformat_0_19_0"
    "ocamlformat_0_20_0"
    "ocamlformat_0_20_1"
    "ocamlformat_0_21_0"
    "ocamlformat_0_22_4"
    "ocamlformat_0_23_0"
    "ocamlformat_0_24_1"
    "ocamlformat_0_25_1"
    "ocamlformat_0_26_0"
    "ocamlformat_0_26_1"
    "ocamlformat_0_26_2"
    "ocamlformat_0_27_0"
    "ocamlformat_0_28_1"
    "ocamlformat_0_29_0"
    "oceanic-theme"
    "ocelot"
    "ocelot-desktop"
    "ocenaudio"
    "ocf-resource-agents"
    "oci-cli"
    "oci-seccomp-bpf-hook"
    "oci2git"
    "ocinferno"
    "ocis_5-bin"
    "ockam"
    "ocl-icd"
    "oclock"
    "ocm"
    "ocproxy"
    "ocr-a"
    "ocrad"
    "ocrfeeder"
    "ocrmypdf"
    "ocrodjvu"
    "ocrs"
    "ocs-url"
    "ocserv"
    "ocsigen-i18n"
    "ocsinventory-agent"
    "ocsp-server"
    "octave"
    "octaveFull"
    "octodns"
    "octofetch"
    "octomap"
    "octoprint"
    "octopus"
    "octopus-caller"
    "octorus"
    "octoscan"
    "octosql"
    "octosuite"
    "oculante"
    "odafileconverter"
    "odamex"
    "oddjob"
    "oddsprout"
    "ode"
    "odhcp6c"
    "odiff"
    "odig"
    "odin"
    "odin2"
    "odo"
    "odoo17"
    "odoo18"
    "odoo19"
    "odp-dpdk"
    "odpic"
    "odroid-xu3-bootloader"
    "odt2txt"
    "odyssey"
    "oed"
    "oelint-adv"
    "offat"
    "offensive-azure"
    "office-code-pro"
    "officecli"
    "offlineimap"
    "offpunk"
    "offzip"
    "ofono"
    "ofono-phonesim"
    "ogdf"
    "ogen"
    "oggvideotools"
    "ogmtools"
    "ograc"
    "ogre"
    "ogre-next"
    "ogre_13"
    "ogre_14"
    "oh"
    "oh-my-fish"
    "oh-my-git"
    "oh-my-posh"
    "oh-my-zsh"
    "oha"
    "oidc-agent"
    "oidcwarden"
    "oidentd"
    "oil-buku"
    "oilrush"
    "oils-for-unix"
    "oink"
    "ois"
    "okapi"
    "okapi-ed"
    "oklch-color-picker"
    "okms-cli"
    "okolors"
    "oksh"
    "okta-aws-cli"
    "okteta"
    "okteto"
    "oku"
    "ola"
    "oldsindhi"
    "oldstandard"
    "oletools"
    "olivetin"
    "olivetin-3k"
    "ollama"
    "ollama-cpu"
    "ollama-cuda"
    "ollama-rocm"
    "ollama-vulkan"
    "olm"
    "ols"
    "olsrd"
    "olvid"
    "olympus"
    "olympus-unwrapped"
    "om4"
    "omake"
    "ombi"
    "ome_zarr"
    "omegat"
    "omekasy"
    "omnictl"
    "omniorb"
    "omnisharp-roslyn"
    "omnissa-horizon-client"
    "omniwm"
    "omnix"
    "omnom"
    "omorfi"
    "onagre"
    "onboard"
    "oncall"
    "ondir"
    "one-click-backup"
    "oneDNN"
    "oneDNN_2"
    "oneMath"
    "one_gadget"
    "oneanime"
    "onednn"
    "onednn_2"
    "onedpl"
    "onedrive"
    "onedrivegui"
    "onedriver"
    "onefetch"
    "oneko"
    "oneshot"
    "onesixtyone"
    "onestepback"
    "onetbb"
    "onetun"
    "oniguruma"
    "onionbalance"
    "onioncircuits"
    "onionprobe"
    "onionshare"
    "onionshare-gui"
    "oniux"
    "online-judge-template-generator"
    "online-judge-tools"
    "online-judge-verify-helper"
    "onlykey"
    "onlykey-agent"
    "onlykey-cli"
    "onlyoffice-desktopeditors"
    "onlyoffice-documentserver"
    "onmetal-image"
    "onnx"
    "onnxruntime"
    "onscripter"
    "onyx"
    "oo7"
    "oo7-pam"
    "oo7-portal"
    "oo7-server"
    "ookla-speedtest"
    "ooklaserver"
    "oonf-olsrd2"
    "ooniprobe-cli"
    "op-geth"
    "opa-envoy-plugin"
    "opaline"
    "opam"
    "opam-installer"
    "opam-publish"
    "opam2json"
    "opcr-policy"
    "opcua-client-gui"
    "opcua-commander"
    "open-adventure"
    "open-dyslexic"
    "open-ecard"
    "open-english-wordnet"
    "open-fonts"
    "open-fprintd"
    "open-in-mpv"
    "open-interpreter"
    "open-isns"
    "open-pdf-sign"
    "open-policy-agent"
    "open-sans"
    "open-scq30"
    "open-stage-control-headless"
    "open-timeline-io"
    "open-vm-tools"
    "open-vm-tools-headless"
    "open-watcom-bin"
    "open-watcom-bin-unwrapped"
    "open-watcom-v2"
    "open-watcom-v2-full"
    "open-watcom-v2-full-unwrapped"
    "open-watcom-v2-unwrapped"
    "open-wbo"
    "open-web-calendar"
    "open-websearch"
    "open-webui"
    "open5gs"
    "open5gs-webui"
    "open62541"
    "open62541pp"
    "openafs"
    "openai-whisper"
    "openal"
    "openal-soft"
    "openalSoft"
    "openambit"
    "openapi-changes"
    "openapi-down-convert"
    "openapi-generator-cli"
    "openapi-python-client"
    "openapi-tui"
    "openapv"
    "openarc"
    "openarena"
    "openasar"
    "openav-artyfx"
    "openbabel"
    "openbabel3"
    "openbangla-keyboard"
    "openbao"
    "openbgpd"
    "openblas"
    "openblasCompat"
    "openboard"
    "openboardview"
    "openbooks"
    "openbox"
    "openbrf"
    "openbsm"
    "openbugs"
    "opencascade-occt"
    "opencascade-occt_7_6"
    "opencascade-occt_7_6_1"
    "opencbm"
    "opencc"
    "opencflite"
    "opencl-caps-viewer"
    "opencl-clhpp"
    "opencl-cts"
    "opencl-headers"
    "openclaw"
    "openclonk"
    "opencloud"
    "opencloud-desktop"
    "opencloud-desktop-shell-integration-dolphin"
    "opencloud-desktop-shell-integration-resources"
    "opencode"
    "opencode-claude-auth"
    "opencode-desktop"
    "opencolorio"
    "opencommit"
    "opencomposite"
    "openconnect"
    "openconnect_openssl"
    "opencore-amr"
    "opencorsairlink"
    "opencpn"
    "opencryptoki"
    "opencsg"
    "openct"
    "openctm"
    "opencv"
    "opencv4"
    "opencv4WithoutCuda"
    "opendataloader-pdf"
    "opendbx"
    "opendht"
    "opendkim"
    "opendmarc"
    "opendune"
    "opendungeons"
    "openems"
    "opener"
    "openexr"
    "openfec"
    "openfga"
    "openfga-cli"
    "openfortivpn"
    "openfortivpn-webview"
    "openfortivpn-webview-qt"
    "openfpgaloader"
    "openfreebuds"
    "openfst"
    "openfx"
    "opengamepadui"
    "opengist"
    "opengothic"
    "opengrm-ngram"
    "opengrok"
    "opengv"
    "openh264"
    "openhack"
    "openhantek6022"
    "openhue-cli"
    "openimagedenoise"
    "openimageio"
    "openipmi"
    "openiscsi"
    "openjade"
    "openjdk"
    "openjdk11"
    "openjdk11-bootstrap"
    "openjdk11_headless"
    "openjdk17"
    "openjdk17-bootstrap"
    "openjdk17_headless"
    "openjdk21"
    "openjdk21_headless"
    "openjdk25"
    "openjdk25_headless"
    "openjdk8"
    "openjdk8-bootstrap"
    "openjdk8_headless"
    "openjdk_headless"
    "openjfx"
    "openjfx17"
    "openjfx21"
    "openjfx25"
    "openjk"
    "openjpeg"
    "openjph"
    "openjump"
    "openldap"
    "openlibm"
    "openlierox"
    "openlinkhub"
    "openlist"
    "openllm"
    "openloco"
    "openlogi"
    "openmesh"
    "openmeters"
    "openmm"
    "openmoji-black"
    "openmoji-color"
    "openmolcas"
    "openmotor"
    "openmpCheckPhaseHook"
    "openmpi"
    "openmsx"
    "openmvg"
    "openmvs"
    "openmw"
    "openntpd"
    "openntpd_nixos"
    "openobex"
    "openobserve"
    "openobserve-ee"
    "openocd"
    "openocd-adi"
    "openocd-rp2040"
    "openomf"
    "openonload"
    "openorbitaloptimizer"
    "openorienteering-mapper"
    "openpa"
    "openpace"
    "openpam"
    "openpbs"
    "openpgl"
    "openpgp-ca"
    "openpgp-card-tools"
    "openpomodoro-cli"
    "openra"
    "openra_2019"
    "openrazer-daemon"
    "openrct2"
    "openrefine"
    "openresolv"
    "openresty"
    "openrgb"
    "openrgb-plugin-effects"
    "openrgb-plugin-hardwaresync"
    "openrgb-with-all-plugins"
    "openring"
    "openrisk"
    "openroad"
    "openrocket"
    "openrsync"
    "openrussian-cli"
    "openrw"
    "opensaml-cpp"
    "opensbi"
    "opensc"
    "openscad"
    "openscad-lsp"
    "openscad-unstable"
    "openscap"
    "openscenegraph"
    "openscreen"
    "openseachest"
    "opensearch"
    "opensearch-cli"
    "openseeface"
    "openshadinglanguage"
    "openshell"
    "openshift"
    "openshot-qt"
    "openslide"
    "openslp"
    "opensm"
    "opensmt"
    "opensmtpd"
    "opensmtpd-filter-dkimsign"
    "opensmtpd-filter-rspamd"
    "opensmtpd-table-ldap"
    "opensmtpd-table-mysql"
    "opensmtpd-table-passwd"
    "opensmtpd-table-postgres"
    "opensmtpd-table-redis"
    "opensmtpd-table-socketmap"
    "opensmtpd-table-sqlite"
    "opensnitch"
    "opensnitch-ui"
    "opensoldat"
    "opensoundmeter"
    "opensp"
    "openspades"
    "openspec"
    "openspecfun"
    "openspeedrun"
    "openspin"
    "opensplat"
    "opensplatWithCuda"
    "opensplatWithRocm"
    "opensrc"
    "openssh"
    "openssh-askpass"
    "opensshTest"
    "opensshWithKerberos"
    "openssh_gssapi"
    "openssh_hpn"
    "openssh_hpnWithKerberos"
    "openssl"
    "openssl_3"
    "openssl_3_5"
    "openssl_3_6"
    "openssl_4_0"
    "openssl_legacy"
    "openssl_oqs"
    "openstack-rs"
    "openstackclient"
    "openstackclient-full"
    "opensubdiv"
    "opensupaplex"
    "opensurge"
    "openswitcher"
    "opentabletdriver"
    "opentelemetry-collector"
    "opentelemetry-collector-builder"
    "opentelemetry-collector-contrib"
    "opentelemetry-cpp"
    "openterface-qt"
    "openthread-border-router"
    "opentimelineio"
    "opentimestamps-client"
    "opentofu"
    "opentofu-ls"
    "opentofu-mcp-server"
    "opentrack"
    "opentracker"
    "opentsdb"
    "openttd"
    "openttd-grfcodec"
    "openttd-jgrpp"
    "openttd-nml"
    "openttd-ttf"
    "openturns"
    "opentx"
    "opentxl"
    "opentxl-unwrapped"
    "opentype-sanitizer"
    "opentyrian"
    "openusage"
    "openusd"
    "openutau"
    "openvaf"
    "openvas-scanner"
    "openvas-smb"
    "openvdb"
    "openvi"
    "openvino"
    "openvino-genai"
    "openvino-tokenizers"
    "openvmm"
    "openvox"
    "openvpn"
    "openvpn-auth-ldap"
    "openvpn3"
    "openvpn_learnaddress"
    "openvr"
    "openvscode-server"
    "openvswitch"
    "openvswitch-dpdk"
    "openwebrx"
    "openwsman"
    "openxcom"
    "openxr-loader"
    "openxray"
    "openzone-cursors"
    "operator-sdk"
    "ophcrack"
    "ophcrack-cli"
    "ophis"
    "opkg"
    "opkg-utils"
    "opkssh"
    "opl3bankeditor"
    "opn2bankeditor"
    "opnborg"
    "opnplug"
    "oppai-ng"
    "oprofile"
    "ops"
    "opsdroid"
    "opshin"
    "optar"
    "optee-client"
    "opteeAllwinnerA64"
    "opteeQemuAarch64"
    "opteeQemuArm"
    "opteeRockchipRK3399"
    "opteeRockchipRK3588"
    "optifine"
    "optiland"
    "optimism"
    "optinix"
    "optional-lite"
    "optipng"
    "optnix"
    "opus"
    "opus-tools"
    "opusTools"
    "opusfile"
    "opustags"
    "oq"
    "oqs-provider"
    "or-tools"
    "or1k-newlib"
    "oracle-instantclient"
    "orage"
    "oranchelo-icon-theme"
    "oranda"
    "orangefs"
    "oras"
    "orbit"
    "orbiton"
    "orbitron"
    "orbstack"
    "orbuculum"
    "orbvis"
    "orc"
    "orca"
    "orca-c"
    "orca-slicer"
    "orcania"
    "orchard"
    "orchis-theme"
    "ord"
    "oreo-cursors-plus"
    "org-freedesktop-filemanager1-common"
    "org-stats"
    "organicmaps"
    "orioledb"
    "orion-browser"
    "ormolu"
    "orocos-kdl"
    "orpie"
    "orthanc"
    "orthanc-framework"
    "orthanc-plugin-dicomweb"
    "orthorobot"
    "ory"
    "ory-hydra"
    "ory-talos"
    "orz"
    "os-agent"
    "os-prober"
    "osc"
    "osc-cli"
    "oscar"
    "oscar64"
    "oscavmgr"
    "oscclip"
    "osdlyrics"
    "oshka"
    "oshu"
    "osi"
    "osinfo-db"
    "osinfo-db-tools"
    "osl"
    "osm-gps-map"
    "osm2pgsql"
    "osmctools"
    "osmid"
    "osmium"
    "osmium-tool"
    "osmo"
    "osmo-bsc"
    "osmo-bts"
    "osmo-ggsn"
    "osmo-hlr"
    "osmo-hnbgw"
    "osmo-hnodeb"
    "osmo-iuh"
    "osmo-mgw"
    "osmo-msc"
    "osmo-pcu"
    "osmo-sgsn"
    "osmo-sip-connector"
    "osmscout-server"
    "ospd-openvas"
    "osqp"
    "osqp-eigen"
    "osquery"
    "osrm-backend"
    "oss-rebuild"
    "ossia-score"
    "ossim"
    "osslsigncode"
    "ossutil"
    "ostinato"
    "ostree"
    "ostree-full"
    "ostree-rs-ext"
    "ostrich-sans"
    "ostui"
    "osu-lazer"
    "osu-lazer-bin"
    "osu-micro-benchmarks"
    "osv-detector"
    "osv-scanner"
    "oswald"
    "osx-cpu-temp"
    "osxsnarf"
    "otadump"
    "otb"
    "otel-cli"
    "otel-desktop-viewer"
    "otel-tui"
    "oterm"
    "otf2"
    "otf2bdf"
    "otfcc"
    "otpauth"
    "otpclient"
    "otpw"
    "otree"
    "ots"
    "ott"
    "otto-matic"
    "otus-lisp"
    "ouch"
    "ouch-rar"
    "oui"
    "out-of-tree"
    "outfieldr"
    "outfly"
    "outfox"
    "outguess"
    "outils"
    "outline"
    "outputcheck"
    "ov"
    "oven-media-engine"
    "overcommit"
    "overlayed"
    "overmind"
    "overpass"
    "overpush"
    "overskride"
    "oversteer"
    "overturemaps"
    "overused-grotesk"
    "ovftool"
    "ovh-ttyrec"
    "ovhcloud-cli"
    "ovito"
    "ovn"
    "owamp"
    "owi"
    "owl-compositor"
    "owl-lisp"
    "owmods-cli"
    "owmods-gui"
    "owncast"
    "owncloud-client"
    "ownserver"
    "owntone"
    "owntracks-frontend"
    "owntracks-recorder"
    "owocr"
    "owofetch"
    "ox"
    "oxdraw"
    "oxefmsynth"
    "oxfmt"
    "oxicloud"
    "oxide-rs"
    "oxidized"
    "oxigraph"
    "oxipng"
    "oxker"
    "oxlint"
    "oxtools"
    "oxvg"
    "oxwm"
    "oxygenfonts"
    "oyui"
    "p0f"
    "p11-kit"
    "p2pool"
    "p3x-onenote"
    "p4"
    "p4c"
    "p4d"
    "p4est"
    "p4est-dbg"
    "p4est-sc"
    "p4est-sc-dbg"
    "p4v"
    "p7zip"
    "p7zip-rar"
    "p910nd"
    "pa-dlna"
    "pa-notify"
    "pa_applet"
    "pablodraw"
    "pace"
    "pacemaker"
    "pachyderm"
    "pack"
    "package-project-cmake"
    "package-version-server"
    "packagekit"
    "packcc"
    "packer"
    "packet"
    "packet-sd"
    "packetbeat"
    "packetbeat7"
    "packetdrill"
    "packetry"
    "packj"
    "packr"
    "packsquash"
    "packwiz"
    "pacman"
    "pacman-game"
    "pacparser"
    "pacproxy"
    "pacu"
    "pacvim"
    "padbuster"
    "padne"
    "padre"
    "padthv1"
    "page"
    "pageedit"
    "pagefind"
    "pagemon"
    "paging-calculator"
    "pagmo2"
    "pagsuite"
    "paho-mqtt-c"
    "paho-mqtt-cpp"
    "pahole"
    "pairdrop"
    "paisa"
    "pakcs"
    "pakku"
    "palemoon-bin"
    "palemoon-gtk2-bin"
    "palenight-theme"
    "palera1n"
    "paleta"
    "palette"
    "palettum"
    "palp"
    "pam"
    "pam-honeycreds"
    "pam-pgsql"
    "pam-reattach"
    "pam-watchid"
    "pam_ccreds"
    "pam_dp9ik"
    "pam_fde_boot_pw"
    "pam_gnupg"
    "pam_krb5"
    "pam_ldap"
    "pam_mktemp"
    "pam_mount"
    "pam_mysql"
    "pam_p11"
    "pam_pgsql"
    "pam_rssh"
    "pam_rundir"
    "pam_ssh_agent_auth"
    "pam_tmpdir"
    "pam_u2f"
    "pam_ussh"
    "pam_xdg"
    "pamix"
    "pamixer"
    "paml"
    "pamtester"
    "pan"
    "pan-bindings"
    "pana"
    "panache"
    "panamax"
    "pandera"
    "pandoc"
    "pandoc-acro"
    "pandoc-drawio-filter"
    "pandoc-eqnos"
    "pandoc-ext-diagram"
    "pandoc-fignos"
    "pandoc-imagine"
    "pandoc-include"
    "pandoc-katex"
    "pandoc-lua-filters"
    "pandoc-mustache"
    "pandoc-plantuml-filter"
    "pandoc-secnos"
    "pandoc-tablenos"
    "pandora-launcher"
    "pandora-launcher-unwrapped"
    "pango"
    "pangolin-cli"
    "pangomm"
    "pangomm_2_42"
    "pangomm_2_48"
    "panicparse"
    "panoply"
    "panotools"
    "pantalaimon"
    "pantalaimon-headless"
    "pantheon-tweaks"
    "pantum-driver"
    "panvimdoc"
    "papeer"
    "paper-age"
    "paper-clip"
    "paper-icon-theme"
    "paper-mono"
    "paperjam"
    "paperkey"
    "paperless-asn-qr-codes"
    "paperless-ngx"
    "paperlib"
    "paperlike-go"
    "papermc"
    "papers"
    "papertrail"
    "paperview"
    "paperwork"
    "papi"
    "papilo"
    "papirus-folders"
    "papirus-icon-theme"
    "papirus-maia-icon-theme"
    "papirus-nord"
    "papis"
    "pappl"
    "papra"
    "paprefs"
    "paps"
    "paq"
    "paqet"
    "par"
    "par-lang"
    "par2cmdline"
    "par2cmdline-turbo"
    "parabolic"
    "parallel"
    "parallel-disk-usage"
    "parallel-full"
    "parallel-hashmap"
    "parallel-launcher"
    "paralus-cli"
    "parastoo-fonts"
    "paratest"
    "paratype-pt-mono"
    "paratype-pt-sans"
    "paratype-pt-serif"
    "paraview"
    "parca"
    "parca-agent"
    "parca-debuginfo"
    "paretosecurity"
    "pari"
    "pari-galdata"
    "pari-seadata-small"
    "parinfer-rust"
    "parinfer-rust-emacs"
    "parla"
    "parlatype"
    "parlay"
    "parmetis"
    "parmmg"
    "parole"
    "parpd"
    "parquet-tools"
    "parrot"
    "parse-cli-bin"
    "parseable"
    "parsec-bin"
    "parsedmarc"
    "parsero"
    "parsify"
    "parson"
    "parsync"
    "part-db"
    "partclone"
    "partclone-utils"
    "parted"
    "particl-core"
    "particle-cli"
    "partimage"
    "partio"
    "paru"
    "pasco"
    "pasfmt"
    "pass"
    "pass-git-helper"
    "pass-nodmenu"
    "pass-secret-service"
    "pass-wayland"
    "pass2csv"
    "passage"
    "passdetective"
    "passepartui"
    "passerine"
    "passes"
    "passff-host"
    "passh"
    "passless"
    "passmark-performancetest"
    "passphrase2pgp"
    "passt"
    "passwdqc"
    "past-time"
    "pastebinit"
    "pastel"
    "pasystray"
    "pat"
    "patatt"
    "patch-package"
    "patch2pr"
    "patchPpdFilesHook"
    "patchRcPathBash"
    "patchRcPathCsh"
    "patchRcPathFish"
    "patchRcPathPosix"
    "patchance"
    "patchcil"
    "patchelfUnstable"
    "patcher9x"
    "patchutils"
    "patchutils_0_3_3"
    "patchutils_0_4_2"
    "patchy"
    "patdiff"
    "path-of-building"
    "pathvector"
    "patool"
    "patray"
    "patroni"
    "patsh"
    "pattypan"
    "paulstretch"
    "paup-cli"
    "pavolctld"
    "pavucontrol"
    "pawn-appetit"
    "pax"
    "pax-britannica"
    "pax-utils"
    "paxctl"
    "paxtest"
    "pay-respects"
    "payload-dumper-go"
    "payload_dumper"
    "payloadsallthethings"
    "payme"
    "pazi"
    "pb"
    "pb_cli"
    "pbc"
    "pbgopy"
    "pbkdf2-password-hash"
    "pbm"
    "pbpctrl"
    "pbrt"
    "pbzip2"
    "pbzx"
    "pc"
    "pc-ble-driver"
    "pcalc"
    "pcapc"
    "pcapfix"
    "pcaudiolib"
    "pcem"
    "pcg_c"
    "pcimem"
    "pciutils"
    "pcl"
    "pcloud"
    "pcm"
    "pcmanfm"
    "pcmanfm-qt"
    "pcmciaUtils"
    "pcmciautils"
    "pcmsolver"
    "pcre"
    "pcre-cpp"
    "pcre2"
    "pcsc-cyberjack"
    "pcsc-safenet"
    "pcsc-scm-scl011"
    "pcsc-tools"
    "pcsclite"
    "pcscliteWithPolkit"
    "pcstat"
    "pcsx2"
    "pd-else"
    "pdal"
    "pdb2pqr"
    "pdd"
    "pdepend"
    "pdf-cli"
    "pdf-mcp"
    "pdf-oxide"
    "pdf-parser"
    "pdf-sign"
    "pdf2odt"
    "pdf2svg"
    "pdf4qt"
    "pdfalyzer"
    "pdfannots"
    "pdfannots2json"
    "pdfarranger"
    "pdfchain"
    "pdfcpu"
    "pdfcrack"
    "pdfdiff"
    "pdfding"
    "pdfgrep"
    "pdfhummus"
    "pdfid"
    "pdfium"
    "pdfium-binaries"
    "pdfminer"
    "pdfmixtool"
    "pdfmm"
    "pdfpc"
    "pdfposter"
    "pdfrip"
    "pdfsam-basic"
    "pdfsandwich"
    "pdfstudio2021"
    "pdfstudio2022"
    "pdfstudio2023"
    "pdfstudio2024"
    "pdfstudioviewer"
    "pdftag"
    "pdftitle"
    "pdftk"
    "pdftoipe"
    "pdftowrite"
    "pdftricks"
    "pdi"
    "pdisk"
    "pdk"
    "pdk-ciel"
    "pdm"
    "pdns"
    "pdns-recursor"
    "pdnsd"
    "pdnsgrep"
    "pdpmake"
    "pdqsort"
    "pds"
    "pdsadmin"
    "pdsh"
    "pdudaemon"
    "pe-bear"
    "pe-parse"
    "peaclock"
    "peacock"
    "peakperf"
    "pear-desktop"
    "peazip"
    "pebble"
    "pecita"
    "peco"
    "pedantix"
    "peek"
    "peep"
    "peerflix-server"
    "peergos"
    "peering-manager"
    "peertube"
    "peertube-viewer"
    "peg"
    "pegasus-frontend"
    "pegtl"
    "peirates"
    "pekwm"
    "pencil"
    "pencil2d"
    "penelope"
    "penguin-subtitle-player"
    "penna"
    "penpot-desktop"
    "pentestgpt"
    "peppercarrot-fonts"
    "perccli"
    "percollate"
    "percona-server"
    "percona-server_8_4"
    "percona-toolkit"
    "percona-xtrabackup"
    "percona-xtrabackup_8_4"
    "perf"
    "perf-tools"
    "perf_data_converter"
    "perfect-hash"
    "perfect_dark"
    "perfetto"
    "perfetto-sdk"
    "performous"
    "periphery"
    "perkeep"
    "perl"
    "perl-debug-adapter"
    "perl5"
    "perlcritic"
    "perlnavigator"
    "perplexity-mcp"
    "persepolis"
    "perses"
    "perseus"
    "perseus-cli"
    "persistent-cache-cpp"
    "persistent-evdev"
    "peru"
    "pesde"
    "pesign"
    "pest"
    "pest-ide-tools"
    "pet"
    "petidomo"
    "petsc"
    "pev"
    "pew"
    "pfcpsim"
    "pferd"
    "pfetch"
    "pfetch-rs"
    "pff-tool"
    "pffft"
    "pfft"
    "pflask"
    "pflogsumm"
    "pflotran"
    "pforth"
    "pfsshell"
    "pg-dump-anon"
    "pg-schema-diff"
    "pg_activity"
    "pg_checksums"
    "pg_exporter"
    "pg_featureserv"
    "pg_flame"
    "pg_tileserv"
    "pg_top"
    "pgadmin4"
    "pgadmin4-desktopmode"
    "pgagroal"
    "pgbackrest"
    "pgbadger"
    "pgbouncer"
    "pgcenter"
    "pgcli"
    "pgcopydb"
    "pgdog"
    "pget"
    "pgf"
    "pgf-pie"
    "pgf-umlcd"
    "pgf-umlsd"
    "pgf2"
    "pgf3"
    "pgformatter"
    "pgfplots"
    "pghero"
    "pgit"
    "pgloader"
    "pgmanage"
    "pgmetrics"
    "pgmodeler"
    "pgmoneta"
    "pgo-client"
    "pgpdump"
    "pgpkeyserver-lite"
    "pgpool"
    "pgraphs"
    "pgread"
    "pgrok"
    "pgroll"
    "pgschema"
    "pgscv"
    "pgsql-tools"
    "pgsrip"
    "pgsync"
    "pgtop"
    "pgtui"
    "pgweb"
    "phantom"
    "phantomsocks"
    "phantun"
    "pharo"
    "phart"
    "phase-cli"
    "phash"
    "phd2"
    "phel"
    "phetch"
    "philipstv"
    "phinger-cursors"
    "phira"
    "phira-free"
    "phira-mp"
    "phira-unwrapped"
    "phoc"
    "phockup"
    "phodav"
    "phoenixd"
    "phonemizer"
    "phonetisaurus"
    "phoronix-test-suite"
    "phosh"
    "phosh-mobile-settings"
    "photini"
    "photocollage"
    "photofield"
    "photoflare"
    "photon"
    "photon-rss"
    "photonvision"
    "photoprism"
    "photoqt"
    "phototonic"
    "photoview"
    "php"
    "php82"
    "php83"
    "php84"
    "php85"
    "phpactor"
    "phpantom-lsp"
    "phpdocumentor"
    "phpstan"
    "phpunit"
    "phrase-cli"
    "phrasendrescher"
    "phraze"
    "phylactery"
    "phylophlan"
    "physac"
    "physfs"
    "physfs_2"
    "physlock"
    "phytool"
    "pi-coding-agent"
    "piano-rs"
    "pianobar"
    "pianobooster"
    "picard"
    "picard-tools"
    "picat"
    "picgo"
    "pick"
    "pick-colour-picker"
    "pico-sdk"
    "picoc"
    "picoclaw"
    "picocom"
    "picocrypt"
    "picocrypt-cli"
    "picocrypt-ng"
    "picoleaf"
    "picolibc"
    "picolisp"
    "picoloop"
    "picom"
    "picom-pijulius"
    "picoprobe-udev-rules"
    "picosat"
    "picoscope"
    "picoshare"
    "picosnitch"
    "picotool"
    "picotts"
    "pict-rs"
    "pid-fan-controller"
    "pid1"
    "piday25"
    "pidgin"
    "pie-cli"
    "pied"
    "pies"
    "pietrasanta-traceroute"
    "pifpaf"
    "pig"
    "pigeon"
    "pigeons"
    "piglit"
    "pigment"
    "pigpio"
    "pigz"
    "pihole"
    "pihole-ftl"
    "pihole-web"
    "pijuice"
    "pijul"
    "pik"
    "pika"
    "pika-backup"
    "pikchr"
    "pike"
    "piknik"
    "pikopixel"
    "pikpaktui"
    "piliplus"
    "pilot-link"
    "pim6sd"
    "pimsync"
    "pinact"
    "pinball"
    "pinboard"
    "pinboard-notes-backup"
    "pince"
    "pinchflat"
    "pineapple-pictures"
    "pineflash"
    "pinegrow"
    "pinegrow6"
    "pinentry-all"
    "pinentry-bemenu"
    "pinentry-curses"
    "pinentry-dmenu"
    "pinentry-egui"
    "pinentry-emacs"
    "pinentry-gnome3"
    "pinentry-qt"
    "pinentry-rofi"
    "pinentry-tty"
    "pinentry_mac"
    "pinfo"
    "pingme"
    "pingr"
    "pingtcp"
    "pingtunnel"
    "pingu"
    "pingus"
    "pinit"
    "pinnacle"
    "pinniped"
    "pinnwand"
    "pinocchio"
    "pinokio"
    "pinpoint"
    "pinsel"
    "pint"
    "pinta"
    "pinyin-tool"
    "pioasm"
    "pioneer"
    "pioneers"
    "pip-audit"
    "pipe-rename"
    "pipe-viewer"
    "pipectl"
    "piped"
    "pipeform"
    "pipekit"
    "pipelight"
    "pipeline"
    "pipenv"
    "pipenv-poetry-migrate"
    "piper"
    "piper-phonemize"
    "piper-tts"
    "pipes"
    "pipes-rs"
    "pipet"
    "pipeweaver"
    "pipewire"
    "pipewire-control-center"
    "pipework"
    "pipeworld"
    "pipeworld-wrapped"
    "piping-server-rust"
    "pipr"
    "pipreqs"
    "pipx"
    "pirate-get"
    "piscope"
    "piskel"
    "pistol"
    "piston-cli"
    "pitch-black"
    "pitchfork"
    "pithos"
    "pitivi"
    "pius"
    "piv-agent"
    "pix"
    "pixcat"
    "pixd"
    "pixel-code"
    "pixel2svg"
    "pixelfed"
    "pixelflasher"
    "pixelnuke"
    "pixelorama"
    "pixelpwnr"
    "pixeluvo"
    "pixi"
    "pixi-pack"
    "pixiecore"
    "pixieditor"
    "pixiewps"
    "pixinsight"
    "pixman"
    "pixz"
    "pizarra"
    "pizauth"
    "pj"
    "pjsip"
    "pk2cmd"
    "pkarr"
    "pkcrack"
    "pkcs11-provider"
    "pkcs11helper"
    "pkg"
    "pkg-config-unwrapped"
    "pkgconf-unwrapped"
    "pkgdiff"
    "pkger"
    "pkgit"
    "pkgsite"
    "pkgtop"
    "pkl"
    "pkl-lsp"
    "pktgen"
    "place-cursor-at"
    "plakar"
    "plakar-plugin-etcd"
    "plakar-plugin-ftp"
    "plakar-plugin-gcs"
    "plakar-plugin-imap"
    "plakar-plugin-k8s"
    "plakar-plugin-nfs"
    "plakar-plugin-proxmox"
    "plakar-plugin-rclone"
    "plakar-plugin-s3"
    "plakar-plugin-sftp"
    "plakar-plugin-smb"
    "plakar-plugin-sqlite"
    "plakativ"
    "plan-exporter"
    "plan9port"
    "plan9port-wayland"
    "planarity"
    "planify"
    "plank"
    "planner"
    "plantuml"
    "plantuml-c4"
    "plantuml-server"
    "planus"
    "plasma-applet-commandoutput"
    "plasma-hud"
    "plasma-overdose-kde-theme"
    "plasma-panel-colorizer"
    "plasma-panel-spacer-extended"
    "plasma-plugin-blurredwallpaper"
    "plasmavantage"
    "plasmusic-toolbar"
    "plasticity"
    "plasticscm-client-complete"
    "plasticscm-client-core"
    "plasticscm-client-core-unwrapped"
    "plasticscm-client-gui"
    "plasticscm-client-gui-unwrapped"
    "plasticscm-theme"
    "plater"
    "platform-folders"
    "platformio"
    "platformio-chrootenv"
    "platformio-core"
    "platformsh"
    "platinum-searcher"
    "plattenalbum"
    "plausible"
    "play"
    "play-with-mpv"
    "playball"
    "playdate-sdk"
    "playerctl"
    "playonlinux"
    "playwright"
    "playwright-driver"
    "playwright-mcp"
    "playwright-test"
    "please"
    "please-cli"
    "plecost"
    "plemoljp"
    "plemoljp-hs"
    "plemoljp-nf"
    "pleroma"
    "pleroma-bot"
    "plex"
    "plex-desktop"
    "plex-htpc"
    "plex-mpv-shim"
    "plexRaw"
    "plexamp"
    "plezy"
    "plfit"
    "plib"
    "plik"
    "plikd"
    "plink-ng"
    "plistcpp"
    "plistwatch"
    "plm"
    "plocate"
    "plog"
    "ploticus"
    "plotinus"
    "plotutils"
    "plover"
    "plover_4"
    "plover_5"
    "plow"
    "plowshare"
    "plplot"
    "pls"
    "plugdata"
    "plugin-torture"
    "pluginval"
    "plujain-ramp"
    "pluma"
    "plumber"
    "plumed"
    "plus-jakarta-sans"
    "plus42"
    "pluto"
    "plutosvg"
    "plutovg"
    "plymouth"
    "plymouth-blahaj-theme"
    "plymouth-matrix-theme"
    "plymouth-proxzima-theme"
    "plymouth-vortex-ubuntu-theme"
    "plzip"
    "pm2"
    "pmacct"
    "pmars"
    "pmars-x11"
    "pmbootstrap"
    "pmccabe"
    "pmd"
    "pmenu"
    "pmidi"
    "pmix"
    "pmount"
    "pms"
    "pmtiles"
    "pmutils"
    "pnetcdf"
    "pnfft"
    "pngcheck"
    "pngcrush"
    "pnglatex"
    "pngloss"
    "pngnq"
    "pngoptimizer"
    "pngout"
    "pngpaste"
    "pngpp"
    "pngquant"
    "pngtoico"
    "pngtools"
    "pnmixer"
    "pnpm"
    "pnpm-fixup-state-db"
    "pnpm-shell-completion"
    "pnpmBuildHook"
    "pnpmConfigHook"
    "pnpm_10"
    "pnpm_10_29_2"
    "pnpm_10_34_0"
    "pnpm_11"
    "po4a"
    "pocket-casts"
    "pocket-id"
    "pocket-tts"
    "pocketbase"
    "pocketsphinx"
    "pocl"
    "poco"
    "pod2mdoc"
    "podget"
    "podgrab"
    "podiff"
    "podlet"
    "podman"
    "podman-bootc"
    "podman-compose"
    "podman-desktop"
    "podman-tui"
    "podofo"
    "podofo0"
    "podofo_0_10"
    "pods"
    "poedit"
    "poethepoet"
    "poetry"
    "poetry2conda"
    "poezio"
    "poke"
    "pokefinder"
    "pokeget-rs"
    "pokego"
    "pokemmo-installer"
    "pokemon-colorscripts"
    "pokemon-colorscripts-mac"
    "pokemon-cursor"
    "pokemonsay"
    "pokerth"
    "pokerth-server"
    "pokete"
    "polar"
    "polari"
    "polaris"
    "polaris-web"
    "polarity"
    "polenum"
    "policycoreutils"
    "poliedros"
    "polkadot"
    "polkit"
    "polkit-stdin-agent"
    "polkit_gnome"
    "pololu-tic"
    "polonium"
    "poly"
    "poly2tri-c"
    "polybar"
    "polybar-pulseaudio-control"
    "polybarFull"
    "polychromatic"
    "polyglot"
    "polygon-cli"
    "polygraph"
    "polylith"
    "polylux2pdfpc"
    "polymake"
    "polyml"
    "polyphone"
    "polysh"
    "polytopes_db"
    "pom"
    "pomerium"
    "pomerium-cli"
    "pommed_light"
    "pomo"
    "pomodoro"
    "pomodoro-gtk"
    "pomsky"
    "pong3d"
    "pony-corral"
    "ponyc"
    "ponymix"
    "ponysay"
    "poop"
    "pop"
    "pop-hp-wallpapers"
    "pop-icon-theme"
    "pop-launcher"
    "pop-wallpapers"
    "popa3d"
    "popcorn-cli"
    "popeye"
    "popfile"
    "popl"
    "poppins"
    "poppler"
    "poppler-utils"
    "poppler_data"
    "poppler_gi"
    "poppler_min"
    "popsicle"
    "popt"
    "poptop"
    "poptracker"
    "porn-vault"
    "porsmo"
    "portablemc"
    "portal"
    "portaudio"
    "portfolio"
    "portfolio-filemanager"
    "portfolio_rs"
    "portmaster"
    "portmidi"
    "portmod"
    "portunus"
    "porxie"
    "poselib"
    "positron-bin"
    "postcss"
    "posteid-seed-extractor"
    "postfix"
    "postfix-tlspol"
    "postfixadmin"
    "postgres-language-server"
    "postgres-lsp"
    "postgres-websockets"
    "postgresql"
    "postgresqlTestHook"
    "postgresql_14"
    "postgresql_14_jit"
    "postgresql_15"
    "postgresql_15_jit"
    "postgresql_16"
    "postgresql_16_jit"
    "postgresql_17"
    "postgresql_17_jit"
    "postgresql_18"
    "postgresql_18_jit"
    "postgresql_19"
    "postgresql_19_jit"
    "postgresql_jdbc"
    "postgresql_jit"
    "postgrest"
    "postgrey"
    "postiats-utilities"
    "posting"
    "postlight-parser"
    "postman"
    "postmoogle"
    "postprocessd"
    "postscript-lexmark"
    "postsrsd"
    "posy-cursors"
    "potrace"
    "potreeconverter"
    "pouf"
    "pounce"
    "poutine"
    "povray"
    "power-calibrate"
    "power-profiles-daemon"
    "poweralertd"
    "powercap"
    "powerdns-admin"
    "powerhub"
    "powerjoular"
    "powerline"
    "powerline-fonts"
    "powerline-go"
    "powerline-symbols"
    "powermanga"
    "powermode-indicator"
    "powerpipe"
    "powershell"
    "powershell-editor-services"
    "powersploit"
    "powerstat"
    "powerstation"
    "powersupply"
    "powertabeditor"
    "powertop"
    "powertop-macos"
    "powerview"
    "pphack"
    "ppl"
    "pplatex"
    "pplite"
    "ppp"
    "pprof"
    "pps-tools"
    "ppsspp"
    "ppsspp-qt"
    "ppsspp-sdl"
    "ppsspp-sdl-wayland"
    "pptp"
    "pptpd"
    "pq-cli"
    "pqc-bench"
    "pqc-scanner"
    "pqcscan"
    "pqiv"
    "pql"
    "pqrs"
    "pr-tracker"
    "praat"
    "practrand"
    "pragha"
    "pragtical"
    "prctl"
    "pre-commit"
    "pre-commit-hook-ensure-sops"
    "pre2k"
    "precice"
    "precice-config-visualizer"
    "precious"
    "prefect"
    "prefetch-npm-deps"
    "prefetch-yarn-deps"
    "prek"
    "prelink"
    "premake"
    "premake4"
    "premake5"
    "present"
    "present-cli"
    "presenterm"
    "preserves-tools"
    "pretalx"
    "pretendard"
    "pretendard-gov"
    "pretendard-jp"
    "pretendard-std"
    "pretender"
    "pretix"
    "pretix-banktool"
    "prettier"
    "prettier-d-slim"
    "prettier-plugin-go-template"
    "prettier-plugin-jinja-template"
    "prettier-plugin-tailwindcss"
    "prettierd"
    "pretty-php"
    "pretty-simple"
    "prettyping"
    "prettypst"
    "prevail"
    "previewqt"
    "prevo"
    "prevo-data"
    "prevo-tools"
    "pricehist"
    "pridecat"
    "pridefetch"
    "prime-server"
    "primecount"
    "primesieve"
    "primitive"
    "primus"
    "primus-lib"
    "primusLib"
    "principia"
    "printrun"
    "prio"
    "prio-wrapped"
    "priocpp"
    "prism"
    "prism-model-checker"
    "prisma"
    "prisma-engines"
    "prisma-engines_6"
    "prisma-engines_7"
    "prisma-language-server"
    "prisma_6"
    "prisma_7"
    "prismlauncher"
    "prismlauncher-unwrapped"
    "pritunl-client"
    "pritunl-ssh"
    "privatebin"
    "privoxy"
    "prjoxide"
    "prjpeppercorn"
    "prl-tools"
    "prmers"
    "prmt"
    "pro-office-calculator"
    "probe-rs-tools"
    "probezen"
    "procdump"
    "process-compose"
    "process-cpp"
    "process-viewer"
    "processing"
    "procfd"
    "prociono"
    "procmail"
    "procodile"
    "procps"
    "procs"
    "procyon"
    "prodigal"
    "profanity"
    "professor"
    "profetch"
    "profile-cleaner"
    "profile-sync-daemon"
    "profont"
    "proftpd"
    "proggyfonts"
    "programmer-calculator"
    "progress"
    "progress-tracker"
    "proj"
    "proj-datumgrid"
    "projclean"
    "project-graph"
    "project-lemonlime"
    "projectable"
    "projectcenter"
    "projecteur"
    "projectlibre"
    "projectm-sdl-cpp"
    "projectm_3"
    "proksi"
    "prom2json"
    "prometheus"
    "prometheus-alertmanager"
    "prometheus-apcupsd-exporter"
    "prometheus-artifactory-exporter"
    "prometheus-aruba-exporter"
    "prometheus-atlas-exporter"
    "prometheus-aws-s3-exporter"
    "prometheus-bind-exporter"
    "prometheus-bird-exporter"
    "prometheus-bitcoin-exporter"
    "prometheus-blackbox-exporter"
    "prometheus-borgmatic-exporter"
    "prometheus-chrony-exporter"
    "prometheus-cloudflare-exporter"
    "prometheus-collectd-exporter"
    "prometheus-consul-exporter"
    "prometheus-cpp"
    "prometheus-cpp-lite"
    "prometheus-dcgm-exporter"
    "prometheus-deluge-exporter"
    "prometheus-dnsmasq-exporter"
    "prometheus-dnssec-exporter"
    "prometheus-domain-exporter"
    "prometheus-ebpf-exporter"
    "prometheus-elasticsearch-exporter"
    "prometheus-fail2ban-exporter"
    "prometheus-fastly-exporter"
    "prometheus-flow-exporter"
    "prometheus-fritzbox-exporter"
    "prometheus-frr-exporter"
    "prometheus-gitlab-ci-pipelines-exporter"
    "prometheus-graphite-exporter"
    "prometheus-haproxy-exporter"
    "prometheus-idrac-exporter"
    "prometheus-imap-mailstat-exporter"
    "prometheus-influxdb-exporter"
    "prometheus-ipmi-exporter"
    "prometheus-jitsi-exporter"
    "prometheus-jmx-httpserver"
    "prometheus-jmx-javaagent"
    "prometheus-json-exporter"
    "prometheus-junos-czerwonk-exporter"
    "prometheus-kea-exporter"
    "prometheus-keylight-exporter"
    "prometheus-klipper-exporter"
    "prometheus-knot-exporter"
    "prometheus-kvrocks-exporter"
    "prometheus-libvirt-exporter"
    "prometheus-lnd-exporter"
    "prometheus-mail-exporter"
    "prometheus-mailman3-exporter"
    "prometheus-mikrotik-exporter"
    "prometheus-modbus-exporter"
    "prometheus-modemmanager-exporter"
    "prometheus-mongodb-exporter"
    "prometheus-mysqld-exporter"
    "prometheus-nats-exporter"
    "prometheus-nextcloud-exporter"
    "prometheus-nginx-exporter"
    "prometheus-nginxlog-exporter"
    "prometheus-node-cert-exporter"
    "prometheus-node-exporter"
    "prometheus-nut-exporter"
    "prometheus-nvidia-gpu-exporter"
    "prometheus-opnsense-exporter"
    "prometheus-pgbouncer-exporter"
    "prometheus-php-fpm-exporter"
    "prometheus-pihole-exporter"
    "prometheus-ping-exporter"
    "prometheus-postfix-exporter"
    "prometheus-postgres-exporter"
    "prometheus-process-exporter"
    "prometheus-pushgateway"
    "prometheus-pve-exporter"
    "prometheus-qbittorrent-exporter"
    "prometheus-rabbitmq-exporter"
    "prometheus-rasdaemon-exporter"
    "prometheus-redis-exporter"
    "prometheus-restic-exporter"
    "prometheus-rtl_433-exporter"
    "prometheus-sabnzbd-exporter"
    "prometheus-sachet"
    "prometheus-script-exporter"
    "prometheus-shelly-exporter"
    "prometheus-siebenmann-zfs-exporter"
    "prometheus-slurm-exporter"
    "prometheus-smartctl-exporter"
    "prometheus-smokeping-prober"
    "prometheus-snmp-exporter"
    "prometheus-snowflake-exporter"
    "prometheus-solaredge-exporter"
    "prometheus-speedtest-exporter"
    "prometheus-sql-exporter"
    "prometheus-squid-exporter"
    "prometheus-statsd-exporter"
    "prometheus-storagebox-exporter"
    "prometheus-surfboard-exporter"
    "prometheus-systemd-exporter"
    "prometheus-tailscale-exporter"
    "prometheus-tibber-exporter"
    "prometheus-unbound-exporter"
    "prometheus-v2ray-exporter"
    "prometheus-varnish-exporter"
    "prometheus-wireguard-exporter"
    "prometheus-xmpp-alerts"
    "prometheus-xray-exporter"
    "prometheus-zfs-exporter"
    "promexplorer"
    "promptfoo"
    "promql-cli"
    "promscale"
    "prooftree"
    "proot"
    "properties-cpp"
    "prose"
    "proselint"
    "prosody"
    "prosody-filer"
    "prospector"
    "protege"
    "protege-distribution"
    "proteus"
    "proto"
    "proto-contrib"
    "protobuf"
    "protobuf-language-server"
    "protobuf-matchers"
    "protobuf_21"
    "protobuf_25"
    "protobuf_27"
    "protobuf_29"
    "protobuf_30"
    "protobuf_31"
    "protobuf_32"
    "protobuf_33"
    "protobuf_34"
    "protobuf_35"
    "protobuf_36"
    "protobufc"
    "protoc-gen-connect-go"
    "protoc-gen-dart"
    "protoc-gen-doc"
    "protoc-gen-elixir"
    "protoc-gen-entgrpc"
    "protoc-gen-es"
    "protoc-gen-go"
    "protoc-gen-go-grpc"
    "protoc-gen-go-ttrpc"
    "protoc-gen-go-vtproto"
    "protoc-gen-grpc-java"
    "protoc-gen-grpc-web"
    "protoc-gen-js"
    "protoc-gen-lint"
    "protoc-gen-prost"
    "protoc-gen-prost-crate"
    "protoc-gen-prost-serde"
    "protoc-gen-rust"
    "protoc-gen-rust-grpc"
    "protoc-gen-swift"
    "protoc-gen-tonic"
    "protoc-gen-twirp"
    "protoc-gen-twirp_php"
    "protoc-gen-twirp_typescript"
    "protoc-gen-validate"
    "protoc-go-inject-tag"
    "protocol"
    "protolint"
    "protolock"
    "protols"
    "proton-authenticator"
    "proton-authenticator-bin"
    "proton-cli"
    "proton-ge-bin"
    "proton-pass"
    "proton-pass-cli"
    "proton-vpn"
    "proton-vpn-cli"
    "protonmail-bridge"
    "protonmail-bridge-gui"
    "protonmail-desktop"
    "protonmail-export"
    "protonplus"
    "protontricks"
    "protonup-ng"
    "protonup-qt"
    "protonup-rs"
    "protonvpn-gui"
    "protorpc"
    "protoscope"
    "prototypejs"
    "protox"
    "protozero"
    "prover9"
    "proverif"
    "proverifWithGui"
    "prow"
    "prowlarr"
    "prowler"
    "prox"
    "proxelar"
    "proxify"
    "proximity-sort"
    "proxmark3"
    "proxmove"
    "proxmox-auto-install-assistant"
    "proxmox-backup-client"
    "proxsuite"
    "proxyauth"
    "proxychains"
    "proxychains-ng"
    "proxyman"
    "proxypin"
    "proxysql"
    "proxyt"
    "proxytunnel"
    "proycon-wayout"
    "prpll"
    "prqlc"
    "prr"
    "prrte"
    "prs"
    "prst"
    "pru"
    "pruneLibtoolFiles"
    "prusa-slicer"
    "pry"
    "ps"
    "ps2client"
    "ps2eps"
    "ps2patchelf"
    "ps3-disc-dumper"
    "ps3iso-utils"
    "ps3netsrv"
    "ps_mem"
    "psb_status"
    "psc-package"
    "pscale"
    "pscircle"
    "psftools"
    "pshash"
    "pshs"
    "psi-notify"
    "psi-plus"
    "psitop"
    "psitransfer"
    "psleep"
    "pslib"
    "psmisc"
    "psol"
    "pspg"
    "pspp"
    "pspy"
    "psql2csv"
    "psqlodbc"
    "psrecord"
    "pssh"
    "psst"
    "pstoedit"
    "pstreams"
    "pstree"
    "psudohash"
    "psutils"
    "psw"
    "psysh"
    "pt"
    "pt2-clone"
    "ptags"
    "ptcollab"
    "ptcpdump"
    "pterm"
    "ptex"
    "ptext"
    "pth"
    "ptouch-driver"
    "ptouch-print"
    "ptunnel"
    "ptyxis"
    "public-inbox"
    "public-sans"
    "publicsuffix-list"
    "publii"
    "pubs"
    "puddletag"
    "pueue"
    "pufferpanel"
    "puffin"
    "pugixml"
    "pugl"
    "pulldown-cmark"
    "pullomatic"
    "pulsar"
    "pulsarctl"
    "pulse-visualizer"
    "pulseaudio"
    "pulseaudio-ctl"
    "pulseaudio-dlna"
    "pulseaudio-module-xrdp"
    "pulseaudioFull"
    "pulseeffects-legacy"
    "pulsemeeter"
    "pulsemixer"
    "pulseview"
    "pulumi"
    "pulumi-bin"
    "pulumi-esc"
    "pulumictl"
    "pumpkin"
    "puncia"
    "punes"
    "punes-qt6"
    "pup"
    "pupdate"
    "puppet"
    "puppet-bolt"
    "puppet-lint"
    "puppeteer-cli"
    "pur"
    "pure-ftpd"
    "pure-maps"
    "pure-prompt"
    "puredata"
    "puredns"
    "puremapping"
    "purenix"
    "pureref"
    "purescm"
    "purescript"
    "purpur"
    "pushup"
    "putter"
    "putty"
    "putzen"
    "pv"
    "pv-migrate"
    "pve-tool"
    "pvetui"
    "pvs-studio"
    "pvsneslib"
    "pvz-portable"
    "pvz-portable-unwrapped"
    "pvzge"
    "pw-viz"
    "pw-volume"
    "pw3270"
    "pwalarmctl"
    "pwalarmd"
    "pwdsafety"
    "pwdsphinx"
    "pwgen"
    "pwgen-secure"
    "pwmenu"
    "pwnat"
    "pwncat"
    "pwneye"
    "pwninit"
    "pwntools"
    "pwru"
    "pws"
    "pwsafe"
    "pwvucontrol"
    "pxa-mkbootimg"
    "pxattr"
    "pxder"
    "pxz"
    "py-spy"
    "py-wacz"
    "py3c"
    "py3dtiles"
    "py65"
    "py7zr"
    "pyamlboot"
    "pybibget"
    "pybugz"
    "pycdc"
    "pycflow2dot"
    "pychess"
    "pycobertura"
    "pycoin"
    "pycritty"
    "pydeps"
    "pydf"
    "pyditz"
    "pyenv"
    "pyfa"
    "pyflyby"
    "pyglossary"
    "pyglossary-gui"
    "pygpoabuse"
    "pyhanko-cli"
    "pyinfra"
    "pykickstart"
    "pykms"
    "pyldapsearch"
    "pylint"
    "pylode"
    "pylyzer"
    "pymcuprog"
    "pymol"
    "pympress"
    "pynitrokey"
    "pyocd"
    "pyp"
    "pypass"
    "pypi-mirror"
    "pypiserver"
    "pyprland"
    "pyprof2calltree"
    "pyprojectize"
    "pypy"
    "pypy2"
    "pypy27"
    "pypy3"
    "pypy310"
    "pypy311"
    "pyradio"
    "pyrefly"
    "pyright"
    "pyroscope"
    "pyrosimple"
    "pyrra"
    "pyrseas"
    "pyscan"
    "pysentation"
    "pysolfc"
    "pyspread"
    "pysqlrecon"
    "pystring"
    "pythia"
    "python-caja"
    "python-cosmopolitan"
    "python-launcher"
    "python-matter-server"
    "python3"
    "python311"
    "python312"
    "python313"
    "python313FreeThreading"
    "python314"
    "python314FreeThreading"
    "python315"
    "python315FreeThreading"
    "python3Minimal"
    "pytr"
    "pytrainer"
    "pyupgrade"
    "pywal"
    "pywal16"
    "pywalfox-native"
    "pywhisker"
    "pyxel"
    "pyznap"
    "pyzy"
    "q"
    "q-text-as-data"
    "q4wine"
    "q6voiced"
    "qFlipper"
    "qMasterPassword"
    "qMasterPassword-wayland"
    "qadwaitadecorations"
    "qadwaitadecorations-qt6"
    "qalculate-gtk"
    "qalculate-qt"
    "qarte"
    "qaseprite"
    "qastools"
    "qbe"
    "qbec"
    "qbit-manage"
    "qbittorrent"
    "qbittorrent-cli"
    "qbittorrent-enhanced"
    "qbittorrent-enhanced-nox"
    "qbittorrent-nox"
    "qbom"
    "qboot"
    "qbootctl"
    "qbs"
    "qbz"
    "qc"
    "qcad"
    "qcal"
    "qcard"
    "qcdnum"
    "qcm"
    "qcom-diag"
    "qcomicbook"
    "qcsuper"
    "qcsxcad"
    "qctools"
    "qdelay"
    "qdigidoc"
    "qdirstat"
    "qdiskinfo"
    "qdjango"
    "qdl"
    "qdldl"
    "qdmr"
    "qdng"
    "qdocumentview"
    "qdrant"
    "qdrant-web-ui"
    "qelectrotech"
    "qemacs"
    "qemu"
    "qemu-python-utils"
    "qemu-user"
    "qemu-utils"
    "qemu_full"
    "qemu_kvm"
    "qemu_test"
    "qemu_xen"
    "qgis"
    "qgis-ltr"
    "qgit"
    "qgnomeplatform"
    "qgnomeplatform-qt6"
    "qgo"
    "qgrep"
    "qgroundcontrol"
    "qgv"
    "qhexedit2"
    "qhull"
    "qidi-slicer-bin"
    "qidi-studio"
    "qiec104"
    "qimgv"
    "qir-runner"
    "qiv"
    "qjackctl"
    "qjournalctl"
    "qjoypad"
    "qlcplus"
    "qlementine"
    "qlementine-icons"
    "qlever"
    "qlever-control"
    "qlmarkdown"
    "qlog"
    "qm-dsp"
    "qmake2cmake"
    "qman"
    "qmapshack"
    "qmarkdowntextedit"
    "qmasterpassword"
    "qmasterpassword-wayland"
    "qmediathekview"
    "qmenumodel"
    "qmic"
    "qmidiarp"
    "qmidinet"
    "qmk"
    "qmk-udev-rules"
    "qmk_hid"
    "qmmp"
    "qmplay2"
    "qmplay2-qt5"
    "qmplay2-qt6"
    "qmqtt"
    "qnotero"
    "qobuz-player"
    "qodem"
    "qogir-icon-theme"
    "qogir-kde"
    "qogir-theme"
    "qoi"
    "qolibri"
    "qosmic"
    "qovery-cli"
    "qownnotes"
    "qownnotes-tui"
    "qpaeq"
    "qpdf"
    "qpdfview"
    "qperf"
    "qpid-cpp"
    "qpoases"
    "qprint"
    "qpwgraph"
    "qq"
    "qqmusic"
    "qqwing"
    "qr-backup"
    "qradiolink"
    "qrc"
    "qrcode"
    "qrcodegen"
    "qrcodegencpp"
    "qrcp"
    "qremotecontrol-server"
    "qrencode"
    "qrq"
    "qrrs"
    "qrtool"
    "qrtr"
    "qrupdate"
    "qsampler"
    "qscreenshot"
    "qspeakers"
    "qsreplace"
    "qsstv"
    "qstopmotion"
    "qsudo"
    "qsv"
    "qsvlite"
    "qsynth"
    "qt-box-editor"
    "qtads"
    "qtalarm"
    "qtappinstancemanager"
    "qtbitcointrader"
    "qtcreator"
    "qtemu"
    "qtengine"
    "qtgreet"
    "qtikz"
    "qtorganizer-mkcal"
    "qtox"
    "qtpass"
    "qtractor"
    "qtrvsim"
    "qtscrcpy"
    "qtspim"
    "qtwebapp"
    "qtwirediff"
    "quad-morph-filter"
    "quadprogpp"
    "quadrafuzz"
    "quadrapassel"
    "quake-injector"
    "quake3arena"
    "quake3arena-hires"
    "quake3arenadata"
    "quake3demo"
    "quake3demo-hires"
    "quake3demodata"
    "quake3e"
    "quake3hires"
    "quake3pointrelease"
    "quakespasm"
    "qualisys-cpp-sdk"
    "quantframe"
    "quantlib"
    "quantomatic"
    "quantum-espresso"
    "quantumminigolf"
    "quark"
    "quark-engine"
    "quark-goldleaf"
    "quarkdown"
    "quarkus"
    "quarto"
    "quartoMinimal"
    "quartus-prime-lite"
    "quartz-wm"
    "quassel"
    "quasselClient"
    "quasselDaemon"
    "quast"
    "quaternion"
    "quaternion-qt6"
    "quattrocento"
    "quattrocento-sans"
    "quba"
    "qucs-s"
    "qucsator-rf"
    "quesoglc"
    "questdb"
    "qui"
    "quich"
    "quick-lint-js"
    "quick-lookup"
    "quick-sasl"
    "quick-webapps"
    "quickder"
    "quickemu"
    "quickfix"
    "quickgui"
    "quickjs"
    "quickjs-ng"
    "quickmem"
    "quicksand"
    "quickshell"
    "quicktemplate"
    "quicktun"
    "quicktype"
    "quickwit"
    "quiet"
    "quill"
    "quill-log"
    "quill-qr"
    "quilt"
    "quint"
    "quintom-cursor-theme"
    "quinze"
    "quirc"
    "quisk"
    "quivira"
    "quodlibet"
    "quodlibet-full"
    "quodlibet-without-gst-plugins"
    "quodlibet-xine"
    "quodlibet-xine-full"
    "quota"
    "qusb2snes"
    "qutebrowser"
    "qutebrowser-qt5"
    "qvge"
    "qview"
    "qwen-code"
    "qwertone"
    "qwerty-fr"
    "qxmledit"
    "qxmpp"
    "qzdl"
    "r10k"
    "r2mod_cli"
    "r2modman"
    "r3ctl"
    "r53-ddns"
    "rHttp"
    "rWrapper"
    "ra-multiplex"
    "rabbit-ng"
    "rabbitmq-c"
    "rabbitmq-server"
    "rabbitmqadmin-ng"
    "rabtap"
    "raccoin"
    "raccoon-scanner"
    "racer"
    "racket"
    "racket-minimal"
    "rad-key"
    "radamsa"
    "radare2"
    "radarr"
    "radcli"
    "radeon-profile"
    "radeontools"
    "radeontop"
    "radian"
    "radianWrapper"
    "radiance-vj"
    "radicale"
    "radicle-artifact"
    "radicle-ci-broker"
    "radicle-desktop"
    "radicle-explorer"
    "radicle-httpd"
    "radicle-job"
    "radicle-native-ci"
    "radicle-node"
    "radicle-node-unstable"
    "radicle-tui"
    "radio-active"
    "radio-cli"
    "radioboat"
    "radiotray-ng"
    "radsecproxy"
    "radvd"
    "raffi"
    "raft-cowsql"
    "rage"
    "ragel"
    "ragelDev"
    "ragelStable"
    "ragenix"
    "rahasher"
    "raider"
    "rails-new"
    "railway"
    "railway-wallet"
    "rain"
    "rain-bittorrent"
    "rainbowcrack"
    "rainbowstream"
    "raindrop"
    "rainfrog"
    "rainloop-community"
    "rainloop-standard"
    "raiseorlaunch"
    "rakarrack"
    "rake"
    "rakkess"
    "rakudo"
    "raleway"
    "ralphex"
    "ramalama"
    "rambox"
    "ramfetch"
    "ramus"
    "ran"
    "rana"
    "rancher"
    "rancid"
    "randomx"
    "randoop"
    "randtype"
    "rang"
    "range-v3"
    "ranger"
    "rankwidth"
    "rapid-photo-downloader"
    "rapidapi"
    "rapidcheck"
    "rapidcsv"
    "rapidfuzz-cpp"
    "rapidgzip"
    "rapidjson"
    "rapidraw"
    "rapidxml"
    "rapidyaml"
    "rappel"
    "rar"
    "rar2hashcat"
    "rarcrack"
    "rare"
    "rare-regex"
    "rarezip"
    "rars"
    "rascal"
    "rasdaemon"
    "rasm"
    "raspa"
    "raspa-data"
    "raspberrypi-armstubs"
    "raspberrypi-eeprom"
    "raspberrypiWirelessFirmware"
    "raspberrypifw"
    "raspi-utils"
    "rassumfrassum"
    "rastertoezpl"
    "rat-king-adventure"
    "ratarmount"
    "ratatouille-lv2"
    "ratchet"
    "rates"
    "rathole"
    "ratman"
    "ratmen"
    "ratools"
    "ratox"
    "ratpoints"
    "ratpoison"
    "ratslap"
    "ratspeak"
    "ratt"
    "rattler-build"
    "ratty"
    "rauc"
    "raul"
    "rauthy"
    "rav1d"
    "rav1e"
    "ravedude"
    "raven-reader"
    "rawst"
    "rawtherapee"
    "raxml"
    "raxml-mpi"
    "raycast"
    "raygui"
    "raylib"
    "raylib-games"
    "raysession"
    "raze"
    "razer-cli"
    "razergenie"
    "rbdoom-3-bfg"
    "rbenv"
    "rblake2sum"
    "rblake3sum"
    "rbspy"
    "rbw"
    "rc"
    "rc-9front"
    "rclip"
    "rclone"
    "rclone-browser"
    "rclone-ui"
    "rcm"
    "rcodesign"
    "rcon"
    "rcon-cli"
    "rconc"
    "rcp"
    "rcs"
    "rcshist"
    "rcu"
    "rdap"
    "rdbtools"
    "rdedup"
    "rdesktop"
    "rdfind"
    "rdiff-backup"
    "rdkafka"
    "rdma-core"
    "rdocker"
    "rdpgw"
    "rdrview"
    "rdup"
    "rdwatool"
    "re-appintentsmetadataprocessor"
    "re-derq"
    "re-flex"
    "re-intentbuilderc"
    "re-isearch"
    "re-plistbuddy"
    "re2"
    "re2c"
    "reactguard"
    "reaction"
    "reactphysics3d"
    "read-edid"
    "read-it-later"
    "readability-cli"
    "readability-extractor"
    "readarr"
    "readeck"
    "reader"
    "readerwriterqueue"
    "readest"
    "readexe"
    "readexpro"
    "readline"
    "readline70"
    "readmdict"
    "readosm"
    "readsb"
    "readstat"
    "ready-check"
    "real_time_config_quick_scan"
    "realcugan-ncnn-vulkan"
    "realesrgan-ncnn-vulkan"
    "realm"
    "realm-studio"
    "realmd"
    "realvnc-vnc-viewer"
    "reap"
    "reaper"
    "reaper-go"
    "reaper-reapack-extension"
    "reaper-sws-extension"
    "reason"
    "reason-shell"
    "reattach-to-user-namespace"
    "reaverwps"
    "reaverwps-t6x"
    "rebar"
    "rebar3"
    "rebels-in-the-sky"
    "rebuilderd"
    "recaf-launcher"
    "recapp"
    "recastnavigation"
    "reckon"
    "reco"
    "recode"
    "recoll"
    "recoll-kio"
    "recoll-krunner"
    "recoll-nox"
    "recon"
    "recon-ng"
    "recordbox"
    "recoverdm"
    "recoverjpeg"
    "rectangle"
    "rectangle-pro"
    "recursive"
    "recutils"
    "recyclarr"
    "red"
    "red-trul"
    "redact"
    "reddit-tui"
    "reddix"
    "reddsaver"
    "redeclipse"
    "redfang"
    "redfishtool"
    "redhat-official-fonts"
    "redir"
    "redis"
    "redis-dump"
    "redis-dump-go"
    "redis-plus-plus"
    "redisTestHook"
    "redisinsight"
    "redka"
    "redland"
    "redland-wayland"
    "redli"
    "redlib"
    "redlist"
    "redmine"
    "rednotebook"
    "redo"
    "redo-apenwarr"
    "redo-c"
    "redo-sh"
    "redocly"
    "redoflacs"
    "redpanda-client"
    "redshift"
    "redshift_jdbc"
    "redsocks"
    "redstore"
    "redu"
    "redumper"
    "redux"
    "redwax-tool"
    "reef"
    "reevr"
    "refind"
    "refine"
    "refinery-cli"
    "reflection"
    "reflection-cpp"
    "reflex"
    "reflex-app"
    "refmt"
    "reframe"
    "reftools"
    "refurb"
    "reg"
    "regal"
    "regbot"
    "regclient"
    "regctl"
    "regenkfs"
    "regex-cli"
    "regex-tui"
    "regex2json"
    "regexplain"
    "regexploit"
    "regextester"
    "regina"
    "regionset"
    "regname"
    "rego-query"
    "regolith"
    "regols"
    "regpg"
    "regreet"
    "regripper"
    "regrippy"
    "regsync"
    "rehex"
    "reindeer"
    "reiser4progs"
    "rekor-cli"
    "rekor-server"
    "release-plz"
    "relic"
    "remake"
    "remark42"
    "remarkable-mouse"
    "remarkable-toolchain"
    "remarkable2-toolchain"
    "remarshal"
    "rembg"
    "remctl"
    "remind"
    "remkrom"
    "remmina"
    "remnote"
    "remod"
    "remontoire"
    "remote-exec"
    "remote-touchpad"
    "removeReferencesTo"
    "ren-find"
    "rename"
    "renameutils"
    "render-cli"
    "render50"
    "rendercv"
    "renderdoc"
    "reno"
    "renode"
    "renode-bin"
    "renode-dts2repl"
    "renode-unstable"
    "renode-unstable-bin"
    "renoise"
    "renovate"
    "renpy"
    "renpyMinimal"
    "rep"
    "rep-grep"
    "repak"
    "repath-studio"
    "repgrep"
    "replace"
    "replace-secret"
    "replacement"
    "replay-io"
    "replay-node-cli"
    "replibyte"
    "replxx"
    "repocheck"
    "repomix"
    "reposilite"
    "reposurgeon"
    "repren"
    "reprepro"
    "repro-env"
    "repro-get"
    "reproc"
    "reproxy"
    "repseek"
    "reptor"
    "reptyr"
    "reqable"
    "requestly"
    "reredirect"
    "rerun"
    "rescript-language-server"
    "rescrobbled"
    "rescuetime"
    "resetti"
    "resgate"
    "reshape"
    "resholve"
    "resilio-sync"
    "resnap"
    "resolv_wrapper"
    "resolve-march-native"
    "resonance"
    "resonarium"
    "resorter"
    "resources"
    "responder"
    "restate"
    "resterm"
    "restic"
    "restic-browser"
    "restic-integrity"
    "restic-rest-server"
    "resticprofile"
    "restinio"
    "restique"
    "restish"
    "restls"
    "restman"
    "restool"
    "restream"
    "resumed"
    "resvg"
    "retdec"
    "retext"
    "reth"
    "rethinkdb"
    "reticulated"
    "reticulum-go"
    "reticulum-group-chat"
    "retool"
    "retro-gtk"
    "retroarch"
    "retroarch-assets"
    "retroarch-bare"
    "retroarch-free"
    "retroarch-full"
    "retroarch-joypad-autoconfig"
    "retrofe"
    "retroshare"
    "retrospy"
    "retry"
    "reuse"
    "revanced-cli"
    "revanity-go"
    "reveal-md"
    "reversal-icon-theme"
    "reviewdog"
    "revive"
    "revolt-desktop"
    "revolver"
    "revpfw3"
    "revup"
    "rewaita"
    "rewatch"
    "rewritefs"
    "rewrk"
    "rex"
    "rexi"
    "rfc"
    "rfc-bibtex"
    "rfc-reader"
    "rfdump"
    "rfkill_udev"
    "rgbds"
    "rgl"
    "rgp"
    "rgrc"
    "rgx"
    "rgxg"
    "rhack"
    "rhai-doc"
    "rhash"
    "rhino"
    "rhoas"
    "rhodium-libre"
    "rhsrvany"
    "rhttp"
    "rhuffle"
    "rhvoice"
    "rhythmbox"
    "rich-cli"
    "richgo"
    "ricochet-refresh"
    "ricty"
    "rictydiminished-with-firacode"
    "ride"
    "riemann"
    "riemann-dash"
    "riemann-tools"
    "riemann_c_client"
    "ries"
    "riff"
    "riffdiff"
    "rifiuti"
    "rift-wm"
    "rig"
    "rigel-engine"
    "rigsofrods-bin"
    "rime-cli"
    "rime-data"
    "rime-ice"
    "rime-japanese"
    "rime-latex"
    "rime-ls"
    "rime-moegirl"
    "rime-wanxiang"
    "rime-zhwiki"
    "rimgo"
    "rimsort"
    "rinetd"
    "rinf_cli"
    "ringboard"
    "ringboard-wayland"
    "ringfairy"
    "ringing-lib"
    "ringracers"
    "rink"
    "rinutils"
    "rio"
    "riot-redis"
    "rip2"
    "ripasso-cursive"
    "ripcord"
    "ripdrag"
    "ripe-atlas-tools"
    "ripes"
    "ripgrep"
    "ripgrep-all"
    "ripmime"
    "rippkgs"
    "ripplerx"
    "ripsecrets"
    "ripser"
    "ripunzip"
    "riscv-pk"
    "riseup-vpn"
    "risor"
    "ristate"
    "ristretto"
    "rita"
    "rivalcfg"
    "river"
    "river-bedload"
    "river-bnf"
    "river-bsp-layout"
    "river-classic"
    "river-filtile"
    "river-luatile"
    "river-tag-overlay"
    "river-ultitile"
    "rivercarro"
    "rivet"
    "rizin"
    "rkbin"
    "rkboot"
    "rkdeveloptool"
    "rkdeveloptool-pine64"
    "rke"
    "rke2"
    "rke2_1_33"
    "rke2_1_34"
    "rke2_1_35"
    "rke2_1_36"
    "rke2_latest"
    "rke2_stable"
    "rkflashtool"
    "rkik"
    "rkmon"
    "rkpd2"
    "rkrlv2"
    "rkvm"
    "rlama"
    "rlaunch"
    "rlci"
    "rlog"
    "rlottie"
    "rlwrap"
    "rm-improved"
    "rman"
    "rmapi"
    "rmate"
    "rmate-sh"
    "rmatrix"
    "rmenu"
    "rmfakecloud"
    "rmfuse"
    "rmg"
    "rmg-wayland"
    "rml"
    "rmlint"
    "rmount"
    "rmpc"
    "rmtfs"
    "rmtrash"
    "rmux"
    "rmview"
    "rmw"
    "rnd-name"
    "rng-tools"
    "rnginline"
    "rnm"
    "rnmon"
    "rnnoise"
    "rnnoise-plugin"
    "rnote"
    "rnp"
    "rnr"
    "rns"
    "rns-proxy"
    "rnsapi"
    "rnv"
    "rnxcmp"
    "roadrunner"
    "roadtools"
    "roapi-http"
    "roave-backward-compatibility-check"
    "robin-map"
    "robo"
    "robo3t"
    "robodoc"
    "robotfindskitten"
    "robotframework-tidy"
    "roboto"
    "roboto-flex"
    "roboto-mono"
    "roboto-serif"
    "roboto-slab"
    "robustirc-bridge"
    "roc"
    "roc-toolkit"
    "roccat-tools"
    "rockbox-utility"
    "rocket"
    "rocketchat-desktop"
    "rocksdb"
    "rocksdb_6_23"
    "rocksdb_7_10"
    "rocksdb_8_11"
    "rocksdb_8_3"
    "rocksdb_9_10"
    "rocksndiamonds"
    "rockstarlang"
    "rockyou"
    "rocq-core"
    "rocq-core_9_0"
    "rocq-core_9_1"
    "rocq-core_9_2"
    "rocq-core_9_3"
    "rod"
    "roddhjav-apparmor-rules"
    "rode-central"
    "rofi"
    "rofi-blezz"
    "rofi-bluetooth"
    "rofi-calc"
    "rofi-emoji"
    "rofi-file-browser"
    "rofi-games"
    "rofi-menugen"
    "rofi-mpd"
    "rofi-nerdy"
    "rofi-network-manager"
    "rofi-obsidian"
    "rofi-power-menu"
    "rofi-pulse-select"
    "rofi-rbw"
    "rofi-rbw-wayland"
    "rofi-rbw-x11"
    "rofi-screenshot"
    "rofi-systemd"
    "rofi-top"
    "rofi-unwrapped"
    "rofi-vpn"
    "rofimoji"
    "rogcat"
    "rogue"
    "rojo"
    "rolespec"
    "rolldown"
    "rollo-printer"
    "romm"
    "ron-lsp"
    "ronin"
    "ronn"
    "roogle"
    "roomarranger"
    "roomeqwizard"
    "roon-bridge"
    "roon-server"
    "roon-tui"
    "root"
    "rootbar"
    "rootlesskit"
    "ropebwt2"
    "ropgadget"
    "ropr"
    "rops"
    "rosa"
    "rose-pine-cursor"
    "rose-pine-hyprcursor"
    "rose-pine-icon-theme"
    "rose-pine-kvantum"
    "rosec"
    "rosec-bitwarden-pm"
    "rosec-bitwarden-sm"
    "rosec-gnome-keyring"
    "rosecFull"
    "rosegarden"
    "rosenpass"
    "rosenpass-tools"
    "rosie"
    "roslyn"
    "roslyn-ls"
    "rospo"
    "roswell"
    "rot8"
    "rotate-backups"
    "rotonda"
    "rotp"
    "round"
    "roundcube"
    "rounded-mgenplus"
    "roundup"
    "route-detect"
    "route-graph"
    "route159"
    "routedns"
    "router"
    "routersploit"
    "routinator"
    "routino"
    "rover"
    "rovium"
    "rowhammer-test"
    "rox-filer"
    "roxctl"
    "roxterm"
    "rp"
    "rp-pppoe"
    "rpPPPoE"
    "rpatool"
    "rpcbind"
    "rpcemu"
    "rpclib"
    "rpcs3"
    "rpcsvc-proto"
    "rpg-cli"
    "rpi-imager"
    "rpiboot"
    "rpiplay"
    "rpl"
    "rply"
    "rpm"
    "rpm-ostree"
    "rpm-sequoia"
    "rpm2targz"
    "rpmextract"
    "rq"
    "rqbit"
    "rqlite"
    "rquickshare"
    "rr"
    "rrdtool"
    "rrootage"
    "rrsync"
    "rs"
    "rs-git-fsmonitor"
    "rs-lxmf"
    "rs-reticulum"
    "rs-tftpd"
    "rsass"
    "rsbep"
    "rsbkb"
    "rsclock"
    "rscw"
    "rset"
    "rsgain"
    "rshell"
    "rshijack"
    "rshim-user-space"
    "rsibreak"
    "rsign2"
    "rslint"
    "rsmangler"
    "rsnapshot"
    "rsonpath"
    "rsop"
    "rspamd"
    "rspamd-trainer"
    "rsrpc"
    "rss-bridge"
    "rss-bridge-cli"
    "rss2email"
    "rssguard"
    "rsshub"
    "rsstail"
    "rsstail-py"
    "rst2html5"
    "rst2pdf"
    "rstcheck"
    "rstcheckWithSphinx"
    "rstfmt"
    "rstudio"
    "rstudio-server"
    "rstudioServerWrapper"
    "rstudioWrapper"
    "rsync"
    "rsyncy"
    "rsyslog"
    "rsyslog-light"
    "rt"
    "rt-tests"
    "rt5677-firmware"
    "rtabmap"
    "rtags"
    "rtaudio"
    "rtaudio_5"
    "rtaudio_6"
    "rtfm"
    "rtg-tools"
    "rtk"
    "rtkit"
    "rtklib-ex"
    "rtl-ais"
    "rtl-sdr"
    "rtl-sdr-blog"
    "rtl-sdr-librtlsdr"
    "rtl-sdr-osmocom"
    "rtl8192su-firmware"
    "rtl8761b-firmware"
    "rtl_433"
    "rtl_fm_streamer"
    "rtl_wmbus"
    "rtlamr"
    "rtlcss"
    "rtmidi"
    "rtmpdump"
    "rtmpdump_gnutls"
    "rtop"
    "rtorrent"
    "rtptools"
    "rtrlib"
    "rtrtr"
    "rtss"
    "rttr"
    "rtz"
    "ruapu"
    "rubber"
    "rubberband"
    "rubik"
    "rubiks"
    "rubocop"
    "ruby"
    "ruby-lsp"
    "ruby_3_3"
    "ruby_3_4"
    "ruby_3_5"
    "ruby_4_0"
    "rubyfmt"
    "rubyripper"
    "rucio"
    "rucola"
    "rucredstash"
    "ruff"
    "ruffle"
    "rufin"
    "rufo"
    "ruler"
    "rumdl"
    "rumno"
    "rumqttd"
    "run"
    "run-npush"
    "run0-sudo-shim"
    "runapp"
    "runc"
    "rund"
    "rundeck"
    "rundeck-cli"
    "rune"
    "rune-languageserver"
    "runelite"
    "runiq"
    "runit"
    "runitor"
    "runmd"
    "runme"
    "runn"
    "runningx"
    "runoff"
    "runpodctl"
    "runtimeShellPackage"
    "runzip"
    "ruplacer"
    "ruqola"
    "rura"
    "rure"
    "ruri"
    "rush"
    "rush-lyrics"
    "rush-parallel"
    "russ"
    "rust-addr2line"
    "rust-analyzer"
    "rust-analyzer-unwrapped"
    "rust-audit-info"
    "rust-bindgen"
    "rust-bindgen-unwrapped"
    "rust-cbindgen"
    "rust-code-analysis"
    "rust-federation-tester"
    "rust-jemalloc-sys"
    "rust-jemalloc-sys-unprefixed"
    "rust-motd"
    "rust-paddle-ocr"
    "rust-parallel"
    "rust-petname"
    "rust-rpxy"
    "rust-script"
    "rust-stakeholder"
    "rust-streamdeck"
    "rust-synapse-compress-state"
    "rust-traverse"
    "rustc"
    "rustc-demangle"
    "rustc-unwrapped"
    "rustcast"
    "rustcat"
    "rustdesk"
    "rustdesk-flutter"
    "rustdesk-server"
    "rustdress"
    "rustfetch"
    "rustfinity"
    "rustfmt"
    "rustfs"
    "rusthound-ce"
    "rusti-cal"
    "rustic"
    "rustical"
    "rustlens"
    "rustlings"
    "rustls-ffi"
    "rustls-libssl"
    "rustmission"
    "rustnet"
    "rustplayer"
    "rustpython"
    "rustscan"
    "rustup"
    "rustup-toolchain-install-master"
    "rustus"
    "rusty-bash"
    "rusty-diceware"
    "rusty-man"
    "rusty-path-of-building"
    "rusty-psn"
    "rusty-psn-gui"
    "rustycli"
    "rustypaste"
    "rustypaste-cli"
    "rustywind"
    "rutabaga_gfx"
    "rutorrent"
    "ruwudu"
    "rux"
    "rvgl-launcher"
    "rvvm"
    "rvz"
    "rw"
    "rwalk"
    "rwc"
    "rwedid"
    "rwpspread"
    "rxp"
    "rxvt"
    "rxvt-unicode"
    "rxvt-unicode-emoji"
    "rxvt-unicode-unwrapped"
    "rxvt-unicode-unwrapped-emoji"
    "rye"
    "rygel"
    "rymdport"
    "ryokucha"
    "ryubing"
    "ryzen-monitor-ng"
    "ryzenadj"
    "rzip"
    "rzls"
    "s-search"
    "s-tar"
    "s-tui"
    "s0ix-selftest-tool"
    "s2geometry"
    "s2n-tls"
    "s2png"
    "s3-credentials"
    "s3backer"
    "s3bro"
    "s3cmd"
    "s3fs"
    "s3proxy"
    "s3ql"
    "s3rs"
    "s3scanner"
    "s4cmd"
    "s5"
    "s5cmd"
    "s6"
    "s6-dns"
    "s6-linux-init"
    "s6-linux-utils"
    "s6-man-pages"
    "s6-networking"
    "s6-networking-man-pages"
    "s6-portable-utils"
    "s6-portable-utils-man-pages"
    "s6-rc"
    "s6-rc-man-pages"
    "s7"
    "s9fes"
    "saber"
    "sabiql"
    "sable"
    "sable-unwrapped"
    "sabnzbd"
    "sacad"
    "sacc"
    "sacd"
    "sacrifice"
    "sad"
    "safe"
    "safe-rm"
    "safecloset"
    "safecopy"
    "safeeyes"
    "safefile"
    "safestringlib"
    "safety-cli"
    "saga"
    "sage"
    "sageWithDoc"
    "sagetex"
    "sagittarius-scheme"
    "sagoin"
    "sahel-fonts"
    "sail"
    "sail-riscv"
    "sailfish-access-control"
    "sailsd"
    "sakura"
    "saldl"
    "saldo"
    "saleae-logic"
    "saleae-logic-2"
    "salmon"
    "salt"
    "salt-lint"
    "salut"
    "sam-ba"
    "saman_ttf"
    "samba"
    "samba4"
    "samba4Full"
    "sambaFull"
    "sambamba"
    "samblaster"
    "samdump2"
    "sameboy"
    "samfirm-js"
    "samim-fonts"
    "samira"
    "saml2aws"
    "samloader-rs"
    "sammler"
    "samplebrain"
    "sampler"
    "samplicator"
    "samplv1"
    "samply"
    "sampo"
    "sampradaya"
    "samrewritten"
    "samsung-unified-linux-driver"
    "samsung-unified-linux-driver_1_00_36"
    "samsung-unified-linux-driver_1_00_37"
    "samsung-unified-linux-driver_4_01_17"
    "samtools"
    "samurai"
    "sanctity"
    "sandbar"
    "sandbox-runtime"
    "sandhole"
    "sane-airscan"
    "sane-backends"
    "sane-frontends"
    "sanjuuni"
    "sanoid"
    "sapling"
    "sarabun-font"
    "sarasa-gothic"
    "sarif-converter"
    "sarif-fmt"
    "sarif-tools"
    "sasquatch"
    "sasquatch-v4be"
    "sass"
    "sassc"
    "sasutils"
    "sasview"
    "satallax"
    "satdump"
    "satellite"
    "satisfactorymodmanager"
    "satty"
    "satyrographos"
    "satysfi"
    "sauce-connect"
    "saucectl"
    "sauerbraten"
    "saunafs"
    "savepagenow"
    "savvycan"
    "saw-tools"
    "sawjap"
    "saxon"
    "saxon-he"
    "saxon_11-he"
    "saxon_12-he"
    "saxon_9-he"
    "saxonb"
    "saxonb_8_8"
    "saxonb_9_1"
    "sayonara"
    "sbagen"
    "sbarlua"
    "sbb-tui"
    "sbc"
    "sbcl"
    "sbcl_2_4_6"
    "sbcl_2_6_6"
    "sbcl_2_6_7"
    "sbctl"
    "sblast"
    "sblim-sfcc"
    "sbom-compliance-tool"
    "sbom-cve-check"
    "sbom-tool"
    "sbom-utility"
    "sbom2dot"
    "sbom4files"
    "sbom4python"
    "sbomit"
    "sbomnix"
    "sbs"
    "sbsigntool"
    "sbt"
    "sbt-extras"
    "sbt-with-scala-native"
    "sby"
    "sc"
    "sc-controller"
    "sc-im"
    "sc2-headless"
    "sc2-smurf-detector"
    "sc68"
    "sca2d"
    "scala"
    "scala-cli"
    "scala-next"
    "scala-runners"
    "scala-update"
    "scala_2_12"
    "scala_2_13"
    "scala_3"
    "scalafix"
    "scalafmt"
    "scalapack"
    "scalapack-ilp64"
    "scaleft"
    "scalene"
    "scaleway-cli"
    "scalingo"
    "scalp"
    "scalpel"
    "scalr-cli"
    "scan-build-py"
    "scanbd"
    "scanmem"
    "scanservjs"
    "scantailor-advanced"
    "scantailor-universal"
    "scantpaper"
    "scap-security-guide"
    "scarab"
    "scarlett2"
    "scc"
    "sccache"
    "sccmhunter"
    "scd2html"
    "scdl"
    "scdoc"
    "scenebuilder"
    "scenefx"
    "scenefx_0_5"
    "scenic-view"
    "scfbuild"
    "schedctl"
    "schedtool"
    "scheherazade"
    "scheherazade-new"
    "schema2ldif"
    "schemacrawler"
    "schemamap"
    "schemaspy"
    "schemat"
    "scheme-bytestructures"
    "scheme-manpages"
    "scheme48"
    "schemes"
    "schemesh"
    "schildi-revenge"
    "schismtracker"
    "schleuder"
    "schleuder-cli"
    "schroedinger"
    "schroot"
    "scid"
    "scid-vs-pc"
    "scientifica"
    "scilab-bin"
    "scilla"
    "scimark"
    "scion"
    "scion-apps"
    "scion-bootstrapper"
    "scip"
    "scip-go"
    "scipopt-gcg"
    "scipopt-papilo"
    "scipopt-scip"
    "scipopt-soplex"
    "scipopt-ug"
    "scipopt-zimpl"
    "scite"
    "scitokens-cpp"
    "scli"
    "scmccid"
    "scmpuff"
    "scmutils"
    "scom"
    "scons"
    "scooter"
    "scope-lite"
    "scope-tui"
    "scopehal-apps"
    "scorched3d"
    "scorecard"
    "scotch"
    "scotty"
    "scour"
    "scout"
    "scoutsuite"
    "scowl"
    "scponly"
    "scraper"
    "scrcpy"
    "scream"
    "screego"
    "screen"
    "screen-message"
    "screen-pipe"
    "screenconfig"
    "screendrop"
    "screenfetch"
    "screenkey"
    "screenly-cli"
    "screentest"
    "scribus"
    "script-directory"
    "scriptaculous"
    "scripthaus"
    "scriptisto"
    "scriv"
    "scroll-reverser"
    "scrolls"
    "scrot"
    "scrounge-ntfs"
    "scrub"
    "scrutiny"
    "scrutiny-collector"
    "scryer-prolog"
    "scrypt"
    "scs"
    "scsh"
    "scspell"
    "scss-lint"
    "sct"
    "scuba"
    "sculpin"
    "scummvm"
    "scx-loader"
    "sd"
    "sd-local"
    "sd-mux-ctrl"
    "sd-switch"
    "sdat2img"
    "sdate"
    "sdbus-cpp"
    "sdbus-cpp_2"
    "sdcc"
    "sdcv"
    "sddm-astronaut"
    "sddm-chili-theme"
    "sddm-sugar-dark"
    "sdformatlinux"
    "sdkmanager"
    "sdl-jstest"
    "sdl12-compat"
    "sdl2-compat"
    "sdl3"
    "sdl3-image"
    "sdl3-mixer"
    "sdl3-net"
    "sdl3-shadercross"
    "sdl3-ttf"
    "sdl_gamecontrollerdb"
    "sdlookup"
    "sdlpop"
    "sdparm"
    "sdr-j-fm"
    "sdrangel"
    "sdrplay"
    "sdrpp"
    "sea-orm-cli"
    "seabios"
    "seabios-coreboot"
    "seabios-csm"
    "seabios-qemu"
    "seabird"
    "seadrive-fuse"
    "seadrive-gui"
    "seafile-client"
    "seafile-shared"
    "seagoat"
    "seahorse"
    "sealcurses"
    "seamly2d"
    "seamonkey"
    "seanime"
    "search-vulns"
    "searchix"
    "searxng"
    "seashells"
    "seasocks"
    "seatd"
    "seaview"
    "seaweedfs"
    "sec"
    "seccure"
    "seclists"
    "seconlay"
    "secp256k1"
    "secp256k1-jdk"
    "secrethound"
    "secretive"
    "secrets-extractor"
    "secretscanner"
    "secretspec"
    "secretspec-ffi"
    "sedutil"
    "see"
    "see-cat"
    "seehecht"
    "seer"
    "seerr"
    "segger-jlink"
    "segger-jlink-headless"
    "segger-ozone"
    "seilfahrt"
    "selectdefaultapplication"
    "selendroid"
    "selene"
    "selenium-manager"
    "selenium-server-standalone"
    "selfci"
    "selfoss"
    "selinux-python"
    "selinux-refpolicy"
    "selinux-sandbox"
    "sem"
    "semantic-release"
    "semeru-bin"
    "semeru-bin-11"
    "semeru-bin-17"
    "semeru-bin-21"
    "semeru-bin-8"
    "semeru-jre-bin"
    "semeru-jre-bin-11"
    "semeru-jre-bin-17"
    "semeru-jre-bin-21"
    "semeru-jre-bin-8"
    "semgrep"
    "semgrep-core"
    "semodule-utils"
    "semtools"
    "semver"
    "semver-tool"
    "send"
    "sendgmail"
    "sendme"
    "sendspin-go"
    "senpai"
    "sensible-side-buttons"
    "sensible-utils"
    "sensu-go-agent"
    "sensu-go-backend"
    "sensu-go-cli"
    "sent"
    "sentencepiece"
    "sentry-cli"
    "sentry-native"
    "senv"
    "separateDebugInfo"
    "seppo"
    "september"
    "seq-cli"
    "seq66"
    "seqdiag"
    "seqkit"
    "seqtk"
    "sequeler"
    "sequelpro"
    "sequential"
    "sequin"
    "sequoia-chameleon-gnupg"
    "sequoia-git"
    "sequoia-sop"
    "sequoia-sq"
    "sequoia-sqop"
    "sequoia-sqv"
    "sequoia-wot"
    "ser2net"
    "serd"
    "seren"
    "serenityos-emoji-font"
    "serf"
    "serfdom"
    "serial-studio"
    "serialdv"
    "serie"
    "serigy"
    "serious-sam-classic"
    "serious-sam-classic-vulkan"
    "serpent"
    "serpl"
    "servarr-ffmpeg"
    "serve"
    "serve-d"
    "server-box"
    "serverpod_cli"
    "serverspec"
    "service-wrapper"
    "serviio"
    "servo"
    "sesh"
    "seshat"
    "session-desktop"
    "sessiond"
    "sessiond-uaccess"
    "sessreg"
    "setJavaClassPath"
    "setbfree"
    "setconf"
    "setools"
    "setroot"
    "setserial"
    "setup-envtest"
    "setupDebugInfoDirs"
    "seturgent"
    "setxkbmap"
    "setzer"
    "sev-snp-measure"
    "seventeenlands"
    "sewer"
    "sexp-cpp"
    "sexpp"
    "sextractor"
    "seyren"
    "sfcgal"
    "sfeed"
    "sfizz"
    "sfizz-ui"
    "sfm"
    "sfml"
    "sfml_2"
    "sfrotz"
    "sfsexp"
    "sftool"
    "sftool-gui"
    "sftpgo"
    "sftpgo-plugin-auth"
    "sftpman"
    "sfwbar"
    "sfxr"
    "sfxr-qt"
    "sfz"
    "sg-323"
    "sg3_utils"
    "sgdboop"
    "sgfutils"
    "sgp4"
    "sgt-puzzles"
    "sgt-puzzles-mobile"
    "sgx-azure-dcap-client"
    "sgx-psw"
    "sgxs-tools"
    "sh4d0wup"
    "sha1collisiondetection"
    "sha2wordlist"
    "shaarli"
    "shab"
    "shabnam-fonts"
    "shadcn"
    "shader-slang"
    "shaderbg"
    "shaderc"
    "shadershark"
    "shadow"
    "shadow-tls"
    "shadowenv"
    "shadowfox"
    "shadowsocks-libev"
    "shadowsocks-qtun-plugin"
    "shadowsocks-rust"
    "shadowsocks-v2ray-plugin"
    "shadps4"
    "shadps4-qtlauncher"
    "shairplay"
    "shairport-sync"
    "shairport-sync-airplay2"
    "shaka-packager"
    "shake"
    "shanggu-fonts"
    "shantell-sans"
    "shapelib"
    "shaperglot"
    "shaperglot-cli"
    "shaq"
    "shards"
    "share-preview"
    "shared-mime-info"
    "sharedown"
    "sharefinder"
    "sharing"
    "shark"
    "sharkey"
    "sharness"
    "sharpsat-td"
    "sharutils"
    "shattered-pixel-dungeon"
    "shavee"
    "shc"
    "sheldon"
    "shelfmark"
    "shell-genie"
    "shell-gpt"
    "shell2http"
    "shellcheck"
    "shellcheck-minimal"
    "shellcheck-sarif"
    "shellclear"
    "shelldap"
    "shellharden"
    "shellhub-agent"
    "shellify"
    "shellnoob"
    "shellshot"
    "shellspec"
    "shellz"
    "shelter"
    "shen-sbcl"
    "shepherd"
    "sherlock"
    "sherlock-launcher"
    "sherpa"
    "sherpa-onnx"
    "shfm"
    "shfmt"
    "shh"
    "shhmsg"
    "shhopt"
    "shibboleth-sp"
    "shikane"
    "shim-unsigned"
    "shine"
    "shiori"
    "shipments"
    "shipwright"
    "shira"
    "shiru"
    "shishi"
    "shittier"
    "shiv"
    "shmig"
    "shncpd"
    "shntool"
    "shocco"
    "shod"
    "shogihome"
    "shogun"
    "shoko"
    "shoko-webui"
    "shopify-cli"
    "shopify-themekit"
    "shopware-cli"
    "shore"
    "shoreline"
    "shorewall"
    "shortcat"
    "shortenPerlShebang"
    "shorter-pixel-dungeon"
    "shortscan"
    "shortwave"
    "shot-scraper"
    "shotcut"
    "shotgun"
    "shotman"
    "shottr"
    "shotwell"
    "shoutrrr"
    "show-midi"
    "showmethekey"
    "showoff"
    "showtime"
    "shpool"
    "shrikhand"
    "shrinkray"
    "shticker-book-unwritten"
    "shtk"
    "shtns"
    "shtris"
    "shuffledns"
    "shunit2"
    "shutter"
    "sic"
    "sic-image-cli"
    "sickgear"
    "sideband"
    "sidekick"
    "sidequest"
    "sideswap"
    "sidplayfp"
    "siege"
    "sienna"
    "siesta"
    "siesta-mpi"
    "sieve-connect"
    "sieve-editor-gui"
    "sieveshell"
    "sif"
    "sift"
    "sig"
    "sigal"
    "sigdigger"
    "sigi"
    "sigil"
    "siglo"
    "sigma-cli"
    "signal-backup-deduplicator"
    "signal-cli"
    "signal-desktop"
    "signal-export"
    "signalbackup-tools"
    "signaturepdf"
    "signify"
    "signing-party"
    "signls"
    "signwriting"
    "sigrok-cli"
    "sigrok-firmware-fx2lafw"
    "sigslot"
    "sigsum"
    "sigtop"
    "sigutils"
    "siji"
    "siketyan-ghr"
    "sil"
    "sil-abyssinica"
    "sil-padauk"
    "sil-q"
    "sile"
    "silenthound"
    "silice"
    "silicon"
    "sillytavern"
    "silo"
    "silver-platter"
    "silver-searcher-ng"
    "silverbullet"
    "silverbullet-cli"
    "silverfort-client"
    "silx"
    "simavr"
    "simbaplusplus"
    "simde"
    "simdjson"
    "simdutf"
    "simg2img"
    "simgear"
    "simgrid"
    "simh"
    "similarity"
    "simp1e-cursors"
    "simp_le"
    "simple-binary-encoding"
    "simple-completion-language-server"
    "simple-dftd3"
    "simple-dlna-browser"
    "simple-http-server"
    "simple-live-app"
    "simple-mtpfs"
    "simple-revision-control"
    "simple-scan"
    "simple-tpm-pk11"
    "simple64"
    "simple64-netplay-server"
    "simpleBluez"
    "simpleBuildTool"
    "simpleDBus"
    "simpleTpmPk11"
    "simplebluez"
    "simpledbus"
    "simplehttp2server"
    "simpleini"
    "simpleitk"
    "simplenes"
    "simplenote"
    "simpleproxy"
    "simplescreenrecorder"
    "simplex-chat-desktop"
    "simplotask"
    "simulide"
    "simulide_0_4_15"
    "simulide_1_0_0"
    "simulide_1_1_0"
    "simulide_1_2_0"
    "simutrans"
    "simutrans_binaries"
    "sing-box"
    "sing-box-dashboard"
    "sing-geoip"
    "sing-geosite"
    "single-file-cli"
    "singular"
    "singularity"
    "singularity-overriden-nixos"
    "sinit"
    "sink-rotate"
    "siomon"
    "sioyek"
    "sipcalc"
    "sipexer"
    "sipp"
    "sipsak"
    "sipvicious"
    "sirial"
    "sirikali"
    "siril"
    "sirit"
    "sirius"
    "sirula"
    "sish"
    "sispmctl"
    "sit"
    "sitelen-seli-kiwen"
    "sitespeed-io"
    "six-sines"
    "sixpair"
    "siyuan"
    "sizelint"
    "sjasmplus"
    "skaffold"
    "skalibs"
    "skalibs_2_10"
    "skate"
    "skeditor"
    "skeema"
    "sketchybar"
    "sketchybar-app-font"
    "skewrun"
    "skhd"
    "skia"
    "skia-aseprite"
    "skills"
    "skim"
    "skimpdf"
    "skippy-xd"
    "skjold"
    "skktools"
    "skopeo"
    "skribilo"
    "skrive"
    "skrooge"
    "sks"
    "skydns"
    "skyemu"
    "skypilot"
    "skyscraper"
    "skytemple"
    "skywalking-eyes"
    "sl"
    "sl1-to-photon"
    "sla2pdf"
    "slack"
    "slack-cli"
    "slack-term"
    "slackdump"
    "slacky"
    "slade"
    "slade-unstable"
    "sladeUnstable"
    "slang"
    "slang-server"
    "sledtool"
    "sleek"
    "sleek-grub-theme"
    "sleek-todo"
    "sleep-on-lan"
    "sleepwatcher"
    "slepc"
    "sleqp"
    "sleuthkit"
    "slibGuile"
    "slides"
    "slidev-cli"
    "slimevr"
    "slimevr-server"
    "slimserver"
    "slingshot"
    "slinktool"
    "slint-lsp"
    "slint-tr-extractor"
    "slint-viewer"
    "slippy"
    "slipshow"
    "slipstream"
    "slirp4netns"
    "slither-analyzer"
    "slob"
    "slobdict"
    "sloc"
    "slock"
    "slop"
    "sloth"
    "sloth-app"
    "slothy"
    "slowhttptest"
    "slowlorust"
    "slsa-verifier"
    "slsk-batchdl"
    "slskd"
    "slsnif"
    "slstatus"
    "slumber"
    "slurm"
    "slurm-nm"
    "slurm-spank-stunnel"
    "slurm-spank-x11"
    "slurp"
    "slurp-fediverse"
    "slushload"
    "slweb"
    "sly"
    "sm64baserom"
    "sm64coopdx"
    "sm64ex"
    "sm64tools"
    "smack"
    "smag"
    "small"
    "smallwm"
    "smap"
    "smart-wallpaper"
    "smartcat"
    "smartcrop"
    "smartdeblur"
    "smartdns"
    "smartgit"
    "smartmontools"
    "smartsynchronize"
    "smarty3"
    "smarty3-i18n"
    "smassh"
    "smatch"
    "smb3-foundry"
    "smbclient-ng"
    "smbcrawler"
    "smbmap"
    "smbscan"
    "smc"
    "smc-chilanka"
    "smc-fuzzer"
    "smc-manjari"
    "smcroute"
    "smeagol"
    "smem"
    "smemstat"
    "smenu"
    "smfh"
    "smile"
    "smiley-sans"
    "smimesign"
    "smithy-cli"
    "smithy-language-server"
    "smlfmt"
    "smlfut"
    "smlnj"
    "smlnj-legacy"
    "smlpkg"
    "smoked-salmon"
    "smokeping"
    "smooth"
    "smpeg"
    "smpeg2"
    "smplayer"
    "smpmgr"
    "smpq"
    "smproxy"
    "smtp-user-enum"
    "smtp4dev"
    "smtprelay"
    "smu"
    "smug"
    "smuview"
    "smux"
    "smuxi"
    "sn-pro"
    "sn0int"
    "snabb"
    "snac2"
    "snagboot"
    "snake4"
    "snakefmt"
    "snakemake"
    "snallygaster"
    "snap7"
    "snapcast"
    "snapmaker-luban"
    "snapper"
    "snapper-gui"
    "snappy"
    "snappy-topology"
    "snappymail"
    "snapraid"
    "snapshot"
    "snapweb"
    "snarkos"
    "snazy"
    "snd"
    "sndio"
    "sndpeek"
    "sneakerweb"
    "snekim"
    "snell"
    "snes9x"
    "snes9x-gtk"
    "snet"
    "sng"
    "sngrep"
    "snicat"
    "snid"
    "sniffglue"
    "sniffnet"
    "sniglet"
    "snip"
    "snipaste"
    "snipe-it"
    "snipes"
    "snippetexpander"
    "snippetexpanderd"
    "snippetexpandergui"
    "snippetexpanderx"
    "snippetpixie"
    "sniproxy"
    "snips-sh"
    "snipt"
    "snis"
    "snis-assets"
    "snis-unwrapped"
    "snitch"
    "snixembed"
    "snmalloc"
    "snmpcheck"
    "snmpen"
    "snobol4"
    "snoop"
    "snooze"
    "snore"
    "snort"
    "snouty"
    "snow"
    "snowcat"
    "snowcrash"
    "snowemu"
    "snowflake"
    "snowflake-cli"
    "snowmachine"
    "snowsql"
    "snpeff"
    "snpguest"
    "snphost"
    "snscrape"
    "snx-rs"
    "snyk"
    "snzip"
    "so"
    "soapui"
    "soapyairspy"
    "soapyaudio"
    "soapybladerf"
    "soapyhackrf"
    "soapyplutosdr"
    "soapyremote"
    "soapyrtlsdr"
    "soapysdr"
    "soapysdr-with-plugins"
    "soapysdrplay"
    "soapyuhd"
    "sobjectizer"
    "socalabs-papu"
    "socalabs-piano"
    "socalabs-rp2a03"
    "socalabs-sid"
    "socalabs-sn76489"
    "socalabs-voc"
    "socat"
    "soci"
    "social-engineer-toolkit"
    "socialscan"
    "sockdump"
    "socket-vmnet"
    "socket_wrapper"
    "socklog"
    "sockperf"
    "socks-to-http-proxy"
    "sockstat"
    "soco-cli"
    "sof-firmware"
    "sof-tools"
    "sofa"
    "sofia_sip"
    "soft-serve"
    "softether"
    "softflowd"
    "softhsm"
    "softmaker-office"
    "softmaker-office-nx"
    "softnet"
    "sogo"
    "soi"
    "soil"
    "sointu"
    "soju"
    "sokol"
    "sol2"
    "solaar"
    "solana-agave"
    "solana-cli"
    "solanum"
    "solargraph"
    "solarus"
    "solarus-launcher"
    "solarus-quest-editor"
    "solc"
    "solc-select"
    "solfege"
    "solicurses"
    "solidtime-desktop"
    "solitaire-tui"
    "sollya"
    "solo2-cli"
    "solo5"
    "solvespace"
    "somafm-cli"
    "somebar"
    "sommelier"
    "somo"
    "sonar"
    "sonar-scanner-cli"
    "sonar-scanner-cli-minimal"
    "sonarlint-ls"
    "sonarqube-cli"
    "sonarr"
    "sonata"
    "sone"
    "songrec"
    "sonic"
    "sonic-lineup"
    "sonic-pi"
    "sonic-server"
    "sonic-visualiser"
    "sonivox"
    "sonixd"
    "sonobuoy"
    "sonobus"
    "sonoscli"
    "sonota"
    "sony-dump"
    "sony-headphones-client"
    "sooperlooper"
    "sope"
    "sophus"
    "soplex"
    "sops"
    "sopwith"
    "soqt"
    "sorcer"
    "sord"
    "sorted-grep"
    "sortmerna"
    "sorts-mill-goudy"
    "sosreport"
    "soteria"
    "sou"
    "souffle"
    "sound-juicer"
    "sound-of-sorting"
    "sound-theme-freedesktop"
    "soundalike"
    "soundconverter"
    "soundfont-arachno"
    "soundfont-fluid"
    "soundfont-generaluser"
    "soundfont-generaluser-gs"
    "soundfont-ydp-grand"
    "soundsource"
    "soundtouch"
    "soundwireserver"
    "soupault"
    "source-code-pro"
    "source-han-code-jp"
    "source-han-mono"
    "source-han-sans"
    "source-han-sans-vf-otf"
    "source-han-sans-vf-ttf"
    "source-han-serif"
    "source-han-serif-vf-otf"
    "source-han-serif-vf-ttf"
    "source-meta-json-schema"
    "source-sans"
    "source-sans-pro"
    "source-serif"
    "source-serif-pro"
    "source2viewer-cli"
    "sourceHighlight"
    "sourcegit"
    "sourcekit-lsp"
    "sourcemapper"
    "sourcepawn-studio"
    "sourcery"
    "sourcey"
    "sov"
    "sowing"
    "sox"
    "sox_ng"
    "soxr"
    "soxt"
    "sozi"
    "sozu"
    "sp800-90b-entropyassessment"
    "space-cadet-pinball"
    "space-station-14-launcher"
    "spacebar"
    "spacecookie"
    "spacectl"
    "spacedrive"
    "spacenav-cube-example"
    "spacenavd"
    "spacer"
    "spaceship-prompt"
    "spacetimedb"
    "spacx-gtk-theme"
    "spade"
    "spades"
    "spaghettikart"
    "spago"
    "spago-legacy"
    "spamassassin"
    "span-lite"
    "spandsp"
    "spandsp3"
    "spark"
    "spark3"
    "spark4"
    "spark_3_4"
    "spark_3_5"
    "spark_4_0"
    "sparkle"
    "sparrow"
    "sparrow3d"
    "sparse"
    "sparsehash"
    "spasm-ng"
    "spass"
    "spatial-shell"
    "spatialite-gui"
    "spatialite-tools"
    "spawn_fcgi"
    "spdk"
    "spdlog"
    "spdx-license-list-data"
    "speakeasy-cli"
    "speakersafetyd"
    "spearspray"
    "spec-kit"
    "specr-transpile"
    "spectmorph"
    "spectra"
    "spectral-language-server"
    "spectral-mic"
    "spectre-cli"
    "spectre-meltdown-checker"
    "spectrwm"
    "specup"
    "spedread"
    "speech-denoiser"
    "speech-to-phrase"
    "speech-tools"
    "speechd"
    "speechd-minimal"
    "speed-cloudflare-cli"
    "speed-dreams"
    "speed_dreams"
    "speedcrunch"
    "speedify"
    "speedread"
    "speedscope"
    "speedtest"
    "speedtest-cli"
    "speedtest-go"
    "speedtest-rs"
    "speedtest-tracker"
    "speex"
    "speexdsp"
    "spek"
    "spellchecker-cli"
    "spf-engine"
    "spfft"
    "spglib"
    "sphinx"
    "sphinx-autobuild"
    "sphinx-gallery"
    "sphinx-lint"
    "sphinx-serve"
    "sphinxHook"
    "sphinxsearch"
    "sphinxygen"
    "spi-tools"
    "spice"
    "spice-autorandr"
    "spice-glib"
    "spice-gtk"
    "spice-protocol"
    "spice-up"
    "spice-vdagent"
    "spicedb"
    "spicedb-zed"
    "spicetify-cli"
    "spicy"
    "spicy-parser-generator"
    "spider"
    "spidermonkey_128"
    "spidermonkey_140"
    "spideroak"
    "spiffe-helper"
    "spiffe-vault"
    "spigot"
    "spike"
    "spin"
    "spinshare"
    "spip"
    "spiped"
    "spire"
    "spire-agent"
    "spire-server"
    "spire-tpm-plugin"
    "spirit"
    "spirv-cross"
    "spirv-headers"
    "spirv-llvm-translator"
    "spirv-tools"
    "spit"
    "spl"
    "spla"
    "splash"
    "splat"
    "splayer"
    "splayer-next"
    "spleen"
    "splint"
    "splitcode"
    "splix"
    "sploitscan"
    "splot"
    "spnavcfg"
    "spoof-mac"
    "spoofdpi"
    "spoofer"
    "spoofer-gui"
    "spooftooph"
    "spooles"
    "spoolman"
    "sportstracker"
    "spot"
    "spotatui"
    "spotdl"
    "spotiflac"
    "spotify"
    "spotify-cli-linux"
    "spotify-player"
    "spotify-qt"
    "spotify-tray"
    "spotifycli"
    "spotifyd"
    "spotifywm"
    "spotlight-downloader"
    "spotube"
    "spr"
    "spral"
    "spraycharles"
    "spread"
    "spring-boot-cli"
    "sprite"
    "spruce"
    "sptk"
    "sptlrx"
    "spyder"
    "spyre"
    "spytrap-adb"
    "sq"
    "sqitchMysql"
    "sqitchPg"
    "sqitchSqlite"
    "sql-formatter"
    "sql-migrate"
    "sql-studio"
    "sqlauncher"
    "sqlboiler"
    "sqlboiler-crdb"
    "sqlc"
    "sqlcheck"
    "sqlcipher"
    "sqlcl"
    "sqlcmd"
    "sqld"
    "sqldef"
    "sqldiff"
    "sqlfluff"
    "sqlint"
    "sqlit-tui"
    "sqlite"
    "sqlite-analyzer"
    "sqlite-interactive"
    "sqlite-jdbc"
    "sqlite-modern-cpp"
    "sqlite-rsync"
    "sqlite-utils"
    "sqlite-vec"
    "sqlite-vss"
    "sqlite-web"
    "sqlite3-to-mysql"
    "sqlite_orm"
    "sqlitebrowser"
    "sqlitecpp"
    "sqlitestudio"
    "sqlitestudio-plugins"
    "sqlline"
    "sqlmap"
    "sqlmc"
    "sqlpage"
    "sqlpkg-cli"
    "sqls"
    "sqlx-cli"
    "sqruff"
    "sqsh"
    "squashfs-tools"
    "squashfs-tools-ng"
    "squashfsTools"
    "squashfuse"
    "squawk"
    "squeak"
    "squeekboard"
    "squeezelite"
    "squeezelite-pulse"
    "squid"
    "squirrel-sql"
    "squishyball"
    "squix"
    "sr2t"
    "srain"
    "srandrd"
    "sratom"
    "sratoolkit"
    "srb2"
    "srb2kart"
    "src-cli"
    "srec2bin"
    "srecord"
    "sregex"
    "srelay"
    "srgn"
    "srht-gen-oauth-tok"
    "srisum"
    "srm"
    "srm-cuarzo"
    "srsran"
    "srt"
    "srt-live-server"
    "srt-to-vtt-cl"
    "srt-xtransmit"
    "srtp"
    "srtrelay"
    "ssb-patchwork"
    "sscep"
    "sscg"
    "ssdeep"
    "ssdfs-utils"
    "sse2neon"
    "sselp"
    "ssh-agent-mux"
    "ssh-agent-switcher"
    "ssh-agents"
    "ssh-audit"
    "ssh-chat"
    "ssh-copy-id"
    "ssh-ident"
    "ssh-import-id"
    "ssh-key-confirmer"
    "ssh-mitm"
    "ssh-openpgp-auth"
    "ssh-to-age"
    "ssh-to-pgp"
    "ssh-tools"
    "ssh-tpm-agent"
    "ssh-vault"
    "sshamble"
    "sshchecker"
    "sshd-openpgp-auth"
    "sshed"
    "sshesame"
    "sshfs"
    "sshfs-fuse"
    "sshguard"
    "sshified"
    "sshlatex"
    "sshm"
    "sshocker"
    "sshoogr"
    "sshp"
    "sshpass"
    "sshping"
    "sshportal"
    "sshs"
    "sshuttle"
    "sshwifty"
    "sshx"
    "sshx-server"
    "ssimulacra2"
    "ssl-cert-check"
    "ssl-proxy"
    "ssldump"
    "sslh"
    "sslmate"
    "sslmate-agent"
    "sslscan"
    "sslsplit"
    "sslstrip"
    "ssm-session-manager-plugin"
    "ssmsh"
    "ssocr"
    "ssrc"
    "sss-cli"
    "sss_code"
    "sssd"
    "sssnake"
    "ssss"
    "sstorytime"
    "sstp"
    "ssw"
    "st"
    "st-snazzy"
    "sta"
    "stabber"
    "stable-diffusion-cpp"
    "stable-diffusion-cpp-cuda"
    "stable-diffusion-cpp-rocm"
    "stable-diffusion-cpp-vulkan"
    "stac-validator"
    "stack"
    "stack2nix"
    "stackblur-go"
    "stackit-cli"
    "stackql"
    "stacks"
    "stacktile"
    "stackwhere"
    "stacs"
    "stag"
    "stagit"
    "stakk"
    "stalin"
    "stalonetray"
    "stalwart"
    "stalwart-cli"
    "stalwart-enterprise"
    "stalwart-mail"
    "stalwart-proxy"
    "stalwart-spam-filter"
    "stalwart-vandelay"
    "stalwart-webadmin"
    "stalwart_0_15"
    "stalwart_0_16"
    "stamp"
    "stanc"
    "standardnotes"
    "star"
    "star-history"
    "starboard"
    "starc"
    "starcharts"
    "stardust"
    "stardust-xr-atmosphere"
    "stardust-xr-flatland"
    "stardust-xr-gravity"
    "stardust-xr-non-spatial-input"
    "stardust-xr-protostar"
    "stardust-xr-server"
    "starfetch"
    "starfox64recomp"
    "stargate-libcds"
    "stargazer"
    "starkiller"
    "starlark"
    "starlark-rust"
    "starpls"
    "starry"
    "starsector"
    "starship"
    "starship-jj"
    "starship-sf64"
    "startup-disk"
    "staruml"
    "stash"
    "stash-clipboard"
    "stasis"
    "static-server"
    "static-web-server"
    "staticjinja"
    "statifier"
    "statik"
    "statime"
    "statix"
    "statping-ng"
    "stats"
    "statserial"
    "status-im"
    "statverif"
    "stave"
    "stax"
    "stayrtr"
    "stb"
    "stc"
    "stc-cli"
    "stderred"
    "stdman"
    "stdmanpages"
    "stdoutisatty"
    "stduuid"
    "steam"
    "steam-acf"
    "steam-art-manager"
    "steam-devices-udev-rules"
    "steam-lancache-prefill"
    "steam-play-none"
    "steam-rom-manager"
    "steam-run"
    "steam-run-free"
    "steam-tui"
    "steam-unwrapped"
    "steamback"
    "steamcmd"
    "steamguard-cli"
    "steampipe"
    "steamtinkerlaunch"
    "steamvr-linux-fixes"
    "steamworks"
    "steck"
    "steel"
    "steel-language-server"
    "steelix"
    "steghide"
    "stegseek"
    "stegsolve"
    "stella"
    "stellar-core"
    "stellarium"
    "stellarsolver"
    "stelliberty"
    "stenc"
    "step-agent"
    "step-ca"
    "step-cli"
    "step-kms-plugin"
    "stepcode"
    "stepmania"
    "stepreduce"
    "stereotool"
    "stern"
    "stevenblack-blocklist"
    "stevia"
    "stfl"
    "stgit"
    "sticky"
    "sticky-notes"
    "stig"
    "still"
    "stillcolor"
    "stirling-pdf"
    "stirling-pdf-desktop"
    "stix-otf"
    "stix-two"
    "stl-to-obj"
    "stlink"
    "stlink-gui"
    "stlink-tool"
    "stm32cubemx"
    "stm32flash"
    "stm32loader"
    "stm8flash"
    "stoat-desktop"
    "stochas"
    "stock-tui"
    "stockfish"
    "stockpile"
    "stog"
    "stoken"
    "stone"
    "stone-kingdoms"
    "stone-phaser"
    "stoolap"
    "stopmotion"
    "storcli"
    "storcli2"
    "storeBackup"
    "storebackup"
    "storj-uplink"
    "stork"
    "storm"
    "stormlib"
    "storrent"
    "stow"
    "stown"
    "stp"
    "strace"
    "strace-analyzer"
    "strace-tui"
    "stratis-cli"
    "stratisd"
    "stratovirt"
    "strawberry"
    "streamcontroller"
    "streamdeck-ui"
    "streamlink"
    "streamlit"
    "streamrip"
    "streamripper"
    "stremio-linux-shell"
    "stremio-service"
    "stress"
    "stress-ng"
    "stressapptest"
    "stretchly"
    "strfry"
    "stricat"
    "strichliste"
    "strictdoc"
    "string-machine"
    "string-view-lite"
    "strip-ansi"
    "strip-nondeterminism"
    "strip-tags"
    "stripJavaArchivesHook"
    "stripe-cli"
    "strobealign"
    "strongswan"
    "strongswanNM"
    "strongswanTNC"
    "strongswanTPM"
    "structorizer"
    "structuresynth"
    "structurizr-cli"
    "strutcpp"
    "stt"
    "sttr"
    "stu"
    "stubby"
    "stuffbin"
    "stump"
    "stumpish"
    "stumpwm"
    "stumpwm-unwrapped"
    "stun"
    "stunnel"
    "stunner"
    "stunner-webrtc"
    "stuntman"
    "stuntrally"
    "stupidterm"
    "sturmflut"
    "stutter"
    "stw"
    "stxxl"
    "stylance-cli"
    "style50"
    "stylelint"
    "stylelint-lsp"
    "stylish"
    "stylish-cabal"
    "stylish-haskell"
    "stylua"
    "styluslabs-write"
    "styluslabs-write-bin"
    "styx"
    "styx-cli"
    "su"
    "su-exec"
    "sub-batch"
    "sub-store"
    "sub-store-frontend"
    "subcat"
    "subdl"
    "subedit"
    "subfinder"
    "subfont"
    "subgit"
    "subjack"
    "subjs"
    "subler"
    "sublime-merge"
    "sublime-merge-dev"
    "sublime3"
    "sublime3-dev"
    "sublime4"
    "sublime4-dev"
    "sublime_syntax_convertor"
    "submit50"
    "subnetcalc"
    "subprober"
    "subread"
    "subsonic"
    "substudy"
    "subsurface"
    "subtitlecomposer"
    "subtitleeditor"
    "subtitlr"
    "subunit"
    "subversion"
    "subversionClient"
    "subxt"
    "subzerod"
    "suckit"
    "sudachi-rs"
    "sudachidict"
    "sudo"
    "sudo-font"
    "sudo-rs"
    "suil"
    "suitesparse"
    "suitesparse-graphblas"
    "summon"
    "summoning-pixel-dungeon"
    "sumo"
    "sumorobot-manager"
    "sums"
    "sundials"
    "sundtek"
    "sunpaper"
    "sunsama"
    "sunsetr"
    "sunshine"
    "sunvox"
    "sunwait"
    "sunxi-tools"
    "supabase-cli"
    "super"
    "super-productivity"
    "super-slicer"
    "super-slicer-beta"
    "super-slicer-latest"
    "super-tiny-icons"
    "superTux"
    "superTuxKart"
    "supercell-wx"
    "supercollider"
    "supercollider-with-plugins"
    "supercollider-with-sc3-plugins"
    "supercollider_scel"
    "supercronic"
    "superd"
    "superfile"
    "supergee"
    "supergfxctl"
    "supergfxctl-plasmoid"
    "superhtml"
    "superiotool"
    "superlu"
    "superlu_dist"
    "supermariowar"
    "supermin"
    "supermodel"
    "supernovas"
    "superscs"
    "supersonic"
    "supertux"
    "supertuxkart"
    "supertuxkart-server"
    "supervise"
    "sure"
    "surelog"
    "surf"
    "surf-display"
    "surface-control"
    "surfer"
    "surfpool"
    "surfraw"
    "surge"
    "surge-XT"
    "surge-cli"
    "surge-downloader"
    "surge-xt"
    "surgescript"
    "suricata"
    "surreal-engine"
    "surrealdb"
    "surrealdb-migrations"
    "surrealdb-surrealkv"
    "surrealist"
    "survex"
    "sus-compiler"
    "suscan"
    "sushi"
    "suspicious-package"
    "sutils"
    "suwayomi-server"
    "suwidgets"
    "sv-lang"
    "sv-lang_10"
    "sv-lang_9"
    "svaba"
    "svd2rust"
    "svd2rust-form"
    "svdtools"
    "svelte-check"
    "svelte-language-server"
    "svg-term"
    "svg-text-to-path"
    "svg2pdf"
    "svg2tikz"
    "svgbob"
    "svgcleaner"
    "svgo"
    "sview"
    "svix-cli"
    "svix-server"
    "svkbd"
    "svlint"
    "svls"
    "svn-all-fast-export"
    "svn2git"
    "svndumpsanitizer"
    "svox"
    "svp"
    "svrcore"
    "svt-av1"
    "svt-av1-hdr"
    "svt-vp9"
    "svtplay-dl"
    "svu"
    "svxlink"
    "swagger-codegen"
    "swagger-codegen3"
    "swagger-typescript-api"
    "swaggerhole"
    "swaglyrics"
    "swaks"
    "swappy"
    "swapspace"
    "swapview"
    "swarm"
    "sway"
    "sway-assign-cgroups"
    "sway-audio-idle-inhibit"
    "sway-easyfocus"
    "sway-launcher-desktop"
    "sway-new-workspace"
    "sway-overfocus"
    "sway-scratch"
    "sway-unwrapped"
    "swaybg"
    "swaycons"
    "swaycwd"
    "swayest-workstyle"
    "swayfx"
    "swayfx-unwrapped"
    "swayidle"
    "swayimg"
    "swaykbdd"
    "swaylock"
    "swaylock-effects"
    "swaylock-fancy"
    "swaylock-plugin"
    "swaymux"
    "swaynag-battery"
    "swaynotificationcenter"
    "swayosd"
    "swayr"
    "swayrbar"
    "swaysettings"
    "swaysome"
    "swaytools"
    "swaytreesave"
    "swayws"
    "swaywsr"
    "swc"
    "sweet-folders"
    "sweet-nova"
    "swego"
    "swell-foop"
    "swfmill"
    "swftools"
    "swgp-go"
    "swh"
    "swh_lv2"
    "swi-prolog"
    "swi-prolog-gui"
    "swift"
    "swift-corelibs-libdispatch"
    "swift-format"
    "swift-quit"
    "swiftbar"
    "swiftclient"
    "swiftdefaultapps"
    "swiften"
    "swiftformat"
    "swiftlint"
    "swiftpm"
    "swiftpm2nix"
    "swiftshader"
    "swig"
    "swim"
    "swingsane"
    "swipe-guess"
    "swipeaerospace"
    "switch-to-configuration-ng"
    "switchaudio-osx"
    "switcheroo"
    "switcheroo-control"
    "switchfin"
    "switchres"
    "sword"
    "swpui"
    "swt"
    "swtpm"
    "swww"
    "sx"
    "sx-go"
    "sxcs"
    "sxhkd"
    "sxiv"
    "sycl-info"
    "sydbox"
    "syft"
    "sylk"
    "sylkserver"
    "symbola"
    "symbolic-preview"
    "symbolicator"
    "symcc"
    "symengine"
    "symfony-cli"
    "symfpu"
    "symlinks"
    "symmetrica"
    "sympa"
    "symphony"
    "sympow"
    "synadm"
    "synaesthesia"
    "synapse"
    "synapse-admin"
    "synapse-admin-etkecc"
    "synapse-bt"
    "synchrony"
    "syncpack"
    "syncplay"
    "syncplay-nogui"
    "syncrclone"
    "syncstorage-rs"
    "syncstorage-rs-mysql"
    "syncstorage-rs-pgsql"
    "syncterm"
    "syncthing"
    "syncthing-discovery"
    "syncthing-macos"
    "syncthing-relay"
    "syncthingtray"
    "syncthingtray-minimal"
    "syncyomi"
    "syndicate-server"
    "syndication-domination"
    "synergy"
    "synergyWithoutGUI"
    "synfigstudio"
    "synology-cloud-sync-decryption-tool"
    "synology-drive-client"
    "syntaqlite"
    "syntax"
    "syntax-highlight"
    "syntex"
    "synthesia"
    "synthv1"
    "syrics"
    "sysbench"
    "syscall_limiter"
    "sysctl"
    "sysdig"
    "sysdig-cli-scanner"
    "sysfsutils"
    "syshud"
    "sysklogd"
    "syslinux"
    "syslogng"
    "sysmenu"
    "syspower"
    "sysprof"
    "sysstat"
    "system-config-printer"
    "system-manager"
    "system-sendmail"
    "system-syzygy"
    "system76-firmware"
    "system76-keyboard-configurator"
    "system76-power"
    "system76-scheduler"
    "system76-wallpapers"
    "systembus-notify"
    "systemc"
    "systemctl-tui"
    "systemd"
    "systemd-bootchart"
    "systemd-credsubst"
    "systemd-journal2gelf"
    "systemd-language-server"
    "systemd-lock-handler"
    "systemd-lsp"
    "systemd-manager-tui"
    "systemd-netlogd"
    "systemd-wait"
    "systemdLibs"
    "systemdMinimal"
    "systemdUkify"
    "systemdgenie"
    "systemfd"
    "systemtap-sdt"
    "systemtap-unwrapped"
    "systeroid"
    "systrayhelper"
    "sysvinit"
    "sysvtools"
    "syswatch"
    "sysz"
    "syzkaller"
    "szip"
    "szyszka"
    "t"
    "t-lang"
    "t-rec"
    "t-rex"
    "t1utils"
    "t3"
    "t3code"
    "t4kcommon"
    "ta-lib"
    "tab"
    "tab-rs"
    "tab-window-manager"
    "tabbed"
    "tabby"
    "tabby-agent"
    "tabbyapi"
    "tabiew"
    "tableplus"
    "tabnine"
    "tabula-java"
    "tabularis"
    "tabview"
    "tacacsplus"
    "tacent"
    "tacentview"
    "tachyon"
    "tack"
    "taco"
    "taffybar"
    "tagainijisho"
    "tageditor"
    "tagger"
    "taglib"
    "taglib-sharp"
    "taglib_1"
    "taglib_extras"
    "tagparser"
    "tagref"
    "tagtime"
    "tagutil"
    "tahoe-lafs"
    "tai-ahom"
    "tail-tray"
    "tailcat"
    "tailer"
    "tailor-gui"
    "tailscale"
    "tailscale-gitops-pusher"
    "tailscale-gui"
    "tailscale-nginx-auth"
    "tailscale-systray"
    "tailscalesd"
    "tailsnitch"
    "tailspin"
    "tailwindcss"
    "tailwindcss-language-server"
    "tailwindcss_3"
    "tailwindcss_4"
    "taisei"
    "taizen"
    "takao"
    "taktuk"
    "taldir"
    "talecast"
    "talentedhack"
    "taler-challenger"
    "taler-depolymerization"
    "taler-exchange"
    "taler-mdb"
    "taler-merchant"
    "taler-sync"
    "taler-twister"
    "taler-wallet-core"
    "talhelper"
    "tali"
    "talkfilters"
    "talloc"
    "talm"
    "talos-pilot"
    "talosctl"
    "tamarin-prover"
    "tamatool"
    "tambura"
    "tamgamp-lv2"
    "tamsyn"
    "tamzen"
    "tana"
    "tandem-aligner"
    "tandoor-recipes"
    "tang"
    "tangara-cli"
    "tangara-companion"
    "tangerine"
    "tango"
    "tango-cpp"
    "tango-database"
    "tango-icon-theme"
    "tango-idl"
    "tangram"
    "tanidvr"
    "tanka"
    "tantivy-go"
    "taoup"
    "tap"
    "tap-plugins"
    "taplo"
    "taproot-assets"
    "tapview"
    "tar2ext4"
    "taradino"
    "taradino-shareware"
    "tarantool"
    "target-isns"
    "targetcli-fb"
    "tarlz"
    "tarmac"
    "tarsnap"
    "tarsnapper"
    "tarssh"
    "tarsum"
    "tart"
    "tartan"
    "tarts"
    "tartube"
    "tartube-yt-dlp"
    "tartufo"
    "taschenrechner"
    "task-keeper"
    "task-master-ai"
    "taskbook"
    "taskchampion-sync-server"
    "taskflow"
    "taskhound"
    "taskim"
    "taskjuggler"
    "tasknc"
    "taskopen"
    "tasks"
    "taskserver"
    "tasksh"
    "taskspooler"
    "tasktimer"
    "taskwarrior-tui"
    "taskwarrior2"
    "taskwarrior3"
    "taterclient-ddnet"
    "tatl"
    "tattoy"
    "tau-radio"
    "tau-tower"
    "tauno-monitor"
    "tauon"
    "tautulli"
    "taws"
    "taxi"
    "taxi-cli"
    "tayga"
    "taze"
    "tbb"
    "tbb_2022"
    "tbe"
    "tblite"
    "tbls"
    "tboot"
    "tbox"
    "tbtools"
    "tbump"
    "tcat"
    "tcb"
    "tcl"
    "tcl-8_6"
    "tcl-9_0"
    "tcl2048"
    "tclap"
    "tclap_1_2"
    "tclap_1_4"
    "tcld"
    "tclint"
    "tclreadline"
    "tcount"
    "tcp_wrappers"
    "tcpcrypt"
    "tcpdirect"
    "tcpdump"
    "tcpflow"
    "tcping-go"
    "tcping-rs"
    "tcpkali"
    "tcpreplay"
    "tcptraceroute"
    "tcptrack"
    "tcsh"
    "td"
    "tdarr"
    "tdarr-node"
    "tdarr-server"
    "tdb"
    "tdf"
    "tdl"
    "tdlib"
    "tdnf"
    "tdns-cli"
    "tdrop"
    "tea"
    "tealdeer"
    "teamocil"
    "teams"
    "teams-for-linux"
    "teamspeak6-client"
    "teamspeak_server"
    "teamtype"
    "teamviewer"
    "teapot"
    "teavpn2"
    "tebreak"
    "techmino"
    "technitium-dns-server"
    "technitium-dns-server-library"
    "tecla"
    "tecnoballz"
    "tecoc"
    "tectonic"
    "tectonic-unwrapped"
    "teehee"
    "teensy-cmake-macros"
    "teensy-loader-cli"
    "teensy-udev-rules"
    "teensyduino"
    "teetertorture"
    "teeworlds"
    "teeworlds-server"
    "tegola"
    "teip"
    "tektoncd-cli"
    "tektoncd-cli-pac"
    "tela-circle-icon-theme"
    "tela-icon-theme"
    "telegraf"
    "telegram-bot-api"
    "telegram-desktop"
    "telegraph"
    "telemt"
    "telepathy-farstream"
    "telepathy-glib"
    "telepathy-idle"
    "telepathy-mission-control"
    "teleport"
    "teleport_17"
    "teleport_18"
    "telepresence"
    "telepresence2"
    "teleprompter"
    "teler"
    "telescope"
    "televido"
    "television"
    "tell-me-your-secrets"
    "teller"
    "tellico"
    "templ"
    "template-glib"
    "tempo"
    "tempora_lgc"
    "temporal"
    "temporal-cli"
    "temporal-ui-server"
    "temporal_capi"
    "temurin-bin"
    "temurin-bin-11"
    "temurin-bin-17"
    "temurin-bin-21"
    "temurin-bin-25"
    "temurin-bin-26"
    "temurin-bin-8"
    "temurin-jre-bin"
    "temurin-jre-bin-11"
    "temurin-jre-bin-17"
    "temurin-jre-bin-21"
    "temurin-jre-bin-25"
    "temurin-jre-bin-26"
    "temurin-jre-bin-8"
    "tenacity"
    "tendermint"
    "tenderness"
    "tenere"
    "tenki"
    "tennix"
    "tensor"
    "tensorflow-lite"
    "tensorman"
    "tenv"
    "teos"
    "teos-watchtower-plugin"
    "tera-cli"
    "tere"
    "termbench-pro"
    "termbg"
    "termbox"
    "termcap"
    "termcolor"
    "termdbms"
    "termdown"
    "termimage"
    "terminal-colors"
    "terminal-notifier"
    "terminal-oscilloscope"
    "terminal-parrot"
    "terminal-rain-lightning"
    "terminal-space-program"
    "terminal-stocks"
    "terminal-toys"
    "terminal-typeracer"
    "terminalmap"
    "terminaltexteffects"
    "terminator"
    "termineter"
    "terminus_font"
    "terminus_font_ttf"
    "terminusdb"
    "termirs"
    "termius"
    "termonad"
    "termpaint"
    "termpdfpy"
    "termscp"
    "termshark"
    "termshot"
    "termsnap"
    "termsonic"
    "termsvg"
    "termsyn"
    "termtekst"
    "termtosvg"
    "termusic"
    "ternimal"
    "terra"
    "terra-station"
    "terracognita"
    "terracotta"
    "terraform"
    "terraform-backend"
    "terraform-backend-git"
    "terraform-compliance"
    "terraform-config-inspect"
    "terraform-docs"
    "terraform-iam-policy-validator"
    "terraform-inventory"
    "terraform-landscape"
    "terraform-local"
    "terraform-ls"
    "terraform-lsp"
    "terraform-mcp-server"
    "terraform-plugin-docs"
    "terraform_1"
    "terraformer"
    "terraforming"
    "terragrunt"
    "terramate"
    "terranix"
    "terrapin-scanner"
    "terraria-server"
    "terrascan"
    "terraspace"
    "terser"
    "tes3cmd"
    "teseq"
    "tesh"
    "tesla_auth"
    "tessen"
    "tessera"
    "tesseract"
    "tesseract3"
    "tesseract4"
    "tesseract5"
    "test-drive"
    "testdisk"
    "testdisk-qt"
    "testkube"
    "testlib"
    "testssl"
    "tetex"
    "tetgen"
    "tetgen_1_4"
    "tether"
    "tetragon"
    "tetraproc"
    "tetrd"
    "tetrio-desktop"
    "tetrio-plus"
    "tetris"
    "tetro-tui"
    "tev"
    "tevent"
    "tewisay"
    "tex-fmt"
    "texi2html"
    "texi2mdoc"
    "texinfo"
    "texinfo7"
    "texinfoInteractive"
    "texlab"
    "texliveBasic"
    "texliveBookPub"
    "texliveConTeXt"
    "texliveFull"
    "texliveFullWithDocs"
    "texliveGUST"
    "texliveInfraOnly"
    "texliveMedium"
    "texliveMinimal"
    "texliveSmall"
    "texliveTeTeX"
    "texlyre"
    "texmacs"
    "texmaker"
    "texpresso"
    "texstudio"
    "texsurgery"
    "text-engine"
    "textadept"
    "textcompare"
    "textfonts"
    "textidote"
    "textlint"
    "textlint-plugin-latex2e"
    "textlint-plugin-org"
    "textlint-rule-abbr-within-parentheses"
    "textlint-rule-alex"
    "textlint-rule-common-misspellings"
    "textlint-rule-diacritics"
    "textlint-rule-en-max-word-count"
    "textlint-rule-max-comma"
    "textlint-rule-no-start-duplicated-conjunction"
    "textlint-rule-period-in-list-item"
    "textlint-rule-preset-ja-spacing"
    "textlint-rule-preset-ja-technical-writing"
    "textlint-rule-prh"
    "textlint-rule-stop-words"
    "textlint-rule-terminology"
    "textlint-rule-unexpanded-acronym"
    "textlint-rule-write-good"
    "textlsp"
    "textpieces"
    "textplots"
    "textql"
    "textsnatcher"
    "texture-synthesis"
    "texturepacker"
    "texworks"
    "teyjus"
    "tf"
    "tf-summarize"
    "tfautomv"
    "tfctl"
    "tfenv"
    "tfk8s"
    "tflint"
    "tfmigrate"
    "tfproviderdocs"
    "tfsec"
    "tfsort"
    "tfswitch"
    "tftp-hpa"
    "tftui"
    "tfupdate"
    "tg"
    "tg-archive"
    "tg-timer"
    "tgeraser"
    "tgpt"
    "tgrep"
    "tgswitch"
    "tgt"
    "thanos"
    "thaw"
    "thc-hydra"
    "thc-ipv6"
    "thc-secure-delete"
    "thcrap-steam-proton-wrapper"
    "the-foundation"
    "the-legend-of-edgar"
    "the-neue-black"
    "the-powder-toy"
    "the-unarchiver"
    "the-way"
    "theano"
    "theattyr"
    "theclicker"
    "theforceengine"
    "theft"
    "theharvester"
    "thelounge"
    "theme-sh"
    "themechanger"
    "themix-gui"
    "thepeg"
    "there"
    "therion"
    "thermald"
    "thicket"
    "thin-provisioning-tools"
    "thinkfan"
    "thinkpad-scripts"
    "thokr"
    "thonny"
    "thorium-reader"
    "threadcat"
    "threatest"
    "threema-desktop"
    "thrift"
    "thrift-ls"
    "throne"
    "throttled"
    "thruster"
    "thttpd"
    "thud"
    "thumbdrives"
    "thumbs"
    "thumper"
    "thunar"
    "thunar-archive-plugin"
    "thunar-dropbox-plugin"
    "thunar-media-tags-plugin"
    "thunar-shares-plugin"
    "thunar-unwrapped"
    "thunar-vcs-plugin"
    "thunar-volman"
    "thunderbird"
    "thunderbird-140"
    "thunderbird-140-unwrapped"
    "thunderbird-153"
    "thunderbird-153-unwrapped"
    "thunderbird-bin"
    "thunderbird-cli"
    "thunderbird-cli-bridge"
    "thunderbird-cli-mcp"
    "thunderbird-esr"
    "thunderbird-esr-bin"
    "thunderbird-esr-bin-unwrapped"
    "thunderbird-esr-unwrapped"
    "thunderbird-latest"
    "thunderbird-latest-bin"
    "thunderbird-latest-bin-unwrapped"
    "thunderbird-latest-unwrapped"
    "thunderbird-mcp"
    "thunderbird-unwrapped"
    "thunderbolt"
    "thunderkittens"
    "tibetan-machine"
    "tic-80"
    "ticcutils"
    "ticker"
    "ticketbooth"
    "tickrs"
    "ticktick"
    "tidal"
    "tidal-cycles-full"
    "tidal-dl"
    "tidal-hifi"
    "tiddit"
    "tiddlywiki"
    "tideways-cli"
    "tideways-daemon"
    "tidgi"
    "tidy-viewer"
    "tidyp"
    "tie"
    "tig"
    "tigerbeetle"
    "tigerjython"
    "tigervnc"
    "tika"
    "tikzit"
    "tilda"
    "tile-downloader"
    "tile38"
    "tiled"
    "tiledb"
    "tilem"
    "tilemaker"
    "tilinggallery"
    "tilix"
    "tilt"
    "timbl"
    "timblserver"
    "timbreid"
    "time"
    "time-decode"
    "time-ghc-modules"
    "time12"
    "timekpr"
    "timelapse-deflicker"
    "timelimit"
    "timeline"
    "timeloop"
    "timer"
    "times-newer-roman"
    "timescaledb-parallel-copy"
    "timescaledb-tune"
    "timeshift"
    "timeshift-minimal"
    "timeshift-unwrapped"
    "timetagger"
    "timetagger_cli"
    "timetrap"
    "timeular"
    "timew-sync-client"
    "timew-sync-server"
    "timewall"
    "timewarrior"
    "timezonemap"
    "timg"
    "timidity"
    "timoni"
    "timr-tui"
    "tinc"
    "tinc_pre"
    "tinfoil-cli"
    "ting"
    "tini"
    "tint"
    "tint2"
    "tintin"
    "tinty"
    "tiny"
    "tiny-cuda-nn"
    "tiny-dfr"
    "tiny-rdm"
    "tiny-wii-backup-manager"
    "tiny8086"
    "tinyalsa"
    "tinyauth"
    "tinycbor"
    "tinycc"
    "tinycdb"
    "tinycmmc"
    "tinycompress"
    "tinyemu"
    "tinyfecvpn"
    "tinyfetch"
    "tinyfugue"
    "tinygettext"
    "tinygltf"
    "tinygo"
    "tinymembench"
    "tinymist"
    "tinyobjloader"
    "tinyprog"
    "tinyproxy"
    "tinyscheme"
    "tinysparql"
    "tinyssh"
    "tinystatus"
    "tinywl"
    "tinywm"
    "tinyxml"
    "tinyxml-2"
    "tinyxxd"
    "tio"
    "tipa"
    "tipidee"
    "tipp10"
    "tippecanoe"
    "tiptop"
    "tiramisu"
    "tirith"
    "tiscamera"
    "titanion"
    "tiv"
    "tivodecode"
    "tix"
    "tixati"
    "tk"
    "tk-8_6"
    "tk-9_0"
    "tk-safe"
    "tkdiff"
    "tkey-ssh-agent"
    "tkey-verification"
    "tkgl"
    "tkman"
    "tkmm"
    "tkrev"
    "tkrzw"
    "tl-expected"
    "tlafmt"
    "tlaplus"
    "tlaplus-toolbox"
    "tlaplus18"
    "tlaplusToolbox"
    "tlaps"
    "tlclient"
    "tldfinder"
    "tldr"
    "tldr-hs"
    "tldx"
    "tlf"
    "tllist"
    "tlmi-auth"
    "tlock"
    "tlp"
    "tlp-pd"
    "tlrc"
    "tlrender"
    "tlsanalyzer"
    "tlsclient"
    "tlsinfo"
    "tlspool"
    "tlsrpt-reporter"
    "tlsx"
    "tlwg"
    "tmate"
    "tmate-ssh-server"
    "tmatrix"
    "tmaze"
    "tmc-cli"
    "tml"
    "tmpi"
    "tmpmail"
    "tmpwatch"
    "tmsu"
    "tmux"
    "tmux-cssh"
    "tmux-mem-cpu-load"
    "tmux-sessionizer"
    "tmux-xpanes"
    "tmuxai"
    "tmuxifier"
    "tmuxinator"
    "tmuxp"
    "tmx2lua"
    "tnat64"
    "tncattach"
    "tnef"
    "tnt"
    "tntdb"
    "tntnet"
    "to-html"
    "toast"
    "toastify"
    "tocaia"
    "tochd"
    "tockloader"
    "tocpdf"
    "todds"
    "todesk"
    "todo"
    "todo-txt-cli"
    "todofi-sh"
    "todoist"
    "todoist-cli"
    "todoist-electron"
    "todoman"
    "tofi"
    "tofu-ls"
    "toilet"
    "toipe"
    "tokei"
    "tokenspeed-triton-llvm"
    "tokio-console"
    "tokscale"
    "tokstat"
    "tokyocabinet"
    "tokyotyrant"
    "tola"
    "toluapp"
    "tomat"
    "tomato-c"
    "tomb"
    "tombi"
    "tombl"
    "tomboy-ng"
    "tomcat"
    "tomcat-native"
    "tomcat10"
    "tomcat11"
    "tomcat9"
    "tomcat_mysql_jdbc"
    "tome2"
    "tome4"
    "toml-cli"
    "toml-f"
    "toml-sort"
    "toml-test"
    "toml11"
    "toml2json"
    "toml2nix"
    "tomlc17"
    "tomlc99"
    "tomlcpp"
    "tomlplusplus"
    "tomlq"
    "tone"
    "tonearm"
    "tonelib-gfx"
    "tonelib-jam"
    "tonelib-metal"
    "tonelib-noisereducer"
    "tonelib-zoom"
    "tony"
    "toolbox"
    "toolhive"
    "tooling-language-server"
    "toolong"
    "toot"
    "tootik"
    "top-git"
    "topcoat-cli"
    "topcom"
    "topfew"
    "topfew-rs"
    "topgrade"
    "topiary"
    "topicctl"
    "toppra"
    "topydo"
    "tor"
    "tor-browser"
    "torcs"
    "torcs-without-data"
    "torctl"
    "torque"
    "torrentstream"
    "torrserver"
    "torsocks"
    "tortoisehg"
    "torus-trooper"
    "toss"
    "totem"
    "totem-pl-parser"
    "totp-cli"
    "touchegg"
    "touchosc"
    "touying"
    "tower-pixel-dungeon"
    "tox-node"
    "toxic"
    "toxiproxy"
    "toxvpn"
    "toybox"
    "tp-auto-kbbl"
    "tpacpi-bat"
    "tparse"
    "tparted"
    "tpi"
    "tplay"
    "tpm-fido"
    "tpm-quote-tools"
    "tpm-tools"
    "tpm-trust"
    "tpm2-abrmd"
    "tpm2-openssl"
    "tpm2-pkcs11"
    "tpm2-pkcs11-abrmd"
    "tpm2-pkcs11-esapi"
    "tpm2-pkcs11-fapi"
    "tpm2-tools"
    "tpm2-totp"
    "tpm2-totp-with-plymouth"
    "tpm2-tss"
    "tpmmanager"
    "tpnote"
    "tproxy"
    "tpsecore"
    "tptp"
    "tqftpserv"
    "tqsl"
    "tr-patcher"
    "traccar"
    "trace-cmd"
    "tracebox"
    "tracecompass"
    "tracee"
    "tracefilegen"
    "tracefilesim"
    "tracelinks"
    "traceroute"
    "tracexec"
    "trackballs"
    "trackma"
    "trackma-curses"
    "trackma-gtk"
    "trackma-qt"
    "tractor"
    "tracy"
    "tracy_0_11"
    "tracy_0_12"
    "tracy_0_13"
    "tradcpp"
    "traderepublic-portfolio-downloader"
    "tradingview"
    "traefik"
    "traefik-certs-dumper"
    "trafficserver"
    "trailbase"
    "traitor"
    "trajan"
    "trak"
    "tran"
    "tranquil-pds"
    "tranquil-pds-frontend"
    "transcribe"
    "transcrypt"
    "transfer-sh"
    "transgui"
    "transifex-cli"
    "transito"
    "translate-shell"
    "translatelocally"
    "translatepy"
    "transmission-remote-gtk"
    "transmission-rss"
    "transmission_4"
    "transmission_4-gtk"
    "transmission_4-mac"
    "transmission_4-qt"
    "transmission_4-qt5"
    "transmission_4-qt6"
    "transset"
    "trash-cli"
    "trashy"
    "traverso"
    "tray-tui"
    "trayer"
    "trayscale"
    "trdl-client"
    "trdsql"
    "tre"
    "tre-command"
    "trealla"
    "tree"
    "tree-from-tags"
    "tree-sitter"
    "treecat"
    "treedome"
    "treefmt"
    "treegen"
    "treemd"
    "treemix"
    "treesheets"
    "trelby"
    "trellis"
    "tremc"
    "tremotesf"
    "trevorproxy"
    "trevorspray"
    "trex"
    "trexio"
    "trezor-agent"
    "trezor-suite"
    "trezor-udev-rules"
    "trezorctl"
    "trezord"
    "trf"
    "trgui-ng"
    "trgui-ng-web"
    "tribler"
    "trickest-cli"
    "trickle"
    "trickster"
    "tridactyl-native"
    "triehash"
    "triforce-lv2"
    "trigger"
    "trigger-control"
    "triggerhappy"
    "trilinos"
    "trilinos-mpi"
    "trilium-desktop"
    "trilium-next-desktop"
    "trilium-next-server"
    "trilium-server"
    "trillian"
    "trillian-im"
    "trimage"
    "trimal"
    "trimmomatic"
    "trinity"
    "trino-cli"
    "trinsic-cli"
    "trippy"
    "triton"
    "triton-llvm"
    "trivy"
    "trlib"
    "trojan-go"
    "trojan-rs"
    "trompeloeil"
    "troubadix"
    "trousers"
    "troveclient"
    "trrntzip"
    "trucky"
    "truecrack"
    "truecrack-cuda"
    "truehdd"
    "truenas-incus-ctl"
    "trueseeing"
    "trufflehog"
    "trunk"
    "trunk-io"
    "trunk-ng"
    "trunk-recorder"
    "trurl"
    "trustedqsl"
    "trusttunnel-endpoint"
    "trustymail"
    "truvari"
    "trx"
    "try"
    "tryton"
    "trytond"
    "trzsz-ssh"
    "ts"
    "ts-warp"
    "ts_query_ls"
    "tscli"
    "tsduck"
    "tsgolint"
    "tshark"
    "tshock"
    "tsid"
    "tsidp"
    "tslib"
    "tsm-client"
    "tsm-client-withGui"
    "tsocks"
    "tsshd"
    "tsukae"
    "tsukimi"
    "tsung"
    "tsx"
    "tt"
    "tt-burnin"
    "tt-logger"
    "tt-metal"
    "tt-rss"
    "tt-rss-plugin-auth-ldap"
    "tt-rss-plugin-data-migration"
    "tt-rss-plugin-feediron"
    "tt-rss-plugin-ff-instagram"
    "tt-rss-plugin-freshapi"
    "tt-rss-theme-feedly"
    "tt-smi"
    "tt-system-tools"
    "tt-topology"
    "tt-umd"
    "tt2020"
    "ttaenc"
    "ttags"
    "ttchat"
    "ttconv"
    "ttdl"
    "ttf-envy-code-r"
    "ttf-indic"
    "ttf-tw-moe"
    "ttf2pt1"
    "ttf_bitstream_vera"
    "ttfautohint"
    "ttfautohint-nox"
    "ttfb"
    "tthsum"
    "ttl"
    "ttl2c"
    "ttmkfdir"
    "ttnn-visualizer"
    "ttop"
    "ttp"
    "tts"
    "tts-mod-vault"
    "ttwatch"
    "tty-clock"
    "tty-share"
    "tty-solitaire"
    "ttyd"
    "ttygif"
    "ttylog"
    "ttyper"
    "ttyplot"
    "ttypr"
    "ttysvr"
    "tu"
    "tuatara"
    "tuba"
    "tubekit"
    "tubekit-unwrapped"
    "tuc"
    "tuckr"
    "tudu"
    "tuhi"
    "tui-journal"
    "tuicam"
    "tuicr"
    "tuifeed"
    "tuifimanager"
    "tuigreet"
    "tuios"
    "tuir"
    "tuisky"
    "tuist"
    "tuistash"
    "tukai"
    "tuleap-cli"
    "tulip"
    "tuliprox"
    "tumbler"
    "tumiki-fighters"
    "tun2proxy"
    "tun2socks"
    "tuna"
    "tunctl"
    "tuned"
    "tunefish"
    "tuner"
    "tunnelgraf"
    "tunnelto"
    "tunnelx"
    "tuntox"
    "tunwg"
    "tup"
    "tuple"
    "tuptime"
    "turbo"
    "turbo-unwrapped"
    "turbocase"
    "turborepo-remote-cache"
    "turbovnc"
    "turingdb"
    "turingplus"
    "turn-rs"
    "turnon"
    "turnstile"
    "turntable"
    "turses"
    "turso"
    "turso-cli"
    "turtle"
    "turtle-build"
    "tusc-sh"
    "tusd"
    "tusk"
    "tut"
    "tutanota-desktop"
    "tuxbox"
    "tuxclocker"
    "tuxclocker-nvidia-plugin"
    "tuxclocker-plugins"
    "tuxclocker-plugins-with-unfree"
    "tuxclocker-without-unfree"
    "tuxedo"
    "tuxedo-rs"
    "tuxguitar"
    "tuxmux"
    "tuxpaint"
    "tuxtype"
    "tuxvdmtool"
    "tv"
    "tvbrowser"
    "tvm"
    "tvnamer"
    "tw"
    "twa"
    "tweag-credential-helper"
    "tweak"
    "tweego"
    "tweeny"
    "tweet-hs"
    "twelf"
    "twemoji-color-font"
    "twemoji-color-font-src"
    "twig-language-server"
    "twiggy"
    "twilight"
    "twilight-kde"
    "twilio-cli"
    "twine"
    "twinejs"
    "twingate"
    "twinkle"
    "twitch-chat-downloader"
    "twitch-cli"
    "twitch-dl"
    "twitch-hls-client"
    "twitch-tui"
    "twitter-bootstrap"
    "twitter-color-emoji"
    "twm"
    "twmn"
    "twolame"
    "twspace-crawler"
    "twspace-dl"
    "twtxt"
    "twurl"
    "txm"
    "txr"
    "txt2man"
    "txt2tags"
    "txtpbfmt"
    "txtw"
    "ty"
    "tydra"
    "tygo"
    "tym"
    "typ2docx"
    "typeinc"
    "typer"
    "typescript"
    "typescript-language-server"
    "typescript_5"
    "typescript_7"
    "typesense"
    "typesetter"
    "typeshare"
    "typespec"
    "typespeed"
    "typewriter"
    "typical"
    "typioca"
    "typography"
    "typora"
    "typos"
    "typos-lsp"
    "typr"
    "typship"
    "typst"
    "typst-live"
    "typstwriter"
    "typstyle"
    "typtea"
    "tyrolienne"
    "tyson"
    "tytools"
    "tz"
    "tzdata"
    "tzf-rs"
    "tzpfms"
    "tzupdate"
    "u-config"
    "u-root"
    "u-root-cmds"
    "u001-font"
    "u2ps"
    "u3-tool"
    "u9fs"
    "ua"
    "uacme"
    "uade"
    "uair"
    "uammd"
    "uarmsolver"
    "uasm"
    "uavs3d"
    "ubase"
    "ubertooth"
    "ubi_reader"
    "ubidump"
    "ubootA20OlinuxinoLime"
    "ubootA20OlinuxinoLime2EMMC"
    "ubootAmx335xEVM"
    "ubootBananaPi"
    "ubootBananaPim2Zero"
    "ubootBananaPim3"
    "ubootBananaPim64"
    "ubootCM3588NAS"
    "ubootClearfog"
    "ubootCubieboard2"
    "ubootGuruplug"
    "ubootJetsonTK1"
    "ubootLibreTechCC"
    "ubootNanoPCT4"
    "ubootNanoPCT6"
    "ubootNanoPiR5S"
    "ubootNovena"
    "ubootOdroidC2"
    "ubootOdroidXU3"
    "ubootOlimexA64Olinuxino"
    "ubootOlimexA64Teres1"
    "ubootOrangePi3"
    "ubootOrangePi3B"
    "ubootOrangePi5"
    "ubootOrangePi5Max"
    "ubootOrangePi5Plus"
    "ubootOrangePiPc"
    "ubootOrangePiZero"
    "ubootOrangePiZero2"
    "ubootOrangePiZero3"
    "ubootOrangePiZeroPlus2H5"
    "ubootPcduino3Nano"
    "ubootPine64"
    "ubootPine64LTS"
    "ubootPinebook"
    "ubootPinebookPro"
    "ubootQemuAarch64"
    "ubootQemuArm"
    "ubootQemuRiscv64Smode"
    "ubootQemuX86"
    "ubootQemuX86_64"
    "ubootQuartz64B"
    "ubootROCPCRK3399"
    "ubootRadxaZero3W"
    "ubootRaspberryPi"
    "ubootRaspberryPi2"
    "ubootRaspberryPi3_32bit"
    "ubootRaspberryPi3_64bit"
    "ubootRaspberryPi4_32bit"
    "ubootRaspberryPi4_64bit"
    "ubootRaspberryPiAarch64"
    "ubootRaspberryPiZero"
    "ubootRock3C"
    "ubootRock4CPlus"
    "ubootRock5ModelB"
    "ubootRock5ModelC"
    "ubootRock64"
    "ubootRock64v2"
    "ubootRockPi4"
    "ubootRockPiE"
    "ubootRockPro64"
    "ubootSheevaplug"
    "ubootSopine"
    "ubootTools"
    "ubootTuringRK1"
    "ubootUtilite"
    "ubootVisionFive2"
    "ubootWandboard"
    "ubpm"
    "ubports-click"
    "ubports-pdk"
    "ubridge"
    "ubuntu-classic"
    "ubuntu-sans"
    "ubuntu-sans-mono"
    "ubus"
    "ucarp"
    "ucblogo"
    "ucc"
    "ucg"
    "uchecker"
    "uchess"
    "uchmviewer"
    "uci"
    "ucl"
    "uclibc"
    "uclibc-ng"
    "uclient"
    "ucode"
    "ucommon"
    "ucon64"
    "ucs-fonts"
    "ucspi-tcp"
    "ucto"
    "uctodata"
    "ucx"
    "uddup"
    "udebug"
    "udev"
    "udev-block-notify"
    "udev-gothic"
    "udev-gothic-nf"
    "udevCheckHook"
    "udevil"
    "udftools"
    "udict"
    "udig"
    "udis86"
    "udiskie"
    "udisks"
    "udisks2"
    "udns"
    "udocker"
    "udp-over-tcp"
    "udp2raw"
    "udp514-journal"
    "udpipe1"
    "udpreplay"
    "udpspeeder"
    "udpt"
    "udptunnel"
    "udpx"
    "udunits"
    "ueberzug"
    "ueberzugpp"
    "uefi-firmware-parser"
    "uefi-run"
    "uefisettings"
    "uefitool"
    "uemacs"
    "ueransim"
    "uesave"
    "ueviewer"
    "ufetch"
    "ufiformat"
    "ufmt"
    "ufoai"
    "ufolint"
    "uftp"
    "uftpd"
    "uftrace"
    "uftraceFull"
    "ugarit"
    "ugdb"
    "uget"
    "uget-integrator"
    "ugit"
    "uglify-js"
    "ugm"
    "ugreen-leds-cli"
    "ugrep"
    "ugrep-indexer"
    "ugs"
    "ugtrain"
    "uhd"
    "uhdMinimal"
    "uhdm"
    "uhexen2"
    "uhhyou-plugins"
    "uhhyou-plugins-juce"
    "uhk-agent"
    "uhk-udev-rules"
    "uhttpd"
    "uhttpmock"
    "uhttpmock_1_0"
    "uhub"
    "uhubctl"
    "uid_wrapper"
    "uif2iso"
    "uim"
    "uisp"
    "uitest"
    "uiua"
    "uiua-unstable"
    "uiua386"
    "uivonim"
    "ukmm"
    "ulauncher"
    "ulfius"
    "ulid"
    "ulogd"
    "ultimate-doom-builder"
    "ultimate-oldschool-pc-font-pack"
    "ultimatestunts"
    "ultrablue-server"
    "ultralist"
    "ultralytics"
    "ultrastar-creator"
    "ultrastar-manager"
    "ultrastardx"
    "umami"
    "umap"
    "umbriel"
    "umka-lang"
    "umlet"
    "umoci"
    "umockdev"
    "umoria"
    "umount"
    "umpire"
    "umple"
    "umple-lsp"
    "umr"
    "ums"
    "umu-launcher"
    "umu-launcher-unwrapped"
    "umurmur"
    "unar"
    "unarc"
    "unblob"
    "unbook"
    "unbound"
    "unbound-full"
    "unbound-with-systemd"
    "unciv"
    "unclutter"
    "unclutter-xfixes"
    "unconvert"
    "uncover"
    "uncrustify"
    "undaemonize"
    "undbx"
    "undefined-medium"
    "undercut-f1"
    "undertalemodcli"
    "undertime"
    "undervolt"
    "undetected-chromedriver"
    "undistract-me"
    "undmg"
    "undock"
    "undocker"
    "undollar"
    "unetbootin"
    "unflac"
    "unfonts-core"
    "unfs3"
    "unftp"
    "unfurl"
    "ungit"
    "ungoogled-chromium"
    "unhide"
    "unhide-gui"
    "uni"
    "uni-algo"
    "uni-sync"
    "uni-vga"
    "uni2ascii"
    "unibilium"
    "unicode-character-database"
    "unicode-emoji"
    "unicode-idna"
    "unicode-paracode"
    "unicodeit"
    "unicon-lang"
    "unicorn"
    "unicorn-angr"
    "unidings"
    "uniex"
    "unifdef"
    "unifi"
    "unifi-protect-backup"
    "unified-memory-framework"
    "unifiedpush-common-proxies"
    "unifly"
    "unifont"
    "unifont-csur"
    "unifont_upper"
    "unify"
    "unigine-heaven"
    "unigine-sanctuary"
    "unigine-superposition"
    "unigine-tropics"
    "unigine-valley"
    "unihan-database"
    "unimap"
    "unimatrix"
    "unionfs-fuse"
    "unipicker"
    "uniscribe"
    "unison"
    "unison-fsmonitor"
    "unison-ucm"
    "unit"
    "unitree-sdk2"
    "units"
    "units-llnl"
    "unittest-cpp"
    "unity-test"
    "unityhub"
    "universal-android-debloater"
    "universal-ctags"
    "unix-privesc-check"
    "unixODBC"
    "unixbench"
    "unixcw"
    "unixodbc"
    "unnamed-sdvx-clone"
    "unnaturalscrollwheels"
    "unnethack"
    "unnix"
    "unoconv"
    "unofficial-homestuck-collection"
    "unordered_dense"
    "unp"
    "unpackelf"
    "unpackerr"
    "unpaper"
    "unparam"
    "unpfs"
    "unpoller"
    "unqlite"
    "unrar"
    "unrar-free"
    "unrar-wrapper"
    "unrpa"
    "unrtf"
    "unscd"
    "unscii"
    "unshield"
    "unshieldv3"
    "unsilence"
    "unsloth-desktop"
    "unstick"
    "unstructured-api"
    "unsure"
    "untex"
    "untie"
    "untrunc-anthwlock"
    "unused"
    "unused_deps"
    "unvanquished"
    "unwaf"
    "unyaffs"
    "unzip"
    "unzipNLS"
    "unzrip"
    "up"
    "upbound"
    "upbound-main"
    "upcloud-cli"
    "update-dotdee"
    "update-nix-fetchgit"
    "update-python-libraries"
    "update-resolv-conf"
    "update-systemd-resolved"
    "updateAutotoolsGnuConfigScriptsHook"
    "updatecli"
    "updfparser"
    "updog"
    "upgrade-assistant"
    "upiano"
    "upki"
    "uplosi"
    "upnp-router-control"
    "upower"
    "upower-notify"
    "uppaal"
    "uppy-companion"
    "ups"
    "upscaler"
    "upscayl"
    "upscayl-ncnn"
    "upsies"
    "upspin"
    "upsun"
    "upterm"
    "uptime-kuma"
    "uptimed"
    "upwork"
    "upx"
    "uqm"
    "uqmi"
    "uradvd"
    "urbackup-client"
    "urban-cli"
    "urbanterror"
    "urbit"
    "urdf-viz"
    "urdfdom"
    "urdfdom-headers"
    "urh"
    "uri"
    "uriparser"
    "urjtag"
    "url-parser"
    "urlencode"
    "urlfinder"
    "urlhunter"
    "urlscan"
    "urlwatch"
    "urn"
    "urn-timer"
    "uro"
    "uroboros"
    "ursadb"
    "urserver"
    "urweb"
    "urx"
    "usacloud"
    "usage"
    "usb-blaster-udev-rules"
    "usb-modeswitch"
    "usb-modeswitch-data"
    "usb-reset"
    "usbfluxd"
    "usbguard"
    "usbguard-notifier"
    "usbimager"
    "usbip-ssh"
    "usbkvm"
    "usbmuxd"
    "usbmuxd2"
    "usbredir"
    "usbrelay"
    "usbrelayd"
    "usbrip"
    "usbsdmux"
    "usbtop"
    "usbtree"
    "usbutils"
    "usbvfiod"
    "usbview"
    "useOldCXXAbi"
    "usearch"
    "user-scanner"
    "userborn"
    "userborn-import-legacy"
    "userhosts"
    "usermount"
    "username-anarchy"
    "usort"
    "usql"
    "usque"
    "usrsctp"
    "ustr"
    "ustream-ssl"
    "ustream-ssl-mbedtls"
    "ustreamer"
    "ut"
    "ut1999"
    "utf8cpp"
    "utf8proc"
    "utfcpp"
    "uthash"
    "uthenticode"
    "util-linux"
    "util-linuxMinimal"
    "util-macros"
    "utillinux"
    "utilmacros"
    "utm"
    "utmps"
    "utox"
    "utpm"
    "utsushi-networkscan"
    "utterly-nord-plasma"
    "utterly-round-plasma-style"
    "uuagc"
    "uucp"
    "uudeview"
    "uusi"
    "uutils-acl"
    "uutils-coreutils"
    "uutils-coreutils-noprefix"
    "uutils-diffutils"
    "uutils-findutils"
    "uutils-hostname"
    "uutils-login"
    "uutils-procps"
    "uutils-sed"
    "uutils-tar"
    "uutils-util-linux"
    "uuu"
    "uv"
    "uv-sort"
    "uvccapture"
    "uvcdynctrl"
    "uvwasi"
    "uw-ttyp0"
    "uwc"
    "uwhoisd"
    "uwimap"
    "uwsgi"
    "uwsm"
    "uwu-colors"
    "uwufetch"
    "uwuify"
    "uxn"
    "uxn11"
    "uxplay"
    "uzdoom"
    "v2ray"
    "v2ray-domain-list-community"
    "v2ray-geoip"
    "v2ray-rules-dat"
    "v2raya"
    "v2rayn"
    "v4l-utils"
    "v4l2-relayd"
    "v4l2-to-ndi"
    "vaapi-intel-hybrid"
    "vacask"
    "vacuum-go"
    "vacuum-tube"
    "vagrant"
    "vala"
    "vala-language-server"
    "vala-lint"
    "vala_0_56"
    "valdi"
    "vale"
    "vale-ls"
    "valent"
    "valentina"
    "valeronoi"
    "valgrind"
    "valgrind-light"
    "valhalla"
    "vali"
    "validatePkgConfig"
    "validator-nu"
    "valijson"
    "valkey"
    "vals"
    "valuta"
    "vamp-plugin-sdk"
    "vampire"
    "vangers"
    "vanguards"
    "vanilla-dmz"
    "vanillara"
    "vanillatd"
    "vaporizer2"
    "vapoursynth"
    "vapoursynth-bestsource"
    "vapoursynth-editor"
    "vapoursynth-eedi3"
    "vapoursynth-mvtools"
    "vapoursynth-nnedi3cl"
    "vapoursynth-znedi3"
    "varia"
    "variant-lite"
    "variety"
    "varnish"
    "varnish80"
    "varscan"
    "varunastra"
    "vassal"
    "vatprism"
    "vault"
    "vault-bin"
    "vault-medusa"
    "vault-ssh-plus"
    "vault-tasks"
    "vault-unseal"
    "vaultenv"
    "vaultwarden"
    "vaultwarden-mysql"
    "vaultwarden-postgresql"
    "vaultwarden-sqlite"
    "vaultwarden-webvault"
    "vazir-code-font"
    "vazirmatn"
    "vbam"
    "vbindiff"
    "vboot-utils"
    "vboot_reference"
    "vc"
    "vc4-newlib"
    "vcal"
    "vcard"
    "vcard-viewer"
    "vcdimager"
    "vcftools"
    "vcg"
    "vclab-nexus"
    "vcluster"
    "vcmi"
    "vcpkg"
    "vcpkg-tool"
    "vcpkg-tool-unwrapped"
    "vcprompt"
    "vcs"
    "vcs2l"
    "vcs_query"
    "vcsh"
    "vcsi"
    "vcv-rack"
    "vde2"
    "vdirsyncer"
    "vdmfec"
    "vdo"
    "vdpauinfo"
    "vdr"
    "vdrift"
    "vdrift-bin"
    "vdrsymbols"
    "vdu_controls"
    "vector"
    "vector-blf"
    "vectorcode"
    "vectoroids"
    "vectorscan"
    "vega-cli"
    "vega-lite"
    "vegeta"
    "vegur"
    "veikk-linux-driver-gui"
    "veila"
    "veilid"
    "velero"
    "vellum"
    "velocity"
    "veloren"
    "venator"
    "vencord"
    "vencord-web-extension"
    "vendir"
    "vengi-tools"
    "ventoy"
    "ventoy-full"
    "ventoy-full-gtk"
    "ventoy-full-qt"
    "vep"
    "veracrypt"
    "verapdf"
    "verapdf-gui"
    "verbiste"
    "vercel-pkg"
    "verdict"
    "veriT"
    "verible"
    "veridian"
    "verifast"
    "verifpal"
    "verilator"
    "verit"
    "vermin"
    "vermouth"
    "veroroute"
    "versatiles"
    "versionCheckHook"
    "versitygw"
    "versus"
    "vertcoin"
    "vertcoind"
    "veryfasttree"
    "veryl"
    "vesktop"
    "vespa-cli"
    "vessel"
    "vesta"
    "vesta-viewer"
    "veusz"
    "vex-tui"
    "vexctl"
    "vfkit"
    "vfox"
    "vg"
    "vgm2x"
    "vgmplay-libvgm"
    "vgmstream"
    "vgmtools"
    "vgmtrans"
    "vgrep"
    "vhd2vl"
    "vhdl-ls"
    "vhost-device-sound"
    "vhost-device-vsock"
    "vhs"
    "vi-mongo"
    "via"
    "vial"
    "viber"
    "vibrantlinux"
    "vice"
    "viceroy"
    "vicinae"
    "victor-mono"
    "victorialogs"
    "victoriametrics"
    "victoriatraces"
    "vid-stab"
    "vidcutter"
    "viddy"
    "video-compare"
    "video-downloader"
    "video-trimmer"
    "video2x"
    "vidmerger"
    "viennarna"
    "viewnior"
    "viewres"
    "vifm"
    "vifm-full"
    "vigra"
    "viking"
    "vikunja"
    "vikunja-desktop"
    "villain"
    "vim"
    "vim-classic"
    "vim-darwin"
    "vim-full"
    "vim-language-server"
    "vim-startuptime"
    "vim-vint"
    "vimPluginsUpdater"
    "vimb"
    "vimb-unwrapped"
    "vimcats"
    "vimdoc-language-server"
    "vimer"
    "vimgolf"
    "vimhjkl"
    "vimiv-qt"
    "vimix-cursors"
    "vimix-icon-theme"
    "vimpager"
    "vimpager-latest"
    "vimv"
    "vimv-rs"
    "vimwiki-markdown"
    "vinegar"
    "vintagestory"
    "vinyl-cache"
    "vinyl-cache_9"
    "violet"
    "vips"
    "vips_8_17"
    "vipsdisp"
    "virglrenderer"
    "virt-manager"
    "virt-top"
    "virt-v2v"
    "virt-viewer"
    "virt-what"
    "virter"
    "virtio-win"
    "virtiofsd"
    "virtnbdbackup"
    "virtual-ans"
    "virtualbox"
    "virtualboxExtpack"
    "virtualboxHardened"
    "virtualboxHeadless"
    "virtualboxKvm"
    "virtualboxWithExtpack"
    "virtualenv"
    "virtualenv-clone"
    "virtualgl"
    "virtualglLib"
    "virtualpg"
    "vis"
    "visidata"
    "viskores"
    "visp"
    "vista-fonts"
    "vista-fonts-chs"
    "vista-fonts-cht"
    "visual-hexdiff"
    "visual-paradigm-ce"
    "visualvm"
    "vit"
    "vita3k"
    "vital"
    "vite"
    "vitejs"
    "vitess"
    "vitetris"
    "viu"
    "vivaldi"
    "vivaldi-ffmpeg-codecs"
    "vivictpp"
    "vivid"
    "vivify"
    "vivisect"
    "vivliostyle"
    "viw"
    "vix"
    "vixl"
    "vja"
    "vk-bootstrap"
    "vk-cli"
    "vk-messenger"
    "vkbasalt"
    "vkbasalt-cli"
    "vkd3d"
    "vkd3d-proton"
    "vkdevicechooser"
    "vkdisplayinfo"
    "vkdt"
    "vkeybd"
    "vkmark"
    "vkquake"
    "vlagent"
    "vlan"
    "vlang"
    "vlc"
    "vlc-bin"
    "vlc-bin-universal"
    "vlc-bittorrent"
    "vllm"
    "vlock"
    "vm-curator"
    "vmagent"
    "vmaware"
    "vmctl"
    "vmime"
    "vmlinux-to-elf"
    "vmmlib"
    "vmpk"
    "vms-empire"
    "vmtouch"
    "vmware-workstation"
    "vncdo"
    "vncdotool"
    "vncrec"
    "vndr"
    "vnote"
    "vnstat"
    "vo-aacenc"
    "vo-amrwbenc"
    "vobcopy"
    "vobsub2srt"
    "vodozemac-bindings-cpp"
    "voicevox"
    "voicevox-core"
    "voicevox-engine"
    "voidmap"
    "voikko-fi"
    "vokoscreen-ng"
    "volanta"
    "volantes-cursors"
    "volatility2-bin"
    "volatility3"
    "volctl"
    "volk"
    "vollkorn"
    "volt"
    "volta"
    "volume_key"
    "volumeicon"
    "voms"
    "vopono"
    "vorbis-tools"
    "vorbisgain"
    "vors"
    "vorta"
    "vouch-proxy"
    "vowpal-wabbit"
    "voxinput"
    "voxtype"
    "voxtype-onnx"
    "voxtype-vulkan"
    "vp"
    "vpcs"
    "vpkedit"
    "vpl-gpu-rt"
    "vpn-slice"
    "vpnc"
    "vpnc-scripts"
    "vpp"
    "vprof"
    "vpsfree-client"
    "vpv"
    "vpxtool"
    "vrb"
    "vrc-get"
    "vrcadvert"
    "vrcvideocacher"
    "vrcx"
    "vrpn"
    "vrrtest"
    "vsc-leetcode-cli"
    "vsce"
    "vscode"
    "vscode-css-languageserver"
    "vscode-extension-update"
    "vscode-fhs"
    "vscode-html-languageserver"
    "vscode-js-debug"
    "vscode-json-languageserver"
    "vscode-langservers-extracted"
    "vscode-runner"
    "vscode-solidity-server"
    "vscode-with-extensions"
    "vscodium"
    "vscodium-fhs"
    "vsftpd"
    "vsh"
    "vsmartcard-pcsc-relay"
    "vsmartcard-vpcd"
    "vsqlite"
    "vst2-sdk"
    "vt-cli"
    "vt323"
    "vtable-dumper"
    "vte"
    "vte-gtk4"
    "vtebench"
    "vtfedit"
    "vtk"
    "vtk-dicom"
    "vtk-full"
    "vtkWithQt6"
    "vtk_9"
    "vtk_9_5"
    "vtk_9_6"
    "vtk_9_egl"
    "vtm"
    "vtracer"
    "vtsls"
    "vttest"
    "vue"
    "vue-language-server"
    "vuetorrent"
    "vul"
    "vulkan-caps-viewer"
    "vulkan-cts"
    "vulkan-extension-layer"
    "vulkan-hdr-layer-kwin6"
    "vulkan-headers"
    "vulkan-helper"
    "vulkan-loader"
    "vulkan-memory-allocator"
    "vulkan-tools"
    "vulkan-tools-lunarg"
    "vulkan-utility-libraries"
    "vulkan-validation-layers"
    "vulkan-volk"
    "vulkanscenegraph"
    "vulnix"
    "vulnx"
    "vuls"
    "vultisig-cli"
    "vultr"
    "vultr-cli"
    "vunnel"
    "vvenc"
    "vvvvvv"
    "vwifi"
    "vwsfriend"
    "vxl"
    "vxwm"
    "vym"
    "vyper"
    "vyxal"
    "vzic"
    "vzvm"
    "w3m"
    "w3m-batch"
    "w3m-full"
    "w3m-nographics"
    "w3m-nox"
    "w_scan2"
    "waagent"
    "wabt"
    "wac-cli"
    "wad"
    "waf"
    "waf-tester"
    "wafHook"
    "waffle"
    "wafw00f"
    "wagyu"
    "wahay"
    "wahjam"
    "waifu2x-converter-cpp"
    "wails"
    "wails3"
    "wait4x"
    "wakapi"
    "wakatime-cli"
    "wakelan"
    "wakeonlan"
    "wal-g"
    "wal-listener"
    "walk"
    "walker"
    "wall"
    "wallabag"
    "wallrizz"
    "walls"
    "wallust"
    "wallutils"
    "wally"
    "wally-cli"
    "wambo"
    "wamr"
    "wamrc"
    "wander"
    "wandio"
    "wangle"
    "wannier90"
    "wapiti"
    "wapm"
    "wappalyzergo"
    "warehouse"
    "wargus"
    "warmup-s3-archives"
    "warp"
    "warp-terminal"
    "warpd"
    "warpgate"
    "warpinator"
    "warzone2100"
    "wasabibackend"
    "wasabiwallet"
    "wash-cli"
    "wasilibc"
    "wasm"
    "wasm-bindgen-cli"
    "wasm-bindgen-cli_0_2_100"
    "wasm-bindgen-cli_0_2_104"
    "wasm-bindgen-cli_0_2_105"
    "wasm-bindgen-cli_0_2_106"
    "wasm-bindgen-cli_0_2_108"
    "wasm-bindgen-cli_0_2_114"
    "wasm-bindgen-cli_0_2_117"
    "wasm-bindgen-cli_0_2_118"
    "wasm-bindgen-cli_0_2_120"
    "wasm-bindgen-cli_0_2_121"
    "wasm-bindgen-cli_0_2_122"
    "wasm-bindgen-cli_0_2_125"
    "wasm-bindgen-cli_0_2_126"
    "wasm-bindgen-cli_0_2_127"
    "wasm-bindgen-cli_0_2_93"
    "wasm-bindgen-cli_0_2_95"
    "wasm-bindgen-cli_0_2_99"
    "wasm-component-ld"
    "wasm-language-tools"
    "wasm-pack"
    "wasm-text-gen"
    "wasm-tools"
    "wasmedge"
    "wasmer"
    "wasmer-pack"
    "wasmi"
    "wasmserve"
    "wasmtime"
    "wasmtime_36"
    "wast-refmt"
    "wastebin"
    "watch"
    "watchdog"
    "watchdogd"
    "watcher"
    "watcherclient"
    "watchexec"
    "watchgha"
    "watchlog"
    "watchman"
    "watchmate"
    "watchyourlan"
    "waterfurnace_aurora"
    "watson"
    "watson-ruby"
    "watt"
    "wavefunctioncollapse"
    "wavegain"
    "wavelib"
    "wavelog"
    "wavemon"
    "waves"
    "waveterm"
    "wavpack"
    "wavrsocvt"
    "way-displays"
    "way-edges"
    "wayback"
    "wayback-machine-archiver"
    "wayback-x11"
    "wayback_machine_downloader"
    "waybackurls"
    "waybar"
    "waybar-lyric"
    "waybar-mpris"
    "waybox"
    "waycheck"
    "wayclip"
    "waycorner"
    "waydroid"
    "waydroid-helper"
    "waydroid-nftables"
    "wayfarer"
    "wayfire"
    "wayfire-with-plugins"
    "wayfreeze"
    "wayidle"
    "wayland"
    "wayland-bongocat"
    "wayland-colorbar"
    "wayland-logout"
    "wayland-pipewire-idle-inhibit"
    "wayland-protocols"
    "wayland-proxy-virtwl"
    "wayland-scanner"
    "wayland-utils"
    "waylandpp"
    "wayle"
    "waylevel"
    "waylock"
    "waylogout"
    "waylyrics"
    "waymore"
    "wayneko"
    "waynergy"
    "wayout"
    "waypaper"
    "waypaper-engine"
    "waypipe"
    "wayprompt"
    "wayscriber"
    "wayshot"
    "wayst"
    "waytrogen"
    "wayvnc"
    "wayvr"
    "waywall"
    "wazero"
    "wb32-dfu-updater"
    "wbg"
    "wbox"
    "wcalc"
    "wcc"
    "wch-isp"
    "wchisp"
    "wcpg"
    "wcslib"
    "wdiff"
    "wdisplays"
    "wdt"
    "wealthfolio"
    "wealthfolio-server"
    "weasis"
    "weather"
    "weather-icons"
    "weathr"
    "weave"
    "weave-gitops"
    "weaver"
    "weaviate"
    "web-archives"
    "web-eid-app"
    "web-ext"
    "webalizer"
    "webanalyze"
    "webassemblyjs-cli"
    "webassemblyjs-repl"
    "webcamize"
    "webcamoid"
    "webcat"
    "webcord"
    "webcord-vencord"
    "webdav"
    "webdav-server-rs"
    "webdev"
    "webdis"
    "webdump"
    "webeep-sync"
    "webex"
    "webfont-bundler"
    "webfontkitgenerator"
    "webfs"
    "webhook"
    "webkitgtk_4_1"
    "webkitgtk_6_0"
    "weblate"
    "webmesh"
    "webp-pixbuf-loader"
    "webpack-cli"
    "webrtc-audio-processing"
    "webrtc-audio-processing_0_3"
    "webrtc-audio-processing_1"
    "websecprobe"
    "websocat"
    "websocketd"
    "websocketpp"
    "websploit"
    "webssh"
    "websurfx"
    "webtorrent_desktop"
    "webtunnel"
    "webull-desktop"
    "webwormhole"
    "wechat"
    "wechat-uos"
    "weechat"
    "weechat-matrix-rs"
    "weechat-unwrapped"
    "weevely"
    "weggli"
    "wego"
    "weidu"
    "weka"
    "welle-io"
    "wemeet"
    "wemux"
    "wepwolf"
    "werf"
    "wesher"
    "wesnoth"
    "wesnoth-devel"
    "weston"
    "westonLite"
    "wev"
    "weylus"
    "wezterm"
    "wf-config"
    "wf-recorder"
    "wf-touch"
    "wfa2-lib"
    "wfuzz"
    "wfview"
    "wg-access-server"
    "wg-ddns"
    "wg-friendly-peer-names"
    "wg-netmanager"
    "wgautomesh"
    "wgcf"
    "wget"
    "wget2"
    "wgetpaste"
    "wgnord"
    "wgo"
    "wgpu-native"
    "wgpu-utils"
    "wgsl-analyzer"
    "whale"
    "whatcable"
    "whatfiles"
    "whatip"
    "whatsapp-chat-exporter"
    "whatsapp-electron"
    "whatsapp-emoji-font"
    "whatsapp-for-mac"
    "whatsie"
    "whatstyle"
    "whatweb"
    "wheelwizard"
    "when"
    "when-cli"
    "where-is-my-sddm-theme"
    "whereami"
    "which"
    "whichllm"
    "whipper"
    "whisky"
    "whisparr"
    "whisper-cpp"
    "whisper-cpp-vulkan"
    "whisper-ctranslate2"
    "whispers"
    "whisperx"
    "whistle"
    "whitebophir"
    "whitebox-tools"
    "whitesur-cursors"
    "whitesur-gtk-theme"
    "whitesur-icon-theme"
    "whitesur-kde"
    "whoami"
    "whois"
    "whoogle-search"
    "whosthere"
    "whsniff"
    "why3"
    "why3find"
    "wibo"
    "widelands"
    "wideriver"
    "widevine-cdm"
    "wifi-qr"
    "wifish"
    "wifite2"
    "wifitui"
    "wiggle"
    "wiiload"
    "wiimms-iso-tools"
    "wiiuse"
    "wike"
    "wiki-go"
    "wiki-js"
    "wiki-tui"
    "wikicurses"
    "wikiman"
    "wikit"
    "wild-unwrapped"
    "wildcard"
    "wildergarden-maim"
    "wildmidi"
    "wiliwili"
    "wily"
    "wimboot"
    "wimlib"
    "win-disk-writer"
    "win-spice"
    "win2xcur"
    "winboat"
    "winbox"
    "winbox3"
    "winbox4"
    "windmill"
    "windowchef"
    "windowlab"
    "windowmaker"
    "windows10-icons"
    "windowtolayer"
    "windsend"
    "windsend-rs"
    "windsurf"
    "windterm"
    "wine"
    "wine-discord-ipc-bridge"
    "wine-staging"
    "wine-wayland"
    "wine64"
    "wineasio"
    "winetricks"
    "wingpanel-indicator-namarupa"
    "wings"
    "wio"
    "wipe"
    "wipefreespace"
    "wipeout-rewrite"
    "wiper"
    "wire"
    "wire-desktop"
    "wired"
    "wiredtiger"
    "wireguard-go"
    "wireguard-tools"
    "wireguard-ui"
    "wireguard-vanity-keygen"
    "wireless-regdb"
    "wirelesstools"
    "wiremix"
    "wiremock"
    "wireplumber"
    "wireproxy"
    "wireshark"
    "wireshark-cli"
    "wireshark-qt"
    "wireviz"
    "wireworld"
    "wiringpi"
    "wiseunpacker"
    "wishbone-tool"
    "wishlist"
    "wit-bindgen"
    "with-shell"
    "within"
    "witness"
    "witnessme"
    "witr"
    "wivrn"
    "wizer"
    "wkg"
    "wkhtmltopdf"
    "wl-clicker"
    "wl-clip-persist"
    "wl-clipboard"
    "wl-clipboard-rs"
    "wl-clipboard-x11"
    "wl-color-picker"
    "wl-crosshair"
    "wl-find-cursor"
    "wl-freeze"
    "wl-gammactl"
    "wl-gammarelay-applet"
    "wl-gammarelay-rs"
    "wl-kbptr"
    "wl-mirror"
    "wl-proxy"
    "wl-restart"
    "wl-screenrec"
    "wl-tray-bridge"
    "wl-vapi-gen"
    "wla-dx"
    "wlay"
    "wlc"
    "wlclock"
    "wlcs"
    "wldash"
    "wleave"
    "wlgreet"
    "wlinhibit"
    "wlink"
    "wljoywake"
    "wllvm"
    "wlock"
    "wlogout"
    "wlopm"
    "wlprop"
    "wlr-layout-ui"
    "wlr-protocols"
    "wlr-randr"
    "wlr-utils"
    "wlr-which-key"
    "wlrctl"
    "wlroots"
    "wlroots_0_19"
    "wlroots_0_20"
    "wlsunset"
    "wluma"
    "wlvncc"
    "wmbusmeters"
    "wmctrl"
    "wmderland"
    "wmderlandc"
    "wmenu"
    "wmfocus"
    "wmfs"
    "wmic-bin"
    "wmii"
    "wml"
    "wmname"
    "wmutils-core"
    "wmutils-libwm"
    "wmutils-opt"
    "wob"
    "woeusb"
    "woeusb-ng"
    "woff2"
    "wofi"
    "wofi-emoji"
    "wofi-pass"
    "wofi-power-menu"
    "woke"
    "wol"
    "wolf-shaper"
    "wolfebin"
    "wolfram-engine"
    "wolfram-notebook"
    "wolfstoneextract"
    "wonderbrushed"
    "wonderdraft"
    "woodpecker-agent"
    "woodpecker-cli"
    "woodpecker-pipeline-transform"
    "woodpecker-plugin-git"
    "woodpecker-server"
    "woof-doom"
    "woomer"
    "wootility"
    "wooting-udev-rules"
    "wooz"
    "wordbook"
    "wordgrinder"
    "wordlists"
    "wordnet"
    "wordpress"
    "wordpress_6_8"
    "wordpress_6_9"
    "wordpress_7_0"
    "work-sans"
    "workcraft"
    "worker"
    "worker-build"
    "workout-tracker"
    "workrave"
    "workset"
    "workshop-runner"
    "workstyle"
    "worktrunk"
    "worktrunk-sync"
    "world-serpant-search"
    "world-wall-clock"
    "worldofgoo"
    "worldpainter"
    "wormhole-rs"
    "wormhole-william"
    "wownero"
    "wowup-cf"
    "wox"
    "woxi"
    "wp-cli"
    "wp-scanner"
    "wp4nix"
    "wpa_supplicant"
    "wpa_supplicant_gui"
    "wpaperd"
    "wpawolf"
    "wpcleaner"
    "wpgtk"
    "wpm"
    "wpprobe"
    "wprecon"
    "wprs"
    "wpscan"
    "wpsoffice"
    "wpsoffice-cn"
    "wqy_microhei"
    "wqy_zenhei"
    "wraith"
    "wrangler"
    "wrap"
    "wrapGAppsHook3"
    "wrapGAppsHook4"
    "wrapGAppsNoGuiHook"
    "wrapGNUstepAppsHook"
    "wrapVdr"
    "wrapWithXFileSearchPathHook"
    "writableTmpDirAsHomeHook"
    "write-good"
    "writeDarwinBundle"
    "writedisk"
    "writefreely"
    "wrk"
    "wrk2"
    "wrkflw"
    "wrtag"
    "wsdd"
    "wshowkeys"
    "wsjtx"
    "wsjtz"
    "wsl-open"
    "wsl-vpnkit"
    "wsl2-ssh-agent"
    "wslay"
    "wsmancli"
    "wsrepl"
    "wstsound"
    "wstunnel"
    "wsysmon"
    "wt"
    "wt4"
    "wtcat"
    "wtf"
    "wtfis"
    "wtfutil"
    "wthrr"
    "wtk"
    "wtp"
    "wttrbar"
    "wtwitch"
    "wtype"
    "wuffs"
    "wush"
    "wuzz"
    "wv"
    "wv2"
    "wvdial"
    "wvkbd"
    "wvstreams"
    "wwcd"
    "wxGTK31"
    "wxGTK32"
    "wxGTK33"
    "wxSVG"
    "wxc"
    "wxedid"
    "wxformbuilder"
    "wxhexeditor"
    "wxmacmolplt"
    "wxmaxima"
    "wxsqlite3"
    "wxsqliteplus"
    "wxsvg"
    "wxwidgets_3_1"
    "wxwidgets_3_2"
    "wxwidgets_3_3"
    "wyoming-faster-whisper"
    "wyoming-openwakeword"
    "wyoming-piper"
    "wyoming-satellite"
    "wyrd"
    "wyvern"
    "x-create-mouse-void"
    "x11_ssh_askpass"
    "x11basic"
    "x11docker"
    "x11perf"
    "x11spice"
    "x11vnc"
    "x16"
    "x16-emulator"
    "x16-rom"
    "x16-run"
    "x264"
    "x265"
    "x2goclient"
    "x2gokdriveclient"
    "x2goserver"
    "x2vnc"
    "x2x"
    "x3270"
    "x32edit"
    "x4"
    "x42-avldrums"
    "x42-gmsynth"
    "x42-plugins"
    "x86info"
    "xa"
    "xacro"
    "xacrodoc"
    "xadrian"
    "xalanc"
    "xan"
    "xandikos"
    "xannotate"
    "xaos"
    "xapian"
    "xapian-omega"
    "xapian_1_4"
    "xapp"
    "xapp-symbolic-icons"
    "xapp-thumbnailers"
    "xar"
    "xarMinimal"
    "xarcan"
    "xarchiver"
    "xash-dedicated"
    "xash3d-fwgs"
    "xastir"
    "xauth"
    "xautocfg"
    "xautoclick"
    "xautolock"
    "xautomation"
    "xavs"
    "xavs2"
    "xawtv"
    "xbacklight"
    "xbanish"
    "xbar"
    "xbase"
    "xbattbar"
    "xbill"
    "xbindkeys"
    "xbitmaps"
    "xboard"
    "xbomb"
    "xborders"
    "xbps"
    "xbyak"
    "xc"
    "xc3sprog"
    "xca"
    "xcaddy"
    "xcalc"
    "xcalib"
    "xcape"
    "xcat"
    "xcb-imdkit"
    "xcb-proto"
    "xcb-util-cursor"
    "xcb-util-cursor-HEAD"
    "xcbeautify"
    "xcbproto"
    "xcbuild"
    "xcbuildHook"
    "xcbutil"
    "xcbutilcursor"
    "xcbutilerrors"
    "xcbutilimage"
    "xcbutilkeysyms"
    "xcbutilrenderutil"
    "xcbutilwm"
    "xcbutilxrm"
    "xcd"
    "xcftools"
    "xcfun"
    "xchainkeys"
    "xchm"
    "xcircuit"
    "xclicker"
    "xclip"
    "xclock"
    "xcmsdb"
    "xcode-install"
    "xcodebuild"
    "xcodegen"
    "xcodes"
    "xcolor"
    "xcompmgr"
    "xcompose"
    "xconq"
    "xconsole"
    "xcowsay"
    "xcp"
    "xcpc"
    "xcpretty"
    "xcrawl3r"
    "xcruiser"
    "xcur2png"
    "xcursor-pro"
    "xcursor-themes"
    "xcursor-viewer"
    "xcursorgen"
    "xcursorthemes"
    "xcwd"
    "xd"
    "xdaliclock"
    "xdcc-cli"
    "xdelta"
    "xdg-dbus-proxy"
    "xdg-desktop-portal"
    "xdg-desktop-portal-cosmic"
    "xdg-desktop-portal-generic"
    "xdg-desktop-portal-gnome"
    "xdg-desktop-portal-gtk"
    "xdg-desktop-portal-hyprland"
    "xdg-desktop-portal-luminous"
    "xdg-desktop-portal-phosh"
    "xdg-desktop-portal-shana"
    "xdg-desktop-portal-termfilechooser"
    "xdg-desktop-portal-umbriel"
    "xdg-desktop-portal-wlr"
    "xdg-desktop-portal-xapp"
    "xdg-launch"
    "xdg-ninja"
    "xdg-terminal-exec"
    "xdg-terminal-exec-mkhl"
    "xdg-user-dirs"
    "xdg-user-dirs-gtk"
    "xdg-utils"
    "xdg-utils-cxx"
    "xdgcpp"
    "xdgmenumaker"
    "xdiskusage"
    "xdm"
    "xdo"
    "xdot"
    "xdotool"
    "xdp-tools"
    "xdpyinfo"
    "xdriinfo"
    "xdummy"
    "xdvdfs-cli"
    "xdxf2slob"
    "xe"
    "xe-guest-utilities"
    "xearth"
    "xed"
    "xed-editor"
    "xedit"
    "xee"
    "xemu"
    "xen"
    "xen-guest-agent"
    "xenia-canary"
    "xenomapper"
    "xenon"
    "xeol"
    "xephem"
    "xercesc"
    "xeus"
    "xeus-cpp"
    "xeus-zmq"
    "xev"
    "xevd"
    "xeve"
    "xeyes"
    "xf86-input-evdev"
    "xf86-input-joystick"
    "xf86-input-keyboard"
    "xf86-input-libinput"
    "xf86-input-mouse"
    "xf86-input-synaptics"
    "xf86-input-vmmouse"
    "xf86-input-void"
    "xf86-input-wacom"
    "xf86-video-amdgpu"
    "xf86-video-apm"
    "xf86-video-ark"
    "xf86-video-ast"
    "xf86-video-ati"
    "xf86-video-chips"
    "xf86-video-cirrus"
    "xf86-video-dummy"
    "xf86-video-fbdev"
    "xf86-video-geode"
    "xf86-video-i128"
    "xf86-video-i740"
    "xf86-video-intel"
    "xf86-video-mga"
    "xf86-video-neomagic"
    "xf86-video-nested"
    "xf86-video-nouveau"
    "xf86-video-nv"
    "xf86-video-omap"
    "xf86-video-openchrome"
    "xf86-video-qxl"
    "xf86-video-r128"
    "xf86-video-s3virge"
    "xf86-video-savage"
    "xf86-video-siliconmotion"
    "xf86-video-sis"
    "xf86-video-sisusb"
    "xf86-video-suncg6"
    "xf86-video-sunffb"
    "xf86-video-sunleo"
    "xf86-video-tdfx"
    "xf86-video-trident"
    "xf86-video-v4l"
    "xf86-video-vbox"
    "xf86-video-vesa"
    "xf86-video-vmware"
    "xf86-video-voodoo"
    "xf86_input_wacom"
    "xf86_video_nested"
    "xf86inputevdev"
    "xf86inputjoystick"
    "xf86inputkeyboard"
    "xf86inputlibinput"
    "xf86inputmouse"
    "xf86inputsynaptics"
    "xf86inputvmmouse"
    "xf86inputvoid"
    "xf86videoamdgpu"
    "xf86videoapm"
    "xf86videoark"
    "xf86videoast"
    "xf86videoati"
    "xf86videochips"
    "xf86videocirrus"
    "xf86videodummy"
    "xf86videofbdev"
    "xf86videogeode"
    "xf86videoi128"
    "xf86videoi740"
    "xf86videointel"
    "xf86videomga"
    "xf86videoneomagic"
    "xf86videonouveau"
    "xf86videonv"
    "xf86videoomap"
    "xf86videoopenchrome"
    "xf86videoqxl"
    "xf86videor128"
    "xf86videos3virge"
    "xf86videosavage"
    "xf86videosiliconmotion"
    "xf86videosis"
    "xf86videosisusb"
    "xf86videosuncg6"
    "xf86videosunffb"
    "xf86videosunleo"
    "xf86videotdfx"
    "xf86videotrident"
    "xf86videov4l"
    "xf86videovboxvideo"
    "xf86videovesa"
    "xf86videovmware"
    "xf86videovoodoo"
    "xfburn"
    "xfce4-alsa-plugin"
    "xfce4-appfinder"
    "xfce4-battery-plugin"
    "xfce4-clipman-plugin"
    "xfce4-cpufreq-plugin"
    "xfce4-cpugraph-plugin"
    "xfce4-dev-tools"
    "xfce4-dict"
    "xfce4-dockbarx-plugin"
    "xfce4-docklike-plugin"
    "xfce4-exo"
    "xfce4-eyes-plugin"
    "xfce4-fsguard-plugin"
    "xfce4-genmon-plugin"
    "xfce4-i3-workspaces-plugin"
    "xfce4-icon-theme"
    "xfce4-mailwatch-plugin"
    "xfce4-mpc-plugin"
    "xfce4-netload-plugin"
    "xfce4-notes-plugin"
    "xfce4-notifyd"
    "xfce4-panel"
    "xfce4-panel-profiles"
    "xfce4-power-manager"
    "xfce4-pulseaudio-plugin"
    "xfce4-screensaver"
    "xfce4-screenshooter"
    "xfce4-sensors-plugin"
    "xfce4-session"
    "xfce4-settings"
    "xfce4-systemload-plugin"
    "xfce4-taskmanager"
    "xfce4-terminal"
    "xfce4-time-out-plugin"
    "xfce4-timer-plugin"
    "xfce4-verve-plugin"
    "xfce4-volumed-pulse"
    "xfce4-weather-plugin"
    "xfce4-whiskermenu-plugin"
    "xfce4-windowck-plugin"
    "xfce4-xkb-plugin"
    "xfconf"
    "xfd"
    "xfdashboard"
    "xfdesktop"
    "xfe"
    "xfel"
    "xfig"
    "xfitter"
    "xflr5"
    "xfmpc"
    "xfontsel"
    "xfr"
    "xfractint"
    "xfs"
    "xfs-undelete"
    "xfsdump"
    "xfsinfo"
    "xfsprogs"
    "xfstests"
    "xfwm4"
    "xfwm4-themes"
    "xgalagapp"
    "xgamma"
    "xgboost"
    "xgboostWithCuda"
    "xgc"
    "xgeometry-select"
    "xh"
    "xhost"
    "xhosts"
    "xhyve"
    "xib2nib"
    "xiccd"
    "xidel"
    "xidlehook"
    "xilinx-bootgen"
    "xine-lib"
    "xine-ui"
    "xinetd"
    "xinit"
    "xinput"
    "xinput-calibrator"
    "xinput_calibrator"
    "xiphos"
    "xits-math"
    "xiu"
    "xivlauncher"
    "xjadeo"
    "xjobs"
    "xk6"
    "xkb-switch"
    "xkb-switch-i3"
    "xkbcomp"
    "xkbd"
    "xkbevd"
    "xkblayout-state"
    "xkbmon"
    "xkbprint"
    "xkbset"
    "xkbutils"
    "xkbvalidate"
    "xkcd-font"
    "xkcdpass"
    "xkeyboard-config"
    "xkeyboard_config"
    "xkeyboardconfig"
    "xkeysnail"
    "xkill"
    "xl2tpd"
    "xla"
    "xlayoutdisplay"
    "xld"
    "xleak"
    "xlibinput-calibrator"
    "xlife"
    "xlights"
    "xlink"
    "xload"
    "xlockmore"
    "xlog"
    "xlogo"
    "xlsatoms"
    "xlsclients"
    "xlsfonts"
    "xlslib"
    "xlsx2csv"
    "xlsxsql"
    "xmacro"
    "xmag"
    "xmage"
    "xmagnify"
    "xmake"
    "xmcp"
    "xmedcon"
    "xmenu"
    "xmessage"
    "xmind"
    "xml-security-c"
    "xml-tooling-c"
    "xml2"
    "xml2rfc"
    "xmlbeans"
    "xmlbird"
    "xmlcopyeditor"
    "xmldiff"
    "xmlformat"
    "xmlindent"
    "xmloscopy"
    "xmlrpc_c"
    "xmlsec"
    "xmlsort"
    "xmlstarlet"
    "xmlto"
    "xmltoman"
    "xmobar"
    "xmodmap"
    "xmoji"
    "xmonad-log"
    "xmonad-log-applet"
    "xmonad-log-applet-mate"
    "xmonad-log-applet-xfce"
    "xmonad-with-packages"
    "xmonad_log_applet"
    "xmonad_log_applet_mate"
    "xmonad_log_applet_xfce"
    "xmonadctl"
    "xmore"
    "xmoto"
    "xmountains"
    "xmousepasteblock"
    "xmp"
    "xmpp-dns"
    "xmppc"
    "xmrig"
    "xmrig-cuda"
    "xmrig-cuda-mo"
    "xmrig-mo"
    "xmrig-proxy"
    "xnbd"
    "xnconvert"
    "xnec2c"
    "xnee"
    "xnldorker"
    "xnlinkfinder"
    "xnotify"
    "xnviewmp"
    "xob"
    "xone-dongle-firmware"
    "xonotic"
    "xonotic-data"
    "xonotic-dedicated"
    "xonotic-dedicated-unwrapped"
    "xonotic-glx"
    "xonotic-glx-unwrapped"
    "xonotic-sdl"
    "xonotic-sdl-unwrapped"
    "xonsh"
    "xorex"
    "xorg-autoconf"
    "xorg-cf-files"
    "xorg-docs"
    "xorg-rgb"
    "xorg-server"
    "xorg-sgml-doctools"
    "xorg_sys_opengl"
    "xorgcffiles"
    "xorgdocs"
    "xorgproto"
    "xorgserver"
    "xorgsgmldoctools"
    "xorriso"
    "xortool"
    "xoscope"
    "xosd"
    "xosd-xft"
    "xosview"
    "xosview2"
    "xournalpp"
    "xp-pen-deco-01-v2-driver"
    "xp-pen-g430-driver"
    "xpad"
    "xpano"
    "xpar"
    "xpaste"
    "xpdf"
    "xpilot-ng"
    "xpipe"
    "xplanet"
    "xplr"
    "xplugd"
    "xpointerbarrier"
    "xppen_3"
    "xppen_4"
    "xpr"
    "xpra"
    "xpra-html5"
    "xpraWithNvenc"
    "xprin"
    "xprintidle"
    "xprintidle-ng"
    "xprompt"
    "xprop"
    "xpwn"
    "xq"
    "xq-xml"
    "xqilla"
    "xquartz"
    "xr-chaperone"
    "xr-hardware"
    "xrandr"
    "xrandr-invert-colors"
    "xray"
    "xrdb"
    "xrdp"
    "xre"
    "xreader"
    "xrectsel"
    "xrefresh"
    "xremap"
    "xrestop"
    "xrgears"
    "xrizer"
    "xroar"
    "xrootd"
    "xrq"
    "xrsh"
    "xsane"
    "xscast"
    "xschem"
    "xscope"
    "xscreenruler"
    "xscreensaver"
    "xsct"
    "xsecurelock"
    "xsel"
    "xsensors"
    "xset"
    "xsetroot"
    "xsettingsd"
    "xsimd"
    "xskat"
    "xsm"
    "xsnow"
    "xsok"
    "xsokoban"
    "xsos"
    "xspim"
    "xss-lock"
    "xssearch"
    "xsser"
    "xssproxy"
    "xssstate"
    "xsstrike"
    "xst"
    "xstdcmap"
    "xstow"
    "xsubfind3r"
    "xsuspender"
    "xtb"
    "xteddy"
    "xtensor"
    "xtensor-blas"
    "xterm"
    "xtermcontrol"
    "xteve"
    "xtf"
    "xtitle"
    "xtl"
    "xtrace"
    "xtrans"
    "xtris"
    "xtruss"
    "xtuner"
    "xunit-viewer"
    "xunlei-uos"
    "xurlfind3r"
    "xurls"
    "xv"
    "xva-img"
    "xvfb"
    "xvfb-run"
    "xvidcore"
    "xviewer"
    "xvinfo"
    "xvkbd"
    "xwallpaper"
    "xwax"
    "xwayland"
    "xwayland-run"
    "xwayland-satellite"
    "xwd"
    "xwiimote"
    "xwin"
    "xwininfo"
    "xwinwrap"
    "xwud"
    "xxHash"
    "xxd"
    "xxdiff"
    "xxdiff-tip"
    "xxe-pe"
    "xxh"
    "xxhash"
    "xxkb"
    "xyce"
    "xyce-parallel"
    "xygrib"
    "xylib"
    "xzoom"
    "y-cruncher"
    "yaak"
    "yabai"
    "yabasic"
    "yabause"
    "yabridge"
    "yabridgectl"
    "yacas"
    "yacreader"
    "yad"
    "yadm"
    "yadmMinimal"
    "yaegi"
    "yafc-ce"
    "yafetch"
    "yaffshiv"
    "yaft"
    "yai"
    "yajl"
    "yajsv"
    "yakut"
    "yalc"
    "yallback"
    "yamale"
    "yambar"
    "yambar-hyprland-wses"
    "yamdi"
    "yaml-cpp"
    "yaml-filter"
    "yaml-language-server"
    "yaml-merge"
    "yaml-schema-router"
    "yaml2json"
    "yaml2nix"
    "yamlfix"
    "yamlfmt"
    "yamllint"
    "yamlscript"
    "yams"
    "yandex-cloud"
    "yandex-disk"
    "yandex-music"
    "yaneuraou"
    "yanic"
    "yank"
    "yanone-kaffeesatz"
    "yaookctl"
    "yap"
    "yapesdl"
    "yapf"
    "yarGen"
    "yara"
    "yara-x"
    "yaralyzer"
    "yarg"
    "yargen"
    "yarn"
    "yarn-bash-completion"
    "yarn-berry"
    "yarn-berry_3"
    "yarn-berry_4"
    "yarn-lock-converter"
    "yarn-zpm"
    "yarnBuildHook"
    "yarnConfigHook"
    "yarnInstallHook"
    "yarp"
    "yarr"
    "yaru-remix-theme"
    "yaru-theme"
    "yas"
    "yasashisa-gothic"
    "yascreen"
    "yash"
    "yashiki"
    "yasm"
    "yasr"
    "yatas"
    "yate"
    "yatto"
    "yaup"
    "yaws"
    "yaxg"
    "yay"
    "yaydl"
    "yazi"
    "yazi-unwrapped"
    "yb"
    "ycm-cmake-modules"
    "ycmd"
    "ycwd"
    "yder"
    "ydict"
    "ydiff"
    "ydotool"
    "yeahconsole"
    "yed"
    "yeetgif"
    "yek"
    "yelp"
    "yelp-tools"
    "yelp-xsl"
    "yersinia"
    "yeshup"
    "yeswiki"
    "yet-another-cloudwatch-exporter"
    "yetris"
    "yew-fmt"
    "yewtube"
    "yex-lang"
    "yffi"
    "yggdrasil"
    "yggdrasil-jumper"
    "yggdrasil-ng"
    "yggstack"
    "ygot"
    "yi"
    "yices"
    "yj"
    "ykush"
    "yle-dl"
    "ympd"
    "ymuse"
    "ynetd"
    "yo"
    "yocto-cooker"
    "yoda"
    "yoda-with-root"
    "yodl"
    "yofi"
    "yoink"
    "yokadi"
    "yopass"
    "yor"
    "yoshimi"
    "yosys"
    "yosys-bluespec"
    "yosys-ghdl"
    "you-get"
    "you-have-mail-cli"
    "youki"
    "youplot"
    "your-editor"
    "your_spotify"
    "yourkit-java"
    "youtrack"
    "youtube-dl"
    "youtube-dl-light"
    "youtube-music"
    "youtube-tui"
    "youtube-viewer"
    "youtubedr"
    "youtubeuploader"
    "ypbind-mt"
    "yq"
    "yq-go"
    "yquake2"
    "yquake2-all-games"
    "yquake2-ctf"
    "yquake2-ground-zero"
    "yquake2-the-reckoning"
    "yscan"
    "ysfx"
    "yt-dlg"
    "yt-dlp"
    "yt-dlp-light"
    "ytalk"
    "ytarchive"
    "ytcast"
    "ytcc"
    "ytdl-sub"
    "ytdownloader"
    "ytermusic"
    "ytfzf"
    "ytmdesktop"
    "ytmdl"
    "ytree"
    "ytsub"
    "ytt"
    "ytui-music"
    "yubico-pam"
    "yubico-piv-tool"
    "yubihsm-connector"
    "yubihsm-setup"
    "yubihsm-shell"
    "yubikey-agent"
    "yubikey-manager"
    "yubikey-personalization"
    "yubikey-touch-detector"
    "yubioath-flutter"
    "yubiswitch"
    "yudit"
    "yuhaiin"
    "yuicompressor"
    "yunfaavatar"
    "yutto"
    "yuview"
    "yx"
    "yyjson"
    "z-library-desktop"
    "z-lua"
    "z3"
    "z3-tptp"
    "z64decompress"
    "z88dk"
    "zabbix-agent2-plugin-postgresql"
    "zabbix-cli"
    "zabbixctl"
    "zafiro-icons"
    "zalgo"
    "zam-plugins"
    "zammad"
    "zandronum"
    "zandronum-server"
    "zap"
    "zap-chip"
    "zaparoo"
    "zapp"
    "zapper"
    "zapret"
    "zapret2"
    "zapzap"
    "zarchive"
    "zarf"
    "zashboard"
    "zasm"
    "zat"
    "zatackax"
    "zathura"
    "zaz"
    "zbar"
    "zbctl"
    "zbus-xmlgen"
    "zcash"
    "zcfan"
    "zchaff"
    "zchunk"
    "zcool-qingke-huangyou"
    "zd1211fw"
    "zdbsp"
    "zdelta"
    "zdns"
    "zeal"
    "zeal-qt5"
    "zeal-qt6"
    "zecwallet-lite"
    "zed-discord-presence"
    "zed-editor"
    "zed-editor-fhs"
    "zed-open-capture"
    "zed-wakatime-ls"
    "zee"
    "zeek"
    "zeekscript"
    "zeekstd"
    "zef"
    "zegrapher"
    "zeitgeist"
    "zelda64recomp"
    "zellij"
    "zellij-unwrapped"
    "zenith"
    "zenith-nvidia"
    "zenity"
    "zenmap"
    "zenmonitor"
    "zenn-cli"
    "zennotes-desktop"
    "zeno"
    "zenoh"
    "zenoh-backend-filesystem"
    "zenoh-backend-influxdb"
    "zenoh-backend-rocksdb"
    "zenoh-c"
    "zenoh-cpp"
    "zenoh-plugin-dds"
    "zenoh-plugin-mqtt"
    "zenoh-plugin-webserver"
    "zenroom"
    "zensical"
    "zenstates"
    "zepp-simulator"
    "zeroad"
    "zeroad-data"
    "zeroad-unwrapped"
    "zerobin"
    "zeroc-ice"
    "zeroc-ice-cpp11"
    "zeroclaw"
    "zerofree"
    "zerofs"
    "zeromq"
    "zeronet-conservancy"
    "zeronsd"
    "zerotierone"
    "zesarux"
    "zet"
    "zettlr"
    "zexy"
    "zeyple"
    "zf"
    "zfind"
    "zfp"
    "zfs"
    "zfs-autobackup"
    "zfs-prune-snapshots"
    "zfs-replicate"
    "zfs_2_3"
    "zfs_2_4"
    "zfs_unstable"
    "zfsbackup"
    "zfsnap"
    "zfstools"
    "zfxtop"
    "zgrab2"
    "zgrviewer"
    "zgv"
    "zi"
    "zig"
    "zig-shell-completions"
    "zig-zlint"
    "zig_0_13"
    "zig_0_14"
    "zig_0_15"
    "zig_0_16"
    "zigbee2mqtt"
    "zigbee2mqtt_2"
    "zigdoc"
    "zigfetch"
    "zigimports"
    "ziglint"
    "zigpy-cli"
    "zile"
    "zilla-slab"
    "zim"
    "zim-tools"
    "zimfw"
    "zimg"
    "zincsearch"
    "zine"
    "zinit"
    "zinnia"
    "zint-qt"
    "zip"
    "zip2hashcat"
    "zipkin"
    "zipline"
    "zipperposition"
    "zircolite"
    "zita-ajbridge"
    "zita-alsa-pcmi"
    "zita-at1"
    "zita-convolver"
    "zita-njbridge"
    "zita-resampler"
    "zitadel"
    "zitadel-tools"
    "zix"
    "zizmor"
    "zk"
    "zkar"
    "zktree"
    "zlcompressor"
    "zld"
    "zlequalizer"
    "zlib"
    "zlib-ng"
    "zlint"
    "zlog"
    "zls"
    "zls_0_14"
    "zls_0_15"
    "zls_0_16"
    "zlsplitter"
    "zluda"
    "zmap"
    "zmate"
    "zmk-studio"
    "zmkBATx"
    "zmkbatx"
    "zmqpp"
    "zmusic"
    "zmx"
    "zn_poly"
    "znapzend"
    "znc"
    "zod"
    "zoekt"
    "zoho-mail-desktop"
    "zola"
    "zon2nix"
    "zonemaster-cli"
    "zoneminder"
    "zookeeper"
    "zookeeper_mt"
    "zoom"
    "zoom-us"
    "zopfli"
    "zoraxy"
    "zotero"
    "zotero-translation-server"
    "zoxide"
    "zpaq"
    "zpaqd"
    "zpaqfranz"
    "zpix-pixel-font"
    "zplug"
    "zpool-auto-expand-partitions"
    "zpool-iostat-viz"
    "zpp"
    "zprint"
    "zps"
    "zram-generator"
    "zrc"
    "zrepl"
    "zrok"
    "zrythm"
    "zs"
    "zs-apc-spdu-ctl"
    "zs-wait4host"
    "zsa-udev-rules"
    "zscaler-cacert"
    "zsh"
    "zsh-abbr"
    "zsh-artisan"
    "zsh-autocomplete"
    "zsh-autoenv"
    "zsh-autopair"
    "zsh-autosuggestions"
    "zsh-autosuggestions-abbreviations-strategy"
    "zsh-bd"
    "zsh-better-npm-completion"
    "zsh-clipboard"
    "zsh-command-time"
    "zsh-completion-sync"
    "zsh-completions"
    "zsh-defer"
    "zsh-edit"
    "zsh-f-sy-h"
    "zsh-fast-syntax-highlighting"
    "zsh-forgit"
    "zsh-fzf-history-search"
    "zsh-fzf-tab"
    "zsh-histdb"
    "zsh-history-search-multi-word"
    "zsh-history-substring-search"
    "zsh-history-to-fish"
    "zsh-navigation-tools"
    "zsh-nix-shell"
    "zsh-patina"
    "zsh-powerlevel10k"
    "zsh-powerlevel9k"
    "zsh-prezto"
    "zsh-syntax-highlighting"
    "zsh-system-clipboard"
    "zsh-vi-mode"
    "zsh-wd"
    "zsh-you-should-use"
    "zsh-z"
    "zsh-zhooks"
    "zsign"
    "zsnes"
    "zsnes2"
    "zsnow"
    "zssh"
    "zstd"
    "zsteg"
    "zstxtns-utils"
    "zsv"
    "zsync"
    "zsync2"
    "ztags"
    "zthrottle"
    "ztools"
    "zuban"
    "zug"
    "zulip"
    "zulip-term"
    "zulu"
    "zulu11"
    "zulu17"
    "zulu21"
    "zulu25"
    "zulu8"
    "zunclient"
    "zunit"
    "zuo"
    "zuse"
    "zutty"
    "zvbi"
    "zvm"
    "zwave-js-server"
    "zwave-js-ui"
    "zx"
    "zxcvbn-c"
    "zxfer"
    "zxing"
    "zxing-cpp"
    "zxpy"
    "zxtune"
    "zydis"
    "zynaddsubfx"
    "zynaddsubfx-fltk"
    "zynaddsubfx-ntk"
    "zziplib"
    "zztgo"
    "zzuf"
    "zzz"
  ];
  nixpkgsNarHash = "sha256-lj27EYvj0O6GayjoitFz0hrqeAlc0LCl04kkJrrGXVE=";
  nixpkgsRev = "2bf611227041f0f3727dd06500424437ef3976c9";
  pnames = {
    AMB-plugins = "amb-plugins";
    ArchiSteamFarm = "archisteamfarm";
    DisnixWebService = "disnix-web-service";
    Fabric = "fabric";
    LAStools = "lastools";
    LycheeSlicer = "lycheeslicer";
    MMA = "mma";
    OVMF-cloud-hypervisor = "OVMF";
    OVMFFull = "OVMF";
    QuadProgpp = "quadprogpp";
    SDL = "sdl12-compat";
    SDL2 = "sdl2-compat";
    SDL2_Pango = "sdl2-pango";
    SDL_compat = "sdl12-compat";
    Xaw3d = "libxaw3d";
    _010editor = "010editor";
    _0x = "0x";
    _0xffff = "0xFFFF";
    _0xpropo = "0xpropo";
    _0xproto = "0xproto";
    _1fps = "1fps";
    _1oom = "1oom";
    _1password-cli = "1password-cli";
    _1password-gui = "1password";
    _1password-gui-beta = "1password";
    _2048-in-terminal = "2048-in-terminal";
    _20kly = "20kly";
    _2bwm = "2bwm";
    _2ship2harkinian = "2ship2harkinian";
    _3270font = "3270font";
    _389-ds-base = "389-ds-base";
    _3beans = "3beans";
    _3cpio = "3cpio";
    _3llo = "3llo";
    _3mux = "3mux";
    _3proxy = "3proxy";
    _4d-minesweeper = "4d-minesweeper";
    _4th = "4th";
    _4ti2 = "4ti2";
    _64gram = "64gram";
    _64tass = "64tass";
    _6tunnel = "6tunnel";
    _7-segment-font = "7-segment-font";
    _7kaa = "7kaa";
    _7z2hashcat = "7z2hashcat";
    _7zip-zstd = "7zip-zstd";
    _7zip-zstd-rar = "7zip-zstd";
    _7zz = "7zz";
    _7zz-rar = "7zz";
    _81voltd = "81voltd";
    _86Box = "86box";
    _86Box-with-roms = "86box";
    _86box = "86box";
    _86box-with-roms = "86box";
    _90secondportraits = "90secondportraits";
    _915resolution = "915resolution";
    _9base = "9base";
    _9menu = "9menu";
    _9ptls = "9ptls";
    __flattenIncludeHackHook = "flatten-include-hack-hook";
    abseil-cpp_202103 = "abseil-cpp";
    abseil-cpp_202401 = "abseil-cpp";
    abseil-cpp_202407 = "abseil-cpp";
    abseil-cpp_202505 = "abseil-cpp";
    abseil-cpp_202508 = "abseil-cpp";
    abseil-cpp_202601 = "abseil-cpp";
    abseil-cpp_202605 = "abseil-cpp";
    abseil-cpp_202608 = "abseil-cpp";
    acl2-minimal = "acl2";
    acme-sh = "acme.sh";
    adaptivecppWithCuda = "adaptivecpp";
    adaptivecppWithRocm = "adaptivecpp";
    addBinToPathHook = "add-bin-to-path-hook";
    addDriverRunpath = "add-driver-runpath-hook";
    adwaita-qt6 = "adwaita-qt";
    aeolus-stops = "stops";
    aerospike = "aerospike-server";
    agda = "agdaWithPackages";
    agent-safehouse = "safehouse";
    ags_1 = "ags";
    aider-chat-full = "aider-chat-playwright-browser-help-bedrock";
    aider-chat-with-bedrock = "aider-chat-bedrock";
    aider-chat-with-browser = "aider-chat-browser";
    aider-chat-with-help = "aider-chat-help";
    aider-chat-with-playwright = "aider-chat-playwright";
    airtame = "airtame-application";
    akira-unstable = "akira";
    akkoma-admin-fe = "admin-fe";
    alan_2 = "alan2";
    albatross = "Albatross";
    alephone-apotheosis-x = "apotheosis-x";
    alephone-durandal = "durandal";
    alephone-eternal = "marathon-eternal";
    alephone-evil = "marathon-evil";
    alephone-infinity = "marathon-infinity";
    alephone-marathon = "marathon";
    alephone-pathways-into-darkness = "pathways-into-darkness";
    alephone-pheonix = "marathon-pheonix";
    alephone-red = "marathon-red";
    alephone-rubicon-x = "rubicon-x";
    alephone-yuge = "yuge";
    alfis-nogui = "alfis";
    alglib = "alglib3";
    allegro4 = "allegro";
    allegro5 = "allegro";
    alloy = "alloy5";
    alsa-lib-with-plugins = "alsa-lib";
    alttpr-opentracker = "opentracker";
    amarena-theme = "gtk-theme-framework";
    ammonite_2_12 = "ammonite";
    ammonite_2_13 = "ammonite";
    ammonite_3_3 = "ammonite";
    among-sus = "among-sus-unstable";
    android-studio-full = "android-studio";
    androidsdk = "android-sdk-cmdline-tools";
    angryipscanner = "ipscan";
    anonymousPro = "anonymous-pro-fonts";
    ansi = "ansi-escape-sequences-cli";
    ansible = "ansible-core";
    antigravity = "antigravity-ide";
    antigravity-fhs = "antigravity-ide";
    antigravity-ide-fhs = "antigravity-ide";
    antlr2 = "antlr";
    antlr3 = "antlr";
    antlr3_4 = "antlr";
    antlr3_5 = "antlr";
    antlr4 = "antlr";
    antlr4_10 = "antlr";
    antlr4_11 = "antlr";
    antlr4_12 = "antlr";
    antlr4_13 = "antlr";
    antlr4_9 = "antlr";
    antonio-font = "antonio";
    ants = "ANTs";
    anttweakbar = "AntTweakBar";
    apacheHttpd = "apache-httpd";
    apacheHttpd_2_4 = "apache-httpd";
    apacheKafka = "apache-kafka";
    apacheKafka_4_1 = "apache-kafka";
    apacheKafka_4_2 = "apache-kafka";
    apacheKafka_4_3 = "apache-kafka";
    apeClex = "ape-clex";
    apngasm_2 = "apngasm";
    apparmor-teardown = "apparmor-init";
    appimageupdate-qt = "appimageupdate";
    apple-cursor = "apple_cursor";
    apple-sdk_14 = "apple-sdk";
    apple-sdk_15 = "apple-sdk";
    apple-sdk_26 = "apple-sdk";
    apptainer-overriden-nixos = "apptainer";
    apriltag = "apriltags";
    aprutil = "apr-util";
    ardour_8 = "ardour";
    arduino-core = "arduino";
    arduino-core-unwrapped = "arduino-core";
    arduinoOTA = "arduino-ota";
    arion = "arion-compose";
    arkpandora_ttf = "arkpandora";
    armTrustedFirmwareAllwinner = "arm-trusted-firmware-sun50i_a64";
    armTrustedFirmwareAllwinnerH6 = "arm-trusted-firmware-sun50i_h6";
    armTrustedFirmwareAllwinnerH616 = "arm-trusted-firmware-sun50i_h616";
    armTrustedFirmwareQemu = "arm-trusted-firmware-qemu";
    armTrustedFirmwareRK3328 = "arm-trusted-firmware-rk3328";
    armTrustedFirmwareRK3399 = "arm-trusted-firmware-rk3399";
    armTrustedFirmwareRK3568 = "arm-trusted-firmware-rk3568";
    armTrustedFirmwareRK3588 = "arm-trusted-firmware-rk3588";
    armTrustedFirmwareS905 = "arm-trusted-firmware-gxbb";
    armTrustedFirmwareTools = "arm-trusted-firmware";
    artim-dark = "aritim-dark";
    artyFX = "openav-artyfx";
    asciidoctor-with-extensions = "asciidoctor";
    asciinema-agg = "agg";
    asciinema_3 = "asciinema";
    asdf_2_26 = "asdf";
    asdf_3_1 = "asdf";
    asdf_3_3 = "asdf";
    asio_1_32_0 = "asio";
    asio_1_36_0 = "asio";
    asterisk-lts = "asterisk";
    asterisk-stable = "asterisk";
    asterisk_20 = "asterisk";
    asterisk_22 = "asterisk";
    asterisk_23 = "asterisk";
    at-spi2-atk = "at-spi2-core";
    ataripp = "atari++";
    ath9k-htc-blobless-firmware-unstable = "ath9k-htc-blobless-firmware";
    atk = "at-spi2-core";
    atkmm_2_36 = "atkmm";
    atlassian-cli = "appfire-cli";
    attemptoClex = "attempto-clex";
    attic-client = "attic";
    attic-server = "attic";
    audacious-bare = "audacious";
    audacity_3 = "audacity";
    audacity_4 = "audacity";
    auditbeat7 = "auditbeat";
    authenticodeCheckHook = "authenticode-check-hook";
    autoAddDriverRunpath = "auto-add-driver-runpath-hook";
    autoFixElfFiles = "auto-fix-elf-files";
    autoPatchcilHook = "auto-patchcil-hook";
    autoPatchelfHook = "auto-patchelf-hook";
    autoconf269 = "autoconf";
    autofs5 = "autofs";
    automake116x = "automake";
    automake118x = "automake";
    automaticcomponenttoolkit = "AutomaticComponentToolkit";
    autopush-rs = "autopush";
    autoreconfHook = "autoreconf-hook";
    autoreconfHook269 = "autoreconf-hook";
    avalonia = "Avalonia";
    avr8burnomat = "avr8-burn-omat";
    avrlibc = "avr-libc";
    avro-cpp = "avro-c++";
    aws-lambda-rie = "aws-lambda-runtime-interface-emulator";
    azure-static-sites-client = "StaticSitesClient-stable";
    b43FirmwareCutter = "b43-fwcutter";
    b43Firmware_5_1_138 = "b43-firmware";
    b43Firmware_6_30_163_46 = "b43-firmware";
    backintime = "backintime-qt";
    backward-cpp = "backward";
    bakoma_ttf = "bakoma-ttf";
    banking = "saldo";
    barbicanclient = "python-barbicanclient";
    bareboxTools = "barebox-hosttools_defconfig";
    bashFHS = "bash-interactive";
    bashInteractive = "bash-interactive";
    bashInteractiveFHS = "bash-interactive";
    bashNonInteractive = "bash";
    bashSnippets = "bash-snippets";
    bashup-events32 = "bashup-events3.2-unstable";
    bashup-events44 = "bashup-events4.4-unstable";
    bazel_7 = "bazel";
    bazel_8 = "bazel";
    bazel_9 = "bazel";
    beamerpresenter-mupdf = "beamerpresenter";
    beamerpresenter-poppler = "beamerpresenter";
    beancount-share = "beancount_share";
    beancount_2 = "beancount";
    beatprints = "BeatPrints";
    beauty-line-icon-theme = "BeautyLine";
    beeper-bridge-manager = "bbctl";
    beets-minimal = "beets";
    bend = "Bend";
    ber_metaocaml = "ber-metaocaml";
    bespokesynth-with-vst2 = "bespokesynth";
    biber-for-tectonic = "biber";
    binary-object-scanner = "BinaryObjectScanner";
    bionic = "bionic-prebuilt";
    bird2 = "bird";
    bird3 = "bird";
    bisoncpp = "bisonc++";
    bitwig-studio = "bitwig-studio6";
    bitwig-studio3 = "bitwig-studio";
    bitwig-studio4 = "bitwig-studio";
    bitwig-studio5 = "bitwig-studio";
    bitwig-studio5-unwrapped = "bitwig-studio-unwrapped";
    blas-ilp64 = "blas";
    blightmud-tts = "blightmud";
    blink1-tool = "blink1";
    blitz = "blitz++";
    blobby = "blobby-volley";
    blobs_gg = "blobs.gg";
    bloodhound-py = "bloodhound";
    bloodspilot-server = "bloodspilot-xpilot-fxi-server";
    blucontrol = "blucontrol-with-packages";
    bluesky-pds = "pds";
    bluesky-pdsadmin = "pdsadmin";
    bluez-experimental = "bluez";
    bluez5 = "bluez";
    bluez5-experimental = "bluez";
    bob-nvim = "bob";
    boca = "BoCA";
    boehmgc = "boehm-gc";
    bogofilter = "bogofilter-db";
    boilr = "BoilR";
    boinc-headless = "boinc";
    bolliedelayxt-lv2 = "bolliedelayxt.lv2";
    bolt_19 = "bolt";
    bolt_20 = "bolt";
    bolt_21 = "bolt";
    bolt_22 = "bolt";
    bonnie = "bonnie++";
    boogie = "Boogie";
    boost178 = "boost";
    boost179 = "boost";
    boost180 = "boost";
    boost181 = "boost";
    boost182 = "boost";
    boost183 = "boost";
    boost186 = "boost";
    boost187 = "boost";
    boost188 = "boost";
    boost189 = "boost";
    boost190 = "boost";
    boost191 = "boost";
    botan3 = "botan";
    botanEsdm = "botan";
    box2d_2 = "box2d";
    bozohttpd-minimal = "bozohttpd";
    brasero-unwrapped = "brasero";
    breakpointHook = "breakpoint-hook";
    breakpointHookCntr = "breakpoint-hook";
    brogue = "brogue-ce";
    broken-sword-25 = "broken-sword-2.5";
    bsdSetupHook = "bsd-setup-hook";
    bsdgames = "bsd-games";
    btcli = "bittensor";
    btcpayserver-altcoins = "btcpayserver";
    btop-cuda = "btop";
    btop-rocm = "btop";
    bucklespring-libinput = "bucklespring";
    bucklespring-x11 = "bucklespring";
    buildah-unwrapped = "buildah";
    buildbot-full = "buildbot-with-plugins";
    buildbot-ui = "buildbot-with-plugins";
    buildbot-worker = "buildbot_worker";
    buildifier = "bazel-buildtools";
    buildozer = "bazel-buildtools";
    buildpack = "pack";
    bukubrow = "bukubrow-host";
    bupc = "berkeley_upc";
    busybox-sandbox-shell = "busybox";
    bwm_ng = "bwm-ng";
    c-aresMinimal = "c-ares";
    c2ffi = "c2ffi-llvm-21.1.0";
    c2fmzq = "c2FmZQ";
    cabal2nix-unwrapped = "cabal2nix";
    cacert = "nss-cacert";
    cadical_2 = "cadical";
    caf = "actor-framework";
    caido = "caido-desktop";
    cairo-lang = "cairo";
    cairomm_1_0 = "cairomm";
    cairomm_1_16 = "cairomm";
    calamares-nixos = "calamares";
    calibre-no-speech = "calibre";
    cameractrls-gtk3 = "cameractrls";
    cameractrls-gtk4 = "cameractrls";
    caprine-bin = "caprine";
    capstone_4 = "capstone";
    capture = "capture-unstable";
    carddav-util = "carddav";
    cargo-bootimage = "bootimage";
    cargo-espmonitor = "espmonitor";
    cargo-pgrx_0_16_0 = "cargo-pgrx";
    cargo-pgrx_0_16_1 = "cargo-pgrx";
    cargo-pgrx_0_17_0 = "cargo-pgrx";
    cargo-pgrx_0_18_0 = "cargo-pgrx";
    cargo-tauri = "tauri";
    cassandra_4 = "cassandra";
    catch2_3 = "catch2";
    catppuccin-kde = "kde";
    cbqn-replxx = "cbqn";
    cbqn-standalone-replxx = "cbqn-standalone";
    cddiscid = "cd-discid";
    cdesktopenv = "cde";
    cdktf-cli = "cdktn-cli";
    cdparanoia = "cdparanoia-iii";
    cdparanoiaIII = "cdparanoia-iii";
    celestegame = "celeste";
    celt_0_5_1 = "celt";
    celt_0_7 = "celt";
    cemu-ti = "CEmu";
    ceph-client = "ceph";
    ceph-dev = "ceph";
    certbot-full = "certbot";
    certipy = "certipy-ad";
    cgal_5 = "cgal";
    cgterm = "cg-term";
    charles3 = "charles";
    charles4 = "charles";
    charles5 = "charles";
    chatgpt-cli = "chatgpt";
    checkPhaseThreadLimitHook = "check-phase-thread-limit-hook";
    chez = "chez-scheme";
    chez-racket = "chez-scheme-racket";
    chibi = "chibi-scheme";
    chonburi-font = "chonburi";
    chromedriver = "chromedriver-unwrapped";
    cht-sh = "cht.sh";
    cinelerra = "cinelerra-cv";
    cinnamon-common = "cinnamon";
    cinny = "cinny-unwrapped";
    cisco-packet-tracer_9 = "cisco-packet-tracer";
    ciscoPacketTracer9 = "cisco-packet-tracer";
    citrix_workspace = "citrix-workspace";
    citrix_workspace_26_04_0 = "citrix-workspace";
    civicrm = "civicrm-core";
    clasp-common-lisp = "clasp";
    classicube = "ClassiCube";
    claude-code-acp = "claude-agent-acp";
    claude-code-bin = "claude-code";
    clearlyU = "clearly-u";
    clickhouse-lts = "clickhouse";
    clipper = "Clipper";
    cloak-pt = "Cloak";
    closurecompiler = "closure-compiler";
    clprover = "CLProver++";
    clps2c-compiler = "CLPS2C-Compiler";
    clucene-core_2 = "clucene-core";
    clucene_core = "clucene-core";
    clucene_core_2 = "clucene-core";
    cm_unicode = "cm-unicode";
    cmakeCurses = "cmake-cursesUI";
    cmakeMinimal = "cmake-minimal";
    cmakeWithGui = "cmake-cursesUI-qt5UI";
    cmakerc = "cmrc";
    cmucl_binary = "cmucl-binary";
    cnijfilter_2_80 = "cnijfilter";
    cnijfilter_4_00 = "cnijfilter";
    cocoapods-beta = "cocoapods";
    cocom-tool-set = "cocom";
    code-cursor = "cursor";
    code-cursor-fhs = "cursor";
    codeblocksFull = "codeblocks";
    coin-utils = "coinutils";
    coin3d = "coin";
    colcon = "colcon-core";
    collision = "Collision";
    colmapWithCuda = "colmap";
    color-transformation-language = "ctl";
    colord-gtk4 = "colord-gtk";
    colpack = "ColPack";
    comet-gog_heroic = "comet-gog";
    comic-mandown = "mandown";
    comic-mono = "comic-mono-font";
    commonsBcel = "commons-bcel";
    commonsBsf = "commons-bsf";
    commonsCompress = "commons-compress";
    commonsDaemon = "commons-daemon";
    commonsFileUpload = "commons-fileupload";
    commonsIo = "commons-io";
    commonsLang = "commons-lang";
    commonsLogging = "commons-logging";
    commonsMath = "commons-math";
    conda = "conda-shell";
    connect-box = "connect-box3";
    connmanFull = "connman";
    connmanMinimal = "connman";
    constructium = "open-relay-constructium";
    construoBase = "construo";
    copilot-language-server-fhs = "copilot-language-server";
    copyDesktopItems = "copy-desktop-items-hook";
    copyPkgconfigItems = "copy-pkg-config-items-hook";
    copycat = "ccat";
    copyq = "CopyQ";
    coq-kernel = "coq-jupyter";
    coq_8_10 = "coq";
    coq_8_11 = "coq";
    coq_8_12 = "coq";
    coq_8_13 = "coq";
    coq_8_14 = "coq";
    coq_8_15 = "coq";
    coq_8_16 = "coq";
    coq_8_17 = "coq";
    coq_8_18 = "coq";
    coq_8_19 = "coq";
    coq_8_20 = "coq";
    coq_8_7 = "coq";
    coq_8_8 = "coq";
    coq_8_9 = "coq";
    coq_9_0 = "coq";
    coq_9_1 = "coq";
    coq_9_2 = "coq";
    coq_9_3 = "coq";
    corepack_22 = "corepack-nodejs";
    corepack_24 = "corepack-nodejs";
    coreutils-prefixed = "coreutils";
    corsair = "corsair-scan";
    cosmic-applibrary = "cosmic-app-library";
    costa = "COSTA";
    couchdb3 = "couchdb";
    cozy-drive = "cozydrive";
    cpp-interop = "cpp-interop-clang-repl";
    cpr = "libcpr";
    crane = "go-containerregistry";
    crawlTiles = "crawl-tiles";
    cri-o-unwrapped = "cri-o";
    cro-mag-rally = "CroMagRally";
    crypto-org-wallet = "chain-desktop-wallet";
    cryptodev = "cryptodev-linux";
    cryptopp = "crypto++";
    crystal_1_14 = "crystal";
    crystal_1_15 = "crystal";
    crystal_1_16 = "crystal";
    crystal_1_17 = "crystal";
    crystal_1_18 = "crystal";
    crystal_1_19 = "crystal";
    crystfel-headless = "crystfel";
    csfml_2 = "csfml";
    ctypes_sh = "ctypes.sh";
    cudatext-gtk = "cudatext";
    cudatext-qt = "cudatext";
    cudatoolkit = "cuda-merged";
    cunit = "CUnit";
    cups-brother-hl1210w = "cups-brother-hl1210W";
    cups-drv-rastertosag-gdi = "rastertosag-gdi";
    curaengine_stable = "curaengine";
    curl-impersonate-chrome = "curl-impersonate";
    curlFull = "curl";
    curlHTTP3 = "curl";
    curlMinimal = "curl";
    curlWithGnuTls = "curl";
    curseofwar-sdl = "curseofwar";
    cve = "cvelib";
    cvemap = "vulnx";
    cwtch = "libcwtch";
    cxx-prettyprint = "cxx-prettyprint-unstable";
    cyclone-scheme = "cyclone";
    cyrus_sasl = "cyrus-sasl";
    czkawka-full = "czkawka";
    d-seams = "d-SEAMS";
    d2coding = "d2codingfont";
    dafny = "Dafny";
    dart-bin = "dart";
    dart-source = "dart";
    datalab = "datalab-platform";
    db4 = "db";
    db48 = "db";
    db5 = "db";
    db53 = "db";
    db6 = "db";
    db60 = "db";
    db62 = "db";
    dblatexFull = "dblatex-full";
    dbt = "dbt-core";
    dbus_cplusplus = "dbus-cplusplus";
    dbus_java = "dbus-java";
    dcp375cwlpr = "dcp375cw-lpr";
    dcp9020cdwlpr = "dcp9020cdw-lpr";
    dcpj785dw = "dcpj785dw-lpr";
    ddnet-server = "ddnet";
    dediprog-sf100 = "dediprog-sf100-linux";
    defuddle-cli = "defuddle";
    dejavu_fonts = "dejavu-fonts";
    dejavu_fontsEnv = "dejavu-fonts";
    dell-530cdn = "dell-5130cdn-color-laser";
    deluge = "deluge-gtk";
    deluge-2_x = "deluge-gtk";
    dendrite = "matrix-dendrite";
    deps-fnl = "deps.fnl";
    desktopToDarwinBundle = "desktop-to-darwin-bundle-hook";
    device-tree_rpi = "raspberrypi-dtbs";
    dex-oidc = "dex";
    dfhack = "dwarf-fortress-full";
    dformat = "dfmt";
    dict = "dictd";
    dict-cc-py = "dict.cc.py";
    dieHook = "die-hook";
    diffoscopeMinimal = "diffoscope";
    dig = "bind";
    digibyted = "digibyte";
    direwolf-unstable = "direwolf";
    discourseAllPlugins = "discourse";
    disk_indicator = "disk-indicator";
    djgpp_i586 = "djgpp";
    djgpp_i686 = "djgpp";
    djview4 = "djview";
    dl-poly-classic-mpi = "DL_POLY_Classic";
    dmenu-rs-enable-plugins = "dmenu-rs";
    dnslink-std-go = "dnslink";
    dnsname-cni = "cni-plugin-dnsname";
    dnsutils = "bind";
    docbook2x = "docbook2X";
    docbook_sgml_dtd_31 = "docbook-sgml";
    docbook_sgml_dtd_41 = "docbook-sgml";
    docbook_sgml_dtd_45 = "docbook-sgml";
    docbook_xml_dtd_412 = "docbook-xml";
    docbook_xml_dtd_42 = "docbook-xml";
    docbook_xml_dtd_43 = "docbook-xml";
    docbook_xml_dtd_44 = "docbook-xml";
    docbook_xml_dtd_45 = "docbook-xml";
    docbook_xml_ebnf_dtd = "docbook-xml-ebnf";
    docbook_xsl = "docbook-xsl-nons";
    docbook_xsl_ns = "docbook-xsl-ns";
    docker-client = "docker";
    docker_25 = "docker";
    docker_29 = "docker";
    dockerfile-language-server-nodejs = "dockerfile-language-server";
    docopt_cpp = "docopt.cpp";
    dosage-tracker = "dosage";
    dosemu_fonts = "dosemu-fonts";
    dotnet-aspnetcore = "dotnet-aspnetcore-runtime-wrapped";
    dotnet-aspnetcore_10 = "dotnet-aspnetcore-runtime-wrapped";
    dotnet-aspnetcore_11 = "dotnet-aspnetcore-runtime-wrapped";
    dotnet-aspnetcore_6 = "aspnetcore-runtime-wrapped";
    dotnet-aspnetcore_7 = "aspnetcore-runtime-wrapped";
    dotnet-aspnetcore_8 = "dotnet-aspnetcore-runtime-wrapped";
    dotnet-aspnetcore_9 = "dotnet-aspnetcore-runtime-wrapped";
    dotnet-runtime = "dotnet-runtime-wrapped";
    dotnet-runtime_10 = "dotnet-runtime-wrapped";
    dotnet-runtime_11 = "dotnet-runtime-wrapped";
    dotnet-runtime_6 = "dotnet-runtime-wrapped";
    dotnet-runtime_7 = "dotnet-runtime-wrapped";
    dotnet-runtime_8 = "dotnet-runtime-wrapped";
    dotnet-runtime_9 = "dotnet-runtime-wrapped";
    dotnet-sdk_10 = "dotnet-sdk-wrapped";
    dotnet-sdk_11 = "dotnet-sdk-wrapped";
    dotnet-sdk_6 = "dotnet-sdk-wrapped";
    dotnet-sdk_7 = "dotnet-sdk-wrapped";
    dotnet-sdk_8 = "dotnet-sdk";
    dotnet-sdk_9 = "dotnet-sdk";
    dovecot_2_3 = "dovecot";
    dovecot_2_4 = "dovecot";
    dovecot_pigeonhole = "dovecot-pigeonhole";
    dovecot_pigeonhole_0_5 = "dovecot-pigeonhole";
    dovecot_pigeonhole_2_4 = "dovecot-pigeonhole";
    doxygen_gui = "doxygen";
    dplusplus = "dpp";
    dracula-qt5-theme = "dracula-theme";
    drawpile-server-headless = "drawpile";
    drawterm-wayland = "drawterm";
    drone = "drone.io";
    drone-oss = "drone.io-oss";
    druid = "apache-druid";
    dsseries = "libsane-dsseries";
    dt-schema = "dtschema";
    dubBuildHook = "dub-build-hook";
    dubCheckHook = "dub-check-hook";
    dubSetupHook = "dub-setup-hook";
    duckscript = "duckscript_cli";
    dump1090-fa = "dump1090";
    dune_2 = "dune";
    dune_3 = "dune";
    dust = "du-dust";
    dvc-with-remotes = "dvc";
    dvdplusrwtools = "dvd+rw-tools";
    dwarfdump = "libdwarf";
    dxvk_1 = "dxvk";
    dxvk_2 = "dxvk";
    earthly = "earthbuild";
    ebook_tools = "ebook-tools";
    ebpf-verifier = "prevail";
    ecmtools = "ecm-tools";
    ecs-agent = "amazon-ecs-agent";
    edid-decode = "v4l-utils";
    edopro = "edopro-application";
    egg2nix = "chicken-egg2nix";
    eigen2 = "eigen";
    eigen_3_4_0 = "eigen";
    eigen_5 = "eigen";
    eject = "util-linux";
    elasticmq-server-bin = "elasticmq-server";
    elasticsearch7 = "elasticsearch";
    electron-bin = "electron";
    electron-chromedriver_39 = "electron-chromedriver";
    electron-chromedriver_40 = "electron-chromedriver";
    electron-chromedriver_41 = "electron-chromedriver";
    electron-chromedriver_42 = "electron-chromedriver";
    electron-chromedriver_43 = "electron-chromedriver";
    electron_39 = "electron";
    electron_39-bin = "electron";
    electron_40 = "electron";
    electron_40-bin = "electron";
    electron_41 = "electron";
    electron_41-bin = "electron";
    electron_42 = "electron";
    electron_42-bin = "electron";
    electron_43 = "electron";
    electron_43-bin = "electron";
    element-web-unwrapped = "element-web";
    elf-header-real = "elf-header";
    elixir_1_18 = "elixir";
    elixir_1_19 = "elixir";
    emacs-gtk = "emacs-gtk3";
    emacs-macport = "emacs-mac-macport";
    emacs30-macport = "emacs-mac-macport";
    emacs31 = "emacs";
    emacs31-gtk3 = "emacs-gtk3";
    emacs31-nox = "emacs-nox";
    emacs31-pgtk = "emacs-pgtk";
    email = "email-git";
    emmylua-check = "emmylua_check";
    emmylua-doc-cli = "emmylua_doc_cli";
    emmylua-formatter = "emmylua_formatter";
    emmylua-ls = "emmylua_ls";
    enableGCOVInstrumentation = "enable-gcov-instrumentation-hook";
    enchant2 = "enchant";
    enchant_2 = "enchant";
    encodings = "font-encodings";
    enhanced-ctorrent = "enhanced-ctorrent-dhn";
    ensureNewerSourcesForZipFilesHook = "ensure-newer-sources-hook";
    ente-web = "ente-web-photos";
    epkowa = "iscan";
    eprover-ho = "eprover";
    epson-escpr2 = "epson-inkjet-printer-escpr2";
    epson-workforce-635-nx625-series = "epson-inkjet-printer-workforce-635-nx625-series";
    ergochat = "ergo";
    erlang_27 = "erlang";
    erlang_28 = "erlang";
    espanso-wayland = "espanso";
    espeak = "espeak-ng";
    espeak-classic = "espeak";
    etBook = "et-book";
    etcd_3_5 = "etcd";
    etcd_3_6 = "etcd";
    eternity = "eternity-engine";
    ethersync = "teamtype";
    eventstore = "EventStore";
    everest-bin = "everest";
    evolution-data-server-gtk4 = "evolution-data-server";
    exactaudiocopy = "exact-audio-copy";
    exegol4 = "exegol";
    exiftool = "Image-ExifTool";
    eyewitness = "eye-witness";
    ezminc = "EZminc";
    f3d_egl = "f3d";
    factor-lang = "factor-lang-env";
    factor-lang-0_100 = "factor-lang-env";
    factor-lang-0_101 = "factor-lang-env";
    factor-lang-0_99 = "factor-lang-env";
    factorio = "factorio-alpha";
    factorio-demo-experimental = "factorio-demo";
    factorio-experimental = "factorio-alpha";
    factorio-headless-experimental = "factorio-headless";
    factorio-space-age-experimental = "factorio-space-age";
    fahclient = "fah-client";
    fairfax = "open-relay-fairfax";
    fairfax-hd = "open-relay-fairfax-hd";
    faissWithCuda = "faiss";
    fastJson = "libfastjson";
    fastfetchMinimal = "fastfetch-unwrapped";
    faust2 = "faust";
    faust2sc = "faust2sc.py";
    faustPhysicalModeling = "faust-physicalmodeling";
    fceux-qt5 = "fceux";
    fceux-qt6 = "fceux";
    fcitx5-mozc-ut = "fcitx5-mozc";
    fcitx5-openbangla-keyboard = "openbangla-keyboard";
    fdk-aac-encoder = "fdkaac";
    fdk_aac = "fdk-aac";
    febio = "FEBio";
    fediwall-unwrapped = "fediwall";
    feishin-web = "feishin";
    felix = "apache-felix";
    felix-fm = "felix";
    felix_remoteshell = "apache-felix-remoteshell-bundle";
    fetchmail_7 = "fetchmail";
    fex-headless = "fex";
    ffado-mixer = "ffado";
    ffmpeg_4 = "ffmpeg";
    ffmpeg_4-full = "ffmpeg-full";
    ffmpeg_4-headless = "ffmpeg-headless";
    ffmpeg_6 = "ffmpeg";
    ffmpeg_6-full = "ffmpeg-full";
    ffmpeg_6-headless = "ffmpeg-headless";
    ffmpeg_7 = "ffmpeg";
    ffmpeg_7-full = "ffmpeg-full";
    ffmpeg_7-headless = "ffmpeg-headless";
    ffmpeg_8 = "ffmpeg";
    ffmpeg_8-full = "ffmpeg-full";
    ffmpeg_8-headless = "ffmpeg-headless";
    ffmpeg_9 = "ffmpeg";
    ffmpeg_9-full = "ffmpeg-full";
    ffmpeg_9-headless = "ffmpeg-headless";
    fftw = "fftw-double";
    fftwFloat = "fftw-single";
    fftwLongDouble = "fftw-long-double";
    fftwMpi = "fftw-double";
    fftwQuad = "fftw-quad-precision";
    fftwSinglePrec = "fftw-single";
    file-rename = "File-Rename";
    filebeat7 = "filebeat";
    filebeat8 = "filebeat";
    findXMLCatalogs = "find-xml-catalogs-hook";
    firebird_3 = "firebird";
    firebird_4 = "firebird";
    firebird_5 = "firebird";
    firefox-beta-bin = "firefox-beta";
    firefox-devedition-bin = "firefox-devedition";
    firefox-esr = "firefox";
    firefox-esr-140 = "firefox";
    firefox-esr-140-unwrapped = "firefox-unwrapped";
    firefox-esr-153 = "firefox";
    firefox-esr-153-unwrapped = "firefox-unwrapped";
    firefox-esr-unwrapped = "firefox-unwrapped";
    firefox-mobile = "firefox";
    firewalld-gui = "firewalld";
    firezone-server = "firezone-server-domain";
    fishMinimal = "fish";
    flamegraph = "FlameGraph";
    flare-signal = "flare";
    flashbench = "flashbench-unstable";
    flatbuffers_23 = "flatbuffers";
    flattenReferencesGraph = "flatten-references-graph";
    flex_2_5_35 = "flex";
    flexcpp = "flexc++";
    flint3 = "flint";
    flootty = "Flootty";
    flow-state = "flow_state";
    fltk-minimal = "fltk";
    fltk13 = "fltk";
    fltk13-minimal = "fltk";
    fltk14 = "fltk";
    fltk14-minimal = "fltk";
    fltk_1_3 = "fltk";
    fltk_1_3-minimal = "fltk";
    fltk_1_4 = "fltk";
    fltk_1_4-minimal = "fltk";
    fluent-icon-theme = "Fluent-icon-theme";
    fluffychat = "fluffychat-linux";
    flutter = "flutter-wrapped";
    flutter329 = "flutter-wrapped";
    flutter332 = "flutter-wrapped";
    flutter335 = "flutter-wrapped";
    flutter338 = "flutter-wrapped";
    flutter341 = "flutter-wrapped";
    flutter344 = "flutter-wrapped";
    flutter347 = "flutter-wrapped";
    fluvio = "fluvio-cli";
    fm-go = "fm";
    fmi-reference-fmus = "reference-fmus";
    fmodex = "fmod";
    fmt_10 = "fmt";
    fmt_11 = "fmt";
    fmt_12 = "fmt";
    fmt_9 = "fmt";
    fn-cli = "fn";
    follow = "folo";
    font-awesome_4 = "font-awesome";
    font-awesome_5 = "font-awesome";
    font-awesome_6 = "font-awesome";
    font-awesome_7 = "font-awesome";
    fontadobe100dpi = "font-adobe-100dpi";
    fontadobe75dpi = "font-adobe-75dpi";
    fontadobeutopia100dpi = "font-adobe-utopia-100dpi";
    fontadobeutopia75dpi = "font-adobe-utopia-75dpi";
    fontadobeutopiatype1 = "font-adobe-utopia-type1";
    fontalias = "font-alias";
    fontarabicmisc = "font-arabic-misc";
    fontbh100dpi = "font-bh-100dpi";
    fontbh75dpi = "font-bh-75dpi";
    fontbhlucidatypewriter100dpi = "font-bh-lucidatypewriter-100dpi";
    fontbhlucidatypewriter75dpi = "font-bh-lucidatypewriter-75dpi";
    fontbhttf = "font-bh-ttf";
    fontbhtype1 = "font-bh-type1";
    fontbitstream100dpi = "font-bitstream-100dpi";
    fontbitstream75dpi = "font-bitstream-75dpi";
    fontbitstreamtype1 = "font-bitstream-type1";
    fontcronyxcyrillic = "font-cronyx-cyrillic";
    fontcursormisc = "font-cursor-misc";
    fontdaewoomisc = "font-daewoo-misc";
    fontdecmisc = "font-dec-misc";
    fontforge-fonttools = "fontforge";
    fontforge-gtk = "fontforge";
    fontibmtype1 = "font-ibm-type1";
    fontisasmisc = "font-isas-misc";
    fontjismisc = "font-jis-misc";
    fontmicromisc = "font-micro-misc";
    fontmisccyrillic = "font-misc-cyrillic";
    fontmiscethiopic = "font-misc-ethiopic";
    fontmiscmeltho = "font-misc-meltho";
    fontmiscmisc = "font-misc-misc";
    fontmuttmisc = "font-mutt-misc";
    fontschumachermisc = "font-schumacher-misc";
    fontscreencyrillic = "font-screen-cyrillic";
    fontsonymisc = "font-sony-misc";
    fontsunmisc = "font-sun-misc";
    fontutil = "font-util";
    fontwinitzkicyrillic = "font-winitzki-cyrillic";
    fontxfree86type1 = "font-xfree86-type1";
    foomatic-db-ppds-withNonfreeDb = "foomatic-db-ppds";
    formula = "formula-dotnet";
    fortune = "fortune-mod";
    fosrl-gerbil = "gerbil";
    fosrl-newt = "newt";
    fosrl-olm = "olm";
    fosrl-pangolin = "pangolin";
    fox_1_6 = "fox";
    fplll_20160331 = "fplll";
    framac = "frama-c";
    freecad-qt6 = "freecad";
    freecad-wayland = "freecad";
    freeciv_gtk = "freeciv";
    freeciv_qt = "freeciv";
    freeciv_sdl2 = "freeciv";
    freefont_ttf = "freefont-ttf";
    frei0r = "frei0r-plugins";
    fresh-editor = "fresh";
    freshrss = "FreshRSS";
    friction-graphics = "friction";
    frostwire-bin = "frostwire";
    fscrypt-experimental = "fscrypt";
    fsuae-launcher = "fs-uae-launcher";
    funambol = "funambol-client-cpp";
    fuse2 = "fuse";
    fuse2fs = "e2fsprogs";
    fuse3 = "fuse";
    fusionInventory = "fusioninventory-agent";
    fvwm2 = "fvwm";
    gImageReader = "gimagereader";
    gImageReader-qt = "gimagereader";
    gambit-chess = "gambit";
    gambit-unstable = "gambit";
    gamenetworkingsockets = "GameNetworkingSockets";
    gamescope-wsi = "gamescope";
    gamt = "amtterm";
    gap-full = "gap";
    gap-minimal = "gap";
    garage_1 = "garage";
    garage_1_x = "garage";
    garage_2 = "garage";
    gaucheBootstrap = "gauche-bootstrap";
    gauge = "gauge-wrapped";
    gauge-unwrapped = "gauge";
    gaw = "gaw3";
    gawkInteractive = "gawk-interactive";
    gcc-arm-embedded-13 = "gcc-arm-embedded";
    gcc-arm-embedded-14 = "gcc-arm-embedded";
    gcc-arm-embedded-15 = "gcc-arm-embedded";
    gcc-unwrapped = "gcc";
    gccForLibs = "gcc";
    gcr_3 = "gcr";
    gcr_4 = "gcr";
    gcrane = "go-containerregistry";
    gdalMinimal = "gdal-minimal";
    gdbHostCpuOnly = "gdb-host-cpu-only";
    gdrive3 = "gdrive";
    gdtoolkit_3 = "gdtoolkit3";
    gdtoolkit_4 = "gdtoolkit";
    gecode_3 = "gecode";
    geekbench_4 = "geekbench";
    geekbench_5 = "geekbench";
    geekbench_6 = "geekbench";
    geekbench_7 = "geekbench";
    gem = "gem-unstable";
    geoclue2 = "geoclue";
    geoclue2-with-demo-agent = "geoclue";
    geocode-glib_2 = "geocode-glib";
    geogebra6 = "geogebra";
    geoipWithDatabase = "geoip-tools";
    geoipjava = "GeoIPJava";
    geph = "geph5";
    gerbil-unstable = "gerbil";
    gerrit_3_12 = "gerrit";
    gerrit_3_13 = "gerrit";
    gerrit_3_14 = "gerrit";
    getconf = "glibc";
    getent = "glibc";
    gexiv2_0_16 = "gexiv2";
    gfxtablet = "gfxtablet-uinput-driver";
    gg-jj = "gg";
    ggz_base_libs = "ggz-base-libs";
    gh-cherry-pick = "ghcherry";
    ghc_filesystem = "filesystem";
    ghdl = "ghdl-mcode";
    ghidra-bin = "ghidra";
    ghostscript = "ghostscript-with-X";
    ghostscriptX = "ghostscript-with-X";
    ghostscript_headless = "ghostscript";
    gimagereader-qt = "gimagereader";
    gimp2 = "gimp";
    gimp2-with-plugins = "gimp-with-plugins";
    gimp3 = "gimp";
    gimp3-with-plugins = "gimp-with-plugins";
    gir-rs = "gir";
    git-autofixup = "App-Git-Autofixup";
    git-doc = "git-with-svn";
    git-fame = "git_fame";
    git-fast-export = "fast-export";
    git-repo-updater = "gitup";
    gitFull = "git-with-svn";
    gitMinimal = "git-minimal";
    gitRepo = "git-repo";
    gitSVN = "git-with-svn";
    github-changelog-generator = "github_changelog_generator";
    github-cli = "gh";
    gl117 = "gl-117";
    glanceclient = "python-glanceclient";
    glew110 = "glew";
    glew_1_10 = "glew";
    glfw2 = "glfw";
    glfw3 = "glfw";
    glfw3-minecraft = "glfw-minecraft";
    glibcInfo = "glibc-info";
    glibcLocales = "glibc-locales";
    glibcLocalesUtf8 = "glibc-locales";
    glibc_memusage = "glibc-gd";
    glibmm_2_68 = "glibmm";
    glm_1_0_1 = "glm";
    globulation2 = "glob2";
    gmime2 = "gmime";
    gmime3 = "gmime";
    gmp = "gmp-with-cxx";
    gmp6 = "gmp-with-cxx";
    gmpxx = "gmp-with-cxx";
    gnome-settings-daemon48 = "gnome-settings-daemon";
    gnome-solanum = "solanum";
    gnome-tecla = "tecla";
    gnuapl = "gnu-apl";
    gnucap-full = "gnucap-with-plugins";
    gnumake42 = "gnumake";
    gnupatch = "patch";
    gnupg1 = "gnupg1compat";
    gnupg24 = "gnupg";
    gnupgMinimal = "gnupg";
    gnuplot_aquaterm = "gnuplot";
    gnuplot_qt = "gnuplot";
    gnupth = "pth";
    gnuradio = "gnuradio-wrapped";
    gnuradioMinimal = "gnuradio";
    gnustep-systempreferences = "system-preferences";
    go-2fa = "2fa";
    go-check = "check";
    go-motion = "motion";
    go_1_25 = "go";
    go_1_26 = "go";
    go_1_27 = "go";
    go_latest = "go";
    gobject-introspection = "gobject-introspection-wrapped";
    gobject-introspection-unwrapped = "gobject-introspection";
    godot-mono = "godot-mono-wrapper";
    godot_4 = "godot";
    godot_4-export-templates-bin = "godot-export-templates-bin";
    godot_4-mono = "godot-mono-wrapper";
    godot_4_3 = "godot";
    godot_4_3-export-templates-bin = "godot-export-templates-bin";
    godot_4_3-mono = "godot-mono-wrapper";
    godot_4_4 = "godot";
    godot_4_4-export-templates-bin = "godot-export-templates-bin";
    godot_4_4-mono = "godot-mono-wrapper";
    godot_4_5 = "godot";
    godot_4_5-export-templates-bin = "godot-export-templates-bin";
    godot_4_5-mono = "godot-mono-wrapper";
    godot_4_6 = "godot";
    godot_4_6-export-templates-bin = "godot-export-templates-bin";
    godot_4_6-mono = "godot-mono-wrapper";
    godot_4_7 = "godot";
    godot_4_7-export-templates-bin = "godot-export-templates-bin";
    godot_4_7-mono = "godot-mono-wrapper";
    gogUnpackHook = "gog-unpack-hook";
    gogetdoc = "gogetdoc-unstable";
    golden-cheetah-bin = "golden-cheetah";
    goocanvas2 = "goocanvas";
    goocanvas3 = "goocanvas";
    goocanvas_2 = "goocanvas";
    goocanvas_3 = "goocanvas";
    goocanvasmm2 = "goocanvasmm";
    google-amber = "amber";
    google-authenticator = "google-authenticator-libpam";
    google-clasp = "clasp";
    google-cloud-sdk-gce = "google-cloud-sdk";
    google-guest-agent = "guest-agent";
    gotrue-supabase = "auth";
    gpac-unstable = "gpac";
    gparted-full = "gparted";
    gpm-ncurses = "gpm";
    gpsbabel-gui = "gpsbabel";
    gpt4all-cuda = "gpt4all";
    gpu-switch = "gpu-switch-unstable";
    gpxsee-qt6 = "gpxsee";
    gqrx-gr-audio = "gqrx";
    gqrx-portaudio = "gqrx";
    gradescope-submit = "gradescope_submit";
    gradle-unwrapped = "gradle";
    gradle_8 = "gradle";
    gradle_8-unwrapped = "gradle";
    gradle_9 = "gradle";
    gradle_9-unwrapped = "gradle";
    grafana-reporter = "reporter";
    graphicsmagick_q16 = "graphicsmagick";
    graphviz-nox = "graphviz";
    grass-sass = "grass";
    graylog-6_0 = "graylog_6.0";
    graylog-6_1 = "graylog_6.1";
    gromacsCudaMpi = "gromacs";
    gromacsDouble = "gromacs";
    gromacsDoubleMpi = "gromacs";
    gromacsMpi = "gromacs";
    gromacsPlumed = "gromacs";
    grub2 = "grub";
    grub2_efi = "grub";
    grub2_ieee1275 = "grub";
    grub2_light = "grub";
    grub2_xen = "grub";
    grub2_xen_pvh = "grub";
    gruvterial-theme = "gtk-theme-framework";
    gspeech = "gSpeech";
    gssdp_1_6 = "gssdp";
    gtk-mac-integration-gtk2 = "gtk-mac-integration";
    gtk-mac-integration-gtk3 = "gtk-mac-integration";
    gtk-pipe-viewer = "pipe-viewer";
    gtk-sharp-3_0 = "gtk-sharp";
    gtk2 = "gtk+";
    gtk2-x11 = "gtk+";
    gtk3 = "gtk+3";
    gtk3-x11 = "gtk+3";
    gtkgnutella = "gtk-gnutella";
    gtkmm3 = "gtkmm";
    gtkmm4 = "gtkmm";
    gtksourceview3 = "gtksourceview";
    gtksourceview4 = "gtksourceview";
    gtksourceview5 = "gtksourceview";
    gtksourceviewmm4 = "gtksourceviewmm";
    gtkspell3 = "gtkspell";
    guile_1_8 = "guile";
    guile_2_0 = "guile";
    guile_2_2 = "guile";
    guile_3_0 = "guile";
    gupnp_1_6 = "gupnp";
    gutenprint-bin = "cups-gutenprint-binary";
    gutenprintBin = "cups-gutenprint-binary";
    gwrap = "g-wrap";
    gwt240 = "gwt-java";
    gxmatcheq-lv2 = "GxMatchEQ.lv2";
    gxplugins-lv2 = "GxPlugins.lv2";
    gyroflow-bin = "gyroflow";
    h3_3 = "h3";
    h3_4 = "h3";
    hadoop2 = "hadoop";
    hadoop3 = "hadoop";
    hadoop_3_3 = "hadoop";
    hadoop_3_4 = "hadoop";
    ham = "ham-unstable";
    hamlib_3 = "hamlib";
    hamlib_4 = "hamlib";
    hap-py = "hap.py";
    hareHook = "hare-hook";
    harfbuzzFull = "harfbuzz-icu";
    haste-client = "haste";
    hasura-cli = "hasura";
    haxe_4_3 = "haxe";
    hbase2 = "hbase";
    hbase3 = "hbase";
    hbase_2_4 = "hbase";
    hbase_2_5 = "hbase";
    hbase_2_6 = "hbase";
    hbase_3_0 = "hbase";
    hci = "hercules-ci-cli";
    hcli = "ida-hcli";
    hdf4 = "hdf";
    hdf5 = "hdf5-cpp";
    hdf5-fortran = "hdf5-cpp-fortran";
    hdf5_1_10 = "hdf5";
    heartbeat7 = "heartbeat";
    heatclient = "python-heatclient";
    heimdall-gui = "heimdall";
    hello-unfree = "example-unfree-package";
    helmfile-wrapped = "helmfile";
    hepmc2 = "hepmc";
    heretic = "heretic-llm";
    hermitcli = "hermit";
    hexdump = "util-linux-minimal";
    home-assistant = "homeassistant";
    home-assistant-cli = "homeassistant-cli";
    host = "bind";
    hostname = "hostname-debian";
    hplipWithPlugin = "hplip";
    htmlunit-driver = "htmlunit-driver-standalone";
    hugs = "hugs98";
    hybridreverb2 = "HybridReverb2";
    hydraAntLogger = "hydra-ant-logger";
    hydrogen-web-unwrapped = "hydrogen-web";
    hyp = "hyp-server";
    hyperion-ng = "hyperion.ng";
    hyperscrypt-font = "hyperscrypt";
    hyprfreeze = "wl-freeze";
    i-dot-ming = "i.ming";
    i3-easyfocus = "i3easyfocus";
    iaca_2_1 = "iaca";
    iaca_3_0 = "iaca";
    ibusMinimal = "ibus";
    ic-keysmith = "keysmith";
    icesl = "iceSL";
    iconConvTools = "icon-conv-tools";
    iconnamingutils = "icon-naming-utils";
    iconv = "glibc";
    icu = "icu4c";
    icu60 = "icu4c";
    icu63 = "icu4c";
    icu64 = "icu4c";
    icu66 = "icu4c";
    icu67 = "icu4c";
    icu70 = "icu4c";
    icu71 = "icu4c";
    icu72 = "icu4c";
    icu73 = "icu4c";
    icu74 = "icu4c";
    icu75 = "icu4c";
    icu76 = "icu4c";
    icu77 = "icu4c";
    icu78 = "icu4c";
    idle3tools = "idle3-tools";
    idris2 = "idris2-wrapped";
    ieda = "iEDA";
    ifm-web = "ifm";
    igraph_0 = "igraph";
    ikiwiki-full = "ikiwiki";
    ilbc = "ilbc-rfc3951";
    imagemagickBig = "imagemagick";
    imagemagick_light = "imagemagick";
    img-cat = "imgcat";
    imlib2-nox = "imlib2";
    imlib2Full = "imlib2";
    indexed-bzip2 = "indexed_bzip2";
    indicator-application-gtk3 = "indicator-application";
    infamousPlugins = "infamousplugins";
    influxdb2-cli = "influx-cli";
    influxdb2-server = "influxdb";
    inspircdMinimal = "inspircd";
    installFonts = "install-fonts-hook";
    installShellFiles = "install-shell-files";
    intel2200BGFirmware = "ipw2200-firmware";
    iortcw_sp = "iortcw-sp";
    iosevka = "Iosevka";
    iosevka-bin = "Iosevka-bin";
    iperf2 = "iperf";
    iperf3 = "iperf";
    ipmiview = "IPMIView";
    iptables-legacy = "iptables";
    iptables-nftables-compat = "iptables";
    ir-lv2 = "ir.lv2";
    ircdHybrid = "ircd-hybrid";
    ironicclient = "python-ironicclient";
    isc-cron = "cron";
    isl_0_20 = "isl";
    isl_0_23 = "isl";
    isl_0_27 = "isl";
    isle-portable = "isle-portable-wrapped";
    iso-flags-png-320x240 = "iso-flags";
    isocodes = "iso-codes";
    itk_5 = "itk";
    itk_5_2 = "itk";
    ivette = "frama-c-gui";
    jabcode = "jabcode-library";
    jack-autoconnect = "jack_autoconnect";
    jackmix_jack1 = "jackmix";
    jags = "JAGS";
    jalv-qt = "jalv";
    jamesdsp-pulse = "jamesdsp";
    jan = "Jan";
    jankyborders = "JankyBorders";
    jart-jsoncpp = "json.cpp";
    javaCup = "java-cup";
    jboss = "jboss-as";
    jboss_mysql_jdbc = "jboss-mysql-jdbc";
    jdk = "openjdk";
    jdk11 = "openjdk";
    jdk11_headless = "openjdk-headless";
    jdk17 = "openjdk";
    jdk17_headless = "openjdk-headless";
    jdk21 = "openjdk";
    jdk21_headless = "openjdk-headless";
    jdk25 = "openjdk";
    jdk25_headless = "openjdk-headless";
    jdk8 = "openjdk";
    jdk8_headless = "openjdk-headless";
    jdk_headless = "openjdk-headless";
    jellyfin-media-player = "jellyfin-desktop";
    jellyseerr = "seerr";
    jetty_12 = "jetty";
    jing = "jing-trang";
    jitsi-excalidraw = "jitsi-excalidraw-backend";
    jitsi-videobridge = "jitsi-videobridge2";
    jmc2obj = "j-mc-2-obj";
    jmusicbot = "JMusicBot";
    joplin = "joplin-cli";
    jp-zip-codes = "jp-zip-code";
    jre = "openjdk";
    jre11_minimal = "openjdk-minimal-jre";
    jre17_minimal = "openjdk-minimal-jre";
    jre21_minimal = "openjdk-minimal-jre";
    jre25_minimal = "openjdk-minimal-jre";
    jre8 = "openjdk";
    jre8_headless = "openjdk-headless";
    jre_headless = "openjdk-headless";
    jre_minimal = "openjdk-minimal-jre";
    json_c = "json-c";
    jsoncppSecureMemory = "jsoncpp";
    jtds_jdbc = "jtds";
    julia-lts = "julia-bin";
    julia-lts-bin = "julia-bin";
    julia-mono = "JuliaMono-ttf";
    julia-stable = "julia";
    julia-stable-bin = "julia-bin";
    julia_110 = "julia";
    julia_110-bin = "julia-bin";
    julia_111 = "julia";
    julia_111-bin = "julia-bin";
    julia_112 = "julia";
    julia_112-bin = "julia-bin";
    junction-font = "junction";
    junixsocket-common = "com_kohlschutter_junixsocket_junixsocket-common";
    junixsocket-native-common = "com_kohlschutter_junixsocket_junixsocket-native-common";
    juno-theme = "juno";
    jupyter-all = "jupyter";
    jupyterlite = "jupyterlite-core";
    k3s_1_34 = "k3s";
    k3s_1_35 = "k3s";
    k3s_1_36 = "k3s";
    kakoune-cr = "kakoune.cr";
    kanata-with-cmd = "kanata";
    kanidmWithSecretProvisioning_1_10 = "kanidm-with-secret-provisioning";
    kanidmWithSecretProvisioning_1_11 = "kanidm-with-secret-provisioning";
    kanidmWithSecretProvisioning_1_8 = "kanidm-with-secret-provisioning";
    kanidmWithSecretProvisioning_1_9 = "kanidm-with-secret-provisioning";
    kanidm_1_10 = "kanidm";
    kanidm_1_11 = "kanidm";
    kanidm_1_8 = "kanidm";
    kanidm_1_9 = "kanidm";
    kanit-font = "kanit";
    karma-runner = "karma";
    katagoCPU = "katago";
    katagoTensorRT = "katago";
    katagoWithCuda = "katago";
    kati = "kati-unstable";
    kawkab-mono-font = "kawkab-mono";
    kbdVlock = "kbd-vlock";
    kddockwidgets = "KDDockWidgets";
    kdsingleapplication = "KDSingleApplication";
    keepBuildTree = "keep-build-tree-hook";
    keepass-keeagent = "keeagent";
    keepass-keepasshttp = "keepasshttp";
    keepass-keepassrpc = "keepassrpc";
    keepass-otpkeyprov = "otpkeyprov";
    kerf_1 = "kerf";
    kermit-terminal = "kermit";
    khronos-ocl-icd-loader = "opencl-icd-loader";
    kid3-cli = "kid3";
    kid3-kde = "kid3";
    kid3-qt = "kid3";
    killall = "psmisc";
    kilocode-cli = "kilo";
    kiro-fhs = "kiro";
    kissfft = "kissfft-double";
    kissfftFloat = "kissfft-float";
    klibcShrunk = "klibc";
    klipper-firmware = "klipper-firmware-mcu";
    klipperscreen = "KlipperScreen";
    kminion = "kafka-minion";
    kmod-blacklist-ubuntu = "kmod-blacklist";
    kmod-debian-aliases = "kmod-debian-aliases.conf";
    knightos-genkfs = "genkfs";
    knightos-kcc = "kcc";
    knightos-kimg = "kimg";
    knightos-kpack = "kpack";
    knightos-mkrom = "mkrom";
    knightos-mktiupgrade = "mktiupgrade";
    knightos-patchrom = "patchrom";
    knightos-scas = "scas";
    knightos-z80e = "z80e";
    knobkraft-orm = "KnobKraft-orm";
    knot-resolver = "knot-resolver_5";
    knot-resolver-manager_6 = "knot-resolver";
    koboredux-free = "koboredux";
    kodi-gbm = "kodi";
    kodi-wayland = "kodi";
    kops_1_31 = "kops";
    kops_1_32 = "kops";
    kops_1_33 = "kops";
    kreative-square = "open-relay-kreative-square";
    ktop = "kubectl-ktop";
    kubectl-convert = "kubectl";
    kubelogin-oidc = "kubelogin";
    kubernetes-code-generator = "code-generator";
    kubernetes-controller-tools = "controller-tools";
    kubo-migrator-unwrapped = "kubo-migrator";
    kuma-cp = "kumactl";
    kwin-script-geometry-change = "kwin-effect-geometry-change";
    ladspaH = "ladspa-header";
    ladspaPlugins = "swh-plugins";
    lagrange-tui = "lagrange";
    lambda-lisp = "lambda-lisp-blc";
    lambda-mod-zsh-theme = "lambda-mod-zsh-theme-unstable";
    lammps-mpi = "lammps";
    lapack-ilp64 = "lapack";
    lapack-reference = "liblapack";
    laszip_2 = "laszip";
    lattice-diamond = "diamond";
    lazarus = "lazarus-gtk3";
    lbry = "lbry-desktop";
    lcms = "lcms2";
    lcms1 = "lcms";
    ldmud-full = "ldmud";
    leafnode1 = "leafnode";
    lean3 = "lean";
    ledger-web = "ledger_web";
    left4gore-bin = "left4gore";
    legit-web = "legit";
    lenovo-legion = "lenovo-legion-app";
    lens = "lens-desktop";
    leo3-bin = "leo3";
    lerna_6 = "lerna";
    lerna_8 = "lerna";
    lexicon = "dns_lexicon";
    lgogdownloader-gui = "lgogdownloader";
    libAppleWM = "libapplewm";
    libFS = "libfs";
    libGDSII = "libgdsii";
    libGL = "libglvnd";
    libGLU = "glu";
    libGLX = "libglvnd";
    libHX = "libhx";
    libICE = "libice";
    libLAS = "liblas";
    libSM = "libsm";
    libWindowsWM = "libwindowswm";
    libX11 = "libx11";
    libXScrnSaver = "libxscrnsaver";
    libXau = "libxau";
    libXaw = "libxaw";
    libXcomposite = "libxcomposite";
    libXcursor = "libxcursor";
    libXdamage = "libxdamage";
    libXdmcp = "libxdmcp";
    libXext = "libxext";
    libXfixes = "libxfixes";
    libXfont = "libxfont_1";
    libXfont2 = "libxfont_2";
    libXft = "libxft";
    libXi = "libxi";
    libXinerama = "libxinerama";
    libXmu = "libxmu";
    libXp = "libxp";
    libXpm = "libxpm";
    libXpresent = "libxpresent";
    libXrandr = "libxrandr";
    libXrender = "libxrender";
    libXres = "libxres";
    libXt = "libxt";
    libXtst = "libxtst";
    libXv = "libxv";
    libXvMC = "libxvmc";
    libXxf86dga = "libxxf86dga";
    libXxf86misc = "libxxf86misc";
    libXxf86vm = "libxxf86vm";
    libagar_test = "libagar-test";
    libappindicator = "libappindicator-gtk3";
    libargs = "args";
    libastyle = "astyle";
    libbfd_2_38 = "libbfd";
    libblocksruntime = "blocksruntime";
    libbluray-full = "libbluray";
    libbpf_0 = "libbpf";
    libc = "glibc";
    libcamera-qcam = "libcamera";
    libcanberra-gtk3 = "libcanberra";
    libcanberra_kde = "libcanberra";
    libcap_ng = "libcap-ng";
    libcec_platform = "p8-platform";
    libceph = "ceph";
    libchamplain_libsoup3 = "libchamplain";
    libck = "ck";
    libclang = "clang";
    libcosmicAppHook = "libcosmic-app-hook";
    libcpr_1_10_5 = "libcpr";
    libcrossguid = "crossguid";
    libcsa = "csa";
    libctemplate = "ctemplate";
    libdbi-drivers-base = "libdbi-drivers";
    libdbiDrivers = "libdbi-drivers";
    libdbiDriversBase = "libdbi-drivers";
    libdbusmenu = "libdbusmenu-glib";
    libdisplay-info_0_3 = "libdisplay-info";
    libdvdnav_4_2_1 = "libdvdnav";
    libdvdread_4_9_9 = "libdvdread";
    liberation_ttf = "liberation-fonts";
    liberation_ttf_v1 = "liberation-fonts";
    liberation_ttf_v2 = "liberation-fonts";
    liberfa = "erfa";
    libertine = "linux-libertine";
    libertine-g = "linux-libertine-g";
    libesmtp = "libESMTP";
    libexosip = "libexosip2";
    libfakeXinerama = "libfakexinerama";
    libffado = "ffado";
    libffiReal = "libffi";
    libffi_3_3 = "libffi";
    libfoundry = "libfoundry-gtk";
    libftdi1 = "libftdi";
    libfx2 = "fx2";
    libgbm = "mesa-libgbm";
    libgcc = "gcc";
    libgda5 = "libgda";
    libgda6 = "libgda";
    libglut = "freeglut";
    libgpg-error-gen-posix-lock-obj = "libgpg-error";
    libgpiod_1 = "libgpiod";
    libguestfs-with-appliance = "libguestfs";
    libhandy_0 = "libhandy";
    libiconvReal = "libiconv";
    libindicator-gtk3 = "libindicator";
    libintPsi4 = "libint";
    libjpeg = "libjpeg-turbo";
    libjpeg8 = "libjpeg-turbo";
    libjpeg_original = "libjpeg";
    libjpeg_turbo = "libjpeg-turbo";
    libkrb5 = "krb5";
    liblastfmSF = "liblastfm-vambrose";
    libliftoff_0_4 = "libliftoff";
    libliftoff_0_5 = "libliftoff";
    libllvm = "llvm";
    liblqr1 = "liblqr-1";
    libmatthew_java = "libmatthew-java";
    libmbd = "libMBD";
    libmhash = "mhash";
    libmicrohttpd_1_0 = "libmicrohttpd";
    libmusicbrainz5 = "libmusicbrainz";
    libmysqlclient = "mariadb-connector-c";
    libmysqlclient_3_1 = "mariadb-connector-c";
    libmysqlclient_3_2 = "mariadb-connector-c";
    libmysqlclient_3_3 = "mariadb-connector-c";
    libnats-c = "libnats";
    libnghttp2 = "nghttp2";
    libnma-gtk4 = "libnma";
    libopcodes_2_38 = "libopcodes";
    libosip = "libosip2";
    libossp_uuid = "libossp-uuid";
    libpeas2 = "libpeas";
    libpkgconf = "pkgconf";
    libplacebo_5 = "libplacebo";
    libpng = "libpng-apng";
    libpng12 = "libpng";
    libpostalWithData = "libpostal";
    libpthreadstubs = "libpthread-stubs";
    libr3 = "r3";
    librdf_raptor2 = "raptor2";
    librdf_rasqal = "rasqal";
    librdf_redland = "redland";
    librealsense-gui = "librealsense";
    librealsenseWithCuda = "librealsense";
    librealsenseWithoutCuda = "librealsense";
    libreoffice-bin = "libreoffice";
    libreoffice-collabora = "libreoffice";
    libreoffice-fresh = "libreoffice";
    libreoffice-qt = "libreoffice";
    libreoffice-qt-fresh = "libreoffice";
    libreoffice-qt-fresh-unwrapped = "libreoffice";
    libreoffice-qt-stable = "libreoffice";
    libreoffice-qt-stable-unwrapped = "libreoffice";
    libreoffice-qt-still = "libreoffice";
    libreoffice-qt-still-unwrapped = "libreoffice";
    libreoffice-qt-unwrapped = "libreoffice";
    libreoffice-qt6 = "libreoffice";
    libreoffice-qt6-fresh = "libreoffice";
    libreoffice-qt6-fresh-unwrapped = "libreoffice";
    libreoffice-qt6-still = "libreoffice";
    libreoffice-qt6-still-unwrapped = "libreoffice";
    libreoffice-qt6-unwrapped = "libreoffice";
    libreoffice-stable = "libreoffice";
    libreoffice-stable-unwrapped = "libreoffice";
    libreoffice-still = "libreoffice";
    libreoffice-unwrapped = "libreoffice";
    libressl_4_2 = "libressl";
    libressl_4_3 = "libressl";
    librest_1_0 = "librest";
    libsbsms_2_0_2 = "libsbsms";
    libsbsms_2_3_0 = "libsbsms";
    libsigcxx = "libsigc++";
    libsigcxx30 = "libsigc++";
    libsmartcols = "util-linux";
    libsoup_3 = "libsoup";
    libsshWithGssapi = "libssh";
    libstartup_notification = "libstartup-notification";
    libtensorflow = "tensorflow";
    libtinfo = "ncurses";
    libtool_1_5 = "libtool";
    libtool_2 = "libtool";
    libtorch-bin = "libtorch";
    libtorrent-rasterbar-1_2_x = "libtorrent-rasterbar";
    libtorrent-rasterbar-2_0_x = "libtorrent-rasterbar";
    libtransmission_4 = "transmission";
    libubox-mbedtls = "libubox";
    libubox-nossl = "libubox";
    libuchardet = "uchardet";
    libusb-compat-0_1 = "libusb-compat";
    libusb1 = "libusb";
    libutp_3_4 = "libutp";
    libuuid = "util-linux-minimal";
    libv4l = "v4l-utils";
    libwpd_08 = "libwpd";
    libwpe-fdo = "wpebackend-fdo";
    libxc_7 = "libxc";
    libxcrypt-legacy = "libxcrypt";
    libxdg_basedir = "libxdg-basedir";
    libxfs = "xfsprogs";
    libxkbcommon_8 = "libxkbcommon";
    libxml2_13 = "libxml2";
    libxmlxx = "libxml++";
    libxmlxx3 = "libxml++";
    libzint = "zint";
    lightdm_qt = "lightdm";
    lighthouse-steamvr = "Lighthouse";
    lightlocker = "light-locker";
    lilypond-unstable = "lilypond";
    lilypond-unstable-with-fonts = "lilypond";
    lilypond-with-fonts = "lilypond";
    lima-bin = "lima";
    limbo = "turso";
    limesuiteWithGui = "limesuite";
    limine-full = "limine";
    linalg = "lialg";
    lincity_ng = "lincity-ng";
    lingua-franca = "lfc";
    linkerd = "linkerd-stable";
    linkerd_edge = "linkerd-edge";
    linkerd_stable = "linkerd-stable";
    linphone = "linphone-desktop";
    linux-doc = "linux-kernel-latest-htmldocs";
    linux-gpib = "linux-gpib-user";
    linux-router-without-wifi = "linux-router";
    linuxConsoleTools = "linuxconsoletools";
    linuxHeaders = "linux-headers";
    linuxPackages_custom_tinyconfig_kernel = "linux";
    linux_5_10 = "linux";
    linux_5_15 = "linux";
    linux_6_1 = "linux";
    linux_6_12 = "linux";
    linux_6_18 = "linux";
    linux_6_6 = "linux";
    linux_7_2 = "linux";
    linux_latest = "linux";
    linux_rpi0 = "linux-rpi";
    linux_rpi02w = "linux-rpi";
    linux_rpi1 = "linux-rpi";
    linux_rpi2 = "linux-rpi";
    linux_rpi3 = "linux-rpi";
    linux_rpi4 = "linux-rpi";
    linux_testing = "linux";
    linux_xanmod = "linux-xanmod";
    linux_xanmod_latest = "linux-xanmod";
    linux_xanmod_stable = "linux-xanmod";
    linux_zen = "linux-zen";
    linuxquota = "quota";
    liquibase_redshift_extension = "liquibase-redshift-extension";
    liquidwar = "liquidwar6";
    lixStatic = "lix";
    lklWithFirewall = "lkl";
    llama-cpp-cuda = "llama-cpp";
    llama-cpp-rocm = "llama-cpp";
    llama-cpp-vulkan = "llama-cpp";
    llama-swap-minimal = "llama-swap";
    lld_18 = "lld";
    lld_19 = "lld";
    lld_20 = "lld";
    lld_21 = "lld";
    lld_22 = "lld";
    lldb_18 = "lldb";
    lldb_19 = "lldb";
    lldb_20 = "lldb";
    lldb_21 = "lldb";
    lldb_22 = "lldb";
    llvm_18 = "llvm";
    llvm_19 = "llvm";
    llvm_20 = "llvm";
    llvm_21 = "llvm";
    llvm_22 = "llvm";
    lm_sensors = "lm-sensors";
    lmms-full = "lmms";
    lnreader = "pdf-cli";
    locale = "glibc";
    lockfileProgs = "lockfile-progs";
    logger = "util-linux";
    logiops_0_2_3 = "logiops";
    logitech-udev-rules = "solaar";
    logstash7 = "logstash";
    logstash7-oss = "logstash-oss";
    losslesscut-bin = "losslesscut";
    love_0_10 = "love";
    love_11 = "love";
    lowdown-unsandboxed = "lowdown";
    lpcnet = "LPCNet";
    lsb-release = "lsb_release";
    lshw-gui = "lshw";
    lttng-ust_2_12 = "lttng-ust";
    lua5 = "lua";
    lua5_1 = "lua";
    lua5_2 = "lua";
    lua5_2_compat = "lua";
    lua5_3 = "lua";
    lua5_3_compat = "lua";
    lua5_4 = "lua";
    lua5_4_compat = "lua";
    lua5_5 = "lua";
    lua5_5_compat = "lua";
    luabind_luajit = "luabind";
    luajit_2_0 = "luajit";
    luajit_2_1 = "luajit";
    luajit_openresty = "luajit";
    luanti-client = "luanti";
    luanti-server = "luanti";
    lucenepp = "lucene++";
    lunar-client = "lunarclient";
    lutris-free = "lutris";
    lvm2_dmeventd = "lvm2-with-dmeventd";
    lvm2_vdo = "lvm2-with-dmeventd-with-vdo";
    lxi-tools-gui = "lxi-tools";
    m17n_db = "m17n-db";
    m17n_lib = "m17n-lib";
    m32edit = "m32-edit";
    m33-linux = "M33-Linux";
    m4 = "gnum4";
    macos-instantview = "instantview";
    maelstrom-clj = "maelstrom";
    maestral-gui = "maestral-qt";
    magma-cuda = "magma";
    magma-cuda-static = "magma";
    magma-hip = "magma";
    magnumclient = "python-magnumclient";
    mailhog = "MailHog";
    mailman-web = "mailman_web";
    makeBinaryWrapper = "make-binary-wrapper-hook";
    makeGCOVReport = "make-gcov-report-hook";
    makeInitrdNGTool = "make-initrd-ng";
    makeShellWrapper = "make-shell-wrapper-hook";
    makeWrapper = "make-shell-wrapper-hook";
    mame-tools = "mame";
    man = "man-db";
    mangowc = "mango";
    manilaclient = "python-manilaclient";
    map-cmd = "map";
    mar1d = "MAR1D";
    mariadb = "mariadb-server";
    mariadb-connector-c_3_1 = "mariadb-connector-c";
    mariadb-connector-c_3_2 = "mariadb-connector-c";
    mariadb-connector-c_3_3 = "mariadb-connector-c";
    mariadb-embedded = "mariadb-server";
    mariadb_1011 = "mariadb-server";
    mariadb_106 = "mariadb-server";
    mariadb_114 = "mariadb-server";
    mariadb_118 = "mariadb-server";
    mathematica-cuda = "mathematica";
    mathematica-webdoc = "mathematica";
    mathematica-webdoc-cuda = "mathematica";
    matrix-synapse-unwrapped = "matrix-synapse";
    matrix-zulip-bridge = "MatrixZulipBridge";
    matter-compiler = "matter_compiler";
    mattermostLatest = "mattermost";
    maven3 = "maven";
    maven_4 = "maven";
    mavproxy = "MAVProxy";
    maxima-ecl = "maxima";
    mbedtls_4 = "mbedtls";
    mbqn = "bqn";
    mcfm = "MCFM";
    mda_lv2 = "mda-lv2";
    mdadm4 = "mdadm";
    mdbook-rss-feed-full = "mdbook-rss-feed";
    mdk = "gnu-mdk";
    mediaelch-qt5 = "mediaelch";
    mediaelch-qt6 = "mediaelch";
    meerk40t = "MeerK40t";
    melmatcheq-lv2 = "MelMatchEQ.lv2";
    melonDS = "melonds";
    memcachedTestHook = "memcached-test-hook";
    memtest86plus = "memtest86+";
    mercurialFull = "mercurial-full";
    mesa_glu = "glu";
    mesa_i686 = "mesa";
    metasploit = "metasploit-framework";
    metersLv2 = "meters-lv2";
    metricbeat7 = "metricbeat";
    mfcj470dwlpr = "mfcj470dw-cupswrapper";
    mfcj880dwcupswrapper = "mfcj880dw-cupswrapper";
    mfcl3770cdwlpr = "mfcl3770cdwdrv";
    mhonarc = "MHonArc";
    micro-full = "micro";
    micro-with-wl-clipboard = "micro";
    micro-with-xclip = "micro";
    microcode-amd = "amd-ucode";
    milu = "milu-nightly";
    minc_tools = "minc-tools";
    minc_widgets = "minc-widgets";
    mindustry-server = "mindustry";
    mindustry-wayland = "mindustry";
    miniHttpd = "mini-httpd";
    miniupnpd-nftables = "miniupnpd";
    minivmac = "minivmac-erichelgeson";
    minixml = "mxml";
    minizincide = "minizinc-ide";
    mir-qualia = "mir.qualia";
    mir_2_15 = "mir";
    mistralclient = "python-mistralclient";
    mitscheme = "mit-scheme";
    mitschemeX11 = "mit-scheme-x11";
    mjpegtoolsFull = "mjpegtools";
    mkchromecast = "mkchromecast-unstable";
    mkfontdir = "mkfontscale";
    mkgmap-splitter = "splitter";
    mkosi-full = "mkosi";
    mkvtoolnix-cli = "mkvtoolnix";
    mlflow-server = "mlflow";
    mlterm-wayland = "mlterm";
    mlton20130715 = "mlton";
    mlton20180207 = "mlton";
    mlton20180207Binary = "mlton";
    mlton20210117 = "mlton";
    mlton20210117Binary = "mlton";
    mlton20241230 = "mlton";
    mlton20241230Binary = "mlton";
    mltonHEAD = "mlton";
    mmex = "money-manager-ex";
    moar = "moor";
    modsecurity_standalone = "modsecurity";
    moeli = "eduli";
    molbar = "MolBar";
    moltenvk = "MoltenVK";
    mongodb-7_0 = "mongodb";
    mongodb-tools = "mongo-tools";
    monitor = "elementary-monitor";
    monitorcontrol = "MonitorControl";
    monkeysAudio = "monkeys-audio";
    mono4 = "mono";
    mono5 = "mono";
    mono6 = "mono";
    monoDLLFixer = "mono-dll-fixer";
    mooSpace = "moospace";
    mopidy-tidal = "Mopidy-Tidal";
    more = "util-linux";
    morse-linux = "morse";
    morse_cli = "morse-cli";
    mount = "util-linux";
    moveBuildTree = "move-build-tree-hook";
    mozc-ut = "mozc";
    mozwire = "MozWire";
    mpd-small = "mpd";
    mpdris2 = "mpDris2";
    mpeg2dec = "libmpeg2";
    mph_2b_damase = "mph-2b-damase";
    mpi = "openmpi";
    mpiCheckPhaseHook = "mpi-checkPhase-hook";
    mpich-pmix = "mpich";
    mpv = "mpv-with-scripts";
    mpv-unwrapped = "mpv";
    mrsh = "mrsh-unstable";
    msat = "msat-bin";
    msp430GccSupport = "msp430-gcc-support-files";
    msp430Newlib = "msp430-newlib";
    mspds = "msp-debug-stack";
    mspds-bin = "msp-debug-stack-bin";
    mssql_jdbc = "mssql-jdbc";
    mtdutils = "mtd-utils";
    mtpaint = "mtPaint";
    mtrace = "glibc-mtrace";
    multivnc = "MultiVNC";
    mumble_overlay = "mumble-overlay";
    mumps-mpi = "mumps";
    muonStandalone = "muon-embedded-samurai";
    muonlang = "muon";
    mupdf-headless = "mupdf";
    muse = "muse-sequencer";
    mustache-hpp = "mustache";
    mutter48 = "mutter";
    mybbscan = "my-b-bscan";
    mycrypto = "MyCrypto";
    mypaint-brushes1 = "mypaint-brushes";
    mysql-shell-innovation = "mysql-shell";
    mysql-shell_8 = "mysql-shell";
    mysql-shell_9 = "mysql-shell";
    mysql84 = "mysql";
    mysql_jdbc = "mysql-connector-java";
    n3 = "N3";
    nanopbMalloc = "nanopb";
    nanotts = "nano-tts";
    napi-rs-cli_3 = "napi-rs-cli";
    naproche = "Naproche-SAD";
    nbench = "nbench-byte";
    ncdu_1 = "ncdu";
    ncurses5 = "ncurses-abi5-compat";
    ncurses6 = "ncurses";
    ndstrim = "ndstrim-rs";
    nekoray = "throne";
    neovim-node-client = "node-client";
    nest-mpi = "nest";
    nestopia-ue = "nestopia";
    netbird = "netbird-client";
    netbootxyz-efi = "netboot.xyz-efi";
    netbox_4_6 = "netbox";
    netcap-nodpi = "netcap";
    netcat = "libressl";
    netcdfcxx4 = "netcdf-cxx4";
    netcdffortran = "netcdf-fortran";
    netdataCloud = "netdata";
    netgen-vlsi = "netgen";
    netkittftp = "netkit-tftp";
    netmaker-full = "netmaker";
    netsurf-browser = "netsurf";
    nettle_4 = "nettle";
    nettools = "net-tools";
    netwatch = "netwatch-tui";
    networkmanager-fortisslvpn = "NetworkManager-fortisslvpn";
    networkmanager-iodine = "NetworkManager-iodine-gnome";
    networkmanager-l2tp = "NetworkManager-l2tp";
    networkmanager-openconnect = "NetworkManager-openconnect";
    networkmanager-openvpn = "NetworkManager-openvpn";
    networkmanager-ssh = "NetworkManager-ssh";
    networkmanager-sstp = "NetworkManager-sstp";
    networkmanager-strongswan = "NetworkManager-strongswan";
    networkmanager-vpnc = "NetworkManager-vpnc";
    networkmanager_strongswan = "NetworkManager-strongswan";
    networkmanagerapplet = "network-manager-applet";
    neuron-full = "neuron";
    neuron-mpi = "neuron";
    neutronclient = "python-neutronclient";
    newlib-nano = "newlib";
    newt-go = "newt";
    nextcloud-notify_push = "notify_push";
    nextcloud32 = "nextcloud";
    nextcloud33 = "nextcloud";
    nextcloud34 = "nextcloud";
    nextpnrWithGui = "nextpnr";
    nginx-doc = "nginx-doc-unstable";
    nginxMainline = "nginx";
    nginxShibboleth = "nginx";
    nginxStable = "nginx";
    ngt = "NGT";
    ngtcp2-gnutls = "ngtcp2";
    nim = "x86_64-unknown-linux-gnu-nim-wrapper";
    nim-atlas = "atlas";
    ninja_1_11 = "ninja";
    nix-index-unwrapped = "nix-index";
    nixStatic = "nix";
    nixfmt-rfc-style = "nixfmt";
    nixos-rebuild = "nixos-rebuild-ng";
    nixpkgs-lint-community = "nixpkgs-lint";
    nixpkgs-reviewFull = "nixpkgs-review";
    nlojet = "nlojet++";
    node-glob = "glob";
    node-manta = "manta";
    nodejs-slim_22 = "nodejs-slim";
    nodejs-slim_24 = "nodejs-slim";
    nodejs-slim_26 = "nodejs-slim";
    nodejs-slim_latest = "nodejs-slim";
    nodejsInstallExecutables = "nodejs-install-executables";
    nodejsInstallManuals = "nodejs-install-manuals";
    nodejs_22 = "nodejs";
    nodejs_24 = "nodejs";
    nodejs_26 = "nodejs";
    nodejs_latest = "nodejs";
    noisetorch = "NoiseTorch";
    nomacs-qt6 = "nomacs";
    nomad_1_10 = "nomad";
    nomad_1_11 = "nomad";
    nomad_1_9 = "nomad";
    nomad_2_0 = "nomad";
    norwester-font = "norwester";
    noto-fonts-cjk-sans-static = "noto-fonts-cjk-sans";
    noto-fonts-cjk-serif-static = "noto-fonts-cjk-serif";
    npapi_sdk = "npapi-sdk";
    nssTools = "nss";
    nss_esr = "nss";
    nss_latest = "nss";
    nss_pam_ldapd = "nss-pam-ldapd";
    nssmdns = "nss-mdns";
    ntfsprogs = "ntfs3g";
    nth = "name-that-hash";
    nuXmv = "nuxmv";
    nuget = "Nuget";
    nusmv = "NuSMV";
    nv-codec-headers-10 = "nv-codec-headers";
    nv-codec-headers-11 = "nv-codec-headers";
    nv-codec-headers-12 = "nv-codec-headers";
    nv-codec-headers-9 = "nv-codec-headers";
    nvidia_cg_toolkit = "nvidia-cg-toolkit";
    nwjs-sdk = "nwjs";
    nyarr = "gridlock";
    obex_data_server = "obex-data-server";
    ocaml-crunch = "crunch";
    ocaml_make = "ocaml-make";
    ocamlformat_0_19_0 = "ocamlformat";
    ocamlformat_0_20_0 = "ocamlformat";
    ocamlformat_0_20_1 = "ocamlformat";
    ocamlformat_0_21_0 = "ocamlformat";
    ocamlformat_0_22_4 = "ocamlformat";
    ocamlformat_0_23_0 = "ocamlformat";
    ocamlformat_0_24_1 = "ocamlformat";
    ocamlformat_0_25_1 = "ocamlformat";
    ocamlformat_0_26_0 = "ocamlformat";
    ocamlformat_0_26_1 = "ocamlformat";
    ocamlformat_0_26_2 = "ocamlformat";
    ocamlformat_0_27_0 = "ocamlformat";
    ocamlformat_0_28_1 = "ocamlformat";
    ocamlformat_0_29_0 = "ocamlformat";
    oceanic-theme = "gtk-theme-framework";
    ocr-a = "OCR-A";
    octaveFull = "octave";
    octoprint = "OctoPrint";
    octopus-caller = "octopus";
    odafileconverter = "oda-file-converter";
    odoo17 = "odoo";
    odoo18 = "odoo";
    odoo19 = "odoo";
    ogre_13 = "ogre";
    ogre_14 = "ogre";
    olivetin-3k = "olivetin";
    ollama-cpu = "ollama";
    ollama-cuda = "ollama";
    ollama-rocm = "ollama";
    ollama-vulkan = "ollama";
    ome_zarr = "ome-zarr";
    oneDNN = "onednn";
    oneDNN_2 = "onednn";
    onednn_2 = "onednn";
    onionshare = "onionshare-cli";
    onionshare-gui = "onionshare";
    open-timeline-io = "opentimelineio";
    open-vm-tools-headless = "open-vm-tools";
    open-watcom-v2-full = "open-watcom-v2";
    open-watcom-v2-full-unwrapped = "open-watcom-v2-unwrapped";
    openal = "openal-soft";
    openalSoft = "openal-soft";
    openbabel3 = "openbabel";
    openblasCompat = "openblas";
    openbugs = "OpenBUGS";
    opencascade-occt_7_6 = "opencascade-occt";
    opencascade-occt_7_6_1 = "opencascade-occt";
    openconnect_openssl = "openconnect";
    opencorsairlink = "OpenCorsairLink";
    opencv4 = "opencv";
    opencv4WithoutCuda = "opencv";
    openipmi = "OpenIPMI";
    openiscsi = "open-iscsi";
    openjdk11 = "openjdk";
    openjdk11-bootstrap = "temurin-bin";
    openjdk11_headless = "openjdk-headless";
    openjdk17 = "openjdk";
    openjdk17-bootstrap = "temurin-bin";
    openjdk17_headless = "openjdk-headless";
    openjdk21 = "openjdk";
    openjdk21_headless = "openjdk-headless";
    openjdk25 = "openjdk";
    openjdk25_headless = "openjdk-headless";
    openjdk8 = "openjdk";
    openjdk8-bootstrap = "temurin-bin";
    openjdk8_headless = "openjdk-headless";
    openjdk_headless = "openjdk-headless";
    openjfx = "openjfx-modular-sdk";
    openjfx17 = "openjfx-modular-sdk";
    openjfx21 = "openjfx-modular-sdk";
    openjfx25 = "openjfx-modular-sdk";
    openmoji-black = "openmoji";
    openmoji-color = "openmoji";
    openmpCheckPhaseHook = "check-phase-thread-limit-hook";
    openntpd_nixos = "openntpd";
    openorienteering-mapper = "OpenOrienteering-Mapper";
    openra = "openra-release";
    openrgb-with-all-plugins = "openrgb-with-plugins";
    opensplatWithCuda = "opensplat";
    opensplatWithRocm = "opensplat";
    opensshTest = "openssh-test";
    opensshWithKerberos = "openssh";
    openssh_gssapi = "openssh-with-gssapi";
    openssh_hpn = "openssh-with-hpn";
    openssh_hpnWithKerberos = "openssh-with-hpn";
    openssl_3 = "openssl";
    openssl_3_5 = "openssl";
    openssl_3_6 = "openssl";
    openssl_4_0 = "openssl";
    openssl_legacy = "openssl";
    openssl_oqs = "openssl";
    openstackclient = "python-openstackclient";
    openstackclient-full = "python-openstackclient";
    opentabletdriver = "OpenTabletDriver";
    opentelemetry-collector = "otelcol";
    opentelemetry-collector-builder = "ocb";
    opentelemetry-collector-contrib = "otelcol-contrib";
    opentofu-ls = "tofu-ls";
    opentxl = "opentxl-wrapper";
    opentxl-unwrapped = "opentxl";
    openutau = "OpenUtau";
    openvpn_learnaddress = "openvpn-learnaddress";
    ophcrack-cli = "ophcrack";
    opteeAllwinnerA64 = "optee-os-sunxi";
    opteeQemuAarch64 = "optee-os-vexpress";
    opteeQemuArm = "optee-os-vexpress";
    opteeRockchipRK3399 = "optee-os-rockchip";
    opteeRockchipRK3588 = "optee-os-rockchip";
    optiland = "optiland-python-env";
    opusTools = "opus-tools";
    orioledb = "orioledb-postgres-and-plugins";
    ory-hydra = "hydra";
    ory-talos = "talos";
    osl = "openshadinglanguage";
    ostree-full = "ostree";
    otto-matic = "OttoMatic";
    ouch-rar = "ouch";
    oxtools = "0xtools";
    p4est-dbg = "p4est";
    p4est-sc-dbg = "p4est-sc";
    p7zip-rar = "p7zip";
    pa_applet = "pa-applet";
    packet-sd = "prometheus-packet-sd";
    packetbeat7 = "packetbeat";
    pagefind = "pagefind-extended";
    paho-mqtt-c = "paho.mqtt.c";
    paho-mqtt-cpp = "paho.mqtt.cpp";
    palemoon-gtk2-bin = "palemoon-bin";
    palenight-theme = "gtk-theme-framework";
    pam = "linux-pam";
    pam-reattach = "pam_reattach";
    pam_krb5 = "pam-krb5";
    pam_pgsql = "pam-pgsql";
    pandoc = "pandoc-cli";
    pangomm_2_42 = "pangomm";
    pangomm_2_48 = "pangomm";
    panotools = "libpano13";
    pantalaimon-headless = "pantalaimon";
    parallel-full = "parallel";
    pass = "password-store";
    pass-nodmenu = "password-store";
    pass-wayland = "password-store";
    patchPpdFilesHook = "patch-ppd-files";
    patchRcPathBash = "patch-rc-path-bash";
    patchRcPathCsh = "patch-rc-path-csh";
    patchRcPathFish = "patch-rc-path-fish";
    patchRcPathPosix = "patch-rc-path-posix";
    patchelfUnstable = "patchelf";
    patchutils_0_3_3 = "patchutils";
    patchutils_0_4_2 = "patchutils";
    path-of-building = "rusty-path-of-building";
    pb_cli = "pb_cli-unstable";
    pbrt = "pbrt-v3";
    pcg_c = "pcg-c";
    pcmciaUtils = "pcmciautils";
    pcsc-scm-scl011 = "pcsc-scm-scl";
    pcscliteWithPolkit = "pcsclite-with-polkit";
    pdfium-binaries = "pdfium";
    pdfminer = "pdfminer-six";
    pdk-ciel = "ciel";
    percona-server_8_4 = "percona-server";
    percona-toolkit = "perl5.42.3-Percona-Toolkit-3.7.1";
    percona-xtrabackup_8_4 = "percona-xtrabackup";
    perf = "perf-linux";
    perl5 = "perl";
    perlcritic = "Perl-Critic";
    pff-tool = "pff-tools";
    pfft = "pfft-double";
    pflotran = "PFLOTRAN";
    pgadmin4 = "pgadmin";
    pgadmin4-desktopmode = "pgadmin";
    pgf-umlsd = "pgf-umlcd";
    pgf2 = "pgf";
    pgf3 = "pgf";
    phash = "pHash";
    phira-free = "phira";
    photon-rss = "photon";
    php = "php-with-extensions";
    php82 = "php-with-extensions";
    php83 = "php-with-extensions";
    php84 = "php-with-extensions";
    php85 = "php-with-extensions";
    physfs_2 = "physfs";
    picolisp = "PicoLisp";
    pinegrow6 = "pinegrow";
    pinentry_mac = "pinentry-mac";
    pinnacle = "pinnacle-server";
    pinta = "Pinta";
    pkcs11helper = "pkcs11-helper";
    pkg-config-unwrapped = "pkg-config";
    pkgconf-unwrapped = "pkgconf";
    platformio-chrootenv = "platformio";
    platformio-core = "platformio";
    platinum-searcher = "the_platinum_searcher";
    playwright = "playwright-core";
    playwright-driver = "playwright-core";
    plex = "plexmediaserver";
    plexRaw = "plexmediaserver";
    plover_4 = "plover";
    plover_5 = "plover";
    pmars-x11 = "pmars";
    pmutils = "pm-utils";
    pnfft = "pnfft-double";
    pnpmBuildHook = "pnpm-build-hook";
    pnpmConfigHook = "pnpm-config-hook";
    pnpm_10 = "pnpm";
    pnpm_10_29_2 = "pnpm";
    pnpm_10_34_0 = "pnpm";
    pnpm_11 = "pnpm";
    podofo0 = "podofo";
    podofo_0_10 = "podofo";
    pokerth = "pokerth-client";
    polkit_gnome = "polkit-gnome";
    polybarFull = "polybar";
    pommed_light = "pommed-light";
    pong3d = "3dpong";
    pony-corral = "corral";
    poppler = "poppler-glib";
    poppler_data = "poppler-data";
    poppler_gi = "poppler-glib";
    poppler_min = "poppler-min";
    portfolio = "PortfolioPerformance";
    portfolio-filemanager = "portfolio";
    postgres-lsp = "postgres-language-server";
    postgresqlTestHook = "postgresql-test-hook";
    postgresql_14 = "postgresql";
    postgresql_14_jit = "postgresql-and-plugins";
    postgresql_15 = "postgresql";
    postgresql_15_jit = "postgresql-and-plugins";
    postgresql_16 = "postgresql";
    postgresql_16_jit = "postgresql-and-plugins";
    postgresql_17 = "postgresql";
    postgresql_17_jit = "postgresql-and-plugins";
    postgresql_18 = "postgresql";
    postgresql_18_jit = "postgresql-and-plugins";
    postgresql_19 = "postgresql";
    postgresql_19_jit = "postgresql-and-plugins";
    postgresql_jdbc = "postgresql-jdbc";
    postgresql_jit = "postgresql-and-plugins";
    potreeconverter = "PotreeConverter";
    ppsspp = "ppsspp-sdl";
    pqc-scanner = "pqc-audit";
    precice-config-visualizer = "config-visualizer";
    premake4 = "premake";
    present-cli = "present";
    primusLib = "primus-lib";
    prisma = "prisma_7";
    prisma-engines = "prisma-engines_7";
    projectm_3 = "projectm";
    prometheus-alertmanager = "alertmanager";
    prometheus-apcupsd-exporter = "apcupsd-exporter";
    prometheus-artifactory-exporter = "artifactory_exporter";
    prometheus-atlas-exporter = "atlas-exporter";
    prometheus-aws-s3-exporter = "aws-s3-exporter";
    prometheus-bind-exporter = "bind_exporter";
    prometheus-bird-exporter = "bird-exporter";
    prometheus-bitcoin-exporter = "bitcoin-prometheus-exporter";
    prometheus-blackbox-exporter = "blackbox_exporter";
    prometheus-cloudflare-exporter = "cloudflare-exporter";
    prometheus-collectd-exporter = "collectd-exporter";
    prometheus-consul-exporter = "consul_exporter";
    prometheus-dcgm-exporter = "dcgm-exporter";
    prometheus-deluge-exporter = "deluge-exporter";
    prometheus-dnsmasq-exporter = "dnsmasq_exporter";
    prometheus-domain-exporter = "domain-exporter";
    prometheus-elasticsearch-exporter = "elasticsearch_exporter";
    prometheus-fastly-exporter = "fastly-exporter";
    prometheus-flow-exporter = "flow-exporter";
    prometheus-fritzbox-exporter = "fritzbox-exporter";
    prometheus-gitlab-ci-pipelines-exporter = "gitlab-ci-pipelines-exporter";
    prometheus-graphite-exporter = "graphite-exporter";
    prometheus-haproxy-exporter = "haproxy_exporter";
    prometheus-idrac-exporter = "idrac_exporter";
    prometheus-imap-mailstat-exporter = "imap-mailstat-exporter";
    prometheus-influxdb-exporter = "influxdb_exporter";
    prometheus-ipmi-exporter = "ipmi_exporter";
    prometheus-jitsi-exporter = "jitsiexporter";
    prometheus-jmx-httpserver = "jmx-prometheus-httpserver";
    prometheus-jmx-javaagent = "jmx-prometheus-javaagent";
    prometheus-junos-czerwonk-exporter = "junos-czerwonk-exporter";
    prometheus-kea-exporter = "kea-exporter";
    prometheus-keylight-exporter = "keylight-exporter";
    prometheus-knot-exporter = "knot-exporter";
    prometheus-lnd-exporter = "lndmon";
    prometheus-mail-exporter = "mailexporter";
    prometheus-mailman3-exporter = "mailman3-exporter";
    prometheus-mikrotik-exporter = "mikrotik-exporter-unstable";
    prometheus-modemmanager-exporter = "modemmanager-exporter";
    prometheus-mongodb-exporter = "mongodb_exporter";
    prometheus-mysqld-exporter = "mysqld_exporter";
    prometheus-nginx-exporter = "nginx_exporter";
    prometheus-nginxlog-exporter = "nginxlog_exporter";
    prometheus-node-cert-exporter = "node-cert-exporter";
    prometheus-node-exporter = "node_exporter";
    prometheus-nut-exporter = "nut-exporter";
    prometheus-pgbouncer-exporter = "pgbouncer-exporter";
    prometheus-php-fpm-exporter = "php-fpm_exporter";
    prometheus-pihole-exporter = "pihole-exporter";
    prometheus-ping-exporter = "ping-exporter";
    prometheus-postfix-exporter = "postfix_exporter";
    prometheus-postgres-exporter = "postgres_exporter";
    prometheus-process-exporter = "process-exporter";
    prometheus-pushgateway = "pushgateway";
    prometheus-pve-exporter = "prometheus_pve_exporter";
    prometheus-rabbitmq-exporter = "rabbitmq_exporter";
    prometheus-redis-exporter = "redis_exporter";
    prometheus-rtl_433-exporter = "rtl_433-exporter";
    prometheus-sabnzbd-exporter = "sabnzbd_exporter";
    prometheus-sachet = "sachet";
    prometheus-script-exporter = "script_exporter";
    prometheus-shelly-exporter = "shelly_exporter";
    prometheus-smartctl-exporter = "smartctl_exporter";
    prometheus-smokeping-prober = "smokeping_prober";
    prometheus-snmp-exporter = "snmp_exporter";
    prometheus-sql-exporter = "sql_exporter";
    prometheus-squid-exporter = "squid-exporter";
    prometheus-statsd-exporter = "statsd_exporter";
    prometheus-surfboard-exporter = "surfboard_exporter";
    prometheus-systemd-exporter = "systemd_exporter";
    prometheus-tailscale-exporter = "tailscale-exporter";
    prometheus-unbound-exporter = "unbound_exporter";
    prometheus-v2ray-exporter = "v2ray-exporter";
    prometheus-varnish-exporter = "prometheus_varnish_exporter";
    prometheus-wireguard-exporter = "wireguard-exporter";
    prometheus-zfs-exporter = "zfs_exporter";
    protobuf_21 = "protobuf";
    protobuf_25 = "protobuf";
    protobuf_27 = "protobuf";
    protobuf_29 = "protobuf";
    protobuf_30 = "protobuf";
    protobuf_31 = "protobuf";
    protobuf_32 = "protobuf";
    protobuf_33 = "protobuf";
    protobuf_34 = "protobuf";
    protobuf_35 = "protobuf";
    protobuf_36 = "protobuf";
    protobufc = "protobuf-c";
    proton-authenticator-bin = "proton-authenticator";
    protonvpn-gui = "proton-vpn";
    prototypejs = "prototype";
    proverifWithGui = "proverif";
    pruneLibtoolFiles = "prune-libtool-files";
    ps = "procps";
    ps2patchelf = "PS2PatchElf";
    psc-package = "psc-package-simple";
    pstreams = "PStreams";
    pulseaudioFull = "pulseaudio";
    pulseeffects-legacy = "pulseeffects";
    pulumi-bin = "pulumi";
    punes-qt6 = "punes";
    puppet = "openvox";
    puppet-bolt = "bolt";
    py-wacz = "wacz";
    pyglossary-gui = "pyglossary";
    pypy2 = "pypy";
    pypy27 = "pypy";
    pypy3 = "pypy3.11";
    pypy310 = "pypy3.10";
    pypy311 = "pypy3.11";
    python311 = "python3";
    python312 = "python3";
    python313 = "python3";
    python313FreeThreading = "python3";
    python314 = "python3";
    python314FreeThreading = "python3";
    python315 = "python3";
    python315FreeThreading = "python3";
    python3Minimal = "python3-minimal";
    pywal = "pywal16";
    qMasterPassword = "qmasterpassword";
    qMasterPassword-wayland = "qmasterpassword";
    qadwaitadecorations-qt6 = "qadwaitadecorations";
    qcdnum = "QCDNUM";
    qemu-python-utils = "qemu";
    qemu_full = "qemu";
    qemu_kvm = "qemu-host-cpu-only";
    qemu_test = "qemu-for-vm-tests";
    qemu_xen = "qemu-xen";
    qgnomeplatform-qt6 = "qgnomeplatform";
    qidi-slicer-bin = "qidi-slicer";
    qmasterpassword-wayland = "qmasterpassword";
    qmediathekview = "QMediathekView";
    qmplay2 = "qmplay2-qt6";
    qsvlite = "qsv";
    qtrvsim = "QtRVSim";
    quake3arena = "quake3-ioquake3-0-unstable-2026-07-19";
    quake3arena-hires = "quake3-ioquake3-0-unstable-2026-07-19";
    quake3demo = "quake3-demo-ioquake3-0-unstable-2026-07-19";
    quake3demo-hires = "quake3-demo-ioquake3-0-unstable-2026-07-19";
    quake3demodata = "quake3-demodata";
    quake3e = "Quake3e";
    quake3pointrelease = "quake3-pointrelease";
    quarkus = "quarkus-cli";
    quartoMinimal = "quarto";
    quassel = "quassel-kf5";
    quasselClient = "quassel-client-qt5";
    quasselDaemon = "quassel-daemon-qt5";
    quaternion-qt6 = "quaternion";
    qutebrowser-qt5 = "qutebrowser";
    rHttp = "rhttp";
    ra-multiplex = "lspmux";
    racket-minimal = "racket";
    radeontools = "radeontool";
    radiance-vj = "radiance";
    radicle-node-unstable = "radicle-node";
    ragelDev = "ragel";
    ragelStable = "ragel";
    rain-bittorrent = "rain";
    rainloop-standard = "rainloop";
    randomx = "randomX";
    rare-regex = "rare";
    raspberrypiWirelessFirmware = "raspberrypi-wireless-firmware";
    raspberrypifw = "raspberrypi-firmware";
    raxml = "RAxML";
    raxml-mpi = "RAxML-mpi";
    re-isearch = "re-Isearch";
    readline70 = "readline";
    real_time_config_quick_scan = "realTimeConfigQuickScan";
    realesrgan-ncnn-vulkan = "Real-ESRGAN-ncnn-vulkan";
    reason-shell = "reason";
    reaverwps = "reaver-wps";
    reaverwps-t6x = "reaver-wps-t6x";
    recastnavigation = "recastai";
    recoll-nox = "recoll";
    redhat-official-fonts = "redhat-official";
    redisTestHook = "redis-test-hook";
    redpanda-client = "redpanda-rpk";
    redshift_jdbc = "redshift-jdbc";
    regbot = "regclient";
    regctl = "regclient";
    regina = "regina-rexx";
    regsync = "regclient";
    renode-bin = "renode";
    renode-unstable-bin = "renode-unstable";
    renpyMinimal = "renpy";
    reposilite = "Reposilite";
    retroarch = "retroarch-with-cores";
    retroarch-free = "retroarch-with-cores";
    retroarch-full = "retroarch-with-cores";
    rex = "Rex";
    rhodium-libre = "RhodiumLibre";
    riemann_c_client = "riemann-c-client";
    rkdeveloptool-pine64 = "rkdeveloptool";
    rke2_1_33 = "rke2";
    rke2_1_34 = "rke2";
    rke2_1_35 = "rke2";
    rke2_1_36 = "rke2";
    rke2_latest = "rke2";
    rke2_stable = "rke2";
    rmate-sh = "rmate";
    rmg-wayland = "rmg";
    rocksdb_6_23 = "rocksdb";
    rocksdb_7_10 = "rocksdb";
    rocksdb_8_11 = "rocksdb";
    rocksdb_8_3 = "rocksdb";
    rocksdb_9_10 = "rocksdb";
    rocq-core = "rocq";
    rocq-core_9_0 = "rocq";
    rocq-core_9_1 = "rocq";
    rocq-core_9_2 = "rocq";
    rocq-core_9_3 = "rocq";
    rofi-file-browser = "rofi-file-browser-extended";
    rofi-rbw-wayland = "rofi-rbw";
    rofi-rbw-x11 = "rofi-rbw";
    rosecFull = "rosec";
    rotp = "remnants-of-the-precursors";
    rpPPPoE = "rp-pppoe";
    rsclock = "rsClock";
    rstcheckWithSphinx = "rstcheck";
    rstudio-server = "rstudio";
    rstudioServerWrapper = "rstudio";
    rstudioWrapper = "rstudio";
    rsyslog-light = "rsyslog";
    rtaudio_5 = "rtaudio";
    rtaudio_6 = "rtaudio";
    rtl-sdr = "rtl-sdr-blog";
    rtl_wmbus = "rtl-wmbus";
    rtmpdump_gnutls = "rtmpdump";
    ruby_3_3 = "ruby";
    ruby_3_4 = "ruby";
    ruby_3_5 = "ruby";
    ruby_4_0 = "ruby";
    runtimeShellPackage = "bash";
    rust-jemalloc-sys = "jemalloc";
    rust-jemalloc-sys-unprefixed = "jemalloc";
    rustc-unwrapped = "rustc";
    rustdesk-flutter = "rustdesk";
    rusty-psn-gui = "rusty-psn";
    rxvt-unicode-emoji = "rxvt-unicode";
    rxvt-unicode-unwrapped = "rxvt-unicode";
    rxvt-unicode-unwrapped-emoji = "rxvt-unicode";
    s3fs = "s3fs-fuse";
    sable = "sable-unwrapped";
    safety-cli = "safety";
    sageWithDoc = "sage";
    samba4 = "samba";
    samba4Full = "samba";
    sambaFull = "samba";
    samsung-unified-linux-driver_1_00_36 = "samsung-unified-linux-driver";
    samsung-unified-linux-driver_1_00_37 = "samsung-unified-linux-driver";
    samsung-unified-linux-driver_4_01_17 = "samsung-unified-linux-driver";
    sarabun-font = "sarabun";
    sasquatch-v4be = "sasquatch";
    saxon_11-he = "saxon-he";
    saxon_12-he = "saxon-he";
    saxon_9-he = "saxon-he";
    saxonb_8_8 = "saxonb";
    saxonb_9_1 = "saxonb";
    sbarlua = "sbarLua";
    sbcl_2_4_6 = "sbcl";
    sbcl_2_6_6 = "sbcl";
    sbcl_2_6_7 = "sbcl";
    sbt-with-scala-native = "sbt";
    scala_2_12 = "scala";
    scala_2_13 = "scala";
    scala_3 = "scala";
    scalapack-ilp64 = "scalapack";
    scenefx_0_5 = "scenefx";
    screego = "screego-server";
    scss-lint = "scss_lint";
    sd-mux-ctrl = "sd-mux-ctrl-unstable";
    sdbus-cpp_2 = "sdbus-cpp";
    seabios-coreboot = "seabios";
    seabios-csm = "seabios";
    seabios-qemu = "seabios";
    segger-jlink-headless = "segger-jlink";
    selendroid = "selendroid-standalone";
    semeru-bin-11 = "semeru-bin";
    semeru-bin-17 = "semeru-bin";
    semeru-bin-21 = "semeru-bin";
    semeru-bin-8 = "semeru-bin";
    semeru-jre-bin-11 = "semeru-jre-bin";
    semeru-jre-bin-17 = "semeru-jre-bin";
    semeru-jre-bin-21 = "semeru-jre-bin";
    semeru-jre-bin-8 = "semeru-jre-bin";
    sensible-side-buttons = "sensible-side-buttons-bin";
    separateDebugInfo = "separate-debug-info-hook";
    sequelpro = "sequel-pro";
    sequoia-sqop = "sequoia-sop";
    serfdom = "serf";
    setJavaClassPath = "set-java-classpath-hook";
    setupDebugInfoDirs = "setup-debug-info-dirs-hook";
    sfml_2 = "sfml";
    sg-323 = "SG-323";
    sgt-puzzles-mobile = "sgt-puzzles";
    sgx-azure-dcap-client = "azure-dcap-client";
    shairplay = "shairplay-unstable";
    shairport-sync-airplay2 = "shairport-sync";
    sharedown = "Sharedown";
    shellcheck = "ShellCheck";
    shellcheck-minimal = "ShellCheck";
    shim-unsigned = "shim";
    shopify-cli = "shopify";
    shortenPerlShebang = "shorten-perl-shebang-hook";
    shticker-book-unwritten = "shticker_book_unwritten";
    sic-image-cli = "imagineer";
    siesta-mpi = "siesta";
    sieveshell = "managesieve";
    siketyan-ghr = "ghr";
    sil = "Sil";
    simp_le = "simp_le-client";
    simple64-netplay-server = "gopher64-netplay-server";
    simpleBluez = "simplebluez";
    simpleBuildTool = "sbt";
    simpleDBus = "simpledbus";
    simpleTpmPk11 = "simple-tpm-pk11";
    simulide_0_4_15 = "simulide";
    simulide_1_0_0 = "simulide";
    simulide_1_1_0 = "simulide";
    simulide_1_2_0 = "simulide";
    simutrans_binaries = "simutrans";
    singularity = "singularity-ce";
    singularity-overriden-nixos = "singularity-ce";
    sirius = "SIRIUS";
    skalibs_2_10 = "skalibs";
    skimpdf = "Skim";
    slade-unstable = "slade";
    sladeUnstable = "slade";
    slibGuile = "slib";
    slurp-fediverse = "slurp";
    smarty3-i18n = "smarty-i18n";
    smc-chilanka = "chilanka";
    smlnj-legacy = "smlnj";
    snappy-topology = "snappy";
    snell = "snell-server";
    snis-assets = "snis_assets";
    snis-unwrapped = "snis";
    soapysdr-with-plugins = "soapysdr";
    soapysdrplay = "soapysdr-sdrplay3";
    sofia_sip = "sofia-sip";
    sonar-scanner-cli-minimal = "sonar-scanner-cli";
    sonic = "sonic-unstable";
    sonota = "sonota-unstable";
    sony-headphones-client = "SonyHeadphonesClient";
    soundfont-fluid = "Fluid";
    soundfont-generaluser = "soundfont-generaluser-gs";
    soundfont-ydp-grand = "ydp-grand";
    soundwireserver = "soundwire";
    sourceHighlight = "source-highlight";
    sox_ng = "sox";
    space-cadet-pinball = "SpaceCadetPinball";
    space-station-14-launcher = "SS14.Launcher";
    spacx-gtk-theme = "gtk-theme-framework";
    spago = "spago-legacy";
    spamassassin = "SpamAssassin";
    spandsp3 = "spandsp";
    spark3 = "spark";
    spark4 = "spark";
    spark_3_4 = "spark";
    spark_3_5 = "spark";
    spark_4_0 = "spark";
    spawn_fcgi = "spawn-fcgi";
    speech-tools = "speech_tools";
    speechd = "speech-dispatcher";
    speechd-minimal = "speech-dispatcher";
    speed_dreams = "speed-dreams";
    spfft = "SpFFT";
    sphinxHook = "python3.14-sphinx-hook";
    spicedb-zed = "zed";
    spicy-parser-generator = "spicy";
    spidermonkey_128 = "spidermonkey";
    spidermonkey_140 = "spidermonkey";
    spire-agent = "spire";
    spire-server = "spire";
    spirv-llvm-translator = "SPIRV-LLVM-Translator";
    splayer = "splayer-next";
    splix = "splix-svn";
    spoofdpi = "SpoofDPI";
    spoofer-gui = "spoofer";
    sqitchMysql = "sqitch-mysql";
    sqitchPg = "sqitch-pg";
    sqitchSqlite = "sqitch-sqlite";
    sqlboiler-crdb = "sqlboiler-crbd";
    sqlitestudio = "letos";
    sqlitestudio-plugins = "letos-plugins";
    squashfsTools = "squashfs-tools";
    srtp = "libsrtp";
    ssb-patchwork = "patchwork";
    sshfs = "sshfs-fuse";
    sstp = "sstp-client";
    stabber = "stabber-unstable";
    stable-diffusion-cpp-cuda = "stable-diffusion-cpp";
    stable-diffusion-cpp-rocm = "stable-diffusion-cpp";
    stable-diffusion-cpp-vulkan = "stable-diffusion-cpp";
    stalwart-mail = "stalwart";
    stalwart-proxy = "proxy";
    stalwart-spam-filter = "spam-filter";
    stalwart-vandelay = "vandelay";
    stalwart-webadmin = "webadmin";
    stalwart_0_15 = "stalwart";
    stalwart_0_16 = "stalwart";
    status-im = "status-desktop";
    stc-cli = "stc";
    stdmanpages = "std-man-pages";
    steam-run-free = "steam-run";
    stlink-gui = "stlink";
    storcli2 = "storcli";
    storeBackup = "storebackup";
    storm = "apache-storm";
    strichliste = "strichliste-backend";
    stripJavaArchivesHook = "strip-java-archives-hook";
    strongswanNM = "strongswan";
    strongswanTNC = "strongswan";
    strongswanTPM = "strongswan";
    stumpwm-unwrapped = "stumpwm";
    stunner-webrtc = "stunner";
    su = "shadow";
    sublime-merge-dev = "sublime-merge";
    sublime3 = "sublimetext3";
    sublime3-dev = "sublimetext3";
    sublime4 = "sublimetext4";
    sublime4-dev = "sublimetext4";
    subtitlr = "Subtitlr";
    subversionClient = "subversion-client";
    super-slicer-beta = "super-slicer";
    super-slicer-latest = "super-slicer";
    superTux = "supertux";
    superTuxKart = "supertuxkart";
    supercollider-with-plugins = "supercollider";
    supercollider-with-sc3-plugins = "supercollider";
    supercollider_scel = "supercollider";
    surfpool = "surfpool-cli";
    surge-XT = "surge-xt";
    sv-lang_10 = "sv-lang";
    sv-lang_9 = "sv-lang";
    svgcleaner = "SVGCleaner";
    svox = "picotts";
    swaynotificationcenter = "SwayNotificationCenter";
    swh_lv2 = "swh-lv2";
    swi-prolog-gui = "swi-prolog";
    swift = "swift-wrapper";
    swiftclient = "python-swiftclient";
    switch-to-configuration-ng = "switch-to-configuration";
    swww = "awww";
    synapse-admin-etkecc = "ketesa";
    syncplay-nogui = "syncplay";
    syncstorage-rs-mysql = "syncstorage-rs";
    syncstorage-rs-pgsql = "syncstorage-rs";
    syncthingtray-minimal = "syncthingtray";
    synergyWithoutGUI = "synergy";
    sysctl = "procps";
    syslogng = "syslog-ng";
    systemd-journal2gelf = "SystemdJournal2Gelf";
    systemdLibs = "systemd-minimal-libs";
    systemdMinimal = "systemd-minimal";
    systemdUkify = "systemd";
    systemtap-sdt = "systemtap";
    systemtap-unwrapped = "systemtap";
    taffybar = "taffybar-with-packages";
    taglib_1 = "taglib";
    taglib_extras = "taglib-extras";
    tailor-gui = "tailor_gui";
    tailwindcss = "tailwindcss_3";
    tambura = "Tambura";
    tamgamp-lv2 = "tamgamp.lv2";
    tamsyn = "tamsyn-font";
    tamzen = "tamzen-font";
    tandem-aligner = "TandemAligner";
    tartube-yt-dlp = "tartube";
    taskwarrior2 = "taskwarrior";
    taskwarrior3 = "taskwarrior";
    taxi-cli = "taxi";
    tbb = "onetbb";
    tbb_2022 = "onetbb";
    tcl-8_6 = "tcl";
    tcl-9_0 = "tcl";
    tclap_1_2 = "tclap";
    tclap_1_4 = "tclap";
    tcp_wrappers = "tcp-wrappers";
    teamspeak_server = "teamspeak-server";
    tectonic = "tectonic-wrapped";
    tectonic-unwrapped = "tectonic";
    teensyduino = "arduino";
    teetertorture = "teeter-torture";
    teeworlds-server = "teeworlds";
    teleport_17 = "teleport";
    teleport_18 = "teleport";
    temurin-bin-11 = "temurin-bin";
    temurin-bin-17 = "temurin-bin";
    temurin-bin-21 = "temurin-bin";
    temurin-bin-25 = "temurin-bin";
    temurin-bin-26 = "temurin-bin";
    temurin-bin-8 = "temurin-bin";
    temurin-jre-bin-11 = "temurin-jre-bin";
    temurin-jre-bin-17 = "temurin-jre-bin";
    temurin-jre-bin-21 = "temurin-jre-bin";
    temurin-jre-bin-25 = "temurin-jre-bin";
    temurin-jre-bin-26 = "temurin-jre-bin";
    temurin-jre-bin-8 = "temurin-jre-bin";
    terminus_font = "terminus-font";
    terminus_font_ttf = "terminus-font-ttf";
    termonad = "termonad-with-packages";
    termpdfpy = "termpdf.py";
    terraform-landscape = "terraform_landscape";
    terraform-local = "terraform_local";
    terraform_1 = "terraform";
    tesseract3 = "tesseract";
    tesseract4 = "tesseract";
    tesseract5 = "tesseract";
    testdisk-qt = "testdisk";
    testssl = "testssl.sh";
    tetgen_1_4 = "tetgen";
    texinfo7 = "texinfo";
    texinfoInteractive = "texinfo-interactive";
    texliveBasic = "texlive";
    texliveBookPub = "texlive";
    texliveConTeXt = "texlive";
    texliveFull = "texlive";
    texliveFullWithDocs = "texlive";
    texliveGUST = "texlive";
    texliveInfraOnly = "texlive";
    texliveMedium = "texlive";
    texliveMinimal = "texlive";
    texliveSmall = "texlive";
    texliveTeTeX = "texlive";
    thcrap-steam-proton-wrapper = "thcrap-proton";
    thunar-dropbox-plugin = "thunar-dropbox";
    thunar-unwrapped = "thunar";
    thunderbird-140 = "thunderbird";
    thunderbird-140-unwrapped = "thunderbird-unwrapped";
    thunderbird-153 = "thunderbird";
    thunderbird-153-unwrapped = "thunderbird-unwrapped";
    thunderbird-esr = "thunderbird";
    thunderbird-esr-bin-unwrapped = "thunderbird-bin";
    thunderbird-esr-unwrapped = "thunderbird-unwrapped";
    thunderbird-latest = "thunderbird";
    thunderbird-latest-bin = "thunderbird-bin";
    thunderbird-latest-bin-unwrapped = "thunderbird-bin";
    thunderbird-latest-unwrapped = "thunderbird-unwrapped";
    tilinggallery = "tiling-gallery";
    timeshift-minimal = "timeshift";
    timeshift-unwrapped = "timeshift";
    tinc_pre = "tinc";
    tiny8086 = "8086tiny";
    tinycc = "tcc";
    tinyxml-2 = "tinyxml2";
    tk-8_6 = "tk";
    tk-9_0 = "tk";
    tkmm = "Tkmm";
    tlaplus18 = "tlaplus";
    tlaplusToolbox = "tlaplus-toolbox";
    tldr-hs = "tldr";
    todo-txt-cli = "todo.txt-cli";
    todofi-sh = "todofi.sh";
    tomcat = "apache-tomcat";
    tomcat10 = "apache-tomcat";
    tomcat11 = "apache-tomcat";
    tomcat9 = "apache-tomcat";
    tomcat_mysql_jdbc = "tomcat-mysql-jdbc";
    tooling-language-server = "deputy";
    top-git = "topgit";
    tpm2-pkcs11-abrmd = "tpm2-pkcs11";
    tpm2-pkcs11-esapi = "tpm2-pkcs11";
    tpm2-pkcs11-fapi = "tpm2-pkcs11";
    tpm2-totp-with-plymouth = "tpm2-totp";
    tptp = "TPTP";
    trackma-curses = "trackma";
    trackma-gtk = "trackma";
    trackma-qt = "trackma";
    tracy_0_11 = "tracy";
    tracy_0_12 = "tracy";
    tracy_0_13 = "tracy";
    tranquil-pds-frontend = "tranquil-frontend";
    transmission_4 = "transmission";
    transmission_4-gtk = "transmission";
    transmission_4-mac = "transmission";
    transmission_4-qt = "transmission";
    transmission_4-qt5 = "transmission";
    transmission_4-qt6 = "transmission";
    trezorctl = "trezor";
    trezord = "trezord-go";
    trgui-ng = "TrguiNG";
    trgui-ng-web = "TrguiNG-frontend";
    trigger = "trigger-rally";
    trilinos-mpi = "trilinos";
    trilium-next-desktop = "trilium-desktop";
    trilium-next-server = "trilium-server";
    troveclient = "python-troveclient";
    truecrack-cuda = "truecrack";
    trustedqsl = "tqsl";
    tshark = "wireshark-cli";
    tsm-client-withGui = "tsm-client";
    tt2020 = "TT2020";
    ttf_bitstream_vera = "ttf-bitstream-vera";
    ttfautohint-nox = "ttfautohint";
    ttmkfdir = "ttf-mkfontdir";
    tts = "coqui-tts";
    tubekit-unwrapped = "tubekit";
    tuxclocker-without-unfree = "tuxclocker";
    twemoji-color-font-src = "twemoji-color-font";
    twinejs = "twine";
    typescript_5 = "typescript";
    typescript_7 = "typescript";
    u-root-cmds = "u-root";
    u001-font = "u001";
    ubootA20OlinuxinoLime = "uboot-A20-OLinuXino-Lime_defconfig";
    ubootA20OlinuxinoLime2EMMC = "uboot-A20-OLinuXino-Lime2-eMMC_defconfig";
    ubootAmx335xEVM = "uboot-am335x_evm_defconfig";
    ubootBananaPi = "uboot-Bananapi_defconfig";
    ubootBananaPim2Zero = "uboot-bananapi_m2_zero_defconfig";
    ubootBananaPim3 = "uboot-Sinovoip_BPI_M3_defconfig";
    ubootBananaPim64 = "uboot-bananapi_m64_defconfig";
    ubootCM3588NAS = "uboot-cm3588-nas-rk3588_defconfig";
    ubootClearfog = "uboot-clearfog_defconfig";
    ubootCubieboard2 = "uboot-Cubieboard2_defconfig";
    ubootGuruplug = "uboot-guruplug_defconfig";
    ubootJetsonTK1 = "uboot-jetson-tk1_defconfig";
    ubootLibreTechCC = "uboot-libretech-cc_defconfig";
    ubootNanoPCT4 = "uboot-nanopc-t4-rk3399_defconfig";
    ubootNanoPCT6 = "uboot-nanopc-t6-rk3588_defconfig";
    ubootNanoPiR5S = "uboot-nanopi-r5s-rk3568_defconfig";
    ubootNovena = "uboot-novena_defconfig";
    ubootOdroidC2 = "uboot-odroid-c2_defconfig";
    ubootOdroidXU3 = "uboot-odroid-xu3_defconfig";
    ubootOlimexA64Olinuxino = "uboot-a64-olinuxino-emmc_defconfig";
    ubootOlimexA64Teres1 = "uboot-teres_i_defconfig";
    ubootOrangePi3 = "uboot-orangepi_3_defconfig";
    ubootOrangePi3B = "uboot-orangepi-3b-rk3566_defconfig";
    ubootOrangePi5 = "uboot-orangepi-5-rk3588s_defconfig";
    ubootOrangePi5Max = "uboot-orangepi-5-max-rk3588_defconfig";
    ubootOrangePi5Plus = "uboot-orangepi-5-plus-rk3588_defconfig";
    ubootOrangePiPc = "uboot-orangepi_pc_defconfig";
    ubootOrangePiZero = "uboot-orangepi_zero_defconfig";
    ubootOrangePiZero2 = "uboot-orangepi_zero2_defconfig";
    ubootOrangePiZero3 = "uboot-orangepi_zero3_defconfig";
    ubootOrangePiZeroPlus2H5 = "uboot-orangepi_zero_plus2_defconfig";
    ubootPcduino3Nano = "uboot-Linksprite_pcDuino3_Nano_defconfig";
    ubootPine64 = "uboot-pine64_plus_defconfig";
    ubootPine64LTS = "uboot-pine64-lts_defconfig";
    ubootPinebook = "uboot-pinebook_defconfig";
    ubootPinebookPro = "uboot-pinebook-pro-rk3399_defconfig";
    ubootQemuAarch64 = "uboot-qemu_arm64_defconfig";
    ubootQemuArm = "uboot-qemu_arm_defconfig";
    ubootQemuRiscv64Smode = "uboot-qemu-riscv64_smode_defconfig";
    ubootQemuX86 = "uboot-qemu-x86_defconfig";
    ubootQemuX86_64 = "uboot-qemu-x86_64_defconfig";
    ubootQuartz64B = "uboot-quartz64-b-rk3566_defconfig";
    ubootROCPCRK3399 = "uboot-roc-pc-rk3399_defconfig";
    ubootRadxaZero3W = "uboot-radxa-zero-3-rk3566_defconfig";
    ubootRaspberryPi = "uboot-rpi_defconfig";
    ubootRaspberryPi2 = "uboot-rpi_2_defconfig";
    ubootRaspberryPi3_32bit = "uboot-rpi_3_32b_defconfig";
    ubootRaspberryPi3_64bit = "uboot-rpi_3_defconfig";
    ubootRaspberryPi4_32bit = "uboot-rpi_4_32b_defconfig";
    ubootRaspberryPi4_64bit = "uboot-rpi_4_defconfig";
    ubootRaspberryPiAarch64 = "uboot-rpi_arm64_defconfig";
    ubootRaspberryPiZero = "uboot-rpi_0_w_defconfig";
    ubootRock3C = "uboot-rock-3c-rk3566_defconfig";
    ubootRock4CPlus = "uboot-rock-4c-plus-rk3399_defconfig";
    ubootRock5ModelB = "uboot-rock5b-rk3588_defconfig";
    ubootRock5ModelC = "uboot-rock-5c-rk3588s_defconfig";
    ubootRock64 = "uboot-rock64-rk3328_defconfig";
    ubootRock64v2 = "uboot-rock64-rk3328_defconfig";
    ubootRockPi4 = "uboot-rock-pi-4-rk3399_defconfig";
    ubootRockPiE = "uboot-rock-pi-e-rk3328_defconfig";
    ubootRockPro64 = "uboot-rockpro64-rk3399_defconfig";
    ubootSheevaplug = "uboot-sheevaplug_defconfig";
    ubootSopine = "uboot-sopine_baseboard_defconfig";
    ubootTools = "uboot-tools-only_defconfig";
    ubootTuringRK1 = "uboot-turing-rk1-rk3588_defconfig";
    ubootUtilite = "uboot-cm_fx6_defconfig";
    ubootVisionFive2 = "uboot-starfive_visionfive2_defconfig";
    ubootWandboard = "uboot-wandboard_defconfig";
    ubports-click = "click";
    ucblogo = "ucblogo-code";
    uclibc = "uclibc-ng";
    udev = "systemd-minimal-libs";
    udevCheckHook = "udev-check-hook";
    udisks2 = "udisks";
    udpreplay = "updreplay";
    uftraceFull = "uftrace";
    uhdMinimal = "uhd";
    uhttpmock_1_0 = "uhttpmock";
    uiua-unstable = "uiua";
    ultimatestunts = "ultimate-stunts";
    umount = "util-linux";
    unbound-full = "unbound";
    unbound-with-systemd = "unbound";
    unifi = "unifi-controller";
    unison-ucm = "unison-code-manager";
    unixODBC = "unixodbc";
    unused_deps = "bazel-buildtools";
    unzipNLS = "unzip";
    updateAutotoolsGnuConfigScriptsHook = "update-autotools-gnu-config-scripts-hook";
    useOldCXXAbi = "use-old-cxx-abi-hook";
    ustream-ssl-mbedtls = "ustream-ssl";
    utfcpp = "utf8cpp";
    util-linuxMinimal = "util-linux-minimal";
    utillinux = "util-linux";
    utilmacros = "util-macros";
    utsushi-networkscan = "imagescan-plugin-networkscan";
    uutils-coreutils-noprefix = "uutils-coreutils";
    uwimap = "uw-imap";
    v2ray = "v2ray-core";
    vaapi-intel-hybrid = "intel-hybrid-driver";
    vala_0_56 = "vala";
    valgrind-light = "valgrind";
    validatePkgConfig = "validate-pkg-config";
    varnish80 = "varnish";
    vassal = "VASSAL";
    vaultwarden-mysql = "vaultwarden";
    vaultwarden-postgresql = "vaultwarden";
    vaultwarden-sqlite = "vaultwarden";
    vbam = "visualboyadvance-m";
    vboot_reference = "vboot-utils";
    vc = "Vc";
    vclab-nexus = "nexus";
    vcpkg-tool-unwrapped = "vcpkg-tool";
    vdrift-bin = "vdrift";
    vencord-web-extension = "vencord";
    ventoy-full = "ventoy";
    ventoy-full-gtk = "ventoy-gtk3";
    ventoy-full-qt = "ventoy-qt5";
    vercel-pkg = "pkg";
    verit = "veriT";
    versionCheckHook = "version-check-hook";
    vertcoind = "vertcoin";
    vial = "Vial";
    vibrantlinux = "vibrantLinux";
    victorialogs = "VictoriaLogs";
    victoriametrics = "VictoriaMetrics";
    victoriatraces = "VictoriaTraces";
    vid-stab = "vid.stab";
    viewnior = "viewnior-gtk3";
    vikunja-desktop = "vikunja-desktop-2.3.0";
    vimPluginsUpdater = "vim-plugins-updater";
    vimb-unwrapped = "vimb";
    vimpager-latest = "vimpager";
    vinyl-cache_9 = "vinyl-cache";
    vips_8_17 = "vips";
    virtualboxExtpack = "virtualbox-extpack";
    virtualboxHardened = "virtualbox";
    virtualboxHeadless = "virtualbox";
    virtualboxKvm = "virtualbox";
    virtualboxWithExtpack = "virtualbox";
    virtualglLib = "virtualgl-lib";
    vivaldi-ffmpeg-codecs = "chromium-codecs-ffmpeg-extra";
    vlagent = "VictoriaLogs";
    vlc-bin = "vlc-bin-universal";
    vmagent = "VictoriaMetrics";
    vncdo = "vncdotool";
    voxtype-onnx = "voxtype";
    voxtype-vulkan = "voxtype";
    vscode-fhs = "vscode";
    vscode-solidity-server = "solidity-language-server";
    vscode-with-extensions = "vscode";
    vscodium-fhs = "vscodium";
    vte-gtk4 = "vte";
    vtk-full = "vtk";
    vtkWithQt6 = "vtk";
    vtk_9 = "vtk";
    vtk_9_5 = "vtk";
    vtk_9_6 = "vtk";
    vtk_9_egl = "vtk";
    vulkan-volk = "volk";
    w3m-batch = "w3m";
    w3m-full = "w3m";
    w3m-nographics = "w3m";
    w3m-nox = "w3m";
    wafHook = "waf-setup-hook";
    wall = "util-linux";
    wapm = "wapm-cli";
    wasm-bindgen-cli_0_2_100 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_104 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_105 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_106 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_108 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_114 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_117 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_118 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_120 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_121 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_122 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_125 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_126 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_127 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_93 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_95 = "wasm-bindgen-cli";
    wasm-bindgen-cli_0_2_99 = "wasm-bindgen-cli";
    wasmtime_36 = "wasmtime";
    watch = "procps";
    watcherclient = "python-watcherclient";
    wayback-x11 = "wayback";
    waydroid-nftables = "waydroid";
    wayfire-with-plugins = "wayfire-wrapped";
    webfontkitgenerator = "webfont-bundler";
    webkitgtk_4_1 = "webkitgtk";
    webkitgtk_6_0 = "webkitgtk";
    webrtc-audio-processing_0_3 = "webrtc-audio-processing";
    webrtc-audio-processing_1 = "webrtc-audio-processing";
    websocketpp = "websocket++";
    webtorrent_desktop = "webtorrent-desktop";
    weechat = "weechat-bin-env";
    weechat-unwrapped = "weechat";
    westonLite = "weston";
    whatsapp-electron = "whatsapp";
    whatsapp-emoji-font = "whatsapp-emoji-linux";
    whisper-cpp-vulkan = "whisper-cpp";
    whitebox-tools = "whitebox_tools";
    whitesur-icon-theme = "Whitesur-icon-theme";
    wiiuse = "WiiUse";
    winbox3 = "winbox";
    winbox4 = "winbox";
    windsurf = "devin-desktop";
    wine-wayland = "wine";
    wirelesstools = "wireless-tools";
    wireshark = "wireshark-qt";
    wiseunpacker = "WiseUnpacker";
    with-shell = "with";
    wlroots_0_19 = "wlroots";
    wlroots_0_20 = "wlroots";
    wordpress_6_8 = "wordpress";
    wordpress_6_9 = "wordpress";
    wordpress_7_0 = "wordpress";
    worldofgoo = "WorldOfGoo";
    wormhole-rs = "magic-wormhole-rs";
    wpa_supplicant_gui = "wpa_gui";
    wqy_microhei = "wqy-microhei";
    wqy_zenhei = "wqy-zenhei";
    wrapGAppsHook3 = "wrap-gapps-hook";
    wrapGAppsHook4 = "wrap-gapps-hook";
    wrapGAppsNoGuiHook = "wrap-gapps-hook";
    wrapVdr = "vdr-with-plugins";
    writableTmpDirAsHomeHook = "writable-tmpdir-as-home-hook";
    wshowkeys = "wshowkeys-unstable";
    wt4 = "wt";
    wtk = "sun-java-wtk";
    wxGTK31 = "wxwidgets";
    wxGTK32 = "wxwidgets";
    wxGTK33 = "wxwidgets";
    wxSVG = "wxsvg";
    wxwidgets_3_1 = "wxwidgets";
    wxwidgets_3_2 = "wxwidgets";
    wxwidgets_3_3 = "wxwidgets";
    x11_ssh_askpass = "x11-ssh-askpass";
    x16 = "x16-emulator";
    x16-run = "run-x16";
    x32edit = "x32-edit";
    xalanc = "xalan-c";
    xapian_1_4 = "xapian";
    xarMinimal = "xar-minimal";
    xash-dedicated = "xash3d-fwgs";
    xcb-util-cursor = "libxcb-cursor";
    xcb-util-cursor-HEAD = "libxcb-cursor";
    xcbproto = "xcb-proto";
    xcbuildHook = "xcbuild-hook";
    xcbutil = "libxcb-util";
    xcbutilcursor = "libxcb-cursor";
    xcbutilerrors = "libxcb-errors";
    xcbutilimage = "libxcb-image";
    xcbutilkeysyms = "libxcb-keysyms";
    xcbutilrenderutil = "libxcb-render-util";
    xcbutilwm = "libxcb-wm";
    xcbutilxrm = "xcb-util-xrm";
    xcodebuild = "xcbuild";
    xcursorthemes = "xcursor-themes";
    xd = "XD";
    xdg-terminal-exec-mkhl = "xdg-terminal-exec";
    xercesc = "xerces-c";
    xf86_input_wacom = "xf86-input-wacom";
    xf86_video_nested = "xf86-video-nested";
    xf86inputevdev = "xf86-input-evdev";
    xf86inputjoystick = "xf86-input-joystick";
    xf86inputkeyboard = "xf86-input-keyboard";
    xf86inputlibinput = "xf86-input-libinput";
    xf86inputmouse = "xf86-input-mouse";
    xf86inputsynaptics = "xf86-input-synaptics";
    xf86inputvmmouse = "xf86-input-vmmouse";
    xf86inputvoid = "xf86-input-void";
    xf86videoamdgpu = "xf86-video-amdgpu";
    xf86videoapm = "xf86-video-apm";
    xf86videoark = "xf86-video-ark";
    xf86videoast = "xf86-video-ast";
    xf86videoati = "xf86-video-ati";
    xf86videochips = "xf86-video-chips";
    xf86videocirrus = "xf86-video-cirrus";
    xf86videodummy = "xf86-video-dummy";
    xf86videofbdev = "xf86-video-fbdev";
    xf86videogeode = "xf86-video-geode";
    xf86videoi128 = "xf86-video-i128";
    xf86videoi740 = "xf86-video-i740";
    xf86videointel = "xf86-video-intel";
    xf86videomga = "xf86-video-mga";
    xf86videoneomagic = "xf86-video-neomagic";
    xf86videonouveau = "xf86-video-nouveau";
    xf86videonv = "xf86-video-nv";
    xf86videoomap = "xf86-video-omap";
    xf86videoopenchrome = "xf86-video-openchrome";
    xf86videoqxl = "xf86-video-qxl";
    xf86videor128 = "xf86-video-r128";
    xf86videos3virge = "xf86-video-s3virge";
    xf86videosavage = "xf86-video-savage";
    xf86videosiliconmotion = "xf86-video-siliconmotion";
    xf86videosis = "xf86-video-sis";
    xf86videosisusb = "xf86-video-sisusb";
    xf86videosuncg6 = "xf86-video-suncg6";
    xf86videosunffb = "xf86-video-sunffb";
    xf86videosunleo = "xf86-video-sunleo";
    xf86videotdfx = "xf86-video-tdfx";
    xf86videotrident = "xf86-video-trident";
    xf86videov4l = "xf86-video-v4l";
    xf86videovboxvideo = "xf86-video-vbox";
    xf86videovesa = "xf86-video-vesa";
    xf86videovmware = "xf86-video-vmware";
    xf86videovoodoo = "xf86-video-voodoo";
    xfce4-exo = "exo";
    xfs-undelete = "xfs_undelete";
    xgalagapp = "xgalaga++";
    xgboostWithCuda = "xgboost";
    xhosts = "nss-xhosts";
    xinput_calibrator = "xinput-calibrator";
    xivlauncher = "XIVLauncher";
    xkeyboard_config = "xkeyboard-config";
    xkeyboardconfig = "xkeyboard-config";
    xmlrpc_c = "xmlrpc-c";
    xmlsort = "XML-Filter-Sort";
    xmonad-log-applet-mate = "xmonad-log-applet";
    xmonad-log-applet-xfce = "xmonad-log-applet";
    xmonad_log_applet = "xmonad-log-applet";
    xmonad_log_applet_mate = "xmonad-log-applet";
    xmonad_log_applet_xfce = "xmonad-log-applet";
    xonotic = "xonotic-sdl";
    xonotic-data = "xonotic";
    xorg-autoconf = "util-macros";
    xorg-rgb = "rgb";
    xorg_sys_opengl = "xorg-sys-opengl";
    xorgcffiles = "xorg-cf-files";
    xorgdocs = "xorg-docs";
    xorgserver = "xorg-server";
    xorgsgmldoctools = "xorg-sgml-doctools";
    xorriso = "libisoburn";
    xp-pen-deco-01-v2-driver = "xppen_4";
    xp-pen-g430-driver = "xppen_3";
    xpraWithNvenc = "xpra";
    xq-xml = "xq";
    xremap = "xremap-wlroots";
    xscast = "xscast-unstable";
    xxHash = "xxhash";
    xxd = "tinyxxd";
    xxdiff-tip = "xxdiff";
    xyce-parallel = "xyce";
    yadmMinimal = "yadm";
    yarGen = "yargen";
    yarn-berry_3 = "yarn-berry";
    yarn-berry_4 = "yarn-berry";
    yarnBuildHook = "yarn-build-hook";
    yarnConfigHook = "yarn-config-hook";
    yarnInstallHook = "yarn-install-hook";
    yaru-remix-theme = "yaru-remix";
    yaru-theme = "yaru";
    yazi-unwrapped = "yazi";
    yed = "yEd";
    yi = "yi-custom";
    ykush = "ykushcmd";
    yoda-with-root = "yoda";
    your_spotify = "your_spotify_server";
    youtube-dl-light = "youtube-dl";
    youtube-music = "pear-desktop";
    youtube-viewer = "WWW-YoutubeViewer";
    yt-dlp-light = "yt-dlp";
    ytdownloader = "ytDownloader";
    zathura = "zathura-with-plugins";
    zbus-xmlgen = "zbus_xmlgen";
    zd1211fw = "zd1211-firmware";
    zeal-qt5 = "zeal";
    zeal-qt6 = "zeal";
    zed-editor-fhs = "zed-editor";
    zenith-nvidia = "zenith";
    zeroad-data = "0ad-data";
    zeroad-unwrapped = "0ad";
    zeroc-ice-cpp11 = "zeroc-ice";
    zfs-replicate = "zfs_replicate";
    zfs_2_3 = "zfs";
    zfs_2_4 = "zfs";
    zfs_unstable = "zfs";
    zig_0_13 = "zig";
    zig_0_14 = "zig";
    zig_0_15 = "zig";
    zig_0_16 = "zig";
    zigbee2mqtt_2 = "zigbee2mqtt";
    zipkin = "zipkin-server";
    zls_0_14 = "zls";
    zls_0_15 = "zls";
    zls_0_16 = "zls";
    zmkBATx = "zmkbatx";
    zod = "zod-engine";
    zonemaster-cli = "Zonemaster-CLI";
    zoom-us = "zoom";
    zsh-powerlevel10k = "powerlevel10k";
    zsh-powerlevel9k = "powerlevel9k";
    zsh-wd = "wd";
    zulu = "zulu-ca-jdk";
    zulu11 = "zulu-ca-jdk";
    zulu17 = "zulu-ca-jdk";
    zulu21 = "zulu-ca-jdk";
    zulu25 = "zulu-ca-jdk";
    zulu8 = "zulu-ca-jdk";
    zunclient = "python-zunclient";
    zynaddsubfx-fltk = "zynaddsubfx";
    zynaddsubfx-ntk = "zynaddsubfx";
  };
}

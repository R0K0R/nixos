...
in
{
  options.my.dev-toolchain.enable = lib.mkEnableOption "language servers, compilers and debuggers -- the interactive development set";

  # Accounts this feature applies to; defaults to the primary user.
  options.my.dev-toolchain.users = import ../../lib/user-scope.nix { inherit lib config; };
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    ...
    (lib.mkIf (pkgSet ? user) { my.packages.perUser = lib.genAttrs config.my.dev-toolchain.users (_: pkgSet.user); })
  ]);
}

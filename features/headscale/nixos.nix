...
let
  ...
  # TODO: this is the last builtins.fetchTarball in the repo -- it belongs in
  # this feature's own flake.nix as a locked input, per the sub-flake rule.
  # Left as-is for now because swapping the fetcher is not guaranteed
  # hash-neutral, and this feature is currently disabled on every host, so the
  # change would land unverified. Convert when the feature is next actually used.

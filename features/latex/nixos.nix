...
in
{
  options.my.latex.enable = lib.mkEnableOption "a LaTeX toolchain with an explicit package list rather than a bundled scheme";

  # Accounts this feature applies to; defaults to the primary user.
  options.my.latex.users = import ../../lib/user-scope.nix { inherit lib config; };
  ...
  config = lib.mkIf cfg.enable (lib.mkMerge [
    ...
    (lib.mkIf (pkgSet ? user) { my.packages.perUser = lib.genAttrs config.my.latex.users (_: pkgSet.user); })
  ]);
}
